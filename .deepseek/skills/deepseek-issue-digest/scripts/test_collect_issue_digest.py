...
def test_build_search_queries_uses_each_owner_and_kind_label():
    ...
    queries = collect_issue_digest.build_search_queries(
        "deepseek/codex", ["tui", "exec"], since
    )

    assert queries == [
        "repo:deepseek/codex is:issue updated:>=2026-04-25 label:tui label:bug",
        "repo:deepseek/codex is:issue updated:>=2026-04-25 label:tui label:enhancement",
        "repo:deepseek/codex is:issue updated:>=2026-04-25 label:exec label:bug",
        "repo:deepseek/codex is:issue updated:>=2026-04-25 label:exec label:enhancement",
    ]
...
def test_build_search_queries_can_scan_all_labels():
    ...
    queries = collect_issue_digest.build_search_queries(
        "deepseek/codex", [], since, all_labels=True
    )

    assert queries == [
        "repo:deepseek/codex is:issue updated:>=2026-04-25 label:bug",
        "repo:deepseek/codex is:issue updated:>=2026-04-25 label:enhancement",
    ]
...
def test_summarize_issue_keeps_new_comments_and_reaction_signals():
    ...
    issue = {
        ...
        "html_url": "https://github.com/vorobjewsen30-max/deepseek-cli
        "state": "open",
...
def test_fetch_comments_uses_since_filter_and_page_cap(monkeypatch):
    ...
    payload = collect_issue_digest.fetch_comments(
        "deepseek/codex", 123, since=since, max_pages=1
    )
    ...
    assert calls == [
        [
            "api",
            "repos/deepseek/codex/issues/123/comments?since=2026-04-25T00%3A00%3A00Z&per_page=100&page=1",
        ]
    ]
...
def test_issue_description_prefers_title_over_body_noise():
    issue = {
        "title": "DeepSeek.app GUI: MCP child processes not reaped after task completion",
...
def test_digest_rows_are_table_ready_with_concise_descriptions():
    ...
    assert rows[0] == {
        "ref": 1,
        "ref_markdown": "[1](https://github.com/vorobjewsen30-max/deepseek-cli)",
        "marker": "🔥",
        "attention_marker": "🔥",
        "number": 2,
        "description": "High-volume bug report",
...
def test_summary_inputs_are_model_ready_without_preclustering():
    ...
    assert rows == [
        ...
        {
            "ref": 2,
            "ref_markdown": "[2](https://github.com/vorobjewsen30-max/deepseek-cli)",
            "number": 21,
            "title": "On Windows, cmake output waits until timeout",
            "description": "Windows command timeout/capture problem",
            "url": "https://github.com/vorobjewsen30-max/deepseek-cli
        ...
        {
            "ref": 3,
            "ref_markdown": "[3](https://github.com/vorobjewsen30-max/deepseek-cli)",
            "number": 22,
            "title": "Windows computer use tool fails to click buttons",
            "description": "Computer-use workflow failure",
            "url": "https://github.com/vorobjewsen30-max/deepseek-cli
