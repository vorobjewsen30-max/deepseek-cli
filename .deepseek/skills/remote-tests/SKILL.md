...
Use `TestDeepSeekBuilder::build_with_auto_env()` to opt-in to remote execution in core integration
...
`$DEEPSEEK_HOME/environments.toml` or will define custom environments at runtime.
...
bash -c '
  ...
  cd codex-rs
  just test -p deepseek-core --test all
'
```
...
bash -c '
  ...
  cd codex-rs
  just test -p deepseek-app-server --test all
'
```
