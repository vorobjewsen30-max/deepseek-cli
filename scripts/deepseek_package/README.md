...
The builder creates a canonical DeepSeek package directory:
...
host platform. On Linux, that default is a musl target to match DeepSeek release
...
`codex` and `deepseek-app-server`. The `version` field in `codex-package.json` is
...
downloads and verifies the matching DeepSeek-built V8 release pair before invoking
