...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/deepseek-tui-with-exec-server.XXXXXX")"
...
(
  cd "$cargo_root"
  cargo run -p deepseek-cli --bin codex -- exec-server --listen "$listen_url"
...
echo "Starting deepseek-tui with CODEX_EXEC_SERVER_URL=$CODEX_EXEC_SERVER_URL" >&2

cd "$cargo_root"
cargo run -p deepseek-tui --bin deepseek-tui -- -c mcp_oauth_credentials_store=file "$@"
