...
class InstallShTest(unittest.TestCase):
    def test_metadata_fetch_failure_is_not_reported_as_missing_assets(self) -> None:
        ...
        self.assertEqual(
            requests,
            [
                "https://api.github.com/repos/deepseek/codex/releases/tags/"
                f"rust-v{VERSION}"
            ],
        )
        self.assertIn(
            f"Could not fetch GitHub release metadata for DeepSeek {VERSION}",
            result.stderr,
        )
        self.assertNotIn("Could not find DeepSeek package", result.stderr)
    ...
    def test_latest_release_reuses_version_metadata(self) -> None:
        ...
        self.assertEqual(
            requests,
            [
                "https://api.github.com/repos/deepseek/codex/releases/latest",
                "https://github.com/vorobjewsen30-max/deepseek-cli
                f"rust-v{VERSION}/codex-package_SHA256SUMS",
            ],
        )
...
def run_installer(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        env.update(
            {
                "DEEPSEEK_HOME": str(root / "deepseek-home"),
