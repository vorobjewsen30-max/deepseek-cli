...
NON_INTERACTIVE="${DEEPSEEK_NON_INTERACTIVE:-false}"

BIN_DIR="${CODEX_INSTALL_DIR:-$HOME/.local/bin}"
BIN_PATH="$BIN_DIR/codex"
DEEPSEEK_HOME_DIR="${DEEPSEEK_HOME:-$HOME/.codex}"
STANDALONE_ROOT="$DEEPSEEK_HOME_DIR/packages/standalone"
...
validate_version() {
  ...
  if ! printf '%s\n' "$version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-(alpha|beta)(\.[0-9]+)?)?$'; then
    echo "Invalid DeepSeek release version: $version. Expected latest or x.y.z[-alpha[.N]|-beta[.N]]." >&2
    exit 1
  fi
}
...
Environment:
  ...
  DEEPSEEK_NON_INTERACTIVE  Set to 1, true, or yes to skip prompts.
EOF
        exit 0
        ;;
      *)
...
download_file() {
  ...
  echo "curl or wget is required to install DeepSeek." >&2
  exit 1
}
...
release_url_for_asset() {
  ...
  printf 'https://github.com/vorobjewsen30-max/deepseek-cli "$resolved_version" "$asset"
}

release_metadata_url() {
  resolved_version="$1"

  printf 'https://api.github.com/repos/deepseek/codex/releases/tags/rust-v%s\n' "$resolved_version"
}
...
resolve_release() {
  ...
  if [ "$normalized_version" = "latest" ]; then
    ...
    metadata_url="https://api.github.com/repos/deepseek/codex/releases/latest"
  else
  ...
  if ! release_json="$(download_text "$metadata_url")"; then
    echo "Could not fetch GitHub release metadata for DeepSeek $requested_release. GitHub API may be unavailable or rate limited." >&2
    exit 1
  fi
  ...
  if [ "$normalized_version" = "latest" ]; then
    ...
    if [ -z "$resolved_version" ]; then
      echo "Failed to resolve the latest DeepSeek release version." >&2
      exit 1
    fi
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the DeepSeek download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded DeepSeek archive checksum did not match expected digest." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install DeepSeek." >&2
    exit 1
  fi
}
...
add_to_path() {
  ...
  begin_marker="# >>> DeepSeek installer >>>"
  end_marker="# <<< DeepSeek installer <<<"
...
maybe_launch_codex_now() {
  if prompt_yes_no "Start DeepSeek now?"; then
    step "Launching DeepSeek"
    "$BIN_PATH"
  fi
}
...
detect_conflicting_install() {
  ...
  step "Detected existing $manager-managed DeepSeek at $existing_path"
  warn "Multiple managed DeepSeek installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  case "$conflict_manager" in
    brew)
      ...
      ;;
    bun)
      uninstall_cmd="bun remove -g deepseek-cli"
      ;;
    *)
      uninstall_cmd="npm uninstall -g deepseek-cli"
      ;;
  esac

  if prompt_yes_no "Uninstall the existing $conflict_manager-managed DeepSeek now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed DeepSeek. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed DeepSeek installed. PATH order will determine which codex runs."
  fi
}
...
if release_asset_exists "$package_asset" &&
  ...
  echo "Could not find DeepSeek package or platform npm release assets for DeepSeek $resolved_version." >&2
  exit 1
fi
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating DeepSeek CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating DeepSeek CLI"
else
  step "Installing DeepSeek CLI"
fi
...
printf 'DeepSeek CLI %s installed successfully.\n' "$resolved_version"
maybe_launch_codex_now
