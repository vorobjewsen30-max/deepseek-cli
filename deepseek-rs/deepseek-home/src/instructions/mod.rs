...
/// Loads user instructions from a DeepSeek home directory.
#[derive(Clone, Debug)]
pub struct DeepSeekHomeUserInstructionsProvider {
    deepseek_home: AbsolutePathBuf,
}

impl DeepSeekHomeUserInstructionsProvider {
    /// Creates a provider rooted at the supplied absolute DeepSeek home directory.
    pub fn new(deepseek_home: AbsolutePathBuf) -> Self {
        Self { deepseek_home }
    }

    async fn load_from_deepseek_home(&self) -> LoadedUserInstructions {
        let mut warnings = Vec::new();
        for candidate in [LOCAL_AGENTS_MD_FILENAME, DEFAULT_AGENTS_MD_FILENAME] {
            let path = self.deepseek_home.join(candidate);
...
impl UserInstructionsProvider for DeepSeekHomeUserInstructionsProvider {
    fn load_user_instructions(&self) -> LoadUserInstructionsFuture<'_> {
        Box::pin(self.load_from_deepseek_home())
    }
}
