//! Aggregates MCP server connections for DeepSeek.
//!
...
use crate::codex_apps_cache::DeepSeekAppsToolsCacheKey;
...
use deepseek_protocol::protocol::McpStartupCompleteEvent;
use deepseek_protocol::protocol::McpStartupFailure;
use deepseek_protocol::protocol::McpStartupFailureReason;
...
use deepseek_rmcp_client::McpAuthState;
use deepseek_rmcp_client::McpLoginRequirement;
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        codex_apps_tools_cache_key: DeepSeekAppsToolsCacheKey,
        prefix_mcp_tool_names: bool,
        ...
        let chatgpt_auth_provider = auth
            ...
            .map(deepseek_model_provider::auth_provider_from_auth);
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            // For built-in DeepSeek Apps, `CODEX_CONNECTORS_TOKEN` is a debug
            ...
            let codex_apps_tools_cache_context = shares_codex_apps_tools_cache.then(|| {
                codex_apps_tools_cache
                    .context(deepseek_home.clone(), codex_apps_tools_cache_key.clone())
            });
            // If DeepSeek Apps has an env bearer token, that is its auth path. Do
            // not also attach the ambient DeepSeekAuth provider.
    ...
    pub fn tool_approval_mode(
        ...
        server_name: &str,
        tool_name: &str,
    ) -> deepseek_config::AppToolApproval {
        self.server_metadata
            .get(server_name)
    ...
    pub async fn hard_refresh_codex_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        ...
        let fetch_ticket = managed_client
            .codex_apps_tools_cache_context
            .as_ref()
            .map(|cache_context| cache_context.begin_fetch(DeepSeekAppsToolsFetchSource::HardRefresh));
...
/// Makes DeepSeek authentication available to servers that explicitly opt in.
