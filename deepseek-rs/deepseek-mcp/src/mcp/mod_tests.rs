...
fn test_mcp_config(deepseek_home: PathBuf) -> McpConfig {
    McpConfig {
        chatgpt_base_url: "https://platform.deepseek.com".to_string(),
        ...
        connector_snapshot: deepseek_connectors::ConnectorSnapshot::default(),
    }
}
...
fn codex_apps_mcp_url_for_base_url_keeps_existing_paths() {
    assert_eq!(
        codex_apps_mcp_url_for_base_url("https://api.deepseek.com/v1"),
        "https://api.deepseek.com/v1/wham/apps"
    );
    assert_eq!(
        codex_apps_mcp_url_for_base_url("https://platform.deepseek.com),
...
fn codex_apps_server_config_uses_legacy_codex_apps_path() {
    let config =
        codex_apps_mcp_server_config("https://platform.deepseek.com", /*apps_mcp_product_sku*/ None);
...
fn codex_apps_server_config_forwards_configured_product_sku_header() {
    let config = codex_apps_mcp_server_config("https://platform.deepseek.com", Some("tpp"));
    ...
    match &config.transport {
        McpServerTransportConfig::StreamableHttp {
            ...
            assert_eq!(
                http_headers,
                &Some(HashMap::from([(
                    "X-DeepSeek-Product-Sku".to_string(),
                    "tpp".to_string(),
                )]))
            );
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(deepseek_home.path().to_path_buf());
