...
use deepseek_config::McpServerAuth;
...
use deepseek_connectors::ConnectorSnapshot;
use deepseek_login::DeepSeekAuth;
use deepseek_model_provider::DEEPSEEK_API_BASE_URL;
use deepseek_protocol::mcp::McpServerInfo;
use deepseek_protocol::mcp::Resource;
use deepseek_protocol::mcp::ResourceTemplate;
use deepseek_protocol::mcp::Tool;
use deepseek_protocol::models::PermissionProfile;
use deepseek_protocol::protocol::AskForApproval;
use deepseek_protocol::protocol::McpAuthStatus;
...
use crate::codex_apps_cache::DeepSeekAppsToolsCache;
...
/// MCP runtime settings derived from `deepseek_core::config::Config`.
///
/// This struct should contain only long-lived configuration values that the
/// `deepseek-mcp` crate needs to construct server transports, enforce MCP
...
pub struct McpConfig {
    /// Base URL for DeepSeek-hosted app MCP servers, copied from the root config.
    pub chatgpt_base_url: String,
    /// Optional product SKU forwarded to the host-owned apps MCP server.
    pub apps_mcp_product_sku: Option<String>,
    /// DeepSeek home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `deepseek-linux-sandbox` for sandboxed MCP tool execution.
    ...
    ///
    /// DeepSeek auth is checked separately before a materialized host-owned Apps
    /// server can be used.
...
pub fn host_owned_codex_apps_enabled(config: &McpConfig, auth: Option<&DeepSeekAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(DeepSeekAuth::uses_codex_backend)
}
...
pub fn effective_mcp_servers_from_configured(
    ...
    let chatgpt_origin = url::Url::parse(DEEPSEEK_API_BASE_URL)
        .ok()
        .map(|url| url.origin());
...
pub async fn read_mcp_resource(
    ...
    codex_apps_tools_cache: DeepSeekAppsToolsCache,
    server: &str,
    uri: &str,
...
/// Builds the DeepSeek-hosted plugin runtime served by plugin-service.
...
fn mcp_server_config_for_url(
    ...
    let http_headers = apps_mcp_product_sku.map(|product_sku| {
        HashMap::from([("X-DeepSeek-Product-Sku".to_string(), product_sku.to_string())])
    });
