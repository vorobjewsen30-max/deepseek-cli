...
fn codex_apps_tools_cache_publishes_newest_shared_snapshot() {
    let deepseek_home = tempdir().expect("tempdir");
    let cache = DeepSeekAppsToolsCache::default();
    ...
    let older_ticket = cache_context_1.begin_fetch(DeepSeekAppsToolsFetchSource::Startup);
    let newer_ticket = cache_context_2.begin_fetch(DeepSeekAppsToolsFetchSource::HardRefresh);
    let server_info = create_test_server_info("DeepSeek Apps");
...
fn codex_apps_tools_cache_keeps_live_publish_when_disk_persistence_fails() {
    let deepseek_home = tempdir().expect("tempdir");
    let deepseek_home_file = deepseek_home.path().join("not-a-directory");
    std::fs::write(&deepseek_home_file, b"occupied").expect("create codex home file");
    let cache_context = DeepSeekAppsToolsCache::default().context(
        deepseek_home_file,
        DeepSeekAppsToolsCacheKey {
            account_id: Some("account-one".to_string()),
            chatgpt_user_id: Some("user-one".to_string()),
    ...
    let published_tools = cache_context.publish_if_newest_accepted(
        cache_context.begin_fetch(DeepSeekAppsToolsFetchSource::HardRefresh),
        &create_test_server_info("DeepSeek Apps"),
        tools.clone(),
    );
...
fn codex_apps_tools_cache_scopes_non_utf8_home_disk_paths() {
    let deepseek_home = PathBuf::from(std::ffi::OsString::from_vec(
        b"/tmp/deepseek-home-\xff".to_vec(),
    ));
    let cache = DeepSeekAppsToolsCache::default();
