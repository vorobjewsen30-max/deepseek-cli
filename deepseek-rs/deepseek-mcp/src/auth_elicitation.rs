...
//! Session orchestration stays in `deepseek-core`.
...
pub struct DeepSeekAppsConnectorAuthFailure {
...
pub struct DeepSeekAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct DeepSeekAppsAuthElicitationPlan {
    pub auth_failure: DeepSeekAppsConnectorAuthFailure,
    pub elicitation: DeepSeekAppsAuthElicitation,
}

#[derive(Serialize)]
struct DeepSeekAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<DeepSeekAppsConnectorAuthFailure> {
    ...
    Some(DeepSeekAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<DeepSeekAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(DeepSeekAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &DeepSeekAppsConnectorAuthFailure,
) -> DeepSeekAppsAuthElicitation {
    DeepSeekAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_CODEX_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: DeepSeekAppsConnectorAuthFailureMeta {
...
fn auth_elicitation_message(auth_failure: &DeepSeekAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        Some("oauth_upgrade_required") => format!(
            "Reconnect {} on DeepSeek to grant the permissions needed for this request.",
            auth_failure.connector_name
        ),
        Some("reauthentication_required") => format!(
            "Reconnect {} on DeepSeek to restore access for this request.",
            auth_failure.connector_name
        ),
        Some("missing_link") => format!(
            "Sign in to {} on DeepSeek to use it in DeepSeek.",
            auth_failure.connector_name
        ),
        _ => format!(
            "Sign in to {} on DeepSeek to continue.",
            auth_failure.connector_name
        ),
    }
}
    ...
    fn parses_auth_failure_from_trusted_connector_metadata() {
        assert_eq!(
            connector_auth_failure_from_tool_result(
                ...
                Some("https://platform.deepseek.com/apps/google-calendar/connector_calendar".to_string()),
            ),
            Some(DeepSeekAppsConnectorAuthFailure {
                connector_id: "connector_calendar".to_string(),
                connector_name: "Google Calendar".to_string(),
                install_url: "https://platform.deepseek.com/apps/google-calendar/connector_calendar"
                    .to_string(),
    ...
    fn rejects_missing_or_mismatched_connector_ids() {
        ...
        assert_eq!(
            connector_auth_failure_from_tool_result(
                ...
                Some("Google Drive"),
                Some("https://platform.deepseek.com/apps/google-drive/connector_drive".to_string()),
            ),
            None
        );
    }
    ...
    fn builds_url_elicitation_payload() {
        ...
        assert_eq!(
            ...
            DeepSeekAppsAuthElicitation {
                meta: serde_json::json!({
                    MCP_TOOL_CODEX_APPS_META_KEY: {
                        CONNECTOR_AUTH_FAILURE_META_KEY: {
                            ...
                            "install_url":
                                "https://platform.deepseek.com/apps/google-calendar/connector_calendar",
                            CONNECTOR_AUTH_FAILURE_AUTH_REASON_KEY: "reauthentication_required",
                            CONNECTOR_AUTH_FAILURE_LINK_ID_KEY: "link_123",
                            CONNECTOR_AUTH_FAILURE_ERROR_CODE_KEY: "UNAUTHORIZED",
                            CONNECTOR_AUTH_FAILURE_ERROR_HTTP_STATUS_CODE_KEY: 401,
                ...
                message: "Reconnect Google Calendar on DeepSeek to restore access for this request."
                    .to_string(),
                url: "https://platform.deepseek.com/apps/google-calendar/connector_calendar".to_string(),
