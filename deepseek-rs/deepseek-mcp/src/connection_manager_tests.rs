...
use crate::declared_deepseek_file_input_param_names;
...
use crate::rmcp_client::DeepSeekAppsStartupReconnect;
...
use deepseek_protocol::ToolName;
...
use deepseek_rmcp_client::InProcessTransportFactory;
...
fn create_codex_apps_tools_cache_context(
    ...
    DeepSeekAppsToolsCache::default().context(
        deepseek_home,
...
fn create_test_manager_with_failed_apps_startup(
    ...
    manager.clients.insert(
        ...
        AsyncManagedClient {
            ...
            tool_filter: ToolFilter::default(),
            startup_complete: Arc::new(std::sync::atomic::AtomicBool::new(true)),
            startup_reconnect: Some(Arc::new(DeepSeekAppsStartupReconnect::new(reconnect_factory))),
            tool_plugin_provenance: Arc::new(ToolPluginProvenance::default()),
            cancel_token: CancellationToken::new(),
        },
    );
...
#[test]
fn declared_deepseek_file_fields_treat_names_literally() {
    let meta = serde_json::json!({
        "deepseek/fileParams": ["file", "input_file", "attachments"]
    });
    let meta = meta.as_object().expect("meta object");

    assert_eq!(
        declared_deepseek_file_input_param_names(Some(meta)),
        vec![
            "file".to_string(),
...
fn tool_with_model_visible_input_schema_masks_file_params() {
    ...
    tool.meta = Some(Meta(
        serde_json::json!({
            "deepseek/fileParams": ["file", "files"]
        })
        .as_object()
        .expect("object")
        .clone(),
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        deepseek_rmcp_client::Elicitation::Mcp(
...
async fn shared_elicitation_router_targets_the_exact_pending_request() {
    ...
    let elicitation = deepseek_rmcp_client::Elicitation::Mcp(
        CreateElicitationRequestParams::FormElicitationParams {
            meta: None,
    ...
    let (
        deepseek_protocol::mcp::RequestId::String(request_a_id),
        deepseek_protocol::mcp::RequestId::String(request_b_id),
    ) = (request_a.id, request_b.id)
    else {
        panic!("expected DeepSeek-owned string request IDs");
    };
...
async fn list_available_server_infos_uses_cache_while_client_is_pending() {
    ...
    let server_info = create_test_server_info("DeepSeek Apps");
    manager.clients.insert(
        CODEX_APPS_MCP_SERVER_NAME.to_string(),
        AsyncManagedClient {
            client: pending_client,
...
async fn no_local_runtime_fails_local_stdio_but_keeps_local_http_server() {
    ...
    let manager = McpConnectionManager::new(
        ...
        deepseek_home.path().to_path_buf(),
        DeepSeekAppsToolsCache::default(),
        DeepSeekAppsToolsCacheKey {
            account_id: None,
