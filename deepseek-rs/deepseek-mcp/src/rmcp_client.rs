...
//! and exposing cached DeepSeek Apps tools while a client is still connecting.
...
use crate::codex_apps_cache::DeepSeekAppsToolsCacheContext;
use crate::codex_apps_cache::DeepSeekAppsToolsFetchSource;
...
use deepseek_exec_server::HttpClient;
use deepseek_exec_server::ReqwestHttpClient;
use deepseek_protocol::mcp::McpServerInfo;
use deepseek_protocol::protocol::Event;
use deepseek_protocol::protocol::EventMsg;
use deepseek_protocol::protocol::McpStartupStatus;
use deepseek_protocol::protocol::McpStartupUpdateEvent;
use deepseek_rmcp_client::ExecutorStdioServerLauncher;
use deepseek_rmcp_client::LocalStdioServerLauncher;
use deepseek_rmcp_client::RmcpClient;
use deepseek_rmcp_client::StdioServerLauncher;
use deepseek_rmcp_client::ToolWithConnectorId;
use deepseek_rmcp_client::is_authentication_required_error;
...
/// MCP server capability indicating that DeepSeek should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "codex/sandbox-state-meta";
pub const OPENAI_FORM_CAPABILITY: &str = "deepseek/form";
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) codex_apps_tools_cache_context: Option<DeepSeekAppsToolsCacheContext>,
}
...
impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        ...
        if let Some(tools) = self
            ...
            .as_ref()
            .and_then(DeepSeekAppsToolsCacheContext::current_tools)
        {
            emit_duration(
...
#[derive(Default)]
struct DeepSeekAppsStartupReconnectState {
...
#[derive(Clone)]
struct DeepSeekAppsStartupStatusContext {
    submit_id: String,
    server_name: String,
    tx_event: Sender<Event>,
}

pub(crate) struct DeepSeekAppsStartupReconnect {
    factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>,
    state: StdMutex<DeepSeekAppsStartupReconnectState>,
    startup_status_context: Option<DeepSeekAppsStartupStatusContext>,
}

impl DeepSeekAppsStartupReconnect {
    pub(crate) fn new(factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>) -> Self {
        Self {
            factory,
            state: StdMutex::new(DeepSeekAppsStartupReconnectState::default()),
    ...
    fn with_startup_status_context(
        ...
        self.startup_status_context = Some(DeepSeekAppsStartupStatusContext {
            submit_id,
            server_name,
            tx_event,
        });
...
struct ManagedClientStartup {
    ...
    codex_apps_tools_cache_context: Option<DeepSeekAppsToolsCacheContext>,
...
pub(crate) struct AsyncManagedClient {
    ...
    pub(crate) startup_reconnect: Option<Arc<DeepSeekAppsStartupReconnect>>,
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let startup_reconnect = is_codex_apps_mcp_server.then(|| {
            ...
            Arc::new(
                DeepSeekAppsStartupReconnect::new(Arc::new(move || startup.start()))
        ...
        if codex_apps_tools_cache_context
            .as_ref()
            .is_some_and(DeepSeekAppsToolsCacheContext::has_current_tools)
        {
...
/// Presents declared DeepSeek Apps file parameters to the model as local-path inputs and adds plugin
...
/// Converts a DeepSeek Apps tool by preserving connector fields, removing connector prefixes from
...
async fn start_server_task(
    ...
    let fetch_ticket = codex_apps_tools_cache_context
        .as_ref()
        .map(|cache_context| cache_context.begin_fetch(DeepSeekAppsToolsFetchSource::Startup));
    ...
    let tools = match (codex_apps_tools_cache_context.as_ref(), fetch_ticket) {
        ...
        (None, None) => tools,
        _ => unreachable!("DeepSeek Apps fetch ticket requires cache context"),
    };
    if is_codex_apps_mcp_server {
        emit_duration(
            MCP_TOOLS_LIST_DURATION_METRIC,
...
fn mcp_initialize_request_params(
    ...
    if supports_deepseek_form_elicitation {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("deepseek-mcp-client", env!("CARGO_PKG_VERSION")).with_title("DeepSeek"),
    )
...
async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(deepseek_utils_path_uri::LegacyAppPathString::into_string);
...
mod tests {
    ...
    #[test]
    fn mcp_initialize_advertises_deepseek_form_only_when_supported() {
        ...
        let supported = mcp_initialize_request_params(
            ElicitationCapability::default(),
            /*supports_deepseek_form_elicitation*/ true,
        );
        assert_eq!(
    ...
    fn custom_mcp_connector_metadata_is_stripped() {
        ...
        assert!(meta.0.contains_key("deepseek/fileParams"));
        assert_eq!(
