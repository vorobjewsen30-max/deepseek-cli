...
use deepseek_config::RequirementsExecPolicy;
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    requirements_exec_policy.add_network_rule(
        "blocked.example.com",
        deepseek_execpolicy::NetworkRuleProtocol::Https,
        Decision::Forbidden,
        /*justification*/ None,
    )?;

    let requirements = ConfigRequirements {
        exec_policy: Some(deepseek_config::Sourced::new(
            deepseek_config::RequirementsExecPolicy::new(requirements_exec_policy),
            deepseek_config::RequirementSource::Unknown,
        )),
        ..ConfigRequirements::default()
    };
...
async fn ignore_user_project_rules_keeps_system_policy_files() {
    ...
    .expect("write policy file");
    let config_file =
        AbsolutePathBuf::from_absolute_path(config_dir.join(deepseek_config::CONFIG_TOML_FILE))
...
async fn ignore_user_config_keeps_user_policy_files() -> std::io::Result<()> {
    let temp = tempdir()?;
    let deepseek_home = temp.path().join("home_ignore_user_config");
    let rules_dir = deepseek_home.join(RULES_DIR_NAME);
...
async fn append_execpolicy_amendment_updates_policy_and_file() {
    let deepseek_home = tempdir().expect("create temp dir");
    let prefix = vec!["echo".to_string(), "hello".to_string()];
    let manager = ExecPolicyManager::default();

    manager
        .append_amendment_and_update(deepseek_home.path(), &ExecPolicyAmendment::from(prefix))
        .await
    ...
    let contents = fs::read_to_string(default_policy_path(deepseek_home.path()))
...
async fn append_execpolicy_amendment_rejects_empty_prefix() {
    ...
    let result = manager
        .append_amendment_and_update(deepseek_home.path(), &ExecPolicyAmendment::from(vec![]))
        .await;
...
async fn exec_policies_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let deepseek_home = temp.path().join("home_execpolicy_nested");
    ...
    fs::create_dir_all(&deepseek_home)?;
...
async fn exec_policies_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    for (name, trust_entries, expected_decision) in cases {
        let deepseek_home = temp.path().join(format!("home_execpolicy_{name}"));
        fs::create_dir_all(&deepseek_home)?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_entries, expect_warning) in cases {
        let deepseek_home = temp.path().join(format!("home_execpolicy_warning_{name}"));
        fs::create_dir_all(&deepseek_home)?;
        write_project_trust_config(&deepseek_home, &trust_entries).await?;
