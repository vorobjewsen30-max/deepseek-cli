...
use deepseek_analytics::DeepSeekCompactionEvent;
use deepseek_analytics::CompactionImplementation;
use deepseek_analytics::CompactionPhase;
use deepseek_analytics::CompactionReason;
use deepseek_analytics::CompactionStatus;
use deepseek_analytics::CompactionStrategy;
use deepseek_analytics::CompactionTrigger;
use deepseek_analytics::now_unix_seconds;
use deepseek_protocol::error::DeepSeekErr;
use deepseek_protocol::error::Result as DeepSeekResult;
use deepseek_protocol::items::ContextCompactionItem;
...
pub use deepseek_prompts::SUMMARIZATION_PROMPT;
pub use deepseek_prompts::SUMMARY_PREFIX;
...
async fn run_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            let error = DeepSeekErr::TurnAborted;
            attempt
                .track(
                    sess.as_ref(),
...
async fn run_compact_task_inner_impl(
    ...
    let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
        ...
        window_id,
        DeepSeekResponsesRequestKind::Compaction(compaction_metadata),
    );
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(err @ (DeepSeekErr::Interrupted | DeepSeekErr::TurnAborted)) => {
                return Err(err);
            }
            Err(e @ DeepSeekErr::SessionBudgetExceeded) => {
                sess.track_turn_codex_error(turn_context.as_ref(), &e);
                let event = EventMsg::Error(e.to_error_event(/*message_prefix*/ None));
                sess.send_event(&turn_context, event).await;
                return Err(e);
            }
            Err(e @ DeepSeekErr::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        codex_error: Option<&DeepSeekErr>,
        ...
        sess.services
            ...
            .track_compaction(DeepSeekCompactionEvent {
                ...
                codex_error_http_status_code: codex_error
                    .and_then(DeepSeekErr::http_status_code_value),
                active_context_tokens_before,
...
pub(crate) fn compaction_status_from_result<T>(result: &DeepSeekResult<T>) -> CompactionStatus {
    match result {
        Ok(_) => CompactionStatus::Completed,
        Err(DeepSeekErr::Interrupted | DeepSeekErr::TurnAborted) => CompactionStatus::Interrupted,
