...
use deepseek_config::merge_toml_values;
...
use deepseek_config::permissions_toml::overlay_network_domain_permissions;
use deepseek_exec_server::LOCAL_FS;
use deepseek_network_proxy::ConfigReloader;
use deepseek_network_proxy::ConfigReloaderFuture;
use deepseek_network_proxy::ConfigState;
use deepseek_network_proxy::NetworkMode;
use deepseek_network_proxy::NetworkProxyConfig;
use deepseek_network_proxy::NetworkProxyConstraintError;
use deepseek_network_proxy::NetworkProxyConstraints;
use deepseek_network_proxy::NetworkProxyState;
use deepseek_network_proxy::build_config_state;
use deepseek_network_proxy::normalize_host;
use deepseek_network_proxy::validate_policy_against_constraints;
...
async fn build_config_state_with_mtimes() -> Result<(ConfigState, Vec<LayerMtime>)> {
    ...
    let (exec_policy, warning) = match load_exec_policy(&config_layer_stack).await {
        ...
        Err(err @ ExecPolicyError::ParsePolicy { .. }) => {
            (deepseek_execpolicy::Policy::empty(), Some(err))
        }
...
fn apply_exec_policy_network_rules(
    ...
    for host in allowed_domains {
        upsert_network_domain(
            config,
            host,
            deepseek_network_proxy::NetworkDomainPermission::Allow,
        );
    }
    for host in denied_domains {
        upsert_network_domain(
            config,
            host,
            deepseek_network_proxy::NetworkDomainPermission::Deny,
        );
    }
}
...
fn upsert_network_domain(
    config: &mut NetworkProxyConfig,
    host: String,
    permission: deepseek_network_proxy::NetworkDomainPermission,
) {
    config
        .network
