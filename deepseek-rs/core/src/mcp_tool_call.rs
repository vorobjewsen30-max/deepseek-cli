...
use crate::mcp_deepseek_file::rewrite_mcp_tool_arguments_for_deepseek_files;
...
use deepseek_analytics::AppInvocation;
use deepseek_analytics::InvocationType;
use deepseek_analytics::build_track_events_context;
...
use deepseek_connectors::AppToolPolicy;
use deepseek_connectors::AppToolPolicyEvaluator;
use deepseek_connectors::AppToolPolicyInput;
use deepseek_features::Feature;
use deepseek_hooks::PermissionRequestDecision;
use deepseek_mcp::CODEX_APPS_MCP_SERVER_NAME;
use deepseek_mcp::MCP_TOOL_CODEX_APPS_META_KEY;
use deepseek_mcp::McpConnectionManager;
use deepseek_mcp::McpPermissionPromptAutoApproveContext;
use deepseek_mcp::SandboxState;
use deepseek_mcp::auth_elicitation_completed_result;
use deepseek_mcp::build_auth_elicitation_plan;
use deepseek_mcp::declared_deepseek_file_input_param_names;
use deepseek_mcp::mcp_permission_prompt_is_auto_approved;
use deepseek_protocol::approvals::ElicitationRequest;
use deepseek_protocol::items::McpToolCallError;
use deepseek_protocol::items::McpToolCallItem;
use deepseek_protocol::items::McpToolCallStatus;
use deepseek_protocol::items::TurnItem;
use deepseek_protocol::mcp::CallToolResult;
use deepseek_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use deepseek_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use deepseek_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use deepseek_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use deepseek_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use deepseek_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use deepseek_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use deepseek_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use deepseek_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use deepseek_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use deepseek_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use deepseek_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use deepseek_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use deepseek_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
use deepseek_protocol::request_user_input::RequestUserInputAnswer;
use deepseek_protocol::request_user_input::RequestUserInputArgs;
use deepseek_protocol::request_user_input::RequestUserInputQuestion;
use deepseek_protocol::request_user_input::RequestUserInputQuestionOption;
...
use deepseek_utils_output_truncation::TruncationPolicy;
use deepseek_utils_output_truncation::truncate_text;
...
async fn handle_approved_mcp_tool_call(
    ...
    let rewrite = rewrite_mcp_tool_arguments_for_deepseek_files(
        sess,
        turn_context,
        arguments_value.clone(),
        metadata.and_then(|metadata| metadata.deepseek_file_input_params.as_deref()),
    )
    .await;
...
async fn maybe_request_codex_apps_auth_elicitation(
    ...
    let install_url = connector_id.map(|connector_id| {
        deepseek_connectors::metadata::connector_install_url(
...
async fn refresh_codex_apps_after_connector_auth(
    ...
    match mcp_tools_result {
        ...
        Err(err) => {
            tracing::warn!("failed to refresh DeepSeek Apps tools after connector auth: {err:#}");
        }
    }
}
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let server_environment_id = manager
        ...
        .unwrap_or(deepseek_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID);
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        permission_profile,
        deepseek_linux_sandbox_exe: step_context.mcp.config().deepseek_linux_sandbox_exe.clone(),
        sandbox_cwd,
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                deepseek_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(step_context: &StepContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == deepseek_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    deepseek_file_input_params: Option<Vec<String>>,
}

const MCP_TOOL_OPENAI_OUTPUT_TEMPLATE_META_KEY: &str = "deepseek/outputTemplate";
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, deepseek_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__deepseek_mcp_decline__";
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    Some(McpToolApprovalMetadata {
        ...
        deepseek_file_input_params: deepseek_file_input_params_for_server(
            server,
...
fn deepseek_file_input_params_for_server(
    server: &str,
    meta: Option<&serde_json::Map<String, serde_json::Value>>,
) -> Option<Vec<String>> {
    (server == CODEX_APPS_MCP_SERVER_NAME)
        .then_some(declared_deepseek_file_input_param_names(meta))
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, deepseek_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
