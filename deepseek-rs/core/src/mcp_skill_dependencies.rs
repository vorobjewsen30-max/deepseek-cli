...
use deepseek_config::ConfigEditsBuilder;
use deepseek_config::McpServerConfig;
use deepseek_config::McpServerTransportConfig;
use deepseek_config::load_global_mcp_servers;
use deepseek_login::default_client::is_first_party_originator;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(deepseek_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&deepseek_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&deepseek_home)
