...
use deepseek_analytics::TurnProfileFact;
use deepseek_analytics::TurnTokenUsageFact;
use deepseek_login::AuthManager;
use deepseek_models_manager::manager::SharedModelsManager;
use deepseek_otel::SessionTelemetry;
use deepseek_otel::TURN_E2E_DURATION_METRIC;
use deepseek_otel::TURN_MEMORY_METRIC;
use deepseek_otel::TURN_NETWORK_PROXY_METRIC;
use deepseek_otel::TURN_TOKEN_USAGE_METRIC;
use deepseek_otel::TURN_TOOL_CALL_METRIC;
...
pub(crate) type SessionTaskResult = DeepSeekResult<Option<String>>;
...
///
/// Implementations encapsulate a specific DeepSeek workflow (regular chat,
...
pub(crate) trait SessionTask: Send + Sync + 'static {
    ...
    /// [`DeepSeekErr::TurnAborted`] completes the task through the aborted-turn
    /// lifecycle instead.
    fn run(
        self: Arc<Self>,
...
impl Session {
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; DeepSeek will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
    ...
    pub async fn on_task_finished(
        ...
        let (last_agent_message, abort_reason) = match task_result {
            ...
            Err(DeepSeekErr::TurnAborted) => (None, Some(TurnAbortReason::Interrupted)),
            Err(err) => {
