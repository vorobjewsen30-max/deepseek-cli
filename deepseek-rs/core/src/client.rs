...
//!
//! `ModelClient` is intended to live for the lifetime of a DeepSeek session and holds the stable
...
use deepseek_api::AgentIdentityTelemetry;
use deepseek_api::ApiError;
use deepseek_api::AuthProvider;
use deepseek_api::CompactClient as ApiCompactClient;
use deepseek_api::CompactionInput as ApiCompactionInput;
use deepseek_api::Compression;
use deepseek_api::MemoriesClient as ApiMemoriesClient;
use deepseek_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use deepseek_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use deepseek_api::Provider as ApiProvider;
use deepseek_api::RawMemory as ApiRawMemory;
use deepseek_api::RealtimeCallClient as ApiRealtimeCallClient;
use deepseek_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use deepseek_api::Reasoning;
use deepseek_api::ReasoningContext;
use deepseek_api::RequestTelemetry;
use deepseek_api::ReqwestTransport;
use deepseek_api::ResponseCreateWsRequest;
use deepseek_api::ResponsesApiRequest;
use deepseek_api::ResponsesClient as ApiResponsesClient;
use deepseek_api::ResponsesOptions as ApiResponsesOptions;
use deepseek_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use deepseek_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use deepseek_api::ResponsesWsRequest;
use deepseek_api::SharedAuthProvider;
use deepseek_api::SseTelemetry;
use deepseek_api::TransportError;
use deepseek_api::WebsocketTelemetry;
use deepseek_api::auth_header_telemetry;
use deepseek_api::build_session_headers;
use deepseek_api::create_text_param_for_request;
use deepseek_api::response_create_client_metadata;
use deepseek_login::AuthManager;
use deepseek_login::DeepSeekAuth;
use deepseek_login::RefreshTokenError;
use deepseek_login::UnauthorizedRecovery;
...
use deepseek_protocol::config_types::Verbosity as VerbosityConfig;
...
use deepseek_protocol::protocol::InternalSessionSource;
use deepseek_protocol::protocol::SessionSource;
use deepseek_protocol::protocol::W3cTraceContext;
use deepseek_rollout_trace::CompactionTraceContext;
use deepseek_rollout_trace::InferenceTraceAttempt;
use deepseek_rollout_trace::InferenceTraceContext;
use deepseek_tools::create_tools_json_for_responses_api;
...
use deepseek_feedback::FeedbackRequestTags;
use deepseek_feedback::emit_feedback_request_tags_with_auth_env;
use deepseek_login::auth::AgentIdentityAuthPolicy;
use deepseek_login::auth_env_telemetry::AuthEnvTelemetry;
use deepseek_login::auth_env_telemetry::collect_auth_env_telemetry;
use deepseek_model_provider::AgentIdentitySessionFallback;
use deepseek_model_provider::ProviderAuthScope;
use deepseek_model_provider::SharedModelProvider;
use deepseek_model_provider::create_model_provider;
#[cfg(test)]
use deepseek_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
...
use deepseek_protocol::error::Result;
use deepseek_response_debug_context::extract_response_debug_context;
use deepseek_response_debug_context::extract_response_debug_context_from_api_error;
use deepseek_response_debug_context::telemetry_api_error_message;
use deepseek_response_debug_context::telemetry_transport_error_message;

pub const OPENAI_BETA_HEADER: &str = "DeepSeek-Beta";
...
pub const X_OPENAI_MEMGEN_REQUEST_HEADER: &str = "x-deepseek-memgen-request";
pub const X_OPENAI_SUBAGENT_HEADER: &str = "x-deepseek-subagent";
pub const X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER: &str =
    "x-responsesapi-include-timing-metrics";
const X_CODEX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
    "x-codex-ws-stream-request-start-ms";
const WS_REQUEST_HEADER_RESPONSES_LITE_CLIENT_METADATA_KEY: &str =
    "ws_request_header_x_deepseek_internal_codex_responses_lite";
const RESPONSES_WEBSOCKETS_V2_BETA_HEADER_VALUE: &str = "responses_websockets=2026-02-06";
const X_OPENAI_INTERNAL_CODEX_RESPONSES_LITE_HEADER: &str =
    "x-deepseek-internal-codex-responses-lite";
...
///
/// This holds configuration and state that should be shared across turns within a DeepSeek session
...
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each DeepSeek turn. Reusing it across turns would replay
...
///
/// API-key sessions send that API bearer. DeepSeek-auth sessions send their bearer plus account id;
/// transceiver is responsible for accepting that same call-create identity on the direct
/// `api.platform.deepseek.com` sideband path.
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a DeepSeek session. Per-turn values
    ...
    pub fn new(
        ...
        let deepseek_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.deepseek_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), deepseek_api_key_env_enabled);
    ...
    pub(crate) async fn compact_conversation_history(
        ...
        responses_metadata: &DeepSeekResponsesMetadata,
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(DeepSeekAuth::auth_mode),
    ...
    fn build_responses_request(
        &self,
        provider: &deepseek_api::Provider,
        prompt: &Prompt,
        model_info: &ModelInfo,
        effort: Option<ReasoningEffortConfig>,
        summary: ReasoningSummaryConfig,
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: deepseek_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &DeepSeekResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth).connect(
                headers,
                deepseek_login::default_client::default_headers(),
                /*turn_state*/ None,
...
impl ModelClientSession {
    ...
    fn responses_request_compression(&self, auth: Option<&DeepSeekAuth>) -> Compression {
        if self.client.state.enable_request_compression
            && auth.is_some_and(DeepSeekAuth::uses_codex_backend)
            && self.client.state.provider.info().is_deepseek()
        {
            Compression::Zstd
        } else {
            Compression::None
    ...
    /// Streams a turn via the DeepSeek Responses API.
    ///
    ...
    /// Permanently disables WebSockets for this DeepSeek session and resets WebSocket state.
    ///
...
///
/// These headers implement DeepSeek-specific conventions:
///
...
pub(crate) fn add_originator_header(headers: &mut ApiHeaderMap, originator: &str) {
    let default_originator = deepseek_login::default_client::originator();
...
fn map_response_stream(
    api_stream: deepseek_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let deepseek_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = deepseek_api::ResponseStream {
        rx_event,
...
/// Handles a 401 response by optionally refreshing DeepSeek tokens once.
///
/// When refresh succeeds, the caller should retry the API call; otherwise
/// the mapped `DeepSeekErr` is returned to the caller.
...
struct WebsocketConnectParams<'a> {
    ...
    api_provider: deepseek_api::Provider,
    api_auth: SharedAuthProvider,
    responses_metadata: &'a DeepSeekResponsesMetadata,
    auth_context: AuthRequestTelemetryContext,
    request_route_telemetry: RequestRouteTelemetry,
}
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(DeepSeekErr::RefreshTokenFailed(failed))
            }
