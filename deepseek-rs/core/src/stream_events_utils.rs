...
use deepseek_memories_read::citations::parse_memory_citation;
use deepseek_memories_read::citations::thread_ids_from_memory_citation;
...
use deepseek_utils_stream_parser::strip_proposed_plan_blocks;
use futures::Future;
use tracing::debug;
...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<deepseek_protocol::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                deepseek_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
...
async fn save_image_generation_result(
    ...
    let bytes = BASE64_STANDARD
        ...
        .map_err(|err| {
            DeepSeekErr::InvalidRequest(format!("invalid image generation payload: {err}"))
        })?;
    let path = image_generation_artifact_path(deepseek_home, session_id, call_id);
    ...
    .await
    {
        ...
        Err(err) => {
            ...
            let output_dir = output_path
                .parent()
                .unwrap_or_else(|| turn_context.config.deepseek_home.clone());
            tracing::warn!(
...
async fn record_image_generation_instructions(
    ...
    let image_output_path =
        image_generation_artifact_path(&turn_context.config.deepseek_home, &session_id, "<image_id>");
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(item.clone()) {
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(DeepSeekErr::Fatal(message));
        }
    }
...
pub(crate) async fn finalize_turn_item(
    ...
    if let TurnItem::AgentMessage(agent_message) = &mut *turn_item {
        ...
        agent_message.content =
            vec![deepseek_protocol::items::AgentMessageContent::Text { text: stripped }];
