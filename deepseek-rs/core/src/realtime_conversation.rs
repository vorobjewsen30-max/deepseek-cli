...
use deepseek_api::RealtimeAudioFrame;
use deepseek_api::RealtimeEvent;
use deepseek_api::RealtimeEventParser;
use deepseek_api::RealtimeSessionConfig;
use deepseek_api::RealtimeSessionMode;
use deepseek_api::RealtimeWebsocketClient;
use deepseek_api::RealtimeWebsocketEvents;
use deepseek_api::RealtimeWebsocketWriter;
use deepseek_api::map_api_error;
...
use deepseek_login::read_deepseek_api_key_from_env;
...
use deepseek_protocol::protocol::RealtimeConversationClosedEvent;
use deepseek_protocol::protocol::RealtimeConversationRealtimeEvent;
use deepseek_protocol::protocol::RealtimeConversationSdpEvent;
use deepseek_protocol::protocol::RealtimeConversationStartedEvent;
use deepseek_protocol::protocol::RealtimeHandoffRequested;
use deepseek_protocol::protocol::RealtimeOutputModality;
...
impl RealtimeConversationManager {
    ...
    async fn start(&self, start: RealtimeStart) -> DeepSeekResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> DeepSeekResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            extra_headers,
            client_managed_handoffs,
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> DeepSeekResult<()> {
        let sender = {
        ...
        let Some(sender) = sender else {
            return Err(DeepSeekErr::InvalidRequest(
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(DeepSeekErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, mut params: ConversationTextParams) -> DeepSeekResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(params)
            .await
            .map_err(|_| DeepSeekErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }
    ...
    pub(crate) async fn append_speech(&self, text: String) -> DeepSeekResult<()> {
    ...
    pub(crate) async fn handoff_complete(&self) -> DeepSeekResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn shutdown(&self) -> DeepSeekResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> DeepSeekResult<PreparedRealtimeConversationStart> {
...
) -> DeepSeekResult<RealtimeSessionConfig> {
    let config = sess.get_config().await;
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> DeepSeekResult<()> {
    let voices = RealtimeVoicesList::builtin();
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), DeepSeekErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_api_key(auth: Option<&DeepSeekAuth>, provider: &ModelProviderInfo) -> DeepSeekResult<String> {
    ...
    if let Some(api_key) = auth.and_then(DeepSeekAuth::api_key) {
        return Ok(api_key.to_string());
    }

    // TODO(aibrahim): Remove this temporary fallback once realtime auth no longer
    // requires API key auth for DeepSeek/SIWC sessions.
    if provider.is_deepseek()
        && let Some(api_key) = read_deepseek_api_key_from_env()
    {
        return Ok(api_key);
    }

    Err(DeepSeekErr::InvalidRequest(
...
fn realtime_request_headers(
    ...
    version: RealtimeWsVersion,
    originator: &str,
) -> DeepSeekResult<Option<HeaderMap>> {
    let mut headers = HeaderMap::new();

    if version == RealtimeWsVersion::V1 {
        headers.insert("deepseek-alpha", HeaderValue::from_static("quicksilver=v1"));
    }
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            DeepSeekErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    codex_error_info: DeepSeekErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
