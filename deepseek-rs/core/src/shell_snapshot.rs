...
impl ShellSnapshot {
    ...
    async fn try_create(
        ...
        let temp_path = deepseek_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.tmp-{nonce}"));

        // Clean the (unlikely) leaked snapshot files.
        let deepseek_home = deepseek_home.clone();
        let cleanup_session_id = session_id;
        tokio::spawn(async move {
            if let Err(err) =
                cleanup_stale_snapshots(&deepseek_home, cleanup_session_id, state_db).await
            {
...
pub async fn cleanup_stale_snapshots(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path =
            find_thread_path_by_id_str(deepseek_home, session_id, state_db.as_deref()).await?;
