...
impl deepseek_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: deepseek_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> deepseek_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<deepseek_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> deepseek_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<deepseek_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> deepseek_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
async fn shutdown_all_threads_bounded_submits_shutdown_to_every_thread() {
    ...
    config.deepseek_home = temp_dir.path().join("deepseek-home").abs();
    config.cwd = config.deepseek_home.abs();
    std::fs::create_dir_all(&config.deepseek_home).expect("create codex home");

    let manager = ThreadManager::with_models_provider_and_home_for_tests(
        DeepSeekAuth::from_api_key("dummy"),
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl deepseek_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
    ...
    impl deepseek_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        ...
        fn contribute<'a>(
            &'a self,
            context: deepseek_extension_api::McpServerContributionContext<'a, Config>,
        ) -> deepseek_extension_api::ExtensionFuture<'a, Vec<deepseek_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                let mut server = deepseek_mcp::codex_apps_mcp_server_config(
                ...
                vec![deepseek_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let mut extensions = deepseek_extension_api::ExtensionRegistryBuilder::new();
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = deepseek_extension_api::ExtensionDataInit::new();
    ...
    let selected_servers = |config: &deepseek_mcp::McpConfig| {
        deepseek_mcp::configured_mcp_servers(config)
            .into_iter()
...
#[tokio::test]
async fn explicit_installation_id_skips_deepseek_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.deepseek_home = temp_dir.path().join("deepseek-home").abs();
    config.cwd = config.deepseek_home.abs();
    ...
    assert!(!config.deepseek_home.join(INSTALLATION_ID_FILENAME).exists());
