...
/// `deepseek_feedback::DeepSeekFeedback::metadata_layer()` is installed, these fields are captured and
...
///
/// ```rust
/// deepseek_core::feedback_tags!(model = "gpt-5", cached = true);
/// deepseek_core::feedback_tags!(provider = provider_id, request_id = request_id);
/// ```
#[macro_export]
