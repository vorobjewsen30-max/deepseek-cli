...
use deepseek_thread_store::ArchiveThreadParams;
...
impl AgentControlHarness {
    ...
    async fn new_with_config(home: TempDir, config: Config) -> Self {
        ...
        let manager = ThreadManager::with_models_provider_home_and_state_for_tests(
            ...
            std::sync::Arc::new(deepseek_exec_server::EnvironmentManager::default_for_tests()),
            state_db.clone(),
        );
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<DeepSeekThread>) {
        let new_thread = self
            .manager
...
async fn persisted_originator(thread: &DeepSeekThread) -> String {
...
async fn wait_for_subagent_notification(parent_thread: &Arc<DeepSeekThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<DeepSeekThread>, message: &str) {
    thread
...
async fn assert_thread_not_loaded(manager: &ThreadManager, thread_id: ThreadId) {
    match manager.get_thread(thread_id).await {
        Err(DeepSeekErr::ThreadNotFound(id)) => assert_eq!(id, thread_id),
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(err, DeepSeekErr::ThreadNotFound(id) if id == thread_id);
}
...
async fn ensure_v2_agent_loaded_reloads_registered_unloaded_agent() {
    ...
    match harness.manager.get_thread(spawned_agent.thread_id).await {
        Err(DeepSeekErr::ThreadNotFound(id)) => assert_eq!(id, spawned_agent.thread_id),
        Err(err) => panic!("expected ThreadNotFound, got {err:?}"),
...
async fn resume_agent_from_rollout_does_not_reopen_v2_descendants() {
    ...
    let resumed_manager = ThreadManager::with_models_provider_home_and_state_for_tests(
        ...
        harness.config.deepseek_home.to_path_buf(),
        std::sync::Arc::new(deepseek_exec_server::EnvironmentManager::default_for_tests()),
...
async fn spawn_agent_respects_max_threads_limit() {
    ...
    let DeepSeekErr::AgentLimitReached {
        max_threads: seen_max_threads,
    } = err
    else {
        panic!("expected DeepSeekErr::AgentLimitReached");
    };
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let DeepSeekErr::AgentLimitReached { max_threads } = err else {
        panic!("expected DeepSeekErr::AgentLimitReached");
    };
