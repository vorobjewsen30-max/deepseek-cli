...
impl AgentControl {
    ...
    pub(crate) async fn ensure_v2_agent_loaded(
        ...
        if self.state.agent_metadata_for_thread(thread_id).is_none() {
            return Err(DeepSeekErr::ThreadNotFound(thread_id));
        }
        ...
        let history = stored_thread
            .history
            .ok_or(DeepSeekErr::ThreadNotFound(thread_id))?
            .items;
        ...
        while let Some((parent_thread_id, parent_depth)) = resume_queue.pop_front() {
            let child_ids = match agent_graph_store
                .list_thread_spawn_children(
                    parent_thread_id,
                    Some(deepseek_agent_graph_store::ThreadSpawnEdgeStatus::Open),
                )
                .await
            {
    ...
    async fn resume_single_agent_from_rollout(
        ...
        thread_id: ThreadId,
        session_source: SessionSource,
    ) -> DeepSeekResult<(ThreadId, MultiAgentVersion)> {
        let state = self.upgrade()?;
        let stored_thread = state
            .read_stored_thread(ReadThreadParams {
                thread_id,
        ...
        let resumed_agent_path = stored_thread
            ...
            .transpose()
            .map_err(|err| DeepSeekErr::InvalidRequest(format!("invalid stored agent path: {err}")))?;
        let resumed_agent_nickname = stored_thread.agent_nickname.clone();
        let resumed_agent_role = stored_thread.agent_role.clone();
        let history = stored_thread
            .history
            .ok_or_else(|| DeepSeekErr::ThreadNotFound(thread_id))?
            .items;
        let initial_history = InitialHistory::Resumed(ResumedHistory {
            conversation_id: thread_id,
            history: Arc::new(history),
