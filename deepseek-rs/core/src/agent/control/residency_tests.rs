...
async fn residency_slot_reservation_unloads_oldest_idle_v2_agent() {
    ...
    config.deepseek_home = temp_home.path().to_path_buf().try_into().unwrap();
    ...
    match manager.get_thread(first.thread_id).await {
        Err(DeepSeekErr::ThreadNotFound(thread_id)) => assert_eq!(thread_id, first.thread_id),
...
async fn interrupted_v2_agent_is_lost_after_residency_eviction() {
    ...
    match err {
        DeepSeekErr::ThreadNotFound(thread_id) => assert_eq!(thread_id, first.thread_id),
...
async fn mark_thread_completed(thread: &DeepSeekThread) {
...
async fn mark_thread_interrupted(thread: &DeepSeekThread) {
    let turn = thread.codex.session.new_default_turn().await;
    thread
        .codex
        .session
...
async fn clear_active_turn(thread: &DeepSeekThread) {
