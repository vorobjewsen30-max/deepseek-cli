...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> DeepSeekThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> DeepSeekResult<String> {
        let state = self.upgrade()?;
    ...
    async fn handle_thread_request_result(
        ...
        if matches!(result, Err(DeepSeekErr::InternalAgentDied)) {
    ...
    pub(crate) fn ensure_agent_known(&self, agent_id: ThreadId) -> DeepSeekResult<AgentMetadata> {
        self.state
            .agent_metadata_for_thread(agent_id)
            .ok_or(DeepSeekErr::ThreadNotFound(agent_id))
    }
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        Err(DeepSeekErr::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> DeepSeekResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
    ...
    ) -> DeepSeekResult<Vec<ListedAgent>> {
        let state = self.upgrade()?;
    ...
    ) -> DeepSeekResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> DeepSeekResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| DeepSeekErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> DeepSeekResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> DeepSeekResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
    ...
    async fn persist_thread_spawn_edge_for_source(
        &self,
        child_thread: &crate::DeepSeekThread,
        child_thread_id: ThreadId,
        session_source: Option<&SessionSource>,
    ) {
        ...
        if let Err(err) = agent_graph_store
            .upsert_thread_spawn_edge(
                parent_thread_id,
                child_thread_id,
                deepseek_agent_graph_store::ThreadSpawnEdgeStatus::Open,
            )
            .await
        {
