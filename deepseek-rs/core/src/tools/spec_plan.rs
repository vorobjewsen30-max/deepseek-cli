...
use deepseek_protocol::deepseek_models::ToolMode;
...
use deepseek_tools::ResponsesApiNamespace;
use deepseek_tools::ResponsesApiNamespaceTool;
use deepseek_tools::TOOL_SEARCH_TOOL_NAME;
use deepseek_tools::ToolCall as ExtensionToolCall;
use deepseek_tools::ToolEnvironmentMode;
use deepseek_tools::ToolExecutor;
...
use deepseek_tools::can_request_original_image_detail;
use deepseek_tools::collect_code_mode_exec_prompt_tool_definitions;
use deepseek_tools::collect_request_plugin_install_entries;
use deepseek_tools::default_namespace_description;
use deepseek_tools::request_user_input_available_modes;
use deepseek_tools::shell_command_backend_for_features;
use deepseek_tools::shell_type_for_model_and_features;
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && deepseek_code_mode::is_code_mode_nested_tool(spec.name())
    {
        deepseek_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
...
fn image_generation_runtime_enabled(turn_context: &TurnContext) -> bool {
    (turn_context
        .provider
        .info()
        .uses_deepseek_actor_authorization()
        || (turn_context.provider.info().requires_deepseek_auth
            && turn_context
                .auth_manager
                .as_deref()
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure != ToolExposure::DirectModelOnly
        && deepseek_code_mode::is_code_mode_nested_tool(&deepseek_tools::code_mode_name_for_tool_name(
            tool_name,
        ))
}
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, deepseek_code_mode::ToolNamespaceDescription> {
    ...
    for spec in specs {
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| deepseek_code_mode::ToolNamespaceDescription {
        ...
        match spec {
            DynamicToolSpec::Function(tool) => {
                let Some(handler) = DynamicToolHandler::new(tool) else {
                    tracing::error!(
                        "Failed to convert dynamic tool {:?} to DeepSeek tool",
                        tool.name
                    );
                    continue;
                };
...
fn append_extension_tool_executors(
    ...
    let tool_mode = effective_tool_mode(turn_context);
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly) {
        reserved_tool_names.insert(ToolName::plain(deepseek_code_mode::PUBLIC_TOOL_NAME));
        reserved_tool_names.insert(ToolName::plain(deepseek_code_mode::WAIT_TOOL_NAME));
    }
    if search_tool_enabled(turn_context)
        && planned_tools
            .runtimes()
...
fn compare_code_mode_tools(
    left: &deepseek_code_mode::ToolDefinition,
    right: &deepseek_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, deepseek_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &deepseek_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, deepseek_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
