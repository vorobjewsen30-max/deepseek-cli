...
#[cfg(target_os = "macos")]
use deepseek_network_proxy::CODEX_PROXY_GIT_SSH_COMMAND_MARKER;
use deepseek_network_proxy::CUSTOM_CA_ENV_KEYS;
...
#[cfg(target_os = "macos")]
use deepseek_network_proxy::PROXY_GIT_SSH_COMMAND_ENV_KEY;
...
async fn test_network_proxy() -> anyhow::Result<NetworkProxy> {
    let state = deepseek_network_proxy::build_config_state(
...
fn maybe_wrap_shell_lc_with_snapshot_does_not_embed_override_values_in_argv() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport DEEPSEEK_API_KEY='snapshot-value'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printf '%s' \"$DEEPSEEK_API_KEY\"".to_string(),
    ];
