...
use deepseek_shell_command::powershell::prefix_powershell_script_with_utf8;
    ...
    async fn run(
        ...
        let (env, managed_network_context) = match managed_network {
            Some(network) => {
                let prepared = network
                    ...
                    .map_err(|err| {
                        ToolError::DeepSeek(DeepSeekErr::Io(io::Error::other(format!(
        ...
        if let UnifiedExecShellMode::ZshFork(zsh_fork_config) = &self.shell_mode {
            ...
            .map_err(|error| match error {
                ...
                error @ ToolError::DeepSeek(_) => error,
            })?;
...
mod tests {
    ...
    use deepseek_tools::ZshForkConfig;
