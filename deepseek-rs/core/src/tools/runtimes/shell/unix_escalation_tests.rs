...
use deepseek_execpolicy::PolicyParser;
use deepseek_execpolicy::RuleMatch;
use deepseek_hooks::Hooks;
use deepseek_hooks::HooksConfig;
use deepseek_network_proxy::PROXY_ACTIVE_ENV_KEY;
use deepseek_network_proxy::PROXY_ENV_KEYS;
...
use deepseek_sandboxing::policy_transforms::effective_permission_profile;
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(deepseek_execpolicy::Policy::empty())),
        ...
        stopwatch: deepseek_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = deepseek_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = deepseek_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    let config_toml_path = turn_context
        .config
        .deepseek_home
        .join(deepseek_config::CONFIG_TOML_FILE);
    ...
    let expected_hook_command =
        deepseek_shell_command::parse_command::shlex_join(&["/usr/bin/touch".to_string(), target_str]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(deepseek_execpolicy::Policy::empty())),
    ...
    let action = tokio::time::timeout(
        ...
        deepseek_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        deepseek_shell_escalation::EscalationDecision::Escalate(
            deepseek_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, deepseek_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        deepseek_shell_escalation::EscalationDecision::Deny {
