...
use deepseek_execpolicy::MatchOptions;
...
use deepseek_protocol::protocol::NetworkPolicyRuleAction;
...
use deepseek_shell_command::bash::parse_shell_lc_plain_commands;
use deepseek_shell_command::bash::parse_shell_lc_single_command_prefix;
use deepseek_shell_escalation::EscalateServer;
use deepseek_shell_escalation::EscalationDecision;
use deepseek_shell_escalation::EscalationExecution;
use deepseek_shell_escalation::EscalationPermissions;
use deepseek_shell_escalation::EscalationPolicy;
use deepseek_shell_escalation::EscalationPolicyFuture;
use deepseek_shell_escalation::EscalationSession;
use deepseek_shell_escalation::ExecParams;
use deepseek_shell_escalation::ExecResult;
use deepseek_shell_escalation::PreparedExec;
use deepseek_shell_escalation::ResolvedPermissionProfile;
use deepseek_shell_escalation::ShellCommandExecutor;
use deepseek_shell_escalation::ShellCommandExecutorFuture;
use deepseek_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        sandbox_policy_cwd,
        windows_sandbox_workspace_roots,
        deepseek_linux_sandbox_exe: ctx.turn.config.deepseek_linux_sandbox_exe.clone(),
        ...
        Ok(stopwatch
            .pause_for(async move {
                ...
                let permission_request = PermissionRequestPayload::bash(
                    deepseek_shell_command::parse_command::shlex_join(&command),
                    /*description*/ None,
                );
...
struct CoreShellCommandExecutor {
    ...
    network: Option<deepseek_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            deepseek_linux_sandbox_exe: self.deepseek_linux_sandbox_exe.as_deref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::DeepSeek(DeepSeekErr::Sandbox(SandboxErr::Timeout {
            output: Box::new(output),
        })));
    }

    if is_likely_sandbox_denied(sandbox, &output) {
        return Err(ToolError::DeepSeek(DeepSeekErr::Sandbox(SandboxErr::Denied {
            output: Box::new(output),
