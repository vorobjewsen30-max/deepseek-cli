...
use deepseek_protocol::deepseek_models::ApplyPatchToolType;
...
use deepseek_protocol::deepseek_models::WebSearchToolType;
...
fn zsh_fork_config_for_spec_plan_tests() -> deepseek_tools::ZshForkConfig {
    let placeholder_exe = deepseek_utils_absolute_path::AbsolutePathBuf::try_from(
...
fn use_actor_authorized_provider(turn: &mut TurnContext) {
    ...
    provider_info.requires_deepseek_auth = false;
    provider_info.http_headers = Some(HashMap::from([
        (
            "x-deepseek-actor-authorization".to_string(),
            "test-actor-authorization".to_string(),
        ),
        (
            "DeepSeek-Account-ID".to_string(),
...
impl ToolExecutor<ExtensionToolCall> for WebRunExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(deepseek_tools::ResponsesApiNamespace {
            name: "web".to_string(),
    ...
    fn handle(&self, _call: ExtensionToolCall) -> deepseek_tools::ToolExecutorFuture<'_> {
        Box::pin(async {
            Ok(Box::new(deepseek_tools::JsonToolOutput::new(json!({}))) as Box<dyn ToolOutput>)
        })
    }
}
...
impl ToolExecutor<ExtensionToolCall> for DeferredExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Function(ResponsesApiTool {
            ...
            strict: true,
            defer_loading: None,
            parameters: deepseek_tools::JsonSchema::object(
                BTreeMap::from([(
                    "message".to_string(),
                    deepseek_tools::JsonSchema::string(/*description*/ None),
                )]),
...
fn dynamic_tool(namespace: Option<&str>, name: &str, defer_loading: bool) -> DynamicToolSpec {
    let function = deepseek_protocol::dynamic_tools::DynamicToolFunctionSpec {
        name: name.to_string(),
    ...
    match namespace {
        Some(namespace) => {
            DynamicToolSpec::Namespace(deepseek_protocol::dynamic_tools::DynamicToolNamespaceSpec {
                name: namespace.to_string(),
                description: format!("{namespace} dynamic tools"),
                tools: vec![
                    deepseek_protocol::dynamic_tools::DynamicToolNamespaceTool::Function(function),
                ],
            })
        }
...
fn plugin_candidates(presentation: ToolSuggestPresentation) -> ToolSuggestCandidates {
    ToolSuggestCandidates {
        tools: vec![DiscoverableTool::Plugin(Box::new(DiscoverablePluginInfo {
            id: "github@deepseek-curated-remote".to_string(),
...
async fn request_user_input_stays_direct_in_code_mode_only() {
    ...
    plan.assert_visible_contains(&[
        "request_user_input",
        deepseek_code_mode::PUBLIC_TOOL_NAME,
        deepseek_code_mode::WAIT_TOOL_NAME,
    ]);
    ...
    let ToolSpec::Freeform(exec) = plan.visible_spec(deepseek_code_mode::PUBLIC_TOOL_NAME) else {
...
async fn shell_zsh_fork_stays_standalone_until_unified_exec_composition_is_enabled() {
    ...
    if deepseek_utils_pty::conpty_supported() {
...
async fn zsh_fork_unified_exec_hides_shell_parameter() {
    ...
    let plan = probe(|turn| {
        ...
        turn.unified_exec_shell_mode =
            deepseek_tools::UnifiedExecShellMode::ZshFork(zsh_fork_config_for_spec_plan_tests());
    })
    .await;
    ...
    let plan = probe(|turn| {
        ...
        turn.environments.turn_environments.push(
            crate::session::turn_context::TurnEnvironment::new(
                "remote".to_string(),
                Arc::new(
                    deepseek_exec_server::Environment::create_for_tests(Some(
