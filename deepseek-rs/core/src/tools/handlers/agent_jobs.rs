...
fn required_state_db(
    session: &Arc<Session>,
) -> Result<Arc<deepseek_state::StateRuntime>, FunctionCallError> {
...
async fn run_agent_job_loop(
    session: Arc<Session>,
    turn: Arc<TurnContext>,
    db: Arc<deepseek_state::StateRuntime>,
    job_id: String,
        ...
        if !cancel_requested && active_items.len() < options.max_concurrency {
            ...
            let pending_items = db
                .list_agent_job_items(
                    job_id.as_str(),
                    Some(deepseek_state::AgentJobItemStatus::Pending),
                    Some(slots),
                )
                .await?;
            for item in pending_items {
                    ...
                    .await
                {
                    ...
                    Err(DeepSeekErr::AgentLimitReached { .. }) => {
...
async fn recover_running_items(
    ...
    let running_items = db
        .list_agent_job_items(
            job_id,
            Some(deepseek_state::AgentJobItemStatus::Running),
            /*limit*/ None,
        )
        .await?;
...
async fn finalize_finished_item(
    ...
    if matches!(item.status, deepseek_state::AgentJobItemStatus::Running) {
...
fn build_worker_prompt(
    job: &deepseek_state::AgentJob,
    item: &deepseek_state::AgentJobItem,
...
fn job_runtime_timeout(job: &deepseek_state::AgentJob) -> Duration {
    job.max_runtime_seconds
        .map(Duration::from_secs)
        .unwrap_or(DEFAULT_AGENT_JOB_ITEM_TIMEOUT)
}

fn started_at_from_item(item: &deepseek_state::AgentJobItem) -> Instant {
...
fn is_item_stale(item: &deepseek_state::AgentJobItem, runtime_timeout: Duration) -> bool {
    let now = chrono::Utc::now();
...
fn render_job_csv(
    headers: &[String],
    items: &[deepseek_state::AgentJobItem],
