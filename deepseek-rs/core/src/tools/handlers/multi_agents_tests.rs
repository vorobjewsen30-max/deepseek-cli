...
use deepseek_protocol::protocol::FileSystemAccessMode;
use deepseek_protocol::protocol::FileSystemPath;
use deepseek_protocol::protocol::FileSystemSandboxEntry;
use deepseek_protocol::protocol::FileSystemSandboxPolicy;
use deepseek_protocol::protocol::InitialHistory;
use deepseek_protocol::protocol::InterAgentCommunication;
use deepseek_protocol::protocol::NetworkSandboxPolicy;
...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.deepseek_home)
        .await
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    deepseek_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn spawn_agent_uses_explorer_role_and_preserves_approval_policy() {
    ...
    let mut config = (*turn.config).clone();
    let provider_info =
        built_in_model_providers(/* deepseek_base_url */ /*deepseek_base_url*/ None)["ollama"].clone();
...
async fn multi_agent_v2_spawn_fork_turns_all_rejects_agent_type_override() {
    ...
    let turn = TurnContext {
        ...
        multi_agent_version: deepseek_protocol::protocol::MultiAgentVersion::V2,
        ..turn
    };
...
async fn spawn_agent_role_service_tier_falls_back_to_supported_parent_tier() {
    ...
    tokio::fs::create_dir_all(&turn.config.deepseek_home)
        .await
        .expect("codex home should be created");
    let role_config_path = turn.config.deepseek_home.as_path().join("tiered-role.toml");
    tokio::fs::write(
        &role_config_path,
        r#"model = "gpt-5.4"
service_tier = "turbo"
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    config.deepseek_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    turn.config = Arc::new(config);
