...
use deepseek_analytics::PluginInstallRequestSource;
use deepseek_analytics::PluginInstallRequested;
use deepseek_analytics::PluginInstallRequestedPlugin;
...
use deepseek_tools::DiscoverableToolAction;
use deepseek_tools::DiscoverableToolType;
use deepseek_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use deepseek_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use deepseek_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use deepseek_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use deepseek_tools::RequestPluginInstallArgs;
use deepseek_tools::RequestPluginInstallResult;
use deepseek_tools::ToolName;
use deepseek_tools::ToolSpec;
use deepseek_tools::all_requested_connectors_picked_up;
use deepseek_tools::build_request_plugin_install_elicitation_request;
use deepseek_tools::filter_request_plugin_install_discoverable_tools_for_client;
use deepseek_tools::verified_connector_install_completed;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        if (requested_tool_type == Some(DiscoverableToolType::Plugin)
            ...
            && turn.app_server_client_name.as_deref() == Some("deepseek-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in deepseek-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.deepseek_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    deepseek_home: &deepseek_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    session: &crate::session::session::Session,
    turn: &crate::session::turn_context::TurnContext,
    manager: &deepseek_mcp::McpConnectionManager,
    tool: &DiscoverableTool,
    auth: Option<&deepseek_login::DeepSeekAuth>,
) -> bool {
    match tool {
