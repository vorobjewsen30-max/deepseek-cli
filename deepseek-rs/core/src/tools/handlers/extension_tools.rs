...
use deepseek_tools::ConversationHistory;
use deepseek_tools::ExtensionTurnItem;
use deepseek_tools::ToolCall as ExtensionToolCall;
use deepseek_tools::ToolEnvironment;
use deepseek_tools::ToolName;
use deepseek_tools::ToolSearchInfo;
use deepseek_tools::ToolSpec;
use deepseek_tools::TurnItemEmissionFuture;
use deepseek_tools::TurnItemEmitter;
...
pub(crate) struct ExtensionToolAdapter(Arc<dyn deepseek_tools::ToolExecutor<ExtensionToolCall>>);

impl ExtensionToolAdapter {
    pub(crate) fn new(executor: Arc<dyn deepseek_tools::ToolExecutor<ExtensionToolCall>>) -> Self {
        Self(executor)
    }
}
...
impl ToolExecutor<ToolInvocation> for ExtensionToolAdapter {
    ...
    fn handle(&self, invocation: ToolInvocation) -> deepseek_tools::ToolExecutorFuture<'_> {
                ...
                ExtensionTurnItem::WebSearch(item) => {
                    ...
                    finalize_turn_item(
                        ...
                        turn.collaboration_mode.mode
                            == deepseek_protocol::config_types::ModeKind::Plan,
                    )
                    .await;
                    item
                }
...
mod tests {
    ...
    use deepseek_extension_api::TurnItemContributor;
    use deepseek_protocol::items::TurnItem;
    ...
    impl deepseek_extension_api::ToolExecutor<deepseek_tools::ToolCall> for StubExtensionExecutor {
        fn tool_name(&self) -> deepseek_tools::ToolName {
            deepseek_tools::ToolName::plain("extension_echo")
        }

        fn spec(&self) -> deepseek_tools::ToolSpec {
            deepseek_tools::ToolSpec::Function(deepseek_tools::ResponsesApiTool {
                name: "extension_echo".to_string(),
                description: "Echoes arguments.".to_string(),
                strict: true,
                parameters: deepseek_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle(&self, _call: deepseek_tools::ToolCall) -> deepseek_tools::ToolExecutorFuture<'_> {
            Box::pin(async {
                Ok(
                    Box::new(deepseek_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn deepseek_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<deepseek_tools::ToolCall>>>,
    }

    impl deepseek_extension_api::ToolExecutor<deepseek_tools::ToolCall> for CapturingExtensionExecutor {
        fn tool_name(&self) -> deepseek_tools::ToolName {
            deepseek_tools::ToolName::plain("extension_echo")
        }
        ...
        fn spec(&self) -> deepseek_tools::ToolSpec {
            deepseek_tools::ToolSpec::Function(deepseek_tools::ResponsesApiTool {
                ...
                strict: false,
                parameters: deepseek_tools::JsonSchema::default(),
                output_schema: None,
                defer_loading: None,
            })
        }

        fn handle(&self, call: deepseek_tools::ToolCall) -> deepseek_tools::ToolExecutorFuture<'_> {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: deepseek_tools::ToolCall,
        ) -> Result<Box<dyn deepseek_tools::ToolOutput>, deepseek_tools::FunctionCallError> {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: deepseek_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = deepseek_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    impl TurnItemContributor for RecordExtensionTurnItemContributor {
        ...
        ) -> deepseek_extension_api::ExtensionFuture<'a, Result<(), String>> {
            Box::pin(async move {
    ...
    async fn extension_completion_runs_turn_item_contributors() {
        ...
        deepseek_tools::TurnItemEmitter::emit_completed(
            &emitter,
    ...
    impl deepseek_extension_api::ToolExecutor<deepseek_tools::ToolCall> for ImageGenerationExtensionExecutor {
        fn tool_name(&self) -> deepseek_tools::ToolName {
            deepseek_tools::ToolName::namespaced("image_gen", "imagegen")
        }
        ...
        async fn handle_call(
            ...
            call.turn_item_emitter
                .emit_started(ExtensionTurnItem::ImageGeneration(
                    deepseek_protocol::items::ImageGenerationItem {
                        id: call.call_id.clone(),
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: deepseek_tools::ToolName::namespaced("image_gen", "imagegen"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: "{}".to_string(),
            },
