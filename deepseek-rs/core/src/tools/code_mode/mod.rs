...
use deepseek_code_mode::CellId;
use deepseek_code_mode::CodeModeNestedToolCall;
use deepseek_code_mode::CodeModeSession;
use deepseek_code_mode::CodeModeSessionProvider;
use deepseek_code_mode::CodeModeToolKind;
use deepseek_code_mode::RuntimeResponse;
...
use deepseek_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
use deepseek_utils_output_truncation::truncate_function_output_items_with_policy;
...
pub(crate) const PUBLIC_TOOL_NAME: &str = deepseek_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = deepseek_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = deepseek_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
impl CodeModeService {
    ...
    pub(crate) async fn execute(
        &self,
        request: deepseek_code_mode::ExecuteRequest,
    ) -> Result<deepseek_code_mode::StartedCell, String> {
        self.session().await?.execute(request).await
    }

    pub(crate) async fn wait(
        &self,
        request: deepseek_code_mode::WaitRequest,
    ) -> Result<deepseek_code_mode::WaitOutcome, String> {
    ...
    pub(crate) fn mark_cell_ready_for_dispatch(&self, cell_id: &deepseek_code_mode::CellId) {
...
mod tests {
    ...
    use crate::tools::context::ToolPayload;
    use deepseek_code_mode::CodeModeToolKind;
    use deepseek_code_mode::ExecuteRequest;
    ...
    async fn missing_process_host_is_reported_without_failing_service_creation() {
        let service = CodeModeService::new(Arc::new(
            ProcessOwnedCodeModeSessionProvider::with_host_program(
                "deepseek-code-mode-host-does-not-exist".into(),
            ),
        ));
