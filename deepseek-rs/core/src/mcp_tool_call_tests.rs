...
use deepseek_config::types::AppConfig;
use deepseek_config::types::AppToolConfig;
use deepseek_config::types::AppToolsConfig;
use deepseek_config::types::ApprovalsReviewer;
use deepseek_config::types::AppsConfigToml;
...
use deepseek_rollout_trace::ToolDispatchInvocation;
use deepseek_rollout_trace::ToolDispatchPayload;
use deepseek_rollout_trace::ToolDispatchRequester;
...
fn approval_metadata(
    ...
    McpToolApprovalMetadata {
        ...
        deepseek_file_input_params: None,
    }
}
...
fn write_sample_plugin_mcp(deepseek_home: &std::path::Path) {
    let plugin_root = deepseek_home.join("plugins/cache/test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".deepseek-plugin")).expect("create plugin manifest dir");
    std::fs::write(
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.deepseek_home)
...
"#,
    ...
    std::fs::write(
        turn_context.config.deepseek_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = deepseek_hooks::list_hooks(HooksConfig {
...
fn attach_trace_bundle(
    ...
    let rollout_thread_trace =
        deepseek_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
            root,
...
fn mcp_app_resource_uri_reads_known_tool_meta_keys() {
    ...
    let output_template = serde_json::json!({
        "deepseek/outputTemplate": "ui://widget/output-template.html",
    });
    assert_eq!(
...
#[test]
fn deepseek_file_params_are_only_honored_for_codex_apps() {
    let meta = serde_json::json!({
        "deepseek/fileParams": ["file"],
    });
    let meta = meta.as_object();

    assert_eq!(
        deepseek_file_input_params_for_server(CODEX_APPS_MCP_SERVER_NAME, meta),
        Some(vec!["file".to_string()])
    );
    assert_eq!(
        deepseek_file_input_params_for_server("minimaltest", meta),
        None
    );
}
...
async fn mcp_tool_call_span_records_error_type_and_error_code() {
    ...
    let span = mcp_tool_call_span(
        ...
        McpToolCallSpanFields {
            ...
            call_id: "call-123",
            server_origin: Some("https://platform.deepseek.com/api/codex/ps/mcp"),
            connector_id: Some("calendar"),
            connector_name: Some("Calendar"),
        },
    );
...
async fn host_owned_codex_apps_manager(
    session: &Session,
    turn_context: &TurnContext,
) -> Arc<deepseek_mcp::McpConnectionManager> {
    ...
    let mcp_servers = HashMap::from([(
        ...
        deepseek_mcp::EffectiveMcpServer::configured(deepseek_mcp::codex_apps_mcp_server_config(
            "https://platform.deepseek.com",
            /*apps_mcp_product_sku*/ None,
        )),
    )]);
    let manager = deepseek_mcp::McpConnectionManager::new(
        &mcp_servers,
        ...
        deepseek_mcp::McpRuntimeContext::new(
        ...
        turn_context.config.deepseek_home.to_path_buf(),
        session.services.mcp_manager.codex_apps_tools_cache(),
        deepseek_mcp::codex_apps_tools_cache_key(auth.as_ref()),
        ...
        deepseek_mcp::ToolPluginProvenance::default(),
        auth.as_ref(),
        /*elicitation_reviewer*/ None,
        deepseek_mcp::ElicitationRequestRouter::default(),
    )
    .await;
    Arc::new(manager)
}
...
async fn codex_apps_auth_elicitation_feature_enabled_requests_elicitation() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        request.id,
        deepseek_protocol::mcp::RequestId::String("codex_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        deepseek_protocol::approvals::ElicitationRequest::Url { .. }
    ));
...
async fn guardian_review_decision_maps_to_mcp_tool_decision() {
    ...
    session.services.guardian_rejections.lock().await.insert(
        ...
        crate::guardian::GuardianRejection {
            ...
            source: deepseek_protocol::protocol::GuardianAssessmentDecisionSource::Agent,
        },
    );
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(deepseek_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .deepseek_home(deepseek_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
"#,
    ...
    let contents = std::fs::read_to_string(deepseek_home.join(CONFIG_TOML_FILE)).expect("read config");
    let parsed: ConfigToml = toml::from_str(&contents).expect("parse config");
    let tool = parsed
        .plugins
        .get("sample@test")
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    ConfigEditsBuilder::new(&deepseek_home)
        .set_project_trust_level(
            project_dir.path(),
            deepseek_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
...
async fn approve_mode_skips_guardian_in_every_permission_mode() {
    ...
    for approval_policy in [
        ...
        turn_context.auth_manager = Some(crate::test_support::auth_manager_from_auth(
            deepseek_login::DeepSeekAuth::create_dummy_chatgpt_auth_for_testing(),
        ));
        turn_context
            .approval_policy
            .set(approval_policy)
