//! Bridges Apps SDK-style `deepseek/fileParams` metadata into DeepSeek's MCP flow.
//!
//! Strategy:
//! - Inspect `_meta["deepseek/fileParams"]` to discover which tool arguments are
//!   file inputs.
//! - At tool execution time, read those files from the primary environment,
//!   upload them to DeepSeek file storage,
//!   and rewrite only the declared arguments into the provided-file payload
//!   shape expected by the downstream Apps tool.
//!
//! The model-facing local-path schema is owned by `deepseek-mcp` alongside MCP tool inventory, so this
//! module only handles uploading the files and rewriting the execution-time arguments.

use crate::session::session::Session;
use crate::session::turn_context::TurnContext;
use deepseek_api::OPENAI_FILE_UPLOAD_LIMIT_BYTES;
use deepseek_api::upload_deepseek_file;
use deepseek_login::DeepSeekAuth;
use deepseek_utils_path_uri::PathUri;
use serde_json::Value as JsonValue;

pub(crate) async fn rewrite_mcp_tool_arguments_for_deepseek_files(
    sess: &Session,
    turn_context: &TurnContext,
    arguments_value: Option<JsonValue>,
    deepseek_file_input_params: Option<&[String]>,
) -> Result<Option<JsonValue>, String> {
    let Some(deepseek_file_input_params) = deepseek_file_input_params else {
    ...
    for field_name in deepseek_file_input_params {
        let Some(value) = arguments.get(field_name) else {
            continue;
        };
        let Some(uploaded_value) =
            rewrite_argument_value_for_deepseek_files(turn_context, auth.as_ref(), field_name, value)
                .await?
        else {
            continue;
        };
...
async fn rewrite_argument_value_for_deepseek_files(
    turn_context: &TurnContext,
...
async fn build_uploaded_argument_value(
    ...
    let Some(auth) = auth else {
        return Err("DeepSeek auth is required to upload files for DeepSeek Apps tools".to_string());
    };
    ...
    let file_name = resolved_path
        ...
        .unwrap_or("file")
        .to_string();
    let upload_auth = deepseek_model_provider::auth_provider_from_auth(auth);
    let uploaded = upload_deepseek_file(
...
mod tests {
    ...
    fn set_primary_environment_cwd(turn_context: &mut TurnContext, cwd: &Path) {
        ...
        turn_context.permission_profile = deepseek_protocol::models::PermissionProfile::Disabled;
    ...
    #[tokio::test]
    async fn deepseek_file_argument_rewrite_requires_declared_file_params() {
        let (session, turn_context) = make_session_and_context().await;
        let arguments = Some(serde_json::json!({
            "file": "/tmp/codex-smoke-file.txt"
        }));

        let rewritten = rewrite_mcp_tool_arguments_for_deepseek_files(
            &session,
    ...
    #[tokio::test]
    async fn rewrite_argument_value_for_deepseek_files_rewrites_scalar_path() {
        use wiremock::Mock;
        use wiremock::MockServer;
        use wiremock::ResponseTemplate;
        use wiremock::matchers::body_json;
        ...
        config.chatgpt_base_url = format!("{}/backend-api", server.uri());
        turn_context.config = Arc::new(config);
        let rewritten = rewrite_argument_value_for_deepseek_files(
            &turn_context,
            Some(&auth),
            "file",
    ...
    #[tokio::test]
    async fn rewrite_argument_value_for_deepseek_files_rewrites_array_paths() {
        use wiremock::Mock;
        use wiremock::MockServer;
        use wiremock::ResponseTemplate;
        use wiremock::matchers::body_json;
    ...
    #[tokio::test]
    async fn rewrite_mcp_tool_arguments_for_deepseek_files_surfaces_upload_failures() {
        ...
        let error = rewrite_mcp_tool_arguments_for_deepseek_files(
            &session,
            &turn_context,
            Some(serde_json::json!({
