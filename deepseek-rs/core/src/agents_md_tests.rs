...
use deepseek_exec_server::ExecutorFileSystemFuture;
use deepseek_exec_server::FileMetadata;
use deepseek_exec_server::FileSystemReadStream;
use deepseek_exec_server::FileSystemSandboxContext;
use deepseek_exec_server::LOCAL_FS;
use deepseek_exec_server::ReadDirectoryEntry;
...
async fn make_config(root: &TempDir, limit: usize, instructions: Option<&str>) -> TestConfig {
    ...
    let user_instructions = instructions.map(|text| UserInstructions {
        text: text.to_owned(),
        source: config.deepseek_home.join(DEFAULT_AGENTS_MD_FILENAME),
    });
    TestConfig {
        config,
        user_instructions,
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.deepseek_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.deepseek_home).unwrap();
...
fn create_skill(deepseek_home: PathBuf, name: &str, description: &str) {
    let skill_dir = deepseek_home.join(format!("skills/{name}"));
