...
use deepseek_network_proxy::ManagedNetworkSandboxContext;
...
fn env_overlay_for_exec_server_keeps_runtime_changes_only() {
    ...
    let request_env = HashMap::from([
        ...
        (
            "DEEPSEEK_SANDBOX_NETWORK_DISABLED".to_string(),
            "1".to_string(),
        ),
    ]);
...
fn exec_server_params_use_path_uri_and_env_policy_overlay_contract() {
    let cwd: deepseek_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let file_system_sandbox_policy =
        deepseek_protocol::permissions::FileSystemSandboxPolicy::unrestricted();
    let network_sandbox_policy = deepseek_protocol::permissions::NetworkSandboxPolicy::Restricted;
    let permission_profile = deepseek_protocol::models::PermissionProfile::Disabled;
    ...
    let mut request = ExecRequest {
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: deepseek_exec_server::ExecEnvPolicy {
                inherit: deepseek_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: deepseek_sandboxing::SandboxType::None,
    ...
    request.exec_server_sandbox = Some(
        deepseek_exec_server::FileSystemSandboxContext::from_permission_profile(permission_profile),
    );
    let first =
        exec_server_params_for_request(/*process_id*/ 123, &request, /*tty*/ true);
    let second =
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let request = ExecCommandRequest {
        ...
        shell_mode: deepseek_tools::UnifiedExecShellMode::Direct,
        network: None,
        tty: true,
    ...
    let deepseek_protocol::protocol::EventMsg::ExecCommandEnd(end_event) = event.msg else {
        panic!("expected ExecCommandEnd event");
    };
    assert_eq!(end_event.call_id, "call-unified-denied");
    assert_eq!(
        end_event.status,
        deepseek_protocol::protocol::ExecCommandStatus::Failed
    );
