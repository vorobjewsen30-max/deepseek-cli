...
use deepseek_exec_server::ExecProcess;
use deepseek_exec_server::ExecProcessEventReceiver;
use deepseek_exec_server::ExecProcessFuture;
use deepseek_exec_server::ProcessId;
use deepseek_exec_server::ProcessSignal;
use deepseek_exec_server::ReadResponse;
use deepseek_exec_server::StartedExecProcess;
use deepseek_exec_server::WriteResponse;
use deepseek_exec_server::WriteStatus;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, deepseek_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, deepseek_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), deepseek_exec_server::ExecServerError> {
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = deepseek_exec_server::Environment::default_for_tests();
