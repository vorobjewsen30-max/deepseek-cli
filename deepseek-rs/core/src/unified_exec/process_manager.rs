...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the DeepSeek sandbox network proxy.";
...
) -> deepseek_exec_server::ExecEnvPolicy {
...
fn exec_server_env_for_request(
    ...
    Option<deepseek_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn exec_server_params_for_request(
    ...
    request: &ExecRequest,
    tty: bool,
) -> deepseek_exec_server::ExecParams {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::DeepSeek(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        .map_err(|err| match err {
            UnifiedExecError::SandboxDenied { output, .. } => {
                ToolError::DeepSeek(DeepSeekErr::Sandbox(SandboxErr::Denied {
    ...
    pub(crate) async fn open_session_with_prepared_exec_env(
        ...
        if request.sandbox == deepseek_sandboxing::SandboxType::WindowsRestrictedToken {
            ...
            let deepseek_home = crate::config::find_deepseek_home().map_err(|err| {
            ...
            let spawned = match request.windows_sandbox_level {
                ...
                deepseek_protocol::config_types::WindowsSandboxLevel::RestrictedToken
        ...
        let spawn_result = if tty {
            deepseek_utils_pty::pty::spawn_process_with_inherited_fds(
                program,
                args,
                native_cwd.as_path(),
                &request.env,
                &request.arg0,
                deepseek_utils_pty::TerminalSize::default(),
                &inherited_fds,
            )
            .await
        } else {
            deepseek_utils_pty::pipe::spawn_process_no_stdin_with_inherited_fds(
                program,
                args,
                native_cwd.as_path(),
                &request.env,
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::DeepSeek(DeepSeekErr::Sandbox(SandboxErr::Denied { output, .. })) => {
                    let output = *output;
