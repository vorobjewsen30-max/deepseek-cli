...
use deepseek_thread_store::ThreadStoreResult;
...
/// Explains why `DeepSeekThread::try_start_turn_if_idle` rejected an automatic
/// idle turn.
...
pub struct DeepSeekThreadSettingsOverrides {
...
pub struct DeepSeekThread {
    pub(crate) codex: DeepSeek,
...
/// (formerly called a conversation) in DeepSeek.
impl DeepSeekThread {
    pub(crate) fn new(
        codex: DeepSeek,
        session_configured: SessionConfiguredEvent,
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(deepseek_extension_api::ThreadResumeInput {
    ...
    /// callers that only hold a `DeepSeekThread`.
    ...
    pub async fn set_deepseek_form_elicitation_support(&self, supported: bool) -> anyhow::Result<()> {
        self.codex
            .session
            .set_deepseek_form_elicitation_support(supported)
            .await
    }
    ...
    pub async fn preview_thread_settings_overrides(
        &self,
        overrides: DeepSeekThreadSettingsOverrides,
    ...
    async fn thread_settings_update(
        ...
        let DeepSeekThreadSettingsOverrides {
            environments,
            workspace_roots,
    ...
    pub async fn submit_with_id(&self, sub: Submission) -> DeepSeekResult<()> {
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> DeepSeekResult<()> {
        if items.is_empty() {
    ...
    pub async fn runtime_mcp_config(&self, config: &crate::config::Config) -> deepseek_mcp::McpConfig {
    ...
    pub async fn increment_out_of_band_elicitation_count(&self) -> DeepSeekResult<u64> {
        let mut guard = self.out_of_band_elicitation_count.lock().await;
        let was_zero = *guard == 0;
        *guard = guard.checked_add(1).ok_or_else(|| {
            DeepSeekErr::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> DeepSeekResult<u64> {
        let mut guard = self.out_of_band_elicitation_count.lock().await;
        if *guard == 0 {
            return Err(DeepSeekErr::InvalidRequest(
