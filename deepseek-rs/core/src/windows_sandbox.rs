...
use deepseek_otel::sanitize_metric_tag_value;
...
pub fn sandbox_setup_is_complete(deepseek_home: &Path) -> bool {
    codex_windows_sandbox::sandbox_setup_is_complete(deepseek_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_deepseek_home: &Path) -> bool {
    false
}
...
#[cfg(target_os = "windows")]
pub fn run_elevated_provisioning_setup(deepseek_home: &Path, real_user: &str) -> anyhow::Result<()> {
    codex_windows_sandbox::run_elevated_provisioning_setup(deepseek_home, real_user)
}
...
#[cfg(not(target_os = "windows"))]
pub fn run_elevated_provisioning_setup(_deepseek_home: &Path, _real_user: &str) -> anyhow::Result<()> {
    anyhow::bail!("elevated Windows sandbox setup is only supported on Windows")
}
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let deepseek_home = request.deepseek_home;
    let setup_deepseek_home = deepseek_home.clone();

    let setup_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        match mode {
            WindowsSandboxSetupMode::Elevated => {
                if !sandbox_setup_is_complete(setup_deepseek_home.as_path()) {
                    run_elevated_setup(
                        &permission_profile,
                        workspace_roots.as_slice(),
                        command_cwd.as_path(),
                        &env_map,
                        setup_deepseek_home.as_path(),
                    )?;
                }
            }
    ...
    ConfigEditsBuilder::new(deepseek_home.as_path())
