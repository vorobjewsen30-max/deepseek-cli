...
use deepseek_git_utils::get_git_remote_urls_assume_git_repo;
use deepseek_git_utils::get_git_repo_root;
use deepseek_git_utils::get_has_changes;
use deepseek_git_utils::get_head_commit_hash;
...
pub async fn detached_memory_responses_metadata(
    ...
    DeepSeekResponsesMetadata {
        request_kind: Some(DeepSeekResponsesRequestKind::Memory),
        subagent_header: subagent_header_value(session_source),
        sandbox: sandbox.map(ToString::to_string),
        workspaces: memory_workspaces(cwd).await,
        ..DeepSeekResponsesMetadata::new(installation_id, session_id, thread_id, window_id)
    }
}
...
impl TurnMetadataState {
    ...
    pub(crate) fn to_responses_metadata(
        ...
        installation_id: String,
        window_id: String,
        request_kind: DeepSeekResponsesRequestKind,
    ) -> DeepSeekResponsesMetadata {
        DeepSeekResponsesMetadata {
            installation_id,
            window_id,
    ...
    fn responses_metadata_template(&self) -> DeepSeekResponsesMetadata {
        DeepSeekResponsesMetadata {
