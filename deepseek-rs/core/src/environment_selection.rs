...
impl TurnEnvironmentSnapshot {
    ...
    #[cfg(test)]
    pub(crate) fn primary_environment(&self) -> Option<Arc<deepseek_exec_server::Environment>> {
        self.primary()
...
"#,
        ...
        let cwd_uri = PathUri::from_abs_path(&cwd);
        let manager =
            EnvironmentManager::from_deepseek_home(temp_dir.path(), Some(test_runtime_paths()))
                .await
