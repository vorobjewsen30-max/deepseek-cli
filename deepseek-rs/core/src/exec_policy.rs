...
use deepseek_execpolicy::AmendError;
use deepseek_execpolicy::Decision;
use deepseek_execpolicy::Error as ExecPolicyRuleError;
...
use deepseek_execpolicy::blocking_append_allow_prefix_rule;
use deepseek_execpolicy::blocking_append_network_rule;
...
use deepseek_shell_command::is_dangerous_command::command_might_be_dangerous;
use deepseek_shell_command::is_safe_command::is_known_safe_command;
use thiserror::Error;
use tokio::fs;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(deepseek_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: deepseek_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(deepseek_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &deepseek_execpolicy::Error) -> String {
...
pub(crate) fn render_decision_for_unmatched_command(
    ...
    let is_known_safe = match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            deepseek_shell_command::is_safe_command::is_safe_powershell_words(command)
        }
    };
    ...
    let command_is_dangerous = match command_origin {
        ...
        #[cfg(windows)]
        ExecPolicyCommandOrigin::PowerShell => {
            deepseek_shell_command::is_dangerous_command::is_dangerous_powershell_words(command)
        }
    };
...
fn default_policy_path(deepseek_home: &Path) -> PathBuf {
    deepseek_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
fn commands_for_exec_policy(command: &[String]) -> ExecPolicyCommands {
    ...
    #[cfg(windows)]
    {
        if let Some(commands) =
            deepseek_shell_command::powershell::parse_powershell_command_into_plain_commands(command)
            && !commands.is_empty()
        {
            return ExecPolicyCommands {
                commands,
