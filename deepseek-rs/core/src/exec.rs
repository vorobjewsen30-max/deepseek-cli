...
use deepseek_protocol::error::SandboxErr;
...
use deepseek_protocol::protocol::ExecCommandOutputDeltaEvent;
use deepseek_protocol::protocol::ExecOutputStream;
use deepseek_sandboxing::SandboxCommand;
use deepseek_sandboxing::SandboxManager;
use deepseek_sandboxing::SandboxTransformRequest;
use deepseek_sandboxing::SandboxType;
use deepseek_sandboxing::SandboxablePreference;
use deepseek_sandboxing::WindowsSandboxFilesystemOverrides;
pub(crate) use deepseek_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use deepseek_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use deepseek_sandboxing::resolve_windows_elevated_filesystem_overrides;
use deepseek_sandboxing::resolve_windows_restricted_token_filesystem_overrides;
#[cfg(test)]
use deepseek_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
use deepseek_sandboxing::windows_sandbox_uses_elevated_backend;
...
use deepseek_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: deepseek_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: deepseek_protocol::config_types::WindowsSandboxLevel,
...
fn network_proxy_environment_error(
    ...
    DeepSeekErr::Io(io::Error::other(format!(
...
pub fn build_exec_request(
    ...
    let mut exec_req = manager
        .transform(SandboxTransformRequest {
            ...
            deepseek_linux_sandbox_exe: deepseek_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
    ...
    .map_err(DeepSeekErr::UnsupportedOperation)?;
    Ok(exec_req)
}
...
pub(crate) async fn execute_exec_request(
    ...
    let cwd = cwd
        .to_abs_path()
        .map_err(|err| DeepSeekErr::InvalidRequest(format!("invalid exec cwd: {err}")))?;
    ...
    let windows_sandbox_policy_cwd = windows_sandbox_policy_cwd
        .to_abs_path()
        .map_err(|err| DeepSeekErr::InvalidRequest(format!("invalid sandbox cwd: {err}")))?;
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        deepseek_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = deepseek_otel::global() {
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_deepseek_home;
    ...
    let deepseek_home = find_deepseek_home().map_err(|err| {
        DeepSeekErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve deepseek_home: {err}"
        )))
    })?;
        ...
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                codex_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
                    ...
                    deepseek_home: deepseek_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(DeepSeekErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, DeepSeekErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(DeepSeekErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(DeepSeekErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(DeepSeekErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
/// DEEPSEEK_SANDBOX_NETWORK_DISABLED=1 is added to the environment of the spawned
/// process.
///
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        deepseek_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                deepseek_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }
