...
use deepseek_protocol::request_user_input::RequestUserInputEvent;
...
async fn forward_ops_preserves_submission_trace_context() {
    ...
    let submission = Submission {
        ...
        trace: Some(deepseek_protocol::protocol::W3cTraceContext {
            traceparent: Some(
...
async fn run_codex_thread_interactive_respects_pre_cancelled_spawn() {
    ...
    assert!(matches!(result, Err(DeepSeekErr::TurnAborted)));
}
