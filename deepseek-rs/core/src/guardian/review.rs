...
use deepseek_analytics::GuardianReviewDecision;
use deepseek_analytics::GuardianReviewFailureReason;
use deepseek_analytics::GuardianReviewTerminalStatus;
use deepseek_analytics::GuardianReviewTrackContext;
use deepseek_analytics::GuardianReviewedAction;
...
use deepseek_protocol::protocol::GuardianAssessmentDecisionSource;
...
impl GuardianReviewError {
    ...
    fn session_with_error_info(err: anyhow::Error, error_info: DeepSeekErrorInfo) -> Self {
        Self::Session {
            message: err.to_string(),
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &deepseek_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        deepseek_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(label))
...
pub(super) async fn guardian_review_session_config(
    ...
    let available_models = session
        .services
        .models_manager
        .list_models(deepseek_models_manager::manager::RefreshStrategy::Offline)
        .await;
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = review_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset
                ...
                .iter()
                .any(|effort| effort.effort == deepseek_protocol::deepseek_models::ReasoningEffort::Low),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| preset.effort == deepseek_protocol::deepseek_models::ReasoningEffort::Low),
            ...
            GuardianReviewError::Session {
                error_info: Some(
                    DeepSeekErrorInfo::ServerOverloaded
                        | DeepSeekErrorInfo::HttpConnectionFailed { .. }
                        | DeepSeekErrorInfo::ResponseStreamConnectionFailed { .. }
                        | DeepSeekErrorInfo::InternalServerError
                        | DeepSeekErrorInfo::ResponseStreamDisconnected { .. }
                ),
                ..
            } | GuardianReviewError::Parse { .. }
        )
