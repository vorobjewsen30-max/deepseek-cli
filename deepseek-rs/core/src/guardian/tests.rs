...
use deepseek_analytics::GuardianApprovalRequestSource;
...
use deepseek_model_provider_info::AMAZON_BEDROCK_GPT_5_4_MODEL_ID;
use deepseek_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
use deepseek_model_provider_info::ModelProviderInfo;
use deepseek_model_provider_info::DEEPSEEK_PROVIDER_ID;
use deepseek_models_manager::manager::StaticModelsManager;
...
// The memories extension depends on deepseek-core, so this probe verifies the nested Guardian config
...
impl deepseek_extension_api::ThreadLifecycleContributor<Config> for GuardianMemoryContextProbe {
    fn on_thread_start<'a>(
        &'a self,
        input: deepseek_extension_api::ThreadStartInput<'a, Config>,
...
impl deepseek_extension_api::ContextContributor for GuardianMemoryContextProbe {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a deepseek_extension_api::ExtensionData,
        thread_store: &'a deepseek_extension_api::ExtensionData,
    ) -> deepseek_extension_api::ExtensionFuture<'a, Vec<deepseek_extension_api::PromptFragment>> {
        Box::pin(async move {
            if thread_store
                .get::<GuardianMemoryContextEnabled>()
                .is_some_and(|enabled| enabled.0)
            {
                vec![deepseek_extension_api::PromptFragment::developer_policy(
        ...
        .record_conversation_items(
            ...
            &[
                ...
                ResponseItem::FunctionCall {
                    ...
                    namespace: None,
                    arguments: "{\"repo\":\"deepseek/codex\"}".to_string(),
                ...
                ResponseItem::FunctionCallOutput {
                    id: None,
                    call_id: "call-1".to_string(),
                    output: deepseek_protocol::models::FunctionCallOutputPayload::from_text(
...
fn guardian_prompt_text(items: &[deepseek_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            deepseek_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
...
async fn build_guardian_prompt_full_mode_preserves_initial_review_format() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The DeepSeek agent has requested the following action:\n"));
        ...
        &FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: deepseek_protocol::permissions::FileSystemSpecialPath::Root,
                },
...
async fn build_guardian_prompt_delta_mode_preserves_original_numbering() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The DeepSeek agent has requested the following next action:\n"));
...
async fn build_guardian_prompt_items_explains_network_access_review_scope() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "deepseek_core__guardian__tests__network_access_guardian_prompt_layout",
...
async fn guardian_request_model_for_auto_review(
    ...
    String,
    String,
    deepseek_analytics::GuardianReviewAnalyticsResult,
)> {
...
-> anyhow::Result<()> {
    ...
    let mut extensions = deepseek_extension_api::ExtensionRegistryBuilder::<Config>::new();
    ...
    assert!(matches!(
        ...
        Some(deepseek_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    let request = request_log.single_request();
    ...
    settings.bind(|| {
        assert_snapshot!(
            "deepseek_core__guardian__tests__guardian_review_request_layout",
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            deepseek_protocol::user_input::UserInput::Text { text, .. } => text,
            deepseek_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    assert!(matches!(
        ...
        Some(deepseek_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "deepseek_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
...
async fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        "parent-active-model",
        Some(deepseek_protocol::deepseek_models::ReasoningEffort::Low),
    )
