...
use anyhow::anyhow;
use deepseek_analytics::GuardianReviewAnalyticsResult;
use deepseek_analytics::GuardianReviewSessionAnalyticsParams;
use deepseek_analytics::GuardianReviewSessionKind;
use deepseek_extension_api::UserInstructions;
...
pub(crate) enum GuardianReviewSessionOutcome {
    ...
    SessionFailed {
        error: anyhow::Error,
        error_info: Option<DeepSeekErrorInfo>,
    },
    TimedOut,
    Aborted,
}
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(
        ...
        Self {
            ...
            deepseek_linux_sandbox_exe: spawn_config.deepseek_linux_sandbox_exe.clone(),
...
impl GuardianReviewSessionManager {
    ...
    #[cfg(test)]
    pub(crate) async fn cache_for_test(&self, codex: DeepSeek) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
    ...
    #[cfg(test)]
    pub(crate) async fn register_ephemeral_for_test(&self, codex: DeepSeek) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            codex.session.get_config().await.as_ref(),
            codex.session.user_instructions().await,
        );
    ...
    let submit_result = run_before_review_deadline(
        ...
        Box::pin(review_session.codex.submit(Op::UserInput {
            ...
            thread_settings: deepseek_protocol::protocol::ThreadSettingsOverrides {
                environments: Some(deepseek_protocol::protocol::TurnEnvironmentSelections::new(
...
pub(crate) fn build_guardian_review_session_config(
    ...
    live_network_config: Option<deepseek_network_proxy::NetworkProxyConfig>,
    active_model: &str,
...
async fn interrupt_and_drain_turn(codex: &DeepSeek, expected_turn_id: &str) -> anyhow::Result<()> {
...
mod tests {
    ...
    async fn guardian_review_session_config_disables_hooks() {
        ...
        assert!(!guardian_config.features.enabled(Feature::DeepSeekHooks));
    }
    ...
    async fn wait_for_guardian_review_preserves_structured_session_error() {
        ...
        tx_event
            .send(Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    codex_error_info: Some(DeepSeekErrorInfo::ServerOverloaded),
                }),
            })
            .await
        ...
        assert_eq!(error_info, Some(DeepSeekErrorInfo::ServerOverloaded));
        assert!(keep_review_session);
        assert!(capture_token_usage);
    }
