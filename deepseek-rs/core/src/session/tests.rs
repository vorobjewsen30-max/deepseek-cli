...
use deepseek_config::ConfigLayerStackOrdering;
use deepseek_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
use deepseek_config::LoaderOverrides;
use deepseek_config::NetworkConstraints;
use deepseek_config::NetworkDomainPermissionToml;
use deepseek_config::NetworkDomainPermissionsToml;
use deepseek_config::RequirementSource;
use deepseek_config::Sourced;
use deepseek_config::loader::project_trust_key;
use deepseek_config::types::McpServerConfig;
use deepseek_config::types::McpServerTransportConfig;
use deepseek_config::types::ToolSuggestDisabledTool;
use deepseek_core_skills::HostSkillsSnapshot;
use core_test_support::test_codex::local_selections;

use deepseek_features::Feature;
use deepseek_login::DeepSeekAuth;
use deepseek_login::auth::AgentIdentityAuthPolicy;
use deepseek_model_provider_info::ModelProviderInfo;
use deepseek_models_manager::bundled_models_response;
use deepseek_models_manager::model_info;
...
use deepseek_protocol::SessionId;
use deepseek_protocol::ThreadId;
use deepseek_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use deepseek_protocol::config_types::ServiceTier;
use deepseek_protocol::config_types::TrustLevel;
use deepseek_protocol::exec_output::ExecToolCallOutput;
use deepseek_protocol::models::ActivePermissionProfile;
use deepseek_protocol::models::AgentMessageInputContent;
use deepseek_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use deepseek_protocol::models::FileSystemPermissions;
use deepseek_protocol::models::FunctionCallOutputBody;
...
use deepseek_protocol::models::SandboxEnforcement;
use deepseek_protocol::deepseek_models::ModelServiceTier;
use deepseek_protocol::permissions::FileSystemAccessMode;
use deepseek_protocol::permissions::FileSystemPath;
use deepseek_protocol::permissions::FileSystemSandboxEntry;
use deepseek_protocol::permissions::FileSystemSandboxPolicy;
use deepseek_protocol::permissions::FileSystemSpecialPath;
use deepseek_protocol::protocol::NonSteerableTurnKind;
use deepseek_protocol::protocol::SandboxPolicy;
use deepseek_protocol::protocol::TurnEnvironmentSelections;
use deepseek_protocol::request_permissions::PermissionGrantScope;
use deepseek_protocol::request_permissions::RequestPermissionProfile;
...
use deepseek_config::config_toml::ProjectConfig;
use deepseek_config::permissions_toml::FilesystemPermissionToml;
use deepseek_config::permissions_toml::FilesystemPermissionsToml;
use deepseek_config::permissions_toml::NetworkToml;
use deepseek_config::permissions_toml::PermissionProfileToml;
use deepseek_config::permissions_toml::PermissionsToml;
use deepseek_execpolicy::Decision;
use deepseek_execpolicy::NetworkRuleProtocol;
use deepseek_execpolicy::Policy;
use deepseek_network_proxy::NetworkProxyConfig;
use deepseek_otel::MetricsClient;
use deepseek_otel::MetricsConfig;
use deepseek_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use deepseek_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use deepseek_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use deepseek_otel::THREAD_SKILLS_TRUNCATED_METRIC;
use deepseek_otel::TelemetryAuthMode;
use deepseek_protocol::config_types::CollaborationMode;
use deepseek_protocol::config_types::ModeKind;
use deepseek_protocol::config_types::Settings;
use deepseek_protocol::models::BaseInstructions;
use deepseek_protocol::models::ContentItem;
use deepseek_protocol::models::InternalChatMessageMetadataPassthrough;
use deepseek_protocol::models::ResponseItem;
use deepseek_protocol::protocol::AskForApproval;
use deepseek_protocol::protocol::DeepSeekErrorInfo;
use deepseek_protocol::protocol::CompactedItem;
use deepseek_protocol::protocol::ConversationAudioParams;
use deepseek_protocol::protocol::CreditsSnapshot;
use deepseek_protocol::protocol::GranularApprovalConfig;
...
use deepseek_protocol::protocol::NetworkApprovalProtocol;
use deepseek_protocol::protocol::RateLimitSnapshot;
use deepseek_protocol::protocol::RateLimitWindow;
use deepseek_protocol::protocol::RealtimeAudioFrame;
use deepseek_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use deepseek_protocol::protocol::RealtimeVoice;
...
use deepseek_protocol::protocol::SessionMeta;
use deepseek_protocol::protocol::SessionMetaLine;
use deepseek_protocol::protocol::SkillScope;
use deepseek_protocol::protocol::Submission;
use deepseek_protocol::protocol::ThreadRolledBackEvent;
use deepseek_protocol::protocol::ThreadSettingsOverrides;
use deepseek_protocol::protocol::TokenCountEvent;
use deepseek_protocol::protocol::TokenUsage;
use deepseek_protocol::protocol::TokenUsageInfo;
use deepseek_protocol::protocol::TurnAbortedEvent;
...
use deepseek_rmcp_client::ElicitationAction;
...
use deepseek_protocol::mcp::CallToolResult as McpCallToolResult;
...
fn test_session_telemetry_without_metadata() -> SessionTelemetry {
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "deepseek-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("deepseek-core-tests");
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        thread_id,
        ModelProviderInfo::create_deepseek_provider(/* base_url */ /*base_url*/ None),
        deepseek_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        deepseek_home.join(deepseek_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<deepseek_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&deepseek_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: deepseek_hooks::StartHookTarget::SessionStart {
                source: deepseek_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn deepseek_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { deepseek_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> deepseek_sandboxing::SandboxablePreference {
            deepseek_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(deepseek_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: deepseek_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let deepseek_home = session.deepseek_home().await;
    std::fs::create_dir_all(&deepseek_home).expect("create codex home");
    let config_toml_path = deepseek_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = deepseek_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let deepseek_home = session.deepseek_home().await;
    std::fs::create_dir_all(&deepseek_home).expect("create codex home");
    let base_config_path = deepseek_home.join(CONFIG_TOML_FILE);
    let profile_config_path = deepseek_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(deepseek_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            ...
            .expect("enable DeepSeek hooks");
    })
    .await?;
    let deepseek_home = session.deepseek_home().await;
    std::fs::create_dir_all(&deepseek_home)?;
    let config_toml_path = deepseek_home.join(CONFIG_TOML_FILE);
    let user_config: deepseek_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = deepseek_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = deepseek_hooks::list_hooks(deepseek_hooks::HooksConfig {
        feature_enabled: true,
    ...
    assert_eq!(
        ...
        deepseek_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: deepseek_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: deepseek_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: deepseek_config::MatcherGroup {
                matcher: None,
                hooks: vec![deepseek_config::HookHandlerConfig::Command {
        ...
        let identity = deepseek_config::TomlValue::try_from(identity)?;
        deepseek_config::version_for_toml(&identity)
    };
...
disabled_tools = [
  ...
  { type = "plugin", id = "slack@deepseek-curated" },
]
"#,
    )
    .expect("write user config");
...
async fn record_inter_agent_communication_preserves_item_id_in_rollout_and_resume() {
    let (mut session, turn_context, _rx) = make_session_and_context_with_auth_and_config_and_rx(
        DeepSeekAuth::from_api_key("Test API Key"),
        Vec::new(),
        |config| {
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl deepseek_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a deepseek_extension_api::ExtensionData,
            thread_store: &'a deepseek_extension_api::ExtensionData,
            turn_store: &'a deepseek_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> deepseek_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let mut builder = deepseek_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl deepseek_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: deepseek_extension_api::TurnStartInput<'a>,
        ) -> deepseek_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl deepseek_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: deepseek_extension_api::TurnErrorInput<'a>,
        ) -> deepseek_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let expected = RecordedTurnError {
        ...
        turn_level_id: turn_context.sub_id.clone(),
        turn_id: turn_context.sub_id.clone(),
        error: DeepSeekErrorInfo::UsageLimitExceeded,
        saw_session_store: true,
        saw_thread_store: true,
    };

    session
        .emit_turn_error_lifecycle(&turn_context, DeepSeekErrorInfo::UsageLimitExceeded)
        .await;
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl deepseek_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &deepseek_extension_api::ExtensionData,
            thread_store: &deepseek_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: deepseek_protocol::protocol::NetworkAccess::Restricted,
    };
...
async fn session_permission_profile_rebinds_runtime_workspace_roots() -> anyhow::Result<()> {
    let deepseek_home = tempfile::TempDir::new()?;
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        config.permissions.approval_policy =
            deepseek_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        deepseek_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "deepseek_core__codex_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: deepseek_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            deepseek_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            deepseek_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            deepseek_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        ...
        Some(DeepSeekErrorInfo::ThreadRollbackFailed)
    );
...
async fn set_rate_limits_retains_previous_credits() {
    let deepseek_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(deepseek_home.path()).await;
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(deepseek_protocol::account::PlanType::Plus),
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        credits: None,
        individual_limit: None,
        plan_type: Some(deepseek_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.codex_error_info == Some(DeepSeekErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(deepseek_home: &Path) -> Config {
...
async fn emit_subagent_session_started_includes_fork_lineage_and_originator() {
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(DeepSeekAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("deepseek-tui".to_string()),
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = deepseek_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        deepseek_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    let deepseek_home = tempfile::tempdir().expect("create codex home");
    let cwd = tempfile::tempdir().expect("create cwd");
    ...
    std::fs::write(
        deepseek_home.path().join(deepseek_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let deepseek_home = parent_config.deepseek_home.clone();
    let skill_dir = deepseek_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&deepseek_exec_server::LOCAL_FS));
    ...
    let role_path = deepseek_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    let deepseek_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(deepseek_home.path()).await;
    config
        .features
    ...
    let auth_manager = AuthManager::from_auth_for_testing(DeepSeekAuth::from_api_key("Test API Key"));
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.deepseek_home.to_path_buf()));
    let mcp_manager = Arc::new(McpManager::new(Arc::clone(&plugins_manager)));
    let skills_service = Arc::new(SkillsService::new(
        config.deepseek_home.clone(),
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(deepseek_code_mode::InProcessCodeModeSessionProvider),
        Arc::new(deepseek_extension_api::ExtensionRegistryBuilder::new().build()),
        deepseek_extension_api::ExtensionDataInit::default(),
        /*supports_deepseek_form_elicitation*/ false,
        AgentControl::default(),
        environment_manager,
        /*inherited_environments*/ None,
        /*analytics_events_client*/ None,
        Arc::new(deepseek_thread_store::LocalThreadStore::new(
            deepseek_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        deepseek_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: deepseek_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(deepseek_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: deepseek_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: deepseek_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: deepseek_extension_api::ExtensionDataInit::default(),
        supports_deepseek_form_elicitation: std::sync::atomic::AtomicBool::new(false),
        agent_control,
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(deepseek_thread_store::LocalThreadStore::new(
            deepseek_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
        ...
        code_mode_service: crate::tools::code_mode::CodeModeService::new(Arc::new(
            deepseek_code_mode::InProcessCodeModeSessionProvider,
        )),
...
async fn load_latest_config_for_session(session: &Session) -> Config {
    let config = session.get_config().await;
    ConfigBuilder::default()
        .deepseek_home(config.deepseek_home.to_path_buf())
...
async fn make_session_with_history_source_and_agent_control_and_rx(
    ...
    let session = Session::new(
        ...
        Arc::new(deepseek_thread_store::LocalThreadStore::new(
            deepseek_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            Some(
                deepseek_state::StateRuntime::init(
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            deepseek_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(deepseek_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(deepseek_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            deepseek_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(deepseek_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = deepseek_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(deepseek_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    &turn_context,
                    call_id,
                    deepseek_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(deepseek_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: deepseek_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(deepseek_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserInput {
            ...
            responsesapi_client_metadata: None,
            additional_context: Default::default(),
            thread_settings: deepseek_protocol::protocol::ThreadSettingsOverrides {
                environments: Some(local_selections(config.cwd.clone())),
                approval_policy: Some(config.permissions.approval_policy.value()),
                approvals_reviewer: Some(deepseek_config::types::ApprovalsReviewer::AutoReview),
                sandbox_policy: Some(config.legacy_sandbox_policy()),
                summary: config.model_reasoning_summary,
                personality: config.personality,
                collaboration_mode: Some(deepseek_protocol::config_types::CollaborationMode {
                    mode: deepseek_protocol::config_types::ModeKind::Default,
                    settings: deepseek_protocol::config_types::Settings {
    ...
    assert_eq!(
        ...
        deepseek_config::types::ApprovalsReviewer::AutoReview
    );
}
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = deepseek_utils_path_uri::PathUri::from_abs_path(&second_cwd);
    turn_context
        .environments
        .turn_environments
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        deepseek_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = deepseek_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(deepseek_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn deepseek_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        deepseek_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl deepseek_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadStopRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: deepseek_extension_api::ThreadStopInput<'a>,
        ) -> deepseek_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl deepseek_extension_api::ThreadLifecycleContributor<crate::config::Config> for LifecycleRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: deepseek_extension_api::ThreadStopInput<'a>,
        ) -> deepseek_extension_api::ExtensionFuture<'a, ()> {
    ...
    impl deepseek_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: deepseek_extension_api::TurnAbortInput<'a>,
        ) -> deepseek_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(deepseek_home).await;
...
async fn refresh_mcp_servers_keeps_the_previous_runtime_alive() {
    ...
    assert!(Arc::ptr_eq(&step_context.mcp, &old_runtime));
    assert_eq!(
        deepseek_mcp::configured_mcp_servers(new_runtime.config()),
...
impl deepseek_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a deepseek_extension_api::ExtensionData,
        thread_store: &'a deepseek_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<deepseek_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    deepseek_extension_api::PromptFragment::developer_policy(
...
-> Arc<deepseek_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = deepseek_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl deepseek_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: deepseek_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<deepseek_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
...
async fn handle_output_item_done_records_image_save_history_message() {
    ...
    let expected_saved_path = crate::stream_events_utils::image_generation_artifact_path(
        &turn_context.config.deepseek_home,
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(deepseek_extension_api::ExtensionData::new(
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<deepseek_thread_store::InMemoryThreadStore> {
    let store = Arc::new(deepseek_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn deepseek_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &deepseek_thread_store::InMemoryThreadStore,
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = deepseek_protocol::user_input::TextElement::new(
        deepseek_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
    ...
    impl deepseek_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            input: deepseek_extension_api::ThreadIdleInput<'a>,
        ) -> deepseek_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn thread_idle_lifecycle_waits_for_trigger_turn_mailbox_work() {
    ...
    impl deepseek_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            _input: deepseek_extension_api::ThreadIdleInput<'a>,
        ) -> deepseek_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            })
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(deepseek_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use deepseek_protocol::protocol::AskForApproval;
    use deepseek_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: deepseek_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: deepseek_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let deepseek_home = temp.path().join("home");
    ...
    write_project_trust_config(&deepseek_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .deepseek_home(deepseek_home)
    ...
    let expected_source_path = deepseek_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let deepseek_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&deepseek_home)?;
        write_project_trust_config(&deepseek_home, &trust_entries).await?;
