...
use deepseek_config::config_toml::OrchestratorFeatureToml;
use deepseek_config::config_toml::OrchestratorToml;
...
use deepseek_features::RolloutBudgetConfigToml;
...
/// DeepSeek actually ran with, not only the user-authored TOML inputs.
...
mod tests {
    ...
    async fn lock_contains_prompts_and_materializes_features() {
        ...
        for spec in deepseek_features::FEATURES {
            assert_eq!(
        ...
        assert_eq!(
            ...
            Some(FeatureToml::Config(CurrentTimeReminderConfigToml {
                ...
                clock_source: Some(deepseek_features::CurrentTimeSource::System),
                delivery_mode: Some(deepseek_features::CurrentTimeReminderDeliveryMode::AnyInference),
    ...
    async fn lock_validation_rejects_codex_version_mismatch_by_default() {
        ...
        let message = error.to_string();
        assert!(
            message.contains("config lock DeepSeek version mismatch"),
            "{message}"
        );
        assert!(
