...
pub(crate) struct SessionConfiguration {
    /// Provider identifier ("deepseek", "openrouter", ...).
    ...
    /// Directory containing all DeepSeek state for this session.
    pub(super) deepseek_home: AbsolutePathBuf,
...
impl SessionConfiguration {
    ...
    pub(crate) fn deepseek_home(&self) -> &AbsolutePathBuf {
        &self.deepseek_home
    }
    ...
    pub(super) fn sandbox_policy(&self) -> SandboxPolicy {
        ...
        deepseek_sandboxing::compatibility_sandbox_policy_for_permission_profile(
            &permission_profile,
            self.cwd(),
        )
    }
            ...
            if let Some(active_permission_profile) = next_configuration.active_permission_profile()
            {
                ...
                config.permissions.network = config
                    ...
                    .map_err(|err| ConstraintError::InvalidValue {
                        ...
                        requirement_source: deepseek_config::RequirementSource::Unknown,
                    })?;
                config
                    .permissions
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        config: Arc<Config>,
        user_instructions: Option<deepseek_extension_api::UserInstructions>,
        ...
        code_mode_session_provider: Arc<dyn deepseek_code_mode::CodeModeSessionProvider>,
        extensions: Arc<deepseek_extension_api::ExtensionRegistry<crate::config::Config>>,
        ...
        let thread_extension_data = deepseek_extension_api::ExtensionData::new_with_init(
            thread_id.to_string(),
        ...
        let auth_and_mcp_fut = async move {
            ...
            let mcp_config = mcp_manager_for_mcp
                ...
                .await;
            let mcp_servers = deepseek_mcp::effective_mcp_servers(&mcp_config, auth.as_ref());
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let trace_agent_path = session_configuration
                .session_source
                .get_agent_path()
                .unwrap_or_else(deepseek_protocol::AgentPath::root);
            let trace_task_name =
            ...
            let config_path = config.deepseek_home.join(CONFIG_TOML_FILE);
            ...
            let auth_mode = auth.map(DeepSeekAuth::auth_mode).map(TelemetryAuthMode::from);
            let account_id = auth.and_then(DeepSeekAuth::get_account_id);
            let account_email = auth.and_then(DeepSeekAuth::get_account_email);
            let originator = session_configuration.originator.clone();
            let terminal_type = user_agent();
            let session_model = session_configuration.collaboration_mode.model().to_string();
            let auth_env_telemetry = collect_auth_env_telemetry(
                &session_configuration.provider,
                auth_manager.deepseek_api_key_env_enabled(),
            );
            ...
            let session_extension_data =
                deepseek_extension_api::ExtensionData::new(session_id.to_string());
            session_extension_data.insert(McpResourceClient::new(Arc::clone(
                &mcp_connection_manager,
            )));
            for contributor in extensions.thread_lifecycle_contributors() {
                contributor.on_thread_start(deepseek_extension_api::ThreadStartInput {
            ...
            let services = SessionServices {
                ...
                mcp_thread_init,
                supports_deepseek_form_elicitation: std::sync::atomic::AtomicBool::new(
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Resumed(_) => deepseek_hooks::SessionStartSource::Resume,
                InitialHistory::New | InitialHistory::Forked(_) => {
                    deepseek_hooks::SessionStartSource::Startup
                }
                InitialHistory::Cleared => deepseek_hooks::SessionStartSource::Clear,
            };
