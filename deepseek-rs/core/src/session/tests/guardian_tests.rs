...
use deepseek_protocol::models::AdditionalPermissionProfile as PermissionProfile;
...
use core_test_support::deepseek_linux_sandbox_exe_or_skip;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                deepseek_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_command_additional_permissions_requests_past_policy_validation() {
    ...
    turn_context_raw.permission_profile = deepseek_protocol::models::PermissionProfile::Disabled;
    let mut config = (*turn_context_raw.config).clone();
    config.deepseek_linux_sandbox_exe = deepseek_linux_sandbox_exe_or_skip!();
    config
        .features
        .enable(Feature::GuardianApproval)
        .expect("test setup should allow enabling guardian approvals");
    ...
    let handler = crate::tools::handlers::ShellCommandHandler::from(
        deepseek_tools::ShellCommandBackendConfig::Classic,
    );
    #[allow(deprecated)]
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: deepseek_tools::ToolName::plain("shell_command"),
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let deepseek_home = tempdir().expect("create codex home");
    ...
    let thread_store = Arc::new(deepseek_thread_store::LocalThreadStore::new(
        deepseek_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));

    let DeepSeekSpawnOk { codex, .. } = DeepSeek::spawn(DeepSeekSpawnArgs {
        config,
        ...
        plugins_manager,
        mcp_manager,
        code_mode_session_provider: Arc::new(deepseek_code_mode::InProcessCodeModeSessionProvider),
        extensions: deepseek_extension_api::empty_extension_registry(),
        ...
        parent_rollout_thread_trace: deepseek_rollout_trace::ThreadTraceContext::disabled(),
        user_shell_override: None,
        parent_trace: None,
        environment_selections: Vec::new(),
        thread_extension_init: deepseek_extension_api::ExtensionDataInit::default(),
