...
use deepseek_analytics::CompactionPhase;
use deepseek_analytics::CompactionReason;
use deepseek_analytics::InvocationType;
use deepseek_analytics::TurnResolvedConfigFact;
use deepseek_analytics::build_track_events_context;
use deepseek_async_utils::OrCancelExt;
use deepseek_core_plugins::RecommendedPluginCandidatesInput;
use deepseek_core_skills::injection::InjectedHostSkillPrompts;
use deepseek_extension_api::TurnInputContext;
use deepseek_extension_api::TurnInputEnvironment;
use deepseek_features::Feature;
use deepseek_git_utils::get_git_repo_root_with_fs;
...
use deepseek_protocol::items::PlanItem;
...
use deepseek_protocol::protocol::AgentMessageContentDeltaEvent;
use deepseek_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use deepseek_protocol::protocol::PlanDeltaEvent;
use deepseek_protocol::protocol::ReasoningContentDeltaEvent;
use deepseek_protocol::protocol::ReasoningRawContentDeltaEvent;
use deepseek_protocol::protocol::SafetyBufferingEvent;
...
use deepseek_tools::filter_request_plugin_install_discoverable_tools_for_client;
use deepseek_utils_stream_parser::AssistantTextChunk;
use deepseek_utils_stream_parser::AssistantTextStreamParser;
use deepseek_utils_stream_parser::ProposedPlanSegment;
use deepseek_utils_stream_parser::extract_proposed_plan_text;
use deepseek_utils_stream_parser::strip_citations;
...
pub(crate) async fn run_turn(
    ...
    turn_extension_data: Arc<deepseek_extension_api::ExtensionData>,
    input: Vec<TurnInput>,
    prewarmed_client_session: Option<ModelClientSession>,
    cancellation_token: CancellationToken,
) -> DeepSeekResult<Option<String>> {
    ...
    if let Err(err) = run_pre_sampling_compact(&sess, &turn_context, &mut client_session).await {
        if matches!(err, DeepSeekErr::TurnAborted) {
            return Err(err);
        }
        let error = err.to_deepseek_protocol_error();
    ...
    loop {
        ...
        let sampling_request_result: DeepSeekResult<_> = async {
            ...
            let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
                ...
                window_id,
                DeepSeekResponsesRequestKind::Turn,
            );
        ...
        match sampling_request_result {
            ...
            Err(err @ DeepSeekErr::TurnAborted) => {
                return Err(err);
            }
            Err(codex_error @ DeepSeekErr::InvalidImageRequest()) => {
                {
                ...
                let error = DeepSeekErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_deepseek_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = deepseek_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = deepseek_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<deepseek_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    responses_metadata: &DeepSeekResponsesMetadata,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> DeepSeekResult<(SamplingRequestResult, Vec<ResponseItem>)> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(DeepSeekErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(DeepSeekErr::ContextWindowExceeded);
            }
            Err(DeepSeekErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(DeepSeekErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
...
fn agent_message_text(item: &deepseek_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            deepseek_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: deepseek_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(deepseek_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &deepseek_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, DeepSeekResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> DeepSeekResult<()> {
...
async fn try_run_sampling_request(
    ...
    prompt: &Prompt,
    cancellation_token: CancellationToken,
) -> DeepSeekResult<SamplingRequestResult> {
    feedback_tags!(
        model = turn_context.model_info.slug.clone(),
        approval_policy = turn_context.approval_policy.value(),
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, DeepSeekResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: DeepSeekResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(deepseek_async_utils::CancelErr::Cancelled) => break Err(DeepSeekErr::TurnAborted),
        };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![deepseek_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
    ...
    if cancellation_token.is_cancelled() {
        return Err(DeepSeekErr::TurnAborted);
    }
