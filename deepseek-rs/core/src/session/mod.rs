...
use deepseek_analytics::SubAgentThreadStartedInput;
use deepseek_analytics::TurnDeepSeekErrorFact;
use deepseek_config::types::AuthKeyringBackendKind;
use deepseek_config::types::OAuthCredentialsStoreMode;
use deepseek_exec_server::Environment;
...
use deepseek_extension_api::PromptSlot;
use deepseek_extension_api::TurnContextContributionInput;
use deepseek_features::FEATURES;
use deepseek_features::Feature;
use deepseek_features::unstable_features_warning_event;
use deepseek_hooks::Hooks;
use deepseek_hooks::HooksConfig;
use deepseek_login::AuthManager;
use deepseek_login::DeepSeekAuth;
use deepseek_login::auth_env_telemetry::collect_auth_env_telemetry;
use deepseek_mcp::McpConnectionManager;
use deepseek_mcp::McpResourceClient;
use deepseek_mcp::McpRuntimeContext;
use deepseek_mcp::codex_apps_tools_cache_key;
use deepseek_models_manager::manager::RefreshStrategy;
use deepseek_models_manager::manager::SharedModelsManager;
use deepseek_network_proxy::NetworkProxy;
use deepseek_network_proxy::NetworkProxyAuditMetadata;
use deepseek_network_proxy::normalize_host;
use deepseek_otel::current_span_trace_id;
use deepseek_otel::current_span_w3c_trace_context;
use deepseek_otel::set_parent_from_w3c_trace_context;
use deepseek_protocol::SessionId;
use deepseek_protocol::ThreadId;
use deepseek_protocol::approvals::ElicitationRequest;
use deepseek_protocol::approvals::ElicitationRequestEvent;
use deepseek_protocol::approvals::ExecPolicyAmendment;
use deepseek_protocol::approvals::NetworkPolicyAmendment;
use deepseek_protocol::approvals::NetworkPolicyRuleAction;
...
use deepseek_protocol::dynamic_tools::DynamicToolResponse;
use deepseek_protocol::dynamic_tools::DynamicToolSpec;
use deepseek_protocol::items::TurnItem;
use deepseek_protocol::items::UserMessageItem;
...
use deepseek_protocol::models::format_allow_prefixes;
...
use deepseek_protocol::protocol::AdditionalContextEntry;
use deepseek_protocol::protocol::FileChange;
use deepseek_protocol::protocol::HasLegacyEvent;
use deepseek_protocol::protocol::InterAgentCommunication;
use deepseek_protocol::protocol::ItemCompletedEvent;
use deepseek_protocol::protocol::ItemStartedEvent;
use deepseek_protocol::protocol::MultiAgentVersion;
use deepseek_protocol::protocol::RawResponseItemEvent;
...
use deepseek_protocol::protocol::ThreadHistoryMode;
use deepseek_protocol::protocol::ThreadSource;
use deepseek_protocol::protocol::TurnAbortReason;
use deepseek_protocol::protocol::TurnContextItem;
use deepseek_protocol::protocol::TurnContextNetworkItem;
...
use deepseek_protocol::protocol::WorldStateItem;
...
use deepseek_protocol::request_permissions::RequestPermissionsArgs;
use deepseek_protocol::request_permissions::RequestPermissionsEvent;
use deepseek_protocol::request_permissions::RequestPermissionsResponse;
use deepseek_protocol::request_user_input::RequestUserInputArgs;
use deepseek_protocol::request_user_input::RequestUserInputResponse;
use deepseek_rmcp_client::ElicitationResponse;
use deepseek_rollout::state_db;
use deepseek_rollout_trace::AgentResultTracePayload;
use deepseek_rollout_trace::ThreadStartedTraceMetadata;
use deepseek_rollout_trace::ThreadTraceContext;
use deepseek_sandboxing::policy_transforms::intersect_permission_profiles;
use deepseek_shell_command::parse_command::parse_command;
use deepseek_terminal_detection::user_agent;
use deepseek_thread_store::CreateThreadParams;
use deepseek_thread_store::LiveThread;
use deepseek_thread_store::LiveThreadInitGuard;
use deepseek_thread_store::LocalThreadStore;
use deepseek_thread_store::ReadThreadParams;
use deepseek_thread_store::ResumeThreadParams;
use deepseek_thread_store::ThreadPersistenceMetadata;
...
#[cfg(test)]
use deepseek_protocol::exec_output::StreamOutput;
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                codex_error_info: Some(DeepSeekErrorInfo::BadRequest),
            },
            ...
            Self::ActiveTurnNotSteerable { turn_kind } => {
                ...
                ErrorEvent {
                    ...
                    codex_error_info: Some(DeepSeekErrorInfo::ActiveTurnNotSteerable {
                        turn_kind: *turn_kind,
                    }),
                }
            }
...
use deepseek_core_plugins::RecommendedPluginCandidatesInput;
use deepseek_git_utils::get_git_repo_root;
use deepseek_mcp::McpConfig;
use deepseek_mcp::compute_auth_statuses;
use deepseek_mcp::effective_mcp_servers;
use deepseek_otel::SessionTelemetry;
use deepseek_otel::THREAD_STARTED_METRIC;
...
use deepseek_protocol::models::LocalImagePreparation;
...
use deepseek_protocol::protocol::ApplyPatchApprovalRequestEvent;
...
use deepseek_protocol::protocol::DeprecationNoticeEvent;
use deepseek_protocol::protocol::ErrorEvent;
...
use deepseek_protocol::protocol::McpServerRefreshConfig;
use deepseek_protocol::protocol::ModelRerouteEvent;
use deepseek_protocol::protocol::ModelRerouteReason;
use deepseek_protocol::protocol::ModelVerification;
use deepseek_protocol::protocol::ModelVerificationEvent;
use deepseek_protocol::protocol::NetworkApprovalContext;
...
use deepseek_protocol::protocol::RequestUserInputEvent;
...
use deepseek_protocol::protocol::SessionNetworkProxyRuntime;
use deepseek_protocol::protocol::StreamErrorEvent;
...
use deepseek_protocol::protocol::TurnModerationMetadataEvent;
use deepseek_protocol::protocol::WarningEvent;
use deepseek_protocol::user_input::UserInput;
use deepseek_tools::UnifiedExecShellMode;
use deepseek_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use deepseek_utils_stream_parser::ProposedPlanSegment;

/// The high-level interface to the DeepSeek system.
...
/// Wrapper returned by [`DeepSeek::spawn`] containing the spawned [`DeepSeek`] and
/// the unique session id.
pub struct DeepSeekSpawnOk {
    pub codex: DeepSeek,
    pub thread_id: ThreadId,
}

pub(crate) struct DeepSeekSpawnArgs {
    ...
    pub(crate) code_mode_session_provider: Arc<dyn deepseek_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<deepseek_extension_api::ExtensionRegistry<crate::config::Config>>,
    ...
    pub(crate) supports_deepseek_form_elicitation: bool,
...
const CYBER_VERIFY_URL: &str = "https://platform.deepseek.com/cyber";
const CYBER_SAFETY_URL: &str = "https://platform.deepseek.com/docs

impl DeepSeek {
    /// Spawn a new [`DeepSeek`] and initialize the session.
    pub(crate) async fn spawn(args: DeepSeekSpawnArgs) -> DeepSeekResult<DeepSeekSpawnOk> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if deepseek_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
        ...
        Self::spawn_internal(DeepSeekSpawnArgs {
            parent_trace,
            ..args
        })
    ...
    async fn spawn_internal(args: DeepSeekSpawnArgs) -> DeepSeekResult<DeepSeekSpawnOk> {
        let DeepSeekSpawnArgs {
            mut config,
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| DeepSeekErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            deepseek_models_manager::manager::RefreshStrategy::Offline
        } else {
            deepseek_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        config
            ...
            .map_err(|err| DeepSeekErr::InvalidRequest(err.to_string()))?;
        ...
        // Generate a unique ID for the lifetime of this DeepSeek session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.deepseek_home)
        })?;
        ...
        Ok(DeepSeekSpawnOk { codex, thread_id })
    }

    /// Submit the `op` wrapped in a `Submission` with a unique ID.
    pub async fn submit(&self, op: Op) -> DeepSeekResult<String> {
    ...
    /// Use sparingly: prefer `submit()` so DeepSeek is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, mut sub: Submission) -> DeepSeekResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| DeepSeekErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub async fn set_thread_memory_mode(
        &self,
        mode: deepseek_protocol::protocol::ThreadMemoryMode,
    ) -> anyhow::Result<()> {
        handlers::persist_thread_memory_mode_update(&self.session, mode).await
    }

    pub async fn shutdown_and_wait(&self) -> DeepSeekResult<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(DeepSeekErr::InternalAgentDied) => {}
    ...
    pub async fn next_event(&self) -> DeepSeekResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> DeepSeekResult<PermissionProfileState> {
...
impl Session {
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &deepseek_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn deepseek_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn deepseek_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn deepseek_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.deepseek_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect DeepSeek performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn user_instructions(&self) -> Option<deepseek_extension_api::UserInstructions> {
    ...
    /// Record a terminal DeepSeekErr before the app-server completion notification is reduced.
    pub(crate) fn track_turn_codex_error(&self, turn_context: &TurnContext, error: &DeepSeekErr) {
        self.services
            .analytics_events_client
            .track_turn_codex_error(TurnDeepSeekErrorFact::from_codex_err(
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        if let EventMsg::Error(error) = &legacy_source
            && error
                .codex_error_info
                .as_ref()
                .is_some_and(DeepSeekErrorInfo::affects_turn_status)
        {
            turn_context
                .terminal_error
                .lock()
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &deepseek_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| deepseek_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&deepseek_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        let Ok(native_environment_cwd) = environment.cwd.to_abs_path() else {
            warn!(
                cwd = %environment.cwd,
                "request_permissions requires a cwd native to the DeepSeek host"
            );
            return Some(RequestPermissionsResponse {
                permissions: RequestPermissionProfile::default(),
                scope: PermissionGrantScope::Turn,
        ...
        if crate::guardian::routes_approval_to_guardian(turn_context.as_ref()) {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                request,
                /*retry_reason*/ None,
                deepseek_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(deepseek_hooks::SessionStartSource::Compact);
        }
    }
    ...
    pub(crate) async fn notify_stream_error(
        ...
        codex_error: DeepSeekErr,
    ) {
        let additional_details = codex_error.to_string();
        let codex_error_info = DeepSeekErrorInfo::ResponseStreamDisconnected {
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<deepseek_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_for_config(
    ...
    Hooks::new(HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::DeepSeekHooks),
