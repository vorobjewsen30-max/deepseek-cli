use super::*;
use deepseek_exec_server::ResolvedSelectedCapabilityRoot;
use deepseek_mcp::ElicitationReviewRequest;
use deepseek_mcp::ElicitationReviewer;
use deepseek_mcp::ElicitationReviewerHandle;
use deepseek_protocol::capabilities::CapabilityRootLocation;
use deepseek_protocol::config_types::ApprovalsReviewer;
use deepseek_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_ELICITATION_APPROVAL_KIND_KEY;
use deepseek_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_ELICITATION_APPROVAL_KIND_MCP_TOOL_CALL;
use deepseek_protocol::mcp_approval_meta::APPROVAL_KIND_TOOL_SUGGESTION as MCP_ELICITATION_APPROVAL_KIND_TOOL_SUGGESTION;
use deepseek_protocol::mcp_approval_meta::APPROVALS_REVIEWER_KEY as MCP_ELICITATION_APPROVALS_REVIEWER_KEY;
use deepseek_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_ELICITATION_CONNECTOR_DESCRIPTION_KEY;
use deepseek_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_ELICITATION_CONNECTOR_ID_KEY;
use deepseek_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_ELICITATION_CONNECTOR_NAME_KEY;
use deepseek_protocol::mcp_approval_meta::REQUEST_TYPE_APPROVAL_REQUEST as MCP_ELICITATION_REQUEST_TYPE_APPROVAL_REQUEST;
use deepseek_protocol::mcp_approval_meta::REQUEST_TYPE_KEY as MCP_ELICITATION_REQUEST_TYPE_KEY;
use deepseek_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_ELICITATION_TOOL_DESCRIPTION_KEY;
use deepseek_protocol::mcp_approval_meta::TOOL_NAME_KEY as MCP_ELICITATION_TOOL_NAME_KEY;
use deepseek_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_ELICITATION_TOOL_PARAMS_KEY;
use deepseek_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_ELICITATION_TOOL_TITLE_KEY;
use deepseek_rmcp_client::Elicitation;
...
impl Session {
    ...
    pub(crate) async fn runtime_mcp_servers(
        ...
        deepseek_mcp::configured_mcp_servers(&self.runtime_mcp_config(config).await)
    }
    ...
    pub async fn request_mcp_server_elicitation(
            ...
            .elicitations_auto_deny()
        {
            return McpServerElicitationOutcome {
                response: Some(ElicitationResponse {
                    action: deepseek_rmcp_client::ElicitationAction::Accept,
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                deepseek_protocol::mcp::RequestId::String(value.to_string())
            }
            rmcp::model::NumberOrString::Number(value) => {
                deepseek_protocol::mcp::RequestId::Integer(value)
            }
        };
    ...
    async fn refresh_mcp_servers_inner(
        ...
        let tool_plugin_provenance = deepseek_mcp::tool_plugin_provenance(&mcp_config);
        ...
        let refreshed_manager = McpConnectionManager::new(
            ...
            mcp_config.deepseek_home.clone(),
    ...
    pub(crate) async fn set_deepseek_form_elicitation_support(
        &self,
        supported: bool,
    ) -> anyhow::Result<()> {
        if self
...
fn plugin_install_elicitation_telemetry_metadata(
    ...
    let deepseek_protocol::approvals::ElicitationRequest::Form {
