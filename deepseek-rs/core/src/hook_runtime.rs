...
use deepseek_analytics::CompactionTrigger;
use deepseek_analytics::HookRunFact;
use deepseek_analytics::build_track_events_context;
use deepseek_hooks::PermissionRequestDecision;
use deepseek_hooks::PermissionRequestOutcome;
use deepseek_hooks::PermissionRequestRequest;
use deepseek_hooks::PostToolUseOutcome;
use deepseek_hooks::PostToolUseRequest;
use deepseek_hooks::PreToolUseOutcome;
use deepseek_hooks::PreToolUseRequest;
use deepseek_hooks::SessionStartOutcome;
use deepseek_hooks::StartHookTarget;
use deepseek_hooks::StopHookTarget;
use deepseek_hooks::StopOutcome;
use deepseek_hooks::SubagentHookContext;
use deepseek_hooks::UserPromptSubmitOutcome;
use deepseek_hooks::UserPromptSubmitRequest;
use deepseek_otel::HOOK_RUN_DURATION_METRIC;
use deepseek_otel::HOOK_RUN_METRIC;
...
use deepseek_protocol::protocol::HookCompletedEvent;
use deepseek_protocol::protocol::HookEventName;
use deepseek_protocol::protocol::HookRunStatus;
use deepseek_protocol::protocol::HookRunSummary;
use deepseek_protocol::protocol::HookSource;
use deepseek_protocol::protocol::HookStartedEvent;
        ...
        let target = match &turn_context.session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn { agent_role, .. })
                if matches!(
                    ...
                    deepseek_hooks::SessionStartSource::Startup
                ) =>
            {
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = deepseek_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = deepseek_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = deepseek_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(deepseek_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: deepseek_hooks::HookEvent::AfterAgent {
                event: deepseek_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            deepseek_hooks::HookResult::Success => continue,
            deepseek_hooks::HookResult::FailedContinue(error) => (error, false),
            deepseek_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(deepseek_protocol::protocol::ErrorEvent {
        message,
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (deepseek_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use deepseek_protocol::protocol::HookEventName;
    use deepseek_protocol::protocol::HookExecutionMode;
    use deepseek_protocol::protocol::HookHandlerType;
    use deepseek_protocol::protocol::HookRunStatus;
    use deepseek_protocol::protocol::HookScope;
    use deepseek_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    deepseek_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
