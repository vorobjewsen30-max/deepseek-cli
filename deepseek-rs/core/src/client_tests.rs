...
use crate::test_support::TestDeepSeekResponsesRequestKind;
use crate::test_support::responses_metadata as test_responses_metadata;
use deepseek_api::AgentIdentityTelemetry;
use deepseek_api::ApiError;
use deepseek_api::ResponseEvent;
...
use deepseek_model_provider::BearerAuthProvider;
use deepseek_model_provider::SharedModelProvider;
use deepseek_model_provider::create_model_provider;
use deepseek_model_provider_info::DEEPSEEK_API_BASE_URL;
...
use deepseek_rollout_trace::ExecutionStatus;
...
use deepseek_rollout_trace::RolloutTrace;
...
async fn compact_uses_bearer_after_agent_identity_session_fallback() -> anyhow::Result<()> {
    ...
    let auth_manager = chatgpt_auth_manager(&deepseek_home, server.uri()).await;
    let mut provider = ModelProviderInfo::create_deepseek_provider(/*base_url*/ None);
    ...
    let output = client
        .compact_conversation_history(
            ...
            CompactConversationRequestSettings {
                effort: None,
                summary: deepseek_protocol::config_types::ReasoningSummary::None,
                service_tier: None,
            },
...
fn write_chatgpt_auth_json(deepseek_home: &std::path::Path) {
    let auth_json = json!({
        "tokens": {
...
async fn chatgpt_auth_manager(
    ...
    write_chatgpt_auth_json(deepseek_home.path());
...
fn started_inference_attempt(temp: &TempDir) -> anyhow::Result<InferenceTraceAttempt> {
    ...
    writer.append(RawTraceEventPayload::DeepSeekTurnStarted {
...
async fn bedrock_unauthorized_error_uses_provider_mapping() {
    ...
    let url = "https://bedrock-mantle.us-east-2.api.aws/deepseek/v1/responses";
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "Amazon Bedrock rejected the request because its AWS signature has expired. Refresh your AWS credentials and retry. If `AWS_BEARER_TOKEN_BEDROCK` is set, update or unset it, then restart DeepSeek, url: {url}"
        )
    );
}
...
fn model_client_with_counting_attestation(
    ...
    let (auth_manager, provider) = if include_attestation {
        (
            ...
            ModelProviderInfo::create_deepseek_provider(Some(DEEPSEEK_API_BASE_URL.to_string())),
        )
    } else {
        (
            None,
