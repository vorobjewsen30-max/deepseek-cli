...
use deepseek_config::ConfigError;
...
use deepseek_config::ConfigRequirementsWithSources;
use deepseek_config::FilesystemDenyReadPattern;
use deepseek_config::LoaderOverrides;
use deepseek_config::RequirementSource;
use deepseek_config::RequirementsLayerEntry;
use deepseek_config::SessionThreadConfig;
use deepseek_config::StaticThreadConfigLoader;
use deepseek_config::ThreadConfigSource;
use deepseek_config::compose_requirements;
use deepseek_config::config_error_from_ignored_toml_fields;
use deepseek_config::config_error_from_toml;
...
use deepseek_config::loader::load_requirements_toml;
...
async fn cli_overrides_resolve_relative_paths_against_cwd() -> std::io::Result<()> {
    let deepseek_home = tempdir().expect("tempdir");
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when DEEPSEEK_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .deepseek_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = deepseek_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        deepseek_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = deepseek_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = deepseek_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when DEEPSEEK_HOME/config.toml does not exist");
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![deepseek_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(deepseek_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(deepseek_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            requirement_source: RequirementSource::MdmManagedPreferences {
                domain: "com.deepseek.codex".to_string(),
...
async fn load_config_layers_includes_cloud_config_bundle() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let deepseek_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&deepseek_home).await?;
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            ConfigLayerSource::User {
                file: AbsolutePathBuf::from_absolute_path(deepseek_home.join(CONFIG_TOML_FILE))?,
                profile: None,
            },
        ]
    );
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_deepseek_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let deepseek_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&deepseek_home).await?;
...
"#;
    ...
    assert_eq!(
        ...
        Some(vec![
            FilesystemDenyReadPattern::from_input(&format!("{}/secrets/**", deepseek_home.display()))
                .expect("bundle requirements path should resolve against deepseek_home")
        ])
    );
...
async fn strict_config_rejects_unknown_cloud_config_key() {
    let tmp = tempdir().expect("tempdir");
    let deepseek_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&deepseek_home)
        .await
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            deepseek_config::SandboxModeRequirement::ReadOnly,
            deepseek_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    deepseek_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let deepseek_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&deepseek_home).await?;
    tokio::fs::write(deepseek_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn deepseek_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let deepseek_home = home_dir.join(".codex");
    tokio::fs::create_dir_all(&deepseek_home).await?;
    tokio::fs::write(
        deepseek_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn deepseek_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#,
    ...
    let deepseek_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&deepseek_home_untrusted).await?;
    make_config_for_test(
        &deepseek_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = deepseek_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let deepseek_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&deepseek_home_unknown).await?;
    tokio::fs::write(
        deepseek_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    for (name, trust_level) in cases {
        let deepseek_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&deepseek_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let deepseek_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&deepseek_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &deepseek_home,
...
mod requirements_exec_policy_tests {
    ...
    use deepseek_config::ConfigRequirementsWithSources;
    use deepseek_config::RequirementSource;
    use deepseek_config::RequirementsExecPolicyDecisionToml;
    use deepseek_config::RequirementsExecPolicyParseError;
    use deepseek_config::RequirementsExecPolicyPatternTokenToml;
    use deepseek_config::RequirementsExecPolicyPrefixRuleToml;
    use deepseek_config::RequirementsExecPolicyToml;
    use deepseek_execpolicy::Decision;
    use deepseek_execpolicy::Evaluation;
    use deepseek_execpolicy::RuleMatch;
    use deepseek_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
