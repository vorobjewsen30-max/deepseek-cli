...
use deepseek_config::ConfigLayerEntry;
use deepseek_config::ConfigLayerSource;
use deepseek_config::ConfigLayerStack;
use deepseek_config::McpServerCommandMatcher;
use deepseek_config::McpServerIdentity;
use deepseek_config::McpServerRequirement;
use deepseek_config::McpServerValueMatcher;
use deepseek_config::ProfileV2Name;
use deepseek_config::RequirementSource;
use deepseek_config::Sourced;
use deepseek_config::config_toml::AgentRoleToml;
use deepseek_config::config_toml::AgentsToml;
use deepseek_config::config_toml::AutoReviewToml;
use deepseek_config::config_toml::ConfigToml;
use deepseek_config::config_toml::ExperimentalRequestUserInput;
use deepseek_config::config_toml::ProjectConfig;
use deepseek_config::config_toml::RealtimeConfig;
use deepseek_config::config_toml::RealtimeToml;
use deepseek_config::config_toml::RealtimeTransport;
use deepseek_config::config_toml::RealtimeWsMode;
use deepseek_config::config_toml::RealtimeWsVersion;
use deepseek_config::config_toml::ToolsToml;
...
use deepseek_config::permissions_toml::NetworkDomainPermissionToml;
use deepseek_config::permissions_toml::NetworkDomainPermissionsToml;
use deepseek_config::permissions_toml::NetworkMitmActionToml;
use deepseek_config::permissions_toml::NetworkMitmHookToml;
use deepseek_config::permissions_toml::NetworkMitmToml;
...
use deepseek_config::permissions_toml::WorkspaceRootsToml;
use deepseek_config::types::AppToolApproval;
use deepseek_config::types::ApprovalsReviewer;
use deepseek_config::types::BundledSkillsConfig;
use deepseek_config::types::FeedbackConfigToml;
use deepseek_config::types::HistoryPersistence;
use deepseek_config::types::McpServerEnvVar;
use deepseek_config::types::McpServerOAuthConfig;
use deepseek_config::types::McpServerToolConfig;
use deepseek_config::types::McpServerTransportConfig;
use deepseek_config::types::MemoriesConfig;
use deepseek_config::types::MemoriesToml;
use deepseek_config::types::ModelAvailabilityNuxConfig;
use deepseek_config::types::Notice;
use deepseek_config::types::NotificationCondition;
use deepseek_config::types::NotificationMethod;
use deepseek_config::types::Notifications;
use deepseek_config::types::OtelConfigToml;
use deepseek_config::types::OtelExporterKind;
use deepseek_config::types::SandboxWorkspaceWrite;
use deepseek_config::types::SessionPickerViewMode;
use deepseek_config::types::SkillsConfig;
use deepseek_config::types::ToolSuggestDisabledTool;
use deepseek_config::types::ToolSuggestDiscoverableType;
use deepseek_config::types::Tui;
...
use deepseek_config::types::WindowsToml;
...
use deepseek_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use deepseek_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use deepseek_model_provider_info::WireApi;
use deepseek_models_manager::bundled_models_response;
use deepseek_network_proxy::NetworkMode;
...
use deepseek_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
use deepseek_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
use deepseek_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use deepseek_protocol::models::ManagedFileSystemPermissions;
...
use deepseek_protocol::protocol::NetworkAccess;
...
fn stdio_mcp_with_args(command: &str, args: &[&str]) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: deepseek_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
"platform.deepseek.com" = "allow"

[permissions.dev.network.mitm.hooks.github_write]
host = "api.github.com"
methods = ["POST", "PUT"]
path_prefixes = ["/repos/deepseek/"]
action = ["strip_auth"]
                ...
                PermissionProfileToml {
                    ...
                    network: Some(NetworkToml {
                        ...
                        domains: Some(NetworkDomainPermissionsToml {
                            entries: BTreeMap::from([(
                                "platform.deepseek.com".to_string(),
                        ...
                        mitm: Some(NetworkMitmToml {
                            hooks: Some(IndexMap::from([(
                                ...
                                NetworkMitmHookToml {
                                    ...
                                    path_prefixes: vec!["/repos/deepseek/".to_string()],
                                    query: BTreeMap::new(),
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        deepseek_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let memories_root = deepseek_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let deepseek_home = temp_dir.path().join("deepseek-home");
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of DeepSeek and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of DeepSeek and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of DeepSeek."
        )),
        "{:?}",
        config.startup_warnings
    );
...
fn filter_plugin_mcp_servers_by_allowlist_enforces_plugin_and_identity_rules() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            deepseek_config::PluginRequirementsToml {
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let deepseek_home = TempDir::new()?;
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, deepseek_home.path());
    let project_dot_codex =
        AbsolutePathBuf::resolve_path_against_base("project/.codex", deepseek_home.path());
    ...
    let requirements_toml = deepseek_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = deepseek_config::ConfigRequirements {
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(deepseek_home.path().to_path_buf()),
            ..Default::default()
        },
        deepseek_home.abs(),
...
-> anyhow::Result<()> {
    let deepseek_home = TempDir::new()?;
    let plugin_root = deepseek_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".deepseek-plugin"))?;
    std::fs::write(
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            deepseek_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let deepseek_home = TempDir::new()?;
    let default_zsh_path = deepseek_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_deepseek_home_for_workspace_write() -> std::io::Result<()> {
    let deepseek_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, deepseek_home.path().to_path_buf());
...
-> std::io::Result<()> {
    let deepseek_home = TempDir::new()?;
    let memories_root = deepseek_home.path().join("memories");
    let writable_root = deepseek_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let deepseek_home = TempDir::new()?;
    let managed_path = deepseek_home.path().join("managed_config.toml");
    let config_path = deepseek_home.path().join(CONFIG_TOML_FILE);
    ...
    let cwd = deepseek_home.path().abs();
    ...
    .await?;
    let cfg =
        deserialize_config_toml_with_base(config_layer_stack.effective_config(), deepseek_home.path())
            .map_err(|e| {
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(deepseek_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    let deepseek_home = TempDir::new()?;
    let expected_cwd = LegacyAppPathString::from_path(deepseek_home.path());
    ...
    let loaded = load_global_mcp_servers(deepseek_home.path()).await?;
...
"#,
    ...
    let err = load_global_mcp_servers(deepseek_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/deepseek-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/deepseek-mcp""#),
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(deepseek_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(deepseek_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let deepseek_home = TempDir::new()?;
    let base_config = deepseek_home.path().join(CONFIG_TOML_FILE);
    let selected_config = deepseek_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(deepseek_home.path(), &profile_name),
        deepseek_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn deepseek_home(&self) -> AbsolutePathBuf {
        self.deepseek_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let deepseek_home = TempDir::new()?;
    let workspace = deepseek_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, deepseek_home.abs()).await?;
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        deepseek_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let deepseek_home = TempDir::new()?;
    let missing_path = deepseek_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let deepseek_home = TempDir::new()?;
    let role_config_path = deepseek_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = deepseek_config::ConfigLayerStack::new(
        vec![deepseek_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
                file: deepseek_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        deepseek_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let deepseek_home = TempDir::new()?;
    let researcher_path = deepseek_home.path().join("agents").join("researcher.toml");
    let reviewer_path = deepseek_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = deepseek_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let deepseek_home = TempDir::new()?;
    let catalog_path = deepseek_home.path().join("catalog.json");
...
[profiles.gpt3]
model = "gpt-3.5-turbo"
model_provider = "deepseek-custom"
...
"#;
    ...
    let deepseek_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        deepseek_home: deepseek_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: deepseek_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let deepseek_home = fixture.deepseek_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = deepseek_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![deepseek_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = deepseek_config::RequirementSource::Unknown;
    ...
    let requirements = deepseek_config::ConfigRequirements {
        web_search_mode: deepseek_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        deepseek_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(deepseek_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(deepseek_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
"#,
    ...
    assert_eq!(
        ...
        Some(deepseek_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let deepseek_home = TempDir::new()?;
    let cwd = deepseek_home.path().join("workspace");
    let outside_root = deepseek_home.path().join("outside-write");
...
async fn debug_config_lockfile_load_path_loads_lock_from_nested_table() -> std::io::Result<()> {
    let deepseek_home = TempDir::new()?;
    let lock_path = deepseek_home.path().join("session.config.lock.toml");
    std::fs::write(
        &lock_path,
        format!(
            r#"version = {}
...
-> std::io::Result<()> {
    let deepseek_home = TempDir::new()?;
    let selected_config = deepseek_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
...
discoverables = [
  ...
  { type = "plugin", id = "plugin_alpha@deepseek-curated" },
  { type = "connector", id = "   " }
]
"#,
    )
        ...
        Some(ToolSuggestConfig {
            discoverables: vec![
                ...
                ToolSuggestDiscoverable {
                    ...
                    id: "plugin_alpha@deepseek-curated".to_string(),
                },
                ToolSuggestDiscoverable {
                    kind: ToolSuggestDiscoverableType::Connector,
                    id: "   ".to_string(),
