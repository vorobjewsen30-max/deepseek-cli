...
async fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let deepseek_home = temp_dir.path().join(".codex");
    let zsh_path = temp_dir.path().join("runtime").join("zsh");
    let arg0_root = deepseek_home.join("tmp").join("arg0");
    let allowed_arg0_dir = arg0_root.join("deepseek-arg0-session");
    let sibling_arg0_dir = arg0_root.join("deepseek-arg0-other-session");
    let execve_wrapper = allowed_arg0_dir.join("deepseek-execve-wrapper");
    ...
    let config = Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(&deepseek_home)?,
    )
    .await?;
...
fn network_toml_ignores_legacy_network_list_keys() {
    let parsed = toml::from_str::<NetworkToml>(
        r#"
allowed_domains = ["platform.deepseek.com"]
"#,
    )
...
fn network_toml_overlays_unix_socket_permissions_by_path() {
    ...
    assert_eq!(
        ...
        Some(deepseek_network_proxy::NetworkUnixSocketPermissions {
            entries: BTreeMap::from([
                (
                    "/tmp/base.sock".to_string(),
...
fn profile_network_proxy_config_keeps_proxy_disabled_for_proxy_policy() {
    ...
    assert_eq!(
        ...
        Some(deepseek_network_proxy::NetworkDomainPermissions {
            entries: vec![deepseek_network_proxy::NetworkDomainPermissionEntry {
                pattern: "platform.deepseek.com".to_string(),
                permission: deepseek_network_proxy::NetworkDomainPermission::Allow,
            }],
        })
    );
}
