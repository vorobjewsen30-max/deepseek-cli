...
use deepseek_config::CloudConfigBundleLoader;
use deepseek_config::ConfigLayerSource;
use deepseek_config::ConfigLayerStack;
use deepseek_config::ConfigLayerStackOrdering;
use deepseek_config::ConfigRequirements;
use deepseek_config::ConfigRequirementsToml;
use deepseek_config::ConstrainedWithSource;
use deepseek_config::FeatureRequirementsToml;
use deepseek_config::McpServerRequirement;
use deepseek_config::PluginRequirementsToml;
use deepseek_config::ProfileV2Name;
use deepseek_config::ResidencyRequirement;
use deepseek_config::SandboxModeRequirement;
use deepseek_config::Sourced;
use deepseek_config::ThreadConfigLoader;
use deepseek_config::config_toml::ConfigLockfileToml;
use deepseek_config::config_toml::ConfigToml;
use deepseek_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use deepseek_config::config_toml::ProjectConfig;
use deepseek_config::config_toml::RealtimeAudioConfig;
use deepseek_config::config_toml::RealtimeConfig;
use deepseek_config::config_toml::ThreadStoreToml;
use deepseek_config::config_toml::validate_model_providers;
use deepseek_config::loader::load_config_layers_state;
use deepseek_config::loader::project_trust_key;
use deepseek_config::permissions_toml::PermissionsToml;
use deepseek_config::sandbox_mode_requirement_for_permission_profile;
use deepseek_config::types::ApprovalsReviewer;
use deepseek_config::types::AuthCredentialsStoreMode;
use deepseek_config::types::AuthKeyringBackendKind;
use deepseek_config::types::History;
use deepseek_config::types::McpServerConfig;
use deepseek_config::types::McpServerDisabledReason;
use deepseek_config::types::MemoriesConfig;
use deepseek_config::types::ModelAvailabilityNuxConfig;
use deepseek_config::types::Notice;
use deepseek_config::types::OAuthCredentialsStoreMode;
use deepseek_config::types::SessionPickerViewMode;
...
use deepseek_config::types::TuiKeymap;
use deepseek_config::types::TuiNotificationSettings;
use deepseek_config::types::TuiPetAnchor;
use deepseek_config::types::UriBasedFileOpener;
use deepseek_config::types::WindowsSandboxModeToml;
use deepseek_core_plugins::PluginsConfigInput;
use deepseek_exec_server::ExecutorFileSystem;
use deepseek_exec_server::LOCAL_FS;
use deepseek_features::CodeModeConfigToml;
use deepseek_features::CurrentTimeReminderConfigToml;
use deepseek_features::CurrentTimeReminderDeliveryMode;
use deepseek_features::CurrentTimeSource;
use deepseek_features::Feature;
use deepseek_features::FeatureConfigSource;
use deepseek_features::FeatureOverrides;
use deepseek_features::FeatureToml;
use deepseek_features::Features;
use deepseek_features::FeaturesToml;
use deepseek_features::MultiAgentV2ConfigToml;
use deepseek_features::NetworkProxyConfigToml;
use deepseek_features::TokenBudgetConfigToml;
use deepseek_git_utils::resolve_root_git_project_for_trust;
use deepseek_install_context::InstallContext;
use deepseek_login::AuthManagerConfig;
use deepseek_login::AuthRouteConfig;
use deepseek_mcp::McpConfig;
use deepseek_mcp::McpPluginAttribution;
use deepseek_mcp::McpServerRegistration;
use deepseek_mcp::ResolvedMcpCatalog;
use deepseek_memories_read::memory_root;
use deepseek_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use deepseek_model_provider_info::ModelProviderInfo;
use deepseek_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use deepseek_model_provider_info::built_in_model_providers;
use deepseek_model_provider_info::merge_configured_model_providers;
use deepseek_models_manager::ModelsManagerConfig;
use deepseek_protocol::config_types::AltScreenMode;
use deepseek_protocol::config_types::AutoCompactTokenLimitScope;
...
use deepseek_protocol::config_types::SandboxMode;
use deepseek_protocol::config_types::ServiceTier;
use deepseek_protocol::config_types::ShellEnvironmentPolicy;
use deepseek_protocol::config_types::TrustLevel;
use deepseek_protocol::config_types::Verbosity;
use deepseek_protocol::config_types::WebSearchConfig;
...
pub use deepseek_thread_store::ExtraConfig;
use deepseek_utils_absolute_path::AbsolutePathBuf;
use deepseek_utils_absolute_path::AbsolutePathBufGuard;
...
pub use deepseek_config::ConfigLoadOptions;
pub use deepseek_config::Constrained;
pub use deepseek_config::ConstraintError;
pub use deepseek_config::ConstraintResult;
pub use deepseek_config::LoaderOverrides;
pub use deepseek_network_proxy::NetworkProxyAuditMetadata;
use deepseek_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use deepseek_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(deepseek_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(deepseek_home.path()).expect("temp dir should resolve"),
    )
    .await
...
impl Permissions {
    ...
    /// selection. Internal DeepSeek-only writable roots are intentionally excluded.
...
// when DeepSeek is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<deepseek_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, DeepSeek will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - DeepSeek
    ...
    ///
    /// ```toml
    /// notify = ["notify-send", "DeepSeek"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send DeepSeek '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the DeepSeek home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that DeepSeek can reach out to for tool calls.
    pub mcp_servers: Constrained<HashMap<String, McpServerConfig>>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by DeepSeek unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/vorobjewsen30-max/deepseek-cli
    /// file: DEEPSEEK_HOME/.credentials.json
    ///       This file will be readable to DeepSeek and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, DeepSeek will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all DeepSeek state (defaults to `~/.codex` but can be
    /// overridden by the `DEEPSEEK_HOME` environment variable).
    pub deepseek_home: AbsolutePathBuf,

    /// Directory where DeepSeek stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where DeepSeek writes log files (defaults to `$DEEPSEEK_HOME/log`).
    pub log_dir: PathBuf,

    /// Directory where DeepSeek writes effective session config lock files.
    pub config_lock_export_dir: Option<AbsolutePathBuf>,

    /// Whether config lock replay ignores DeepSeek version drift between the
    ...
    /// Path to the current DeepSeek executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub codex_self_exe: Option<PathBuf>,

    /// Path to the `deepseek-linux-sandbox` executable. This must be set if
    /// [`deepseek_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `deepseek-linux-sandbox`.
    pub deepseek_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `deepseek-execve-wrapper` executable used for shell
    ...
    /// Base URL for requests to DeepSeek (as opposed to the DeepSeek API).
    pub chatgpt_base_url: String,

    /// Whether DeepSeek-owned clients should respect host system proxy settings.
    ...
    /// When set, restricts вход через DeepSeek to one or more workspace identifiers.
    ...
    /// When `true`, checks for DeepSeek updates on startup and surfaces update prompts.
    /// Set to `false` only if your DeepSeek updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across DeepSeek product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across DeepSeek product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: deepseek_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn deepseek_home(&self) -> PathBuf {
        self.deepseek_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn deepseek_home(mut self, deepseek_home: PathBuf) -> Self {
        self.deepseek_home = Some(deepseek_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let deepseek_home = match deepseek_home {
            Some(deepseek_home) => AbsolutePathBuf::from_absolute_path(deepseek_home)?,
            None => find_deepseek_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&deepseek_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = deepseek_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    deepseek_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(deepseek_config::io_error_from_config_error(
...
impl Config {
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &deepseek_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) async fn to_mcp_config_with_plugin_registrations(
        ...
        McpConfig {
            ...
            deepseek_home: self.deepseek_home.to_path_buf(),
            ...
            deepseek_linux_sandbox_exe: self.deepseek_linux_sandbox_exe.clone(),
            ...
            connector_snapshot:
                deepseek_connectors::ConnectorSnapshot::from_plugin_capability_summaries(
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.deepseek_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        let deepseek_home = find_deepseek_home()?;
        Self::load_default_with_cli_overrides_for_deepseek_home(
            deepseek_home.to_path_buf(),
            cli_overrides,
        )
        .await
    }

    /// Load a default configuration for a specific DeepSeek home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_deepseek_home(
        ...
        let cli_layer = deepseek_config::build_cli_overrides_layer(&cli_overrides);
        deepseek_config::merge_toml_values(&mut merged, &cli_layer);
        let deepseek_home = AbsolutePathBuf::from_absolute_path_checked(deepseek_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &deepseek_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// in [ConfigToml], such as `cwd`, `codex_self_exe`, `deepseek_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_loader_overrides(
    ...
    load_config_as_toml_with_cli_and_load_options(deepseek_home, cwd, cli_overrides, loader_overrides)
        .await
}
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(deepseek_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let config_layer_stack = load_config_layers_state(
        ...
        &cli_overrides,
        options,
        &deepseek_config::NoopThreadConfigLoader,
    )
    .await?;

    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, deepseek_home).map_err(|e| {
...
/// Patch `DEEPSEEK_HOME/config.toml` project state to set trust level.
/// Use with caution.
pub fn set_project_trust_level(
    ...
    ConfigEditsBuilder::new(deepseek_home)
        .set_project_trust_level(project_path, trust_level)
        .apply_blocking()
}

/// Save the default OSS provider preference to config.toml
pub fn set_default_oss_provider(deepseek_home: &Path, provider: &str) -> std::io::Result<()> {
    deepseek_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &deepseek_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            deepseek_protocol::permissions::FileSystemSandboxEntry {
                path: deepseek_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: deepseek_protocol::permissions::FileSystemAccessMode::Deny,
            }
        } else {
            ...
            deepseek_protocol::permissions::FileSystemSandboxEntry {
                path: deepseek_protocol::permissions::FileSystemPath::Path { path },
                access: deepseek_protocol::permissions::FileSystemAccessMode::Deny,
            }
        };
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&deepseek_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&deepseek_home);
        ...
        let model_providers =
            merge_configured_model_providers(built_in_model_providers(deepseek_base_url), cfg.model_providers)
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(deepseek_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| deepseek_home.join("log").to_path_buf());
        let sqlite_home = cfg
            .sqlite_home
            .as_ref()
            .map(AbsolutePathBuf::to_path_buf)
            .or_else(|| resolve_sqlite_home_env(&resolved_cwd))
            .unwrap_or_else(|| deepseek_home.to_path_buf());
            ...
            && constrained_approval_policy.value() == AskForApproval::Never
        {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; DeepSeek would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode.",
            ));
        }
        if permission_profile_was_constrained {
        ...
        let config = Self {
            ...
            chatgpt_base_url: cfg
                .chatgpt_base_url
                .unwrap_or("https://api.deepseek.com/v1/".to_string()),
...
pub fn find_deepseek_home() -> std::io::Result<AbsolutePathBuf> {
    deepseek_utils_home_dir::find_deepseek_home()
}

/// Returns the path to the folder where DeepSeek logs are stored. Does not verify
