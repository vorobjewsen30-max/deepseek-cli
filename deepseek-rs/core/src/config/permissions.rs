...
use deepseek_config::permissions_toml::NetworkUnixSocketPermissionToml;
use deepseek_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use deepseek_features::NetworkProxyDomainPermissionToml;
use deepseek_features::NetworkProxyModeToml;
use deepseek_features::NetworkProxyUnixSocketPermissionToml;
use deepseek_network_proxy::NetworkMode;
use deepseek_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use deepseek_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
use deepseek_protocol::permissions::project_roots_glob_pattern;
...
/// for DeepSeek to function. These should always be added to the
...
pub(crate) fn get_readable_roots_required_for_codex_runtime(
    ...
    let arg0_root = AbsolutePathBuf::from_absolute_path(deepseek_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older DeepSeek versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of DeepSeek. Filesystem access will remain restricted. Upgrade DeepSeek if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{}` is not recognized by this version of DeepSeek and will be ignored. Upgrade DeepSeek if this path is required.",
                subpath.display()
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of DeepSeek and will be ignored. Upgrade DeepSeek if this path is required."
            ),
        },
    );
}
