...
/// Start an interactive sub-DeepSeek thread and return IO channels.
///
...
pub(crate) async fn run_codex_thread_interactive(
    ...
    let DeepSeekSpawnOk { codex, .. } = Box::pin(DeepSeek::spawn(DeepSeekSpawnArgs {
        config,
        ...
        supports_deepseek_form_elicitation: parent_session
            .services
...
async fn shutdown_delegate(codex: &DeepSeek) {
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                deepseek_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                deepseek_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                deepseek_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            deepseek_protocol::request_user_input::RequestUserInputAnswer {
...
) -> deepseek_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = deepseek_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, deepseek_protocol::protocol::ReviewDecision::Abort)
                .await;
            deepseek_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
