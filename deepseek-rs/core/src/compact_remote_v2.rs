...
use deepseek_rollout_trace::CompactionCheckpointTracePayload;
...
async fn run_remote_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            ...
            attempt
                .track(
                    sess.as_ref(),
                    deepseek_analytics::CompactionStatus::Interrupted,
                    Some(&error),
                    analytics_details,
                )
                .await;
    ...
    match result {
        Ok(()) => Ok(()),
        Err(err @ DeepSeekErr::TurnAborted) => Err(err),
        Err(err) => {
...
) -> DeepSeekResult<RemoteCompactionV2Output> {
...
async fn collect_compaction_output(
    ...
    if compaction_count != 1 {
        return Err(DeepSeekErr::Fatal(format!(
...
mod tests {
    ...
    fn response_stream(events: Vec<DeepSeekResult<ResponseEvent>>) -> ResponseStream {
