...
use deepseek_utils_string::to_ascii_json_string;
...
pub(crate) enum DeepSeekResponsesRequestKind {
    Turn,
    Prewarm,
...
impl DeepSeekResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            DeepSeekResponsesRequestKind::Turn => ("turn", None),
            DeepSeekResponsesRequestKind::Prewarm => ("prewarm", None),
            DeepSeekResponsesRequestKind::Compaction(metadata) => ("compaction", Some(metadata)),
            DeepSeekResponsesRequestKind::Memory => ("memory", None),
        }
    }

    fn has_turn_identity(self) -> bool {
        !matches!(self, DeepSeekResponsesRequestKind::Memory)
    }
}
...
/// Caller-owned snapshot of DeepSeek metadata sent to ResponsesAPI.
///
/// The full DeepSeek turn metadata blob is transported canonically as
...
/// truth.
#[derive(Clone, Debug)]
pub struct DeepSeekResponsesMetadata {
    pub(crate) installation_id: String,
    pub(crate) session_id: String,
    pub(crate) thread_id: String,
    pub(crate) turn_id: Option<String>,
    pub(crate) window_id: String,
    pub(crate) request_kind: Option<DeepSeekResponsesRequestKind>,
...
impl DeepSeekResponsesMetadata {
    pub(crate) fn new(
    ...
    fn turn_metadata_payload(&self) -> DeepSeekTurnMetadataPayload<'_> {
        ...
        let has_turn_identity =
            request_kind.is_none_or(DeepSeekResponsesRequestKind::has_turn_identity);
        let has_request_identity =
            request_kind.is_some_and(DeepSeekResponsesRequestKind::has_turn_identity);
        DeepSeekTurnMetadataPayload {
            ...
            compaction,
            // responsesapi_client_metadata enriches the DeepSeek turn metadata blob, not literal
            // top-level Responses client_metadata. Reserved DeepSeek-owned keys are filtered when
...
#[derive(Serialize)]
struct DeepSeekTurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]
