...
use deepseek_config::types::OtelExporterKind as Kind;
use deepseek_config::types::OtelHttpProtocol as Protocol;
use deepseek_features::Feature;
use deepseek_login::default_client::originator;
use deepseek_otel::OtelExporter;
use deepseek_otel::OtelHttpProtocol;
use deepseek_otel::OtelProvider;
use deepseek_otel::OtelSettings;
use deepseek_otel::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
/// Filter predicate for exporting only DeepSeek-owned events via OTEL.
/// Keeps events that originated from deepseek_otel module
pub fn codex_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("deepseek_otel")
}
...
pub fn record_process_start(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let _ = deepseek_otel::record_process_start_once(metrics, originator);
}
...
pub fn install_sqlite_telemetry(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let telemetry = deepseek_rollout::sqlite_telemetry_recorder(metrics.clone(), originator);
