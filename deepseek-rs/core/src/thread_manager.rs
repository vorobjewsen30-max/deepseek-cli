...
use crate::codex_thread::DeepSeekThread;
...
use crate::session::DeepSeek;
use crate::session::DeepSeekSpawnArgs;
use crate::session::DeepSeekSpawnOk;
use crate::session::INITIAL_SUBMIT_ID;
use crate::session::resolve_multi_agent_version;
use crate::tasks::InterruptedTurnHistoryMarker;
use crate::tasks::interrupted_turn_history_marker;
use deepseek_agent_graph_store::AgentGraphStore;
use deepseek_agent_graph_store::LocalAgentGraphStore;
...
use deepseek_code_mode::CodeModeSessionProvider;
use deepseek_code_mode::InProcessCodeModeSessionProvider;
use deepseek_code_mode::ProcessOwnedCodeModeSessionProvider;
...
use deepseek_extension_api::ExtensionRegistry;
...
use deepseek_login::default_client::DEEPSEEK_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
use deepseek_model_provider_info::DEEPSEEK_PROVIDER_ID;
...
use deepseek_thread_store::InMemoryThreadStore;
use deepseek_thread_store::LocalThreadStore;
use deepseek_thread_store::LocalThreadStoreConfig;
use deepseek_thread_store::ReadThreadByRolloutPathParams;
use deepseek_thread_store::ReadThreadParams;
use deepseek_thread_store::StoredThread;
use deepseek_thread_store::ThreadMetadataPatch;
use deepseek_thread_store::ThreadStore;
use deepseek_thread_store::ThreadStoreError;
use deepseek_thread_store::UpdateThreadMetadataParams;
...
struct TempDeepSeekHomeGuard {
    path: PathBuf,
}

impl Drop for TempDeepSeekHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created DeepSeek thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<DeepSeekThread>,
...
pub struct ThreadManager {
    ...
    _test_deepseek_home_guard: Option<TempDeepSeekHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<deepseek_protocol::dynamic_tools::DynamicToolSpec>,
    pub metrics_service_name: Option<String>,
    pub parent_trace: Option<W3cTraceContext>,
    pub environments: Vec<TurnEnvironmentSelection>,
    pub thread_extension_init: ExtensionDataInit,
    pub supports_deepseek_form_elicitation: bool,
}
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<DeepSeekThread>>>>,
...
pub fn thread_store_from_config(
    ...
    match &config.experimental_thread_store {
        ThreadStoreConfig::Local => {
            ...
            {
                deepseek_rollout::spawn_rollout_compression_worker(config.deepseek_home.to_path_buf());
            }
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        let deepseek_home = config.deepseek_home.clone();
        ...
        Self {
            ...
            _test_deepseek_home_guard: None,
        }
    }

    /// Construct with a dummy AuthManager containing the provided DeepSeekAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let deepseek_home = std::env::temp_dir().join(format!(
        ...
        manager._test_deepseek_home_guard = Some(TempDeepSeekHomeGuard { path: deepseek_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided DeepSeekAuth and codex home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let skills_deepseek_home = match AbsolutePathBuf::from_absolute_path_checked(&deepseek_home) {
            Ok(deepseek_home) => deepseek_home,
            Err(err) => panic!("test deepseek_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite_home: deepseek_home.clone(),
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(deepseek_home, /*config_model_catalog*/ None),
                environment_manager,
                skills_service,
                plugins_manager,
                mcp_manager,
    ...
    pub fn validate_environment_selections(
        ...
        for environment in environments {
            if !environment_ids.insert(environment.environment_id.as_str()) {
                return Err(DeepSeekErr::InvalidRequest(format!(
            ...
            self.state
                ...
                .ok_or_else(|| {
                    DeepSeekErr::InvalidRequest(format!(
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> DeepSeekResult<Arc<DeepSeekThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `DeepSeekThread`/`LiveThread`, so metadata changes stay ordered
        ...
        self.state
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    DeepSeekErr::ThreadNotFound(thread_id)
                }
    ...
    pub async fn list_agent_subtree_thread_ids(
        ...
        if let Some(agent_graph_store) = self.state.agent_graph_store() {
            for descendant_id in agent_graph_store
                ...
                .await
                .map_err(|err| {
                    DeepSeekErr::Fatal(format!("failed to load thread-spawn descendants: {err}"))
                })?
            {
    ...
    pub async fn start_thread(&self, config: Config) -> DeepSeekResult<NewThread> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<deepseek_protocol::dynamic_tools::DynamicToolSpec>,
    ) -> DeepSeekResult<NewThread> {
    ...
    async fn start_thread_with_options_and_fork_source(
        ...
        Box::pin(self.state.spawn_thread_with_source(
            ...
            options.supports_deepseek_form_elicitation,
    ...
    /// as `Arc<DeepSeekThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<DeepSeekThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> DeepSeekResult<Arc<DeepSeekThread>> {
        let threads = self.threads.read().await;
        match threads.get(&thread_id) {
            Some(thread) if !thread.session_source.is_internal() => Ok(thread.clone()),
            Some(_) | None => Err(DeepSeekErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => DeepSeekErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }

    /// Send an operation to a thread by ID.
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> DeepSeekResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<DeepSeekThread>> {
    ...
    async fn effective_originator(
        ...
        let env_originator = std::env::var(DEEPSEEK_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
            .is_ok()
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        let DeepSeekSpawnOk {
            codex, thread_id, ..
        } = Box::pin(DeepSeek::spawn(DeepSeekSpawnArgs {
            config,
    ...
    async fn finalize_thread_spawn(
        ...
        let session_configured = match event {
            ...
            _ => {
                return Err(DeepSeekErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(DeepSeekThread::new(
                    codex,
        ...
        Err(DeepSeekErr::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> deepseek_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return deepseek_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(deepseek_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> DeepSeekErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => DeepSeekErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => DeepSeekErr::InvalidRequest(message),
        err => DeepSeekErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> DeepSeekErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => DeepSeekErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => DeepSeekErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => DeepSeekErr::UnsupportedOperation(format!(
            "thread metadata update is not supported by this store: {operation}"
        )),
        err => DeepSeekErr::Fatal(format!(
