...
pub use deepseek_connectors::AppBranding;
pub use deepseek_connectors::AppInfo;
pub use deepseek_connectors::AppMetadata;
use deepseek_connectors::ConnectorDirectoryCacheContext;
use deepseek_connectors::ConnectorDirectoryCacheKey;
use deepseek_connectors::app_is_enabled;
use deepseek_connectors::apps_config_from_layer_stack;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = deepseek_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        deepseek_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_cached_accessible_connectors_from_mcp_tools(
    ...
    if !config
        .features
        .apps_enabled_for_auth(auth.as_ref().is_some_and(DeepSeekAuth::uses_codex_backend))
    {
        return Some(Vec::new());
    }
...
fn accessible_connectors_cache_key(
    ...
    let chatgpt_user_id = auth.and_then(DeepSeekAuth::get_chatgpt_user_id);
    let is_workspace_account = auth.is_some_and(DeepSeekAuth::is_workspace_account);
    AccessibleConnectorsCacheKey {
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + deepseek_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    deepseek_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
fn collect_accessible_connectors_from_mcp_tools<'a>(
    ...
    let tools = mcp_tools.filter_map(|tool| {
        ...
        Some(deepseek_connectors::accessible::AccessibleConnectorTool {
    ...
    deepseek_connectors::accessible::collect_accessible_connectors(tools)
}
