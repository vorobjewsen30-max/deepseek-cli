...
This crate implements the business logic for DeepSeek. It is designed to be used by the various DeepSeek UIs written in Rust.
...
Note that `deepseek-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
Expects the binary containing `deepseek-core` to run the equivalent of `codex sandbox` when `arg0` is `deepseek-linux-sandbox`. See the `deepseek-arg0` crate for details.
...
binary shipped with DeepSeek and DeepSeek surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
DeepSeek also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so DeepSeek rejects sandboxed shell commands that would enter the
...
Expects the binary containing `deepseek-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--codex-run-as-apply-patch`. See the
`deepseek-arg0` crate for details.
