...
use deepseek_protocol::protocol::ExecPolicyAmendment;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestDeepSeek) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured DeepSeek helper binary
...
impl Expectation {
    fn verify(&self, test: &TestDeepSeek, result: &CommandResult) -> Result<()> {
        match self {
...
fn assert_active_workspace_permission_profile(test: &TestDeepSeek) {
    assert_eq!(
...
async fn wait_for_completion_without_approval(test: &TestDeepSeek) {
    let event = wait_for_event(&test.codex, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestDeepSeek) {
...
async fn wait_for_spawned_thread(test: &TestDeepSeek) -> Result<Arc<DeepSeekThread>> {
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Print an ssh command that recreates the current DeepSeek sandbox remotely."
    )
...
"#
    ...
    assert_eq!(
        ...
        "remote_bash.py should use the allowlisted inherited profile and managed configuration to reconstruct the DeepSeek sandbox"
    );
