...
enum TestDeepSeekHome {
    Persistent(PathBuf),
    Temporary(TempDir),
}

impl TestDeepSeekHome {
    fn path(&self) -> &Path {
        match self {
...
fn deepseek_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestDeepSeekHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        // The elevated backend provisions machine-local sandbox users. Bazel
        // retries run in the same Windows VM, so keep DEEPSEEK_HOME stable within
        // the test temp root and let setup reconcile its persisted ACL state.
        let deepseek_home = PathBuf::from(test_tmpdir).join(name);
        std::fs::create_dir_all(&deepseek_home)
            .with_context(|| format!("create stable test DEEPSEEK_HOME {}", deepseek_home.display()))?;
        return Ok(TestDeepSeekHome::Persistent(deepseek_home));
    }

    Ok(TestDeepSeekHome::Temporary(TempDir::new()?))
}
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    for helper_name in ["codex-windows-sandbox-setup", "codex-command-runner"] {
        let helper = deepseek_utils_cargo_bin::cargo_bin(helper_name)?;
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    let deepseek_home =
        deepseek_home_for_windows_sandbox_test("windows-restricted-token-deny-read-deepseek-home")?;
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    let deepseek_home = deepseek_home_for_windows_sandbox_test("windows-elevated-deny-read-deepseek-home")?;
    let _deepseek_home_guard = EnvVarGuard::set("DEEPSEEK_HOME", deepseek_home.path().as_os_str());
    ...
    let public = cwd.join("public.txt");
    let setup_marker = deepseek_home.path().join(".sandbox").join("setup_marker.json");
    ...
    assert!(
        sandbox_setup_is_complete(deepseek_home.path()),
