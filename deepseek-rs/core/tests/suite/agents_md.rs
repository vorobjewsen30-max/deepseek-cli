...
async fn agents_instructions(mut builder: TestDeepSeekBuilder) -> Result<String> {
...
async fn submit_thread_turn(thread: &Arc<deepseek_core::DeepSeekThread>, prompt: &str) -> Result<()> {
    thread
        .submit(Op::UserInput {
...
async fn loads_user_instructions_without_a_primary_environment() -> Result<()> {
    ...
    let provider = Arc::new(RecordingUserInstructionsProvider::new(Arc::new(
        DeepSeekHomeUserInstructionsProvider::new(AbsolutePathBuf::try_from(
