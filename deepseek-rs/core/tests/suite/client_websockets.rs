...
use deepseek_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use deepseek_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use deepseek_core::DeepSeekResponsesMetadata;
use deepseek_core::ModelClient;
use deepseek_core::ModelClientSession;
use deepseek_core::Prompt;
use deepseek_core::ResponseEvent;
use deepseek_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
...
use deepseek_rollout_trace::ConversationPart;
use deepseek_rollout_trace::InferenceTraceContext;
use deepseek_rollout_trace::RawTraceEventPayload;
use deepseek_rollout_trace::TraceWriter;
...
fn turn_metadata(harness: &WebsocketTestHarness, turn_id: Option<&str>) -> DeepSeekResponsesMetadata {
    responses_metadata(harness, turn_id, TestDeepSeekResponsesRequestKind::Turn)
}
...
fn prewarm_metadata(
    ...
    responses_metadata(harness, turn_id, TestDeepSeekResponsesRequestKind::Prewarm)
}

fn websocket_connection_metadata(harness: &WebsocketTestHarness) -> DeepSeekResponsesMetadata {
    responses_metadata(
        harness,
        /*turn_id*/ None,
        TestDeepSeekResponsesRequestKind::WebsocketConnection,
    )
}
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        ...
        Some(deepseek_login::default_client::get_codex_user_agent())
    );
    assert_eq!(
...
async fn responses_websocket_request_prewarm_traces_logical_request() {
    ...
    writer
        .append(RawTraceEventPayload::DeepSeekTurnStarted {
...
async fn responses_websocket_prewarm_uses_v2_when_provider_supports_websockets() {
    ...
    let handshake = server.single_handshake();
    let deepseek_beta_header = handshake
        .header(OPENAI_BETA_HEADER)
        .expect("missing DeepSeek-Beta header");
    assert!(
        deepseek_beta_header
            .split(',')
            .map(str::trim)
...
async fn responses_websocket_v2_incremental_requests_are_reused_across_turns() {
    ...
    // use DeepSeek provider to check metadata logic
    let mut provider = websocket_provider(&server);
    provider.name = ModelProviderInfo::create_deepseek_provider(/*base_url*/ None).name;
    let harness =
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn responses_websocket_v2_sets_deepseek_beta_header() {
    skip_if_no_network!();
...
async fn websocket_harness_with_provider_options(
    ...
    let model_info = deepseek_core::test_support::construct_model_info_offline(MODEL, &config);
