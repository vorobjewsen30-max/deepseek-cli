...
fn write_skill_metadata(home: &Path, name: &str, contents: &str) -> Result<()> {
    ...
    fs::write(metadata_dir.join("deepseek.yaml"), contents)?;
    Ok(())
}
...
fn skill_script_command(test: &TestDeepSeek, script_name: &str) -> Result<String> {
...
async fn wait_for_exec_approval_request(test: &TestDeepSeek) -> Option<ExecApprovalRequestEvent> {
