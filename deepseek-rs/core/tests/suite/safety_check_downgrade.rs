...
fn disabled_text_turn(test: &TestDeepSeek, text: &str) -> Op {
...
async fn deepseek_model_header_mismatch_emits_warning_event() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let server = start_mock_server().await;
    let response =
        sse_response(sse_completed("resp-1")).insert_header("DeepSeek-Model", SERVER_MODEL);
...
async fn cyber_policy_response_emits_typed_error_without_retry() -> Result<()> {
    ...
    assert_eq!(error.codex_error_info, Some(DeepSeekErrorInfo::CyberPolicy));
...
async fn response_model_field_mismatch_emits_warning_when_header_matches_requested() -> Result<()> {
    ...
    .insert_header("DeepSeek-Model", REQUESTED_MODEL);
    let _mock = mount_response_once(&server, response).await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn deepseek_model_header_mismatch_only_emits_one_warning_per_turn() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    .insert_header("DeepSeek-Model", SERVER_MODEL);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn deepseek_model_header_casing_only_mismatch_does_not_warn() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let server = start_mock_server().await;
    let requested_header = REQUESTED_MODEL.to_ascii_uppercase();
    let response = sse_response(sse_completed("resp-1"))
        .insert_header("DeepSeek-Model", requested_header.as_str());
    let _mock = mount_response_once(&server, response).await;
