...
use deepseek_core::REVIEW_PROMPT;
use deepseek_core::config::Config;
use deepseek_core::review_format::render_review_output_text;
use deepseek_protocol::models::ContentItem;
use deepseek_protocol::models::ResponseItem;
use deepseek_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use deepseek_protocol::protocol::EventMsg;
use deepseek_protocol::protocol::ExitedReviewModeEvent;
use deepseek_protocol::protocol::Op;
use deepseek_protocol::protocol::ReviewCodeLocation;
use deepseek_protocol::protocol::ReviewFinding;
use deepseek_protocol::protocol::ReviewLineRange;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under DeepSeek sandbox network restrictions.
    skip_if_no_network!();
    ...
    let codex = new_conversation_for_server(&server, deepseek_home.clone(), |_| {}).await;
    ...
    assert_eq!(
        request.header("x-deepseek-subagent").as_deref(),
        Some("review")
    );
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let codex = new_conversation_for_server(&server, deepseek_home.clone(), |cfg| {
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = deepseek_home.path().join("resume.jsonl");
    {
        ...
        let user = deepseek_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = deepseek_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let codex =
        resume_conversation_for_server(&server, deepseek_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let codex = new_conversation_for_server(&server, deepseek_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, deepseek_home, resume_path)
        .await
