...
use deepseek_config::types::McpServerAuth;
...
use deepseek_exec_server::HttpRedirectPolicy;
use deepseek_exec_server::HttpRequestParams;
use deepseek_login::DeepSeekAuth;
use deepseek_mcp::MCP_SANDBOX_STATE_META_CAPABILITY;
...
use deepseek_utils_cargo_bin::cargo_bin;
...
fn auto_approved_user_turn(fixture: &TestDeepSeek, text: impl Into<String>) -> Op {
...
fn remote_aware_environment_id() -> String {
    if is_remote_test_environment() {
        ...
        deepseek_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
async fn deepseek_form_capability_is_advertised_to_mcp_servers() -> anyhow::Result<()> {
    assert_deepseek_form_capability_advertisement(/*expected*/ true).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn deepseek_form_capability_is_not_advertised_by_default() -> anyhow::Result<()> {
    assert_deepseek_form_capability_advertisement(/*expected*/ false).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn deepseek_form_capability_updates_for_loaded_thread() -> anyhow::Result<()> {
    skip_if_wine_exec!(
        Ok(()),
    ...
    fixture
        .codex
        .set_deepseek_form_elicitation_support(/*supported*/ true)
        .await?;
...
async fn assert_deepseek_form_capability_advertisement(expected: bool) -> anyhow::Result<()> {
    skip_if_wine_exec!(
        Ok(()),
        "requires a Windows test_stdio_server in the Wine-exec environment"
    );
    ...
    if expected {
        builder = builder.with_deepseek_form_elicitation();
    }
...
async fn stdio_mcp_tool_call_includes_sandbox_state_meta() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        SandboxState {
            ...
            deepseek_linux_sandbox_exe: fixture.config.deepseek_linux_sandbox_exe.clone(),
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: deepseek_protocol::deepseek_models::ReasoningEffort::Medium,
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that DeepSeek should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: DeepSeek can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so DeepSeek will call the MCP echo tool
    ...
    // Phase 3: configure DeepSeek with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert DeepSeek begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.codex, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn configured_chatgpt_base_url_does_not_grant_mcp_chatgpt_auth() -> anyhow::Result<()> {
    ...
    let fixture = test_codex()
        .with_auth(DeepSeekAuth::create_dummy_chatgpt_auth_for_testing())
        .with_pre_build_hook(move |deepseek_home| {
            fs::write(
...
/// This test writes to a fallback credentials file in DEEPSEEK_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire DEEPSEEK_HOME through the code.
#[test]
#[serial(deepseek_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so DeepSeek will call the OAuth-backed
    ...
    // Phase 3: seed an isolated DEEPSEEK_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _deepseek_home_guard = EnvVarGuard::set("DEEPSEEK_HOME", temp_home.path().as_os_str());
    ...
    // Phase 4: configure DeepSeek with the OAuth-backed Streamable HTTP MCP
    ...
    // Phase 7: assert DeepSeek begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.codex, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the DeepSeek fixture.
