...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let specific = ModelInfo {
        display_name: "GPT 5.3 DeepSeek".to_string(),
    ...
    let manager = deepseek_core::test_support::models_manager_with_provider(
        deepseek_home.path().to_path_buf(),
        deepseek_core::test_support::auth_manager_from_auth(auth),
        provider,
    );
...
fn bundled_default_model_slug() -> String {
    deepseek_core::test_support::all_model_presets()
        .iter()
