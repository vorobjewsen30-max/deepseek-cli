...
use deepseek_models_manager::model_info::model_info_from_slug;
...
async fn remote_tool_mode_selector_overrides_feature_flags() -> Result<()> {
    ...
    assert!(
        direct_tools
            .iter()
            .all(|name| name != deepseek_code_mode::PUBLIC_TOOL_NAME
                && name != deepseek_code_mode::WAIT_TOOL_NAME),
    ...
    assert_eq!(
        ...
        vec![
            ...
            deepseek_code_mode::PUBLIC_TOOL_NAME.to_string(),
            deepseek_code_mode::WAIT_TOOL_NAME.to_string(),
    ...
    assert!(
        tool_names(&unsupported_response.body)
            .iter()
            .any(|name| name == deepseek_code_mode::PUBLIC_TOOL_NAME)
    );
    assert_eq!(
