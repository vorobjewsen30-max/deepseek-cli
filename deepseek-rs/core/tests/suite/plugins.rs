...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    write_plugin_skill_plugin(deepseek_home.as_ref());
    write_plugin_app_plugin(deepseek_home.as_ref());
    let test_codex = build_apps_enabled_plugin_test_codex(
        &server,
        Arc::clone(&deepseek_home),
    ...
    codex
        .submit(Op::UserInput {
            items: vec![deepseek_protocol::user_input::UserInput::Text {
                text: "hello".into(),
...
async fn explicit_plugin_mentions_use_apps_for_chatgpt_dual_surface_plugins() -> Result<()> {
    ...
    write_plugin_skill_plugin(deepseek_home.as_ref());
    write_plugin_mcp_plugin(deepseek_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(deepseek_home.as_ref());

    let test_codex =
        build_apps_enabled_plugin_test_codex(&server, deepseek_home, apps_server.chatgpt_base_url)
            .await?;
    let codex = Arc::clone(&test_codex.codex);
    ...
    codex
        .submit(Op::UserInput {
            items: vec![deepseek_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
    ...
    assert!(
        ...
        "expected plugin MCP guidance to be suppressed for DeepSeek auth: {developer_messages:?}"
    );
    assert!(
        developer_messages
            .iter()
    ...
    assert!(
        request
            ...
            .is_none(),
        "plugin MCP tool should not leak into the request for DeepSeek auth"
    );
    ...
    assert!(
        echo_tool.is_none(),
        "plugin MCP tool should be suppressed for DeepSeek auth"
    );
...
async fn explicit_plugin_mentions_keep_non_conflicting_mcp_for_chatgpt_auth() -> Result<()> {
    ...
    write_plugin_skill_plugin(deepseek_home.as_ref());
    write_plugin_mcp_plugin(deepseek_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin_with_name(deepseek_home.as_ref(), "sample_app");
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let deepseek_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(deepseek_home.as_ref());
    let test_codex = build_analytics_plugin_test_codex(&server, deepseek_home).await?;
    let codex = Arc::clone(&test_codex.codex);
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "deepseek_plugin_used")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        serde_json::json!(deepseek_login::default_client::originator().value)
    );
