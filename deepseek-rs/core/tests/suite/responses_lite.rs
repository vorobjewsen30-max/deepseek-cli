...
const RESPONSES_LITE_HEADER: &str = "x-deepseek-internal-codex-responses-lite";

fn responses_extensions(auth: &DeepSeekAuth) -> Arc<ExtensionRegistry<Config>> {
...
fn configure_image_capable_model(model_info: &mut deepseek_protocol::deepseek_models::ModelInfo) {
...
async fn responses_lite_exposes_standalone_tools_for_actor_authorized_provider() -> Result<()> {
    ...
    let mut builder = test_codex()
        ...
        .with_config(|config| {
            ...
            config.model_provider.requires_deepseek_auth = false;
