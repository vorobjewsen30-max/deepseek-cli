...
async fn run_turn(test: &TestDeepSeek, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestDeepSeek, prompt: &str) -> anyhow::Result<Duration> {
    let start = Instant::now();
    run_turn(test, prompt).await?;
    Ok(start.elapsed())
}

async fn build_codex_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestDeepSeek> {
