...
use deepseek_protocol::items::SleepItem;
...
async fn build_codex(server: &StreamingSseServer) -> Arc<DeepSeekThread> {
    test_codex()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .expect("build streaming DeepSeek test session")
        .codex
}

async fn submit_user_input(codex: &DeepSeekThread, text: &str) {
    codex
        .submit(Op::UserInput {
...
async fn submit_danger_full_access_user_turn(test: &TestDeepSeek, text: &str) {
...
async fn steer_user_input(codex: &DeepSeekThread, text: &str) {
    codex
        .steer_input(
            vec![UserInput::Text {
                text: text.to_string(),
...
async fn submit_queue_only_agent_mail(codex: &DeepSeekThread, text: &str) {
    codex
...
async fn wait_for_reasoning_item_started(codex: &DeepSeekThread) {
    wait_for_event(codex, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(codex: &DeepSeekThread, text: &str) {
...
async fn wait_for_turn_complete(codex: &DeepSeekThread) {
    wait_for_event(codex, |event| matches!(event, EventMsg::TurnComplete(_))).await;
}

async fn wait_for_sleep_item_started(codex: &DeepSeekThread, call_id: &str, duration_ms: u64) {
...
async fn wait_for_sleep_item_completed(codex: &DeepSeekThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(codex, |event| {
        matches!(
            event,
...
async fn steer_interrupts_wait_agent_and_is_sent_in_follow_up_request() {
    ...
    let codex = test_codex()
        ...
        .build_with_streaming_server(&server)
        .await
        .expect("build DeepSeek test session")
        .codex;
