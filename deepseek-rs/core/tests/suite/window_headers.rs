...
async fn submit_user_turn(codex: &Arc<DeepSeekThread>, text: &str) -> Result<()> {
    codex
        .submit(Op::UserInput {
...
async fn submit_compact_turn(codex: &Arc<DeepSeekThread>) -> Result<()> {
...
async fn shutdown_thread(codex: &Arc<DeepSeekThread>) -> Result<()> {
