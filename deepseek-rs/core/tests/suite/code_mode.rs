...
use deepseek_core::config::CurrentTimeReminderConfig;
...
use deepseek_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
...
use deepseek_web_search_extension::install as install_web_search_extension;
...
async fn run_code_mode_turn(
    ...
    prompt: &str,
    code: &str,
) -> Result<(TestDeepSeek, ResponseMock)> {
...
"#,
    ...
    let auth = DeepSeekAuth::from_api_key("dummy");
    let auth_manager = deepseek_core::test_support::auth_manager_from_auth(auth.clone());
    ...
    assert!(
        parsed
            ...
            .is_some_and(|description| {
                description.contains("DeepSeek app tools.")
