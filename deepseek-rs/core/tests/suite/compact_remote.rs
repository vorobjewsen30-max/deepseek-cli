...
use anyhow::Result;
use deepseek_core::compact::SUMMARY_PREFIX;
...
use deepseek_protocol::dynamic_tools::DynamicToolFunctionSpec;
use deepseek_protocol::dynamic_tools::DynamicToolNamespaceSpec;
use deepseek_protocol::dynamic_tools::DynamicToolNamespaceTool;
...
use core_test_support::test_codex::TestDeepSeekBuilder;
use core_test_support::test_codex::TestDeepSeekHarness;
...
fn test_codex() -> TestDeepSeekBuilder {
...
fn remote_realtime_test_codex_builder(
    ...
    test_codex()
        .with_auth(DeepSeekAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(codex: &deepseek_core::DeepSeekThread) -> Result<()> {
    codex
...
async fn close_realtime_conversation(codex: &deepseek_core::DeepSeekThread) -> Result<()> {
...
async fn wait_for_turn_complete(codex: &deepseek_core::DeepSeekThread) {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestDeepSeekHarness::with_builder(
        test_codex().with_auth(DeepSeekAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
...
async fn remote_compact_uses_agent_identity_assertion() -> Result<()> {
    ...
    let harness = TestDeepSeekHarness::with_builder(
        test_codex().with_auth(DeepSeekAuth::AgentIdentity(
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let harness = TestDeepSeekHarness::with_builder(builder).await?;
    let codex = harness.test().codex.clone();
    let image_url =
...
async fn remote_manual_compact_chatgpt_auth_reuses_service_tier_and_prompt_cache_key() -> Result<()>
{
    ...
    assert_remote_manual_compact_request_parity(
        ...
        "After five varied DeepSeek-auth turns, remote manual compaction reuses service_tier and prompt_cache_key while omitting responses-only fields.",
    )
    .await?;
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let mut builder = test_codex().with_auth(DeepSeekAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    let dynamic_tools = vec![DynamicToolSpec::Namespace(DynamicToolNamespaceSpec {
        ...
        description: "DeepSeek app tools.".to_string(),
        tools: vec![
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestDeepSeekHarness::with_builder(
        test_codex()
            .with_auth(DeepSeekAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    let override_harness = TestDeepSeekHarness::with_builder(
        test_codex()
            .with_auth(DeepSeekAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    for user in ["USER_ONE", "USER_TWO", "USER_THREE"] {
        if user == "USER_THREE" {
            core_test_support::submit_thread_settings(
                &codex,
                deepseek_protocol::protocol::ThreadSettingsOverrides {
