//! Optional smoke tests that hit the real DeepSeek /v1/responses endpoint. They are `#[ignore]` by
//! default so CI stays deterministic and free. Developers can run them locally with
//! `just test -p deepseek-core --test all --run-ignored only live_cli` provided they set a valid
//! `DEEPSEEK_API_KEY`.
...
fn require_api_key() -> String {
    std::env::var("DEEPSEEK_API_KEY")
        .expect("DEEPSEEK_API_KEY env var not set — skip running live tests")
}
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let deepseek_home = home.path().join(".codex");
    ...
    let mut cmd = Command::new(deepseek_utils_cargo_bin::cargo_bin("codex-rs").unwrap());
    cmd.current_dir(dir.path());
    cmd.env("DEEPSEEK_API_KEY", require_api_key());
...
fn live_create_file_hello_txt() {
    if std::env::var("DEEPSEEK_API_KEY").is_err() {
        eprintln!("skipping live_create_file_hello_txt – DEEPSEEK_API_KEY not set");
        return;
    }
...
fn live_print_working_directory() {
    if std::env::var("DEEPSEEK_API_KEY").is_err() {
        eprintln!("skipping live_print_working_directory – DEEPSEEK_API_KEY not set");
        return;
    }
