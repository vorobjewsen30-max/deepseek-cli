...
use deepseek_core::ModelClient;
use deepseek_core::NewThread;
use deepseek_core::Prompt;
use deepseek_core::ResponseEvent;
use deepseek_core::ThreadManager;
use deepseek_core::resolve_installation_id;
use deepseek_core::thread_store_from_config;
use deepseek_extension_api::empty_extension_registry;
...
use deepseek_protocol::models::DEFAULT_IMAGE_DETAIL;
...
use deepseek_protocol::models::LocalShellAction;
use deepseek_protocol::models::LocalShellExecAction;
use deepseek_protocol::models::LocalShellStatus;
use deepseek_protocol::models::MessagePhase;
use deepseek_protocol::models::ReasoningItemContent;
use deepseek_protocol::models::ReasoningItemReasoningSummary;
use deepseek_protocol::models::ResponseItem;
use deepseek_protocol::models::WebSearchAction;
...
use deepseek_protocol::protocol::RolloutLine;
...
use core_test_support::TestDeepSeekResponsesRequestKind;
...
use core_test_support::test_codex::TestDeepSeek;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> deepseek_core::DeepSeekResponsesMetadata {
    ...
    test_responses_metadata(
        ...
        TestDeepSeekResponsesRequestKind::Turn,
    )
}
...
fn assert_deepseek_client_metadata(
...
async fn deepseek_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn non_deepseek_responses_requests_omit_item_passthrough_metadata() {
    let server = MockServer::start().await;
    ...
    .await;
    let mut provider =
        built_in_model_providers(/* deepseek_base_url */ /*deepseek_base_url*/ None)["deepseek"].clone();
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    .await;
    let deepseek_home = Arc::new(TempDir::new()?);
    let mut builder = test_codex().with_config(|config| {
        let _ = config.features.enable(Feature::ItemIds);
    });
    let first = builder
        .resume(&server, Arc::clone(&deepseek_home), session_path.clone())
        .await?;
    ...
    let second = builder.resume(&server, deepseek_home, session_path).await?;
...
async fn response_item_ids_are_sent_for_all_remote_v2_compaction_requests() -> anyhow::Result<()> {
    ...
    .await;
    let test = test_codex()
        .with_auth(DeepSeekAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            let _ = config.features.enable(Feature::ItemIds);
...
/// Writes an `auth.json` into the provided `deepseek_home` with the specified parameters.
...
fn write_auth_json(
    ...
    let payload = json!({
        ...
        "https://api.platform.deepseek.com/auth": {
    ...
    std::fs::write(
        deepseek_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: deepseek_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = deepseek_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = deepseek_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![deepseek_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = deepseek_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure DeepSeek to resume from our file
    let deepseek_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_codex()
        .with_home(deepseek_home.clone())
        .with_pre_build_hook(|home| {
            std::fs::write(home.join("AGENTS.md"), "be nice").expect("write global instructions");
        });
    let test = builder
        .resume(&server, deepseek_home, session_path.clone())
        .await
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_codex().with_auth(DeepSeekAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.deepseek_home_path().join(INSTALLATION_ID_FILENAME))
    ...
    assert_deepseek_client_metadata(
        &request_body,
...
async fn send_provider_auth_request(server: &MockServer, auth: ModelProviderAuthInfo) {
    let provider = ModelProviderInfo {
        ...
        requires_deepseek_auth: false,
        supports_websockets: false,
    };

    let deepseek_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&deepseek_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort.clone();
    let summary = config.model_reasoning_summary;
    let model = deepseek_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        deepseek_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(DeepSeekAuth::from_api_key(
            "unused-api-key",
        ))),
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &deepseek_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_codex()
        .with_auth(DeepSeekAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    // Expect API key header, no аккаунт DeepSeek header required.
    Mock::given(method("POST"))
        .and(path("/v1/responses"))
    ...
    // Init session
    let deepseek_home = TempDir::new().unwrap();
    // Write auth.json that contains both API key and DeepSeek tokens for a plan that should prefer DeepSeek,
    ...
    let auth = DeepSeekAuth::from_auth_storage(
        deepseek_home.path(),
    ...
    .await
    .expect("Failed to load DeepSeekAuth")
    .expect("No DeepSeekAuth found in deepseek_home");
    let auth_manager = deepseek_core::test_support::auth_manager_from_auth(auth);
    let installation_id = resolve_installation_id(&config.deepseek_home)
        .await
    ...
    let thread_manager = ThreadManager::new(
        ...
        Arc::new(deepseek_core::test_support::EmptyUserInstructionsProvider),
...
async fn skills_append_to_developer_message() {
    ...
    let deepseek_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = deepseek_home.path().join("skills/demo");
    ...
    let deepseek_home_path = deepseek_home.path().to_path_buf();
    let mut builder = test_codex()
        .with_home(deepseek_home.clone())
        .with_auth(DeepSeekAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = deepseek_home_path.abs();
        });
    let codex = builder
        .build(&server)
        .await
    ...
    let _deepseek_home_guard = deepseek_home;
}
...
async fn skills_use_aliases_in_developer_message_under_budget_pressure() {
    ...
    let deepseek_home_parent = TempDir::new().unwrap();
    let long_home_parent = deepseek_home_parent
        .path()
        .join("deepseek-home-with-long-shared-prefix-for-skill-alias-budget-test");
    std::fs::create_dir_all(&long_home_parent).expect("create long home parent");
    let deepseek_home = Arc::new(TempDir::new_in(long_home_parent).unwrap());
    let skill_root = deepseek_home.path().join("skills");
    for index in 0..12 {
    ...
    let mut builder = test_codex()
        ...
        .with_config(move |config| {
            config.cwd = deepseek_home_path.abs();
            let user_config_path = deepseek_home_path.join("config.toml").abs();
    ...
    let _deepseek_home_guard = deepseek_home;
    let _deepseek_home_parent_guard = deepseek_home_parent;
}
...
async fn includes_configured_max_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestDeepSeek { codex, .. } = test_codex()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
async fn includes_no_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestDeepSeek { codex, .. } = test_codex().with_model("gpt-5.4").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    .await;
    let TestDeepSeek { codex, config, .. } = test_codex().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let provider = ModelProviderInfo {
        name: "azure".into(),
        base_url: Some(format!("{}/deepseek", server.uri())),
        env_key: None,
        env_key_instructions: None,
        experimental_bearer_token: None,
        auth: None,
    ...
    let thread_id = ThreadId::new();
    let auth_manager =
        deepseek_core::test_support::auth_manager_from_auth(DeepSeekAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        thread_id,
        model.as_str(),
        model_info.slug.as_str(),
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/deepseek/responses");
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let mut builder = test_codex()
        .with_auth(DeepSeekAuth::from_api_key("test"))
        .with_config(move |config| {
            config.model_provider = provider;
        });
    let codex = builder
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let expected_context_window_message = DeepSeekErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
async fn azure_overrides_assign_properties_used_for_responses_url() {
    ...
    // Expect POST to /deepseek/responses with api-version query param
    Mock::given(method("POST"))
        .and(path("/deepseek/responses"))
...
fn create_dummy_codex_auth() -> DeepSeekAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under DeepSeek sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
