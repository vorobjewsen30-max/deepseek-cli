...
fn repo_root() -> std::path::PathBuf {
    deepseek_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
fn personal_access_token_exec_command(server: &MockServer, home: &TempDir) -> Command {
    let bin = deepseek_utils_cargo_bin::cargo_bin("codex").unwrap();
    let mut cmd = Command::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-c")
        .arg(format!("deepseek_base_url=\"{}/api/codex\"", server.uri()))
        .arg("-c")
        .arg(format!("chatgpt_base_url=\"{}/backend-api\"", server.uri()))
        .arg("-C")
        .arg(repo_root())
        .arg("hello?");
    cmd.env("DEEPSEEK_HOME", home.path())
        .env(DEEPSEEK_ACCESS_TOKEN_ENV_VAR, PERSONAL_ACCESS_TOKEN)
        .env("CODEX_AUTHAPI_BASE_URL", server.uri())
        .env_remove(DEEPSEEK_API_KEY_ENV_VAR)
...
impl Drop for ChildProcessCleanupGuard {
    fn drop(&mut self) {
        #[cfg(unix)]
        {
            let _ = deepseek_utils_pty::process_group::kill_process_group(self.0);
        }
...
async fn responses_mode_stream_cli_supports_personal_access_tokens() {
    ...
    assert_eq!(request.header("x-deepseek-fedramp").as_deref(), Some("true"));
    server.verify().await;
}
...
async fn responses_mode_stream_cli_does_not_attempt_oauth_refresh_for_personal_access_tokens_after_401()
 {
    ...
    Mock::given(method("POST"))
        ...
        .and(header("x-deepseek-fedramp", "true"))
...
/// Ensures `deepseek_base_url` config override routes built-in deepseek provider requests.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn responses_mode_stream_cli_supports_deepseek_base_url_config_override() {
    skip_if_no_network!();
    ...
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-c")
        .arg(format!("deepseek_base_url=\"{}/v1\"", server.uri()))
        .arg("-C")
        .arg(&repo_root)
        .arg("hello?");
    cmd.env("DEEPSEEK_HOME", home.path())
...
/// received by a mock DeepSeek Responses endpoint.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn exec_cli_applies_model_instructions_file() {
    ...
    // DeepSeek to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    cmd.arg("exec")
        ...
        .arg(&prompt);
    cmd.env("DEEPSEEK_HOME", home.path())
        .env(DEEPSEEK_API_KEY_ENV_VAR, "dummy");
    ...
    let bin2 = deepseek_utils_cargo_bin::cargo_bin("codex").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("DEEPSEEK_HOME", home.path())
        .env("DEEPSEEK_API_KEY", "dummy");

    let output2 = run_cli_command(&mut cmd2).unwrap();
    assert!(output2.status.success(), "resume deepseek-cli run failed");
