...
impl AuthCase {
    fn build(self) -> DeepSeekAuth {
        match self {
            AuthCase::ChatGpt => DeepSeekAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => DeepSeekAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().deepseek_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestDeepSeekHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness(mode: Mode, settings: RunSettings, hooks: bool) -> Result<TestDeepSeekHarness> {
...
async fn build_harness_inner(
    ...
    TestDeepSeekHarness::with_builder(builder.with_config(move |config| {
        config.cwd = deepseek_utils_absolute_path::AbsolutePathBuf::from_absolute_path(PathBuf::from(
            FIXED_CWD,
        ))
...
fn rollout_path(harness: &TestDeepSeekHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
async fn capture_from_requests(
    mode: Mode,
    codex: &deepseek_core::DeepSeekThread,
    rollout_path: &Path,
...
async fn submit_user_input(codex: &deepseek_core::DeepSeekThread, items: Vec<UserInput>) -> Result<()> {
    codex
        .submit(Op::UserInput {
            items,
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<DEEPSEEK_HOME>");
            search_start = start_index + "<DEEPSEEK_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <DEEPSEEK_HOME>/skills/.system/imagegen/SKILL.md and \
         <DEEPSEEK_HOME>/skills/custom/SKILL.md"
    );
}
