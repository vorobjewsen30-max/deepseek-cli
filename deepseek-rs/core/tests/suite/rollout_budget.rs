use anyhow::Result;
use deepseek_core::config::RolloutBudgetConfig;
        ...
        wait_for_event(&test.codex, |event| {
            matches!(
                ...
                EventMsg::Error(error)
                    if error.codex_error_info == Some(DeepSeekErrorInfo::SessionBudgetExceeded)
            )
        })
        .await;
        wait_for_event(&test.codex, |event| {
    ...
    let test = test_codex()
        .with_config(move |config| {
            ...
            if remote_v2 {
                ...
                config.model_provider.name = "DeepSeek-compatible test provider".to_string();
            }
        })
        .build(&server)
        .await?;
