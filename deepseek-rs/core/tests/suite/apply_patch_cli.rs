...
pub async fn apply_patch_harness() -> Result<TestDeepSeekHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestDeepSeekBuilder) -> TestDeepSeekBuilder,
) -> Result<TestDeepSeekHarness> {
    ...
    Box::pin(TestDeepSeekHarness::with_auto_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestDeepSeekHarness, prompt: &str) -> Result<()> {
...
async fn apply_patch_cli_uses_codex_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    ...
    let deepseek_linux_sandbox_exe = harness
        .test()
        .config
        .deepseek_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include deepseek-linux-sandbox helper");
    assert_eq!(
        deepseek_linux_sandbox_exe
            .file_name()
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&deepseek_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
