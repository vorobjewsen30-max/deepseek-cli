...
use deepseek_core::config::Constrained;
use deepseek_exec_server::CopyOptions;
use deepseek_exec_server::CreateDirectoryOptions;
use deepseek_exec_server::FileSystemSandboxContext;
use deepseek_exec_server::LOCAL_ENVIRONMENT_ID;
use deepseek_exec_server::REMOTE_ENVIRONMENT_ID;
...
async fn unified_exec_test(server: &wiremock::MockServer) -> Result<TestDeepSeek> {
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections = deepseek_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn wait_for_completion_without_patch_approval(test: &TestDeepSeek) {
    let event = wait_for_event(&test.codex, |event| {
        matches!(
            event,
            EventMsg::ApplyPatchApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn explicit_remote_shell_runs_in_remote_cwd() -> Result<()> {
    ...
    let (shell, command) = match test_target_os() {
        TestTargetOs::Linux => (
            "bash",
            r#"case "$PWD" in /tmp/deepseek-core-test-cwd-*) ;; *) echo "unexpected cwd: $PWD" >&2; exit 1 ;; esac"#,
        ),
        TestTargetOs::Windows => (
            "powershell",
            r#"$cwd = (Get-Location).Path; if ($cwd -notlike 'C:\deepseek-core-test-cwd-*') { Write-Error "unexpected cwd: $cwd"; exit 1 }"#,
        ),
        TestTargetOs::MacOs => unreachable!("remote test targets do not run macOS"),
    };
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/deepseek-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
