use deepseek_config::types::Personality;
...
use deepseek_protocol::deepseek_models::ModelInstructionsVariables;
use deepseek_protocol::deepseek_models::ModelMessages;
...
use deepseek_protocol::deepseek_models::default_input_modalities;
...
async fn personality_does_not_mutate_base_instructions_without_template() {
    let deepseek_home = TempDir::new().expect("create temp dir");
    ...
    let model_info = deepseek_core::test_support::construct_model_info_offline("gpt-5.4", &config);
    assert_eq!(
...
async fn base_instructions_override_disables_personality_template() {
    ...
    let model_info =
        deepseek_core::test_support::construct_model_info_offline("gpt-5.3-codex", &config);
...
async fn remote_model_friendly_personality_instructions_with_feature() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex()
        .with_auth(deepseek_login::DeepSeekAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            config
                .features
                .enable(Feature::Personality)
