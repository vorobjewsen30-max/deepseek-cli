...
async fn wait_for_snapshot(deepseek_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = deepseek_home.join("shell_snapshots");
...
async fn run_snapshot_command_with_options(
    ...
    let deepseek_home = test.home.path().to_path_buf();
    ...
    .await;
    let snapshot_path = wait_for_snapshot(&deepseek_home).await?;
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.deepseek_home));
...
async fn shell_command_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    ...
    let harness = TestDeepSeekHarness::with_builder(builder).await?;
    let deepseek_home = harness.test().home.path().to_path_buf();
    ...
    assert!(snapshot_path.starts_with(deepseek_home));
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let deepseek_home = home.path().to_path_buf();
