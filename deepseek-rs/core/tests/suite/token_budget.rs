...
use deepseek_core::config::TokenBudgetConfig;
...
async fn assert_context_compaction_item_lifecycle(codex: &std::sync::Arc<deepseek_core::DeepSeekThread>) {
...
async fn token_budget_context_uses_new_window_after_compaction() -> Result<()> {
    ...
    let mut model_provider = built_in_model_providers(/*deepseek_base_url*/ None)["deepseek"].clone();
...
async fn token_budget_mid_turn_auto_compaction_resets_before_active_follow_up() -> Result<()> {
    ...
    .await;
    let mut model_provider = built_in_model_providers(/*deepseek_base_url*/ None)["deepseek"].clone();
    model_provider.name = "DeepSeek (test)".into();
    model_provider.base_url = Some(format!("{}/v1", server.uri()));
    model_provider.supports_websockets = false;
    let test = test_codex()
        .with_config(move |config| {
