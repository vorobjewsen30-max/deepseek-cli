...
//! request payload that DeepSeek would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(DEEPSEEK_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<DeepSeekThread>) {
    let base_url = format!("{}/v1", server.uri());
    let model = model.map(str::to_string);
    let mut builder = test_codex().with_config(move |config| {
        config.model_provider.name = "Non-DeepSeek Model provider".to_string();
...
async fn user_turn(conversation: &Arc<DeepSeekThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<DeepSeekThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<DeepSeekThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<DeepSeekThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager = deepseek_core::test_support::auth_manager_from_auth(
        deepseek_login::DeepSeekAuth::from_api_key("dummy"),
    );
