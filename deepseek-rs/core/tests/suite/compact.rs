use anyhow::Result;
use anyhow::anyhow;
use deepseek_core::compact::SUMMARIZATION_PROMPT;
...
fn non_deepseek_model_provider(server: &MockServer) -> ModelProviderInfo {
    let mut provider =
        built_in_model_providers(/* deepseek_base_url */ /*deepseek_base_url*/ None)["deepseek"].clone();
    provider.name = "DeepSeek (test)".into();
...
fn local_compaction_provider(server: &wiremock::MockServer) -> ModelProviderInfo {
    let mut provider = built_in_model_providers(/*deepseek_base_url*/ None)["deepseek"].clone();
    provider.name = "DeepSeek-compatible test provider".to_string();
    provider.base_url = Some(format!("{}/v1", server.uri()));
    provider.supports_websockets = false;
    provider
}
...
async fn assert_compaction_uses_turn_lifecycle_id(codex: &std::sync::Arc<deepseek_core::DeepSeekThread>) {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn DeepSeek.
    let model_provider = non_deepseek_model_provider(&server);
    ...
    // Shut down DeepSeek to flush rollout entries before inspecting the file.
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    let hook_inputs = read_hook_inputs(&test.deepseek_home_path().join("pre_compact_block_log.jsonl"));
...
async fn compact_hooks_respect_matchers_and_post_runs_after_compaction() {
    ...
    let hook_inputs =
        read_hook_inputs(&test.deepseek_home_path().join("post_compact_manual_log.jsonl"));
    assert_eq!(hook_inputs.len(), 1);
    let input = &hook_inputs[0];
...
async fn multiple_auto_compact_per_task_runs_after_token_limit_hit() {
    ...
    let non_deepseek_provider_name = non_deepseek_model_provider(&server).name;
    let codex = test_codex()
        .with_config(move |config| {
            config.model_provider.name = non_deepseek_provider_name;
        })
        .build(&server)
        .await
        .expect("build codex")
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        deepseek_protocol::models::ResponseItem::Message {
            id: None,
        ...
        deepseek_protocol::models::ResponseItem::Compaction {
            id: None,
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let mut model_provider = non_deepseek_model_provider(&server);
