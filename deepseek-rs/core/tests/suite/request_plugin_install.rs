...
use deepseek_protocol::approvals::ElicitationAction;
...
const REMOTE_CALENDAR_PLUGIN_CONFIG_ID: &str = "calendar@deepseek-curated-remote";
...
async fn start_install_turn(test: &TestDeepSeek, prompt: &str) -> Result<ElicitationRequestEvent> {
...
async fn endpoint_mode_injects_candidates_hides_list_and_rejects_invented_ids() -> Result<()> {
            ...
            sse(vec![
                ...
                ev_function_call(
                    ...
                    &serde_json::to_string(&json!({
                        "plugin_id": "invented@deepseek-curated-remote",
    ...
    assert!(contextual_user_message.contains("github@deepseek-curated-remote"));
    assert!(contextual_user_message.contains("google-calendar@deepseek-curated-remote"));
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "deepseek_plugin_install_requested")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": turn_id,
                "model_slug": "gpt-5.4",
                "product_client_id": deepseek_login::default_client::originator().value,
            }
        })
    );
