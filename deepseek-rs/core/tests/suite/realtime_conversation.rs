...
use deepseek_protocol::protocol::RealtimeNoopRequested;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from DeepSeek.";
const STARTUP_CONTEXT_OPEN_TAG: &str = "<startup_context>";
const STARTUP_CONTEXT_CLOSE_TAG: &str = "</startup_context>";
const REALTIME_BACKEND_PROMPT: &str = deepseek_prompts::BACKEND_PROMPT;
...
const SILENT_CONTEXT_PREFIX: &str = "[BACKEND] Silent DeepSeek context. Do not speak, acknowledge, or summarize this item. Wait for an explicit speakable handoff or direct user request.";
...
fn run_realtime_conversation_test_in_subprocess(
    ...
    for &key in deepseek_network_proxy::PROXY_ENV_KEYS {
        command.env_remove(key);
    }
    match deepseek_api_key {
        Some(deepseek_api_key) => {
            command.env(DEEPSEEK_API_KEY_ENV_VAR, deepseek_api_key);
        }
        None => {
            command.env_remove(DEEPSEEK_API_KEY_ENV_VAR);
        }
    }
...
async fn seed_recent_thread(
    ...
    let mut metadata_builder = deepseek_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_start_uses_deepseek_env_key_fallback_with_chatgpt_auth() -> Result<()> {
    if std::env::var_os(REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR).is_none() {
        return run_realtime_conversation_test_in_subprocess(
            "suite::realtime_conversation::conversation_start_uses_deepseek_env_key_fallback_with_chatgpt_auth",
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(err.codex_error_info, Some(DeepSeekErrorInfo::BadRequest));
    assert_eq!(err.message, "conversation is not running");
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let resumed_thread = test
        ...
        .resume_thread_with_history(
            ...
            auth_manager_from_auth(DeepSeekAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
