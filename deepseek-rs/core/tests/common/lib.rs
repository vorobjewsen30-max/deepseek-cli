...
use deepseek_utils_cargo_bin::CargoBinError;
use ctor::ctor;
...
pub use deepseek_core::test_support::TestDeepSeekResponsesRequestKind;
pub use deepseek_core::test_support::responses_metadata;
use deepseek_utils_absolute_path::AbsolutePathBuf;
pub use deepseek_utils_absolute_path::test_support::PathBufExt;
pub use deepseek_utils_absolute_path::test_support::PathExt;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    deepseek_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    deepseek_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(deepseek_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = deepseek_utils_cargo_bin::repo_root()
        .ok()
...
pub async fn load_default_config_for_test(deepseek_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        deepseek_linux_sandbox_exe: Some(
            find_deepseek_linux_sandbox_exe().expect("should find binary for deepseek-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_deepseek_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    deepseek_utils_cargo_bin::cargo_bin("deepseek-linux-sandbox")
}

pub async fn wait_for_event<F>(
    codex: &DeepSeekThread,
    predicate: F,
) -> deepseek_protocol::protocol::EventMsg
where
    F: FnMut(&deepseek_protocol::protocol::EventMsg) -> bool,
{
...
pub async fn wait_for_mcp_server(codex: &DeepSeekThread, server_name: &str) -> anyhow::Result<()> {
...
pub async fn wait_for_event_match<T, F>(codex: &DeepSeekThread, matcher: F) -> T
where
    F: Fn(&deepseek_protocol::protocol::EventMsg) -> Option<T>,
{
...
pub fn sandbox_env_var() -> &'static str {
    deepseek_core::spawn::DEEPSEEK_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    deepseek_core::spawn::DEEPSEEK_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    deepseek_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    deepseek_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    deepseek_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
macro_rules! skip_if_no_network {
    () => {{
        if ::std::env::var($crate::sandbox_network_env_var()).is_ok() {
            println!(
                "Skipping test because it cannot execute when network is disabled in a DeepSeek sandbox."
            );
            return;
        }
    }};
...
#[macro_export]
macro_rules! deepseek_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_deepseek_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("deepseek-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
