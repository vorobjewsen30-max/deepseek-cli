...
use deepseek_core::TimeProvider;
use deepseek_core::config::Config;
use deepseek_core::resolve_installation_id;
use deepseek_core::shell::Shell;
use deepseek_core::shell::get_shell_by_model_provided_path;
use deepseek_core::thread_store_from_config;
use deepseek_exec_server::CreateDirectoryOptions;
use deepseek_exec_server::ExecutorFileSystem;
use deepseek_exec_server::RemoveOptions;
...
use deepseek_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
pub fn local(cwd: AbsolutePathBuf) -> TurnEnvironmentSelection {
    TurnEnvironmentSelection {
        environment_id: deepseek_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
...
#[derive(Debug)]
pub struct TestEnv {
    environment: deepseek_exec_server::Environment,
...
impl TestEnv {
    pub async fn local() -> Result<Self> {
        ...
        let environment =
            deepseek_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)?;
    ...
    pub fn environment(&self) -> &deepseek_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                deepseek_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
            ...
            let selection = TurnEnvironmentSelection {
                environment_id: deepseek_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: cwd_uri.clone(),
            };
...
pub struct TestDeepSeekBuilder {
...
impl TestDeepSeekBuilder {
    ...
    pub fn with_auth(mut self, auth: DeepSeekAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    pub fn with_deepseek_form_elicitation(mut self) -> Self {
        self.supports_deepseek_form_elicitation = true;
        self
    }
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestDeepSeek> {
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let deepseek_linux_sandbox_exe = Some(
            crate::find_deepseek_linux_sandbox_exe()
                .context("should find binary for deepseek-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let deepseek_linux_sandbox_exe = None;
        let local_runtime_paths = deepseek_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            deepseek_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            deepseek_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            deepseek_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        resume_from: Option<PathBuf>,
        test_env: TestEnv,
        environment_manager: Arc<deepseek_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestDeepSeek> {
        let auth = self.auth.clone();
        let state_db = deepseek_core::init_state_db(&config).await;
        let thread_store = thread_store_from_config(&config, state_db.clone());
        let installation_id = resolve_installation_id(&config.deepseek_home).await?;
        let user_instructions_provider =
            self.user_instructions_provider.clone().unwrap_or_else(|| {
                Arc::new(DeepSeekHomeUserInstructionsProvider::new(
                    config.deepseek_home.clone(),
                ))
            });
        let thread_manager = ThreadManager::new(
            &config,
            deepseek_core::test_support::auth_manager_from_auth(auth.clone()),
            SessionSource::Exec,
            ...
            thread_store,
            deepseek_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    deepseek_core::test_support::resume_thread_from_rollout_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        path,
                        auth_manager,
                        user_shell_override,
                        self.supports_deepseek_form_elicitation,
                    ),
                )
                .await?
            }
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    deepseek_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_deepseek_form_elicitation,
            ...
            (None, None) => {
                ...
                Box::pin(
                    thread_manager.start_thread_with_options(StartThreadOptions {
                        ...
                        supports_deepseek_form_elicitation: self.supports_deepseek_form_elicitation,
                    }),
                )
                .await?
            }
    ...
    async fn prepare_config(
        ...
        let model_provider = ModelProviderInfo {
            ...
            ..built_in_model_providers(/*deepseek_base_url*/ None)["deepseek"].clone()
        };
        ...
        if let Ok(path) = deepseek_utils_cargo_bin::cargo_bin("codex") {
            config.codex_self_exe = Some(path);
        } else if let Ok(path) = deepseek_utils_cargo_bin::cargo_bin("deepseek-exec") {
            // `deepseek-exec` also supports `--codex-run-as-apply-patch`, so use it
            ...
            && let Some(bin_dir) = exe.parent().and_then(|parent| parent.parent())
        {
            ...
            let deepseek_exec = bin_dir.join("deepseek-exec");
            if codex.is_file() {
                config.codex_self_exe = Some(codex);
            } else if deepseek_exec.is_file() {
                config.codex_self_exe = Some(deepseek_exec);
            }
        }
...
pub struct TestDeepSeek {
    pub home: Arc<TempDir>,
    pub cwd: Arc<TempDir>,
    pub codex: Arc<DeepSeekThread>,
...
impl TestDeepSeek {
    ...
    pub fn deepseek_home_path(&self) -> &Path {
        self.config.deepseek_home.as_path()
    }
...
pub struct TestDeepSeekHarness {
    server: MockServer,
    test: TestDeepSeek,
}

impl TestDeepSeekHarness {
    ...
    pub async fn with_builder(mut builder: TestDeepSeekBuilder) -> Result<Self> {
        let server = start_mock_server().await;
        let test = builder.build(&server).await?;
        Ok(Self { server, test })
    }

    pub async fn with_auto_env_builder(mut builder: TestDeepSeekBuilder) -> Result<Self> {
        let server = start_mock_server().await;
...
pub fn test_codex() -> TestDeepSeekBuilder {
    TestDeepSeekBuilder {
        ...
        auth: DeepSeekAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],
