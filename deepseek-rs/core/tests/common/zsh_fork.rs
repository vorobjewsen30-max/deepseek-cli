...
use crate::test_codex::TestDeepSeek;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = deepseek_utils_cargo_bin::cargo_bin("deepseek-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `deepseek-execve-wrapper` binary");
        return Ok(None);
    };
