...
use deepseek_backend_client::ConfigBundleResponse;
use deepseek_backend_client::DeliveredTomlFragment;
use deepseek_config::AbsolutePathBuf;
use deepseek_config::CloudConfigFragment;
use deepseek_config::CloudConfigTomlBundle;
use deepseek_config::CloudRequirementsFragment;
use deepseek_config::CloudRequirementsTomlBundle;
...
fn write_auth_json(deepseek_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
    std::fs::write(deepseek_home.join("auth.json"), serde_json::to_string(&value)?)?;
    Ok(())
}

fn create_test_cache(deepseek_home: &Path) -> CloudConfigBundleCache {
    CloudConfigBundleCache::new(AbsolutePathBuf::resolve_path_against_base(deepseek_home, "/"))
}
...
async fn auth_manager_with_agent_identity_business_plan() -> Arc<AuthManager> {
    ...
    AuthManager::from_auth_for_testing(DeepSeekAuth::AgentIdentity(
...
impl BundleClient for StaticBundleClient {
    async fn get_bundle(&self, _auth: &DeepSeekAuth) -> Result<CloudConfigBundle, BundleRequestError> {
...
impl BundleClient for TokenBundleClient {
    async fn get_bundle(&self, auth: &DeepSeekAuth) -> Result<CloudConfigBundle, BundleRequestError> {
        self.request_count.fetch_add(1, Ordering::SeqCst);
        if matches!(
...
async fn get_bundle_ignores_invalid_cache_and_refetches() {
    let deepseek_home = tempdir().expect("tempdir");
    let cache = create_test_cache(deepseek_home.path());
    cache
        .save(
...
async fn get_bundle_unauthorized_without_recovery_uses_generic_message() {
    ...
    let fetcher = Arc::new(UnauthorizedBundleClient {
        message:
            "GET https://api.deepseek.com/v1/wham/config/bundle failed: 401; content-type=text/html; body=<html>nope</html>"
                .to_string(),
        request_count: AtomicUsize::new(0),
    });
    let deepseek_home = tempdir().expect("tempdir");
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(deepseek_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            deepseek_backend_client::DeliveredRequirementsToml {
