...
use deepseek_utils_path_uri::PathUriParseError;
pub use parser::Hunk;
...
/// Special argv[1] flag used when the DeepSeek executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `deepseek-apply-patch` (to avoid forcing
/// `deepseek-arg0` to depend on `deepseek-core`), it remains part of the "codex core"
