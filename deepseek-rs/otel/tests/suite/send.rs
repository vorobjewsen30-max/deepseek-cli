...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "deepseek-cli"), ("env", "prod")])?;

    metrics.counter_with_description(
        "codex.turns",
        "Total number of DeepSeek turns.",
        /*inc*/ 1,
    ...
    assert_eq!(counter.description(), "Total number of DeepSeek turns.");
    ...
    assert_eq!(gauge.description(), "Number of active DeepSeek operations.");
