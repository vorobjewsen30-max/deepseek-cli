...
fn auth_env_metadata() -> AuthEnvTelemetryMetadata {
    AuthEnvTelemetryMetadata {
        deepseek_api_key_env_present: true,
        deepseek_api_key_env_present: false,
        deepseek_api_key_env_enabled: true,
...
fn otel_export_routing_policy_routes_user_prompt_log_and_trace_events() {
    ...
    assert!(
        logs.iter()
            .all(|log| { log.record.target().map(Cow::as_ref) == Some("deepseek_otel.log_only") })
    );
...
fn otel_export_routing_policy_routes_api_request_auth_observability() {
    ...
    assert_eq!(
        conversation_log_attrs
            .get("auth.env_deepseek_api_key_present")
            .map(String::as_str),
        Some("true")
    );
    assert_eq!(
    ...
    assert_eq!(
        request_log_attrs
            .get("auth.env_deepseek_api_key_enabled")
            .map(String::as_str),
        Some("true")
    );
    assert_eq!(
