...
use deepseek_otel::PLUGIN_INSTALL_ELICITATION_SENT_METRIC;
use deepseek_otel::PLUGIN_INSTALL_SUGGESTION_METRIC;
...
fn manager_attaches_metadata_tags_to_metrics() -> Result<()> {
    let (metrics, exporter) = build_metrics_with_defaults(&[("service", "deepseek-cli")])?;
    ...
    let expected = BTreeMap::from([
        ...
        ("service".to_string(), "deepseek-cli".to_string()),
...
fn manager_records_plugin_install_elicitation_sent_metric() -> Result<()> {
    ...
    manager.record_plugin_install_elicitation_sent("plugin", "slack@deepseek-curated", "Slack");
    manager.shutdown_metrics()?;
