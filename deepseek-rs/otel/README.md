...
`deepseek-otel` is the OpenTelemetry integration crate for DeepSeek. It provides:

- Provider wiring for log/trace/metric exporters (`deepseek_otel::OtelProvider`
  and `deepseek_otel::provider`).
- Session-scoped business event emission via `deepseek_otel::SessionTelemetry`.
- Low-level metrics APIs via `deepseek_otel::metrics`.
- Trace-context helpers via `deepseek_otel::trace_context` and crate-root re-exports.
...
```rust
use deepseek_otel::config::OtelExporter;
use deepseek_otel::config::OtelHttpProtocol;
use deepseek_otel::config::OtelSettings;
use deepseek_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "deepseek-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    deepseek_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, DeepSeek
...
DeepSeek-specific session events. Rich session/business events should go through
...
`deepseek-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using DeepSeek-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use deepseek_otel::config::{OtelExporter, OtelHttpProtocol};
