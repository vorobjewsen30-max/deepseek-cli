...
> **Privacy:** Rollout tracing is not telemetry. DeepSeek does **not** upload or
...
during a DeepSeek session. It records raw runtime evidence into a local bundle on
...
Hot-path DeepSeek code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how DeepSeek got
there.
...
```mermaid
flowchart TD
    subgraph Runtime["deepseek-core runtime"]
...
DeepSeek session fail just because diagnostic recording failed. Core emits raw
...
DeepSeek rollout/session being observed. Keeping those separate lets us reason about
...
`deepseek_rollout_trace::replay_bundle` directly.
