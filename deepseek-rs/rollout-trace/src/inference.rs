...
use crate::model::DeepSeekTurnId;
...
///
/// A DeepSeek turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one DeepSeek turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        codex_turn_id: DeepSeekTurnId,
...
impl InferenceTraceAttempt {
    ...
    /// trace-specific serialization rules. That keeps deepseek-core focused on
    ...
    /// Records a provider stream that DeepSeek intentionally stopped consuming.
    ///
...
/// DeepSeek received it, so this helper restores that content in the raw payload.
