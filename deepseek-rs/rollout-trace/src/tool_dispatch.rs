...
use deepseek_protocol::models::SearchToolCallParams;
use serde::Serialize;
...
/// Core-facing request data for the canonical DeepSeek tool boundary.
pub struct ToolDispatchInvocation {
    pub thread_id: AgentThreadId,
    pub codex_turn_id: DeepSeekTurnId,
...
/// Raw invocation payload for the canonical DeepSeek tool boundary.
#[derive(Serialize)]
...
fn suppresses_tool_dispatch_trace(invocation: &ToolDispatchInvocation) -> bool {
    matches!(invocation.payload, ToolDispatchPayload::Custom { .. })
        ...
        && invocation.tool_name == deepseek_code_mode::PUBLIC_TOOL_NAME
}
