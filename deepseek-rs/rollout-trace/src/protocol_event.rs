//! Mapping from DeepSeek protocol events into raw rollout-trace events.
//!
...
//! `deepseek-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `deepseek-rollout-trace`.
//!
...
use deepseek_protocol::protocol::ExecCommandStatus;
...
use deepseek_protocol::protocol::PatchApplyStatus;
...
use crate::DeepSeekTurnId;
use crate::ExecutionStatus;
use crate::RawTraceEventPayload;

pub(crate) struct DeepSeekTurnTraceEvent {
    pub context_turn_id: DeepSeekTurnId,
...
pub(crate) fn codex_turn_trace_event(
    ...
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<DeepSeekTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let codex_turn_id = event.turn_id.clone();
            Some(DeepSeekTurnTraceEvent {
                context_turn_id: codex_turn_id.clone(),
                payload: RawTraceEventPayload::DeepSeekTurnStarted {
                    codex_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(DeepSeekTurnTraceEvent {
                context_turn_id: codex_turn_id.clone(),
                payload: RawTraceEventPayload::DeepSeekTurnEnded {
                    codex_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a deepseek_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a deepseek_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a deepseek_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a deepseek_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a deepseek_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a deepseek_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a deepseek_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a deepseek_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [deepseek_protocol::parse_command::ParsedCommand],
