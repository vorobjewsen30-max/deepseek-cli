...
impl ThreadTraceContext {
    ...
    ///
    /// Trace startup is best-effort. A tracing failure must not make the DeepSeek
    ...
    /// Emits typed DeepSeek turn lifecycle events from protocol lifecycle events.
    ...
    /// what DeepSeek did while executing that boundary.
    pub fn record_tool_call_event(&self, codex_turn_id: impl Into<DeepSeekTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_codex_turn_id: impl Into<DeepSeekTurnId>,
    ...
    pub fn record_codex_turn_started(&self, codex_turn_id: impl Into<DeepSeekTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(
            codex_turn_id.clone(),
            RawTraceEventPayload::DeepSeekTurnStarted {
                codex_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    pub fn start_code_cell_trace(
        &self,
        codex_turn_id: impl Into<DeepSeekTurnId>,
    ...
    /// Builds reusable inference trace context for one DeepSeek turn.
    ///
