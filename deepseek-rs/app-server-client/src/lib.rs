...
//!
//! This crate wraps [`deepseek_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](deepseek_app_server::in_process::InProcessClientHandle),
...
pub use deepseek_app_server::app_server_control_socket_path;
pub use deepseek_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
pub use deepseek_app_server::in_process::InProcessServerEvent;
use deepseek_app_server::in_process::InProcessStartArgs;
use deepseek_app_server::in_process::LogDbLayer;
pub use deepseek_app_server::in_process::StateDbHandle;
use deepseek_app_server_protocol::ClientInfo;
use deepseek_app_server_protocol::ClientNotification;
...
use deepseek_app_server_protocol::Result as JsonRpcResult;
...
use deepseek_config::NoopThreadConfigLoader;
use deepseek_config::RemoteThreadConfigLoader;
use deepseek_config::ThreadConfigLoader;
use deepseek_config::config_toml::ConfigToml;
use deepseek_core::config::Config;
pub use deepseek_core::otel_init::build_provider as build_otel_provider;
use deepseek_core::personality_migration::PersonalityMigrationStatus;
use deepseek_core::personality_migration::maybe_migrate_personality;
...
/// module exists so clients can remove a direct `deepseek-core` dependency
/// while legacy startup/config paths are migrated to RPCs.
pub mod legacy_core {
    pub use deepseek_core::check_execpolicy_for_warnings;
    pub use deepseek_core::format_exec_policy_error_with_source;

    pub mod config {
        pub use deepseek_core::config::*;

        pub mod edit {
            pub use deepseek_core::config::edit::*;
        }
    }
}
...
pub async fn migrate_personality_if_needed(
    ...
    let status = maybe_migrate_personality(deepseek_home, config_toml, state_db).await?;
    match status {
...
pub struct InProcessClientStartArgs {
    ...
    pub enable_deepseek_api_key_env: bool,
    ...
    /// Whether MCP servers may send `deepseek/form` elicitation requests.
    pub mcp_server_deepseek_form_elicitation: bool,
...
impl InProcessClientStartArgs {
    ...
    pub fn initialize_params(&self) -> InitializeParams {
        let capabilities = InitializeCapabilities {
            ...
            mcp_server_deepseek_form_elicitation: self.mcp_server_deepseek_form_elicitation,
        };
    ...
    fn into_runtime_start_args(self) -> InProcessStartArgs {
        ...
        InProcessStartArgs {
            ...
            enable_deepseek_api_key_env: self.enable_deepseek_api_key_env,
            initialize,
...
/// - [`deepseek_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        ...
        let mut handle =
            deepseek_app_server::in_process::start(args.into_runtime_start_args()).await?;
...
impl AppServerClient {
    pub fn deepseek_home(&self, local_deepseek_home: &AbsolutePathBuf) -> Option<AppServerPath> {
        match self {
            Self::InProcess(_) => Some(AppServerPath::from_app_server(
                local_deepseek_home.display().to_string(),
            )),
            Self::Remote(client) => client.deepseek_home().map(AppServerPath::from_app_server),
        }
    }
...
mod tests {
    ...
    use deepseek_app_server_protocol::ConfigRequirementsReadResponse;
    use deepseek_app_server_protocol::GetAccountResponse;
    use deepseek_app_server_protocol::JSONRPCMessage;
    use deepseek_app_server_protocol::JSONRPCRequest;
    use deepseek_app_server_protocol::JSONRPCResponse;
    use deepseek_app_server_protocol::ServerNotification;
    use deepseek_app_server_protocol::SessionSource as ApiSessionSource;
    ...
    use deepseek_core::init_state_db;
    use deepseek_uds::UnixListener;
    use deepseek_utils_absolute_path::AbsolutePathBuf;
    use futures::SinkExt;
    ...
    async fn build_test_config_for_deepseek_home(deepseek_home: &Path) -> Config {
        ...
        {
            Ok(config) => config,
            Err(_) => Config::load_default_with_cli_overrides_for_deepseek_home(
    ...
    async fn start_test_client_with_capacity(
        ...
        let deepseek_home = TempDir::new().expect("temp dir");
        let config = Arc::new(build_test_config_for_deepseek_home(deepseek_home.path()).await);
        ...
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            client_name: "deepseek-app-server-client-test".to_string(),
        ...
        write_websocket_message(
            ...
            JSONRPCMessage::Response(JSONRPCResponse {
                ...
                result: serde_json::json!({
                    "userAgent": "deepseek_cli_rs/9.8.7-test (Test OS; x86_64) rust",
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            deepseek_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn agent_message_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::AgentMessageDelta(
            deepseek_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: "thread".to_string(),
                turn_id: "turn".to_string(),
                item_id: "item".to_string(),
                delta: delta.to_string(),
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(deepseek_app_server_protocol::ItemCompletedNotification {
            ...
            turn_id: "turn".to_string(),
            completed_at_ms: 0,
            item: deepseek_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(deepseek_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
            turn: deepseek_app_server_protocol::Turn {
                id: "turn".to_string(),
                items_view: deepseek_app_server_protocol::TurnItemsView::Full,
                items: Vec::new(),
                status: deepseek_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
                completed_at: Some(0),
                duration_ms: Some(1),
    ...
    #[test]
    fn remote_initialize_params_forward_deepseek_form_capability() {
        let mut args = test_remote_connect_args("ws://localhost/rpc".to_string());
        args.mcp_server_deepseek_form_elicitation = true;

        assert!(
            args.initialize_params()
                .capabilities
                .expect("initialize capabilities")
                .mcp_server_deepseek_form_elicitation
        );
    }
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: deepseek_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<deepseek_app_server_protocol::ThreadReadResponse>(
    ...
    async fn forward_in_process_event_preserves_transcript_notifications_under_backpressure() {
        ...
        assert!(matches!(
            ...
            InProcessServerEvent::ServerNotification(ServerNotification::ItemCompleted(
                ...
                &notification.item,
                deepseek_app_server_protocol::ThreadItem::AgentMessage { text, .. } if text == "hello"
            )
        ));
        assert!(matches!(
            &events[4],
            InProcessServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                notification
            )) if notification.turn.status == deepseek_app_server_protocol::TurnStatus::Completed
        ));
    }
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        assert_eq!(client.deepseek_home(), Some("/server/.codex"));
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: deepseek_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
        ...
        for event in &remaining_events {
            match event {
                ...
                AppServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                    ...
                    == deepseek_app_server_protocol::TurnStatus::Completed =>
                {
    ...
    fn event_requires_delivery_marks_transcript_and_terminal_events() {
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                deepseek_app_server_protocol::ServerNotification::TurnCompleted(
                    deepseek_app_server_protocol::TurnCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn: deepseek_app_server_protocol::Turn {
                            id: "turn".to_string(),
                            items_view: deepseek_app_server_protocol::TurnItemsView::Full,
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                deepseek_app_server_protocol::ServerNotification::AgentMessageDelta(
                    deepseek_app_server_protocol::AgentMessageDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                deepseek_app_server_protocol::ServerNotification::ItemCompleted(
                    deepseek_app_server_protocol::ItemCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        completed_at_ms: 0,
                        item: deepseek_app_server_protocol::ThreadItem::AgentMessage {
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                deepseek_app_server_protocol::ServerNotification::ExternalAgentConfigImportCompleted(
                    deepseek_app_server_protocol::ExternalAgentConfigImportCompletedNotification {
        ...
        assert!(!event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                deepseek_app_server_protocol::ServerNotification::CommandExecutionOutputDelta(
                    deepseek_app_server_protocol::CommandExecutionOutputDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
    ...
    #[tokio::test]
    async fn runtime_start_args_forward_environment_manager_and_deepseek_form_capability() {
        ...
        let runtime_args = InProcessClientStartArgs {
            ...
            client_version: "0.0.0-test".to_string(),
            experimental_api: true,
            mcp_server_deepseek_form_elicitation: true,
            opt_out_notification_methods: Vec::new(),
    ...
    async fn runtime_start_args_use_remote_thread_config_loader_when_configured() {
        ...
        assert_eq!(
            err.code(),
            deepseek_config::ThreadConfigLoadErrorCode::RequestFailed
        );
    }
