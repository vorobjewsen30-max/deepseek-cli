...
impl RemoteAppServerClient {
    ...
    pub fn deepseek_home(&self) -> Option<&str> {
        self.deepseek_home.as_deref()
    }
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        let (pending_events, server_version, deepseek_home) = initialize_remote_connection(
            &mut stream,
            &endpoint,
            initialize_params,
            INITIALIZE_TIMEOUT,
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
                ...
                Some(Ok(Message::Text(text))) => {
                    ...
                    match message {
                        JSONRPCMessage::Response(response) if response.id == initialize_request_id => {
                            ...
                            deepseek_home = response
                                .result
                                .get("codexHome")
                                .and_then(serde_json::Value::as_str)
                                .filter(|deepseek_home| !deepseek_home.is_empty())
                                .map(str::to_string);
                            break Ok(());
                        }
    ...
    Ok((pending_events, server_version, deepseek_home))
}
