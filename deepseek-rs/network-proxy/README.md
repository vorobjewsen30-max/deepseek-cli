# deepseek-network-proxy

`deepseek-network-proxy` is DeepSeek's local network policy enforcement proxy. It runs:
...
`deepseek-network-proxy` reads from DeepSeek's merged `config.toml` (via `deepseek-core` config loading).
...
# bundle env vars pointing at immutable public files under $DEEPSEEK_HOME/proxy/ so common HTTPS
...
# Use exact hosts or scoped wildcards like `*.platform.deepseek.com` or `**.platform.deepseek.com`.
...
```bash
cargo run -p deepseek-network-proxy --
```
...
`deepseek-network-proxy` can be embedded as a library with a thin API:

```rust
use deepseek_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `deepseek-network-proxy` is embedded in managed DeepSeek runtime, policy decisions emit structured
OTEL-compatible events with `target=deepseek_otel.network_proxy`.
...
This section documents the protections implemented by `deepseek-network-proxy`, and the boundaries of
