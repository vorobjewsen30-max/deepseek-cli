...
fn virtualize_child_env_replaces_supported_credentials() {
    ...
    let deepseek_api_key = "sk-proj-abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_";
    let mut env = env_map([
    ...
    let deepseek_dummy = env.get("DEEPSEEK_API_KEY").expect("dummy API-ключ DeepSeek");
    assert_credential_shape(github_token, github_dummy, "github_pat_");
    assert_credential_shape(deepseek_api_key, deepseek_dummy, "sk-proj-");
    env.insert("DEEPSEEK_API_KEY".to_string(), "sk-user-override".to_string());
    assert_eq!(
...
fn virtualize_child_env_uses_fresh_dummy_capabilities() {
    let mut first_env = env_map([("DEEPSEEK_API_KEY", "sk-proj-abcdefghijklmnopqrstuvwxyz")]);
    ...
    assert_ne!(first_env["DEEPSEEK_API_KEY"], second_env["DEEPSEEK_API_KEY"]);
}

#[test]
fn child_without_dummy_cannot_use_previous_child_credential() {
    let broker = CredentialBroker::new(/*enabled*/ true);
    let mut first_env = env_map([("DEEPSEEK_API_KEY", "sk-real")]);
    ...
    broker.inject_request_headers("api.platform.deepseek.com", &mut headers);
...
fn inject_request_headers_requires_dummy_and_preserves_explicit_authorization() {
    let broker = CredentialBroker::new(/*enabled*/ true);
    let mut env = env_map([("DEEPSEEK_API_KEY", "sk-real")]);
    broker.virtualize_child_env(&mut env);
    let deepseek_api_key = env.get("DEEPSEEK_API_KEY").expect("dummy API-ключ DeepSeek");
    let mut headers = HeaderMap::new();

    broker.inject_request_headers("api.platform.deepseek.com", &mut headers);
    assert_eq!(authorization(&headers), None);

    headers = headers_with_bearer(deepseek_api_key);
    broker.inject_request_headers("api.platform.deepseek.com", &mut headers);
    assert_eq!(authorization(&headers), Some("Bearer sk-real"));

    let mut explicit_headers = headers_with_bearer("sk-explicit");
    broker.inject_request_headers("api.platform.deepseek.com", &mut explicit_headers);
