...
const AUDIT_TARGET: &str = "deepseek_otel.network_proxy";
...
mod tests {
    ...
    async fn evaluate_host_policy_emits_domain_event_for_decider_allow_override() {
        ...
        assert!(event.target.starts_with("deepseek_otel."));
        assert_eq!(
    ...
    async fn evaluate_host_policy_emits_metadata_fields() {
        let metadata = NetworkProxyAuditMetadata {
            ...
            originator: Some("deepseek_cli_rs".to_string()),
        ...
        assert_eq!(event.field("originator"), Some("deepseek_cli_rs"));
