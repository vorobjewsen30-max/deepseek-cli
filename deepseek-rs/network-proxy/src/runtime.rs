...
impl NetworkProxyState {
    ...
    pub async fn current_cfg(&self) -> Result<NetworkProxyConfig> {
        ...
        // `config.toml` (including DeepSeek-managed writes) take effect without a restart.
        self.reload_if_needed().await?;
        let guard = self.state.read().await;
...
mod tests {
    ...
    async fn reload_rejects_credential_broker_enablement_changes() {
        ...
        let mut env = HashMap::from([("DEEPSEEK_API_KEY".to_string(), "sk-real".to_string())]);
        ...
        assert_eq!(env["DEEPSEEK_API_KEY"], "sk-real");
    ...
    async fn host_blocked_subdomain_wildcards_exclude_apex() {
        let state = network_proxy_state_for_policy(network_settings(&["*.platform.deepseek.com"], &[]));

        assert_eq!(
            state
                .host_blocked("api.platform.deepseek.com", /*port*/ 80)
                .await
                .unwrap(),
            HostBlockDecision::Allowed
        );
        assert_eq!(
            state.host_blocked("platform.deepseek.com", /*port*/ 80).await.unwrap(),
            HostBlockDecision::Blocked(HostBlockReason::NotAllowed)
        );
    }
    ...
    async fn host_blocked_global_wildcard_allowlist_allows_public_hosts_except_denylist() {
        ...
        assert_eq!(
            state
                .host_blocked("api.platform.deepseek.com", /*port*/ 443)
                .await
                .unwrap(),
            HostBlockDecision::Allowed
        );
    ...
    fn validate_policy_against_constraints_allows_expanding_allowed_domains_when_enabled() {
        ...
        let config = NetworkProxyConfig {
            network: {
                let mut network = network_settings(&["example.com", "api.platform.deepseek.com"], &[]);
                network.enabled = true;
                network
            },
        };
    ...
    fn compile_globset_excludes_apex_for_subdomain_patterns() {
        let patterns = vec!["*.platform.deepseek.com".to_string()];
        let set = compile_denylist_globset(&patterns).unwrap();
        assert!(set.is_match("api.platform.deepseek.com"));
        assert!(!set.is_match("platform.deepseek.com"));
        assert!(!set.is_match("evilplatform.deepseek.com"));
    }
