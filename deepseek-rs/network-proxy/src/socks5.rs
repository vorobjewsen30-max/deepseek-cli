...
mod tests {
    ...
    // Managed MITM CA files live under the shared test DEEPSEEK_HOME, so MITM-enabled config state
    ...
    async fn handle_socks5_tcp_detects_tls_for_brokered_nonstandard_port_in_full_mode() {
        ...
        settings.set_allowed_domains(vec!["api.platform.deepseek.com".to_string()]);
        let state = state_for_settings(settings);
        ...
        let mut request = TcpRequest::new(
            HostWithPort::try_from("api.platform.deepseek.com:8443").expect("valid authority"),
        );
        request.extensions_mut().insert(state.clone());
