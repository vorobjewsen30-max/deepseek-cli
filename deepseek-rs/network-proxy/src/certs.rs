...
fn managed_ca_dir() -> Result<PathBuf> {
    let deepseek_home =
        find_deepseek_home().context("failed to resolve DEEPSEEK_HOME for managed MITM CA")?;
    Ok(deepseek_home.join(MANAGED_MITM_CA_DIR).to_path_buf())
}
...
/// Returns whether `path` points at a current DeepSeek-generated MITM CA bundle.
