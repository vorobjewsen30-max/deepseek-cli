...
pub fn current_log_file_path_for_deepseek_home(deepseek_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(deepseek_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_deepseek_home_uses_sandbox_dir() {
        let deepseek_home = Path::new("deepseek-home");

        assert_eq!(
            current_log_file_path_for_deepseek_home(deepseek_home),
            current_log_file_path(&deepseek_home.join(".sandbox"))
        );
    }
}
