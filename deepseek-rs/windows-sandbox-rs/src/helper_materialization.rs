...
pub(crate) fn helper_bin_dir(deepseek_home: &Path) -> PathBuf {
    sandbox_bin_dir(deepseek_home)
}
...
pub(crate) fn resolve_helper_for_launch(
    ...
    match copy_helper_if_needed(kind, deepseek_home, log_dir) {
        Ok(path) => {
            log_note(
                &format!(
...
pub fn resolve_current_exe_for_launch(deepseek_home: &Path, fallback_executable: &str) -> PathBuf {
    let source = match std::env::current_exe() {
        Ok(path) => path,
        Err(_) => return PathBuf::from(fallback_executable),
    };
    resolve_exe_for_launch(&source, deepseek_home)
}

pub fn resolve_exe_for_launch(source: &Path, deepseek_home: &Path) -> PathBuf {
    let Some(file_name) = source.file_name() else {
        return source.to_path_buf();
    };
    let destination = helper_bin_dir(deepseek_home).join(file_name);
    match copy_from_source_if_needed(source, &destination) {
        Ok(_) => destination,
        Err(err) => {
            let sandbox_log_dir = crate::sandbox_dir(deepseek_home);
            log_note(
                &format!(
...
pub(crate) fn copy_helper_if_needed(
    ...
    let cache_key = format!("{}|{}", kind.file_name(), deepseek_home.display());
    ...
    let destination = helper_destination_for_source(kind, deepseek_home, &source)?;
    log_note(
        &format!(
...
fn helper_destination_for_source(
    ...
    Ok(helper_bin_dir(deepseek_home).join(file_name))
}
...
mod tests {
    ...
    fn helper_bin_dir_is_under_sandbox_bin() {
        let deepseek_home = Path::new(r"C:\Users\example\.codex");

        assert_eq!(
            PathBuf::from(r"C:\Users\example\.codex\.sandbox-bin"),
            helper_bin_dir(deepseek_home)
        );
    }
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_destination = helper_bin_dir(&deepseek_home).join(materialized_file_name(
