...
const DEEPSEEK_HOME_FLAG: &str = "--deepseek-home";
...
pub fn create_windows_sandbox_command_args_for_permission_profile(
    ...
    let mut args = vec![
        ...
        DEEPSEEK_HOME_FLAG.to_string(),
        deepseek_home.to_string_lossy().into_owned(),
...
async fn run_windows_sandbox_wrapper_request(request: WindowsSandboxWrapperRequest) -> Result<i32> {
    ...
    let spawned =
        crate::spawn_windows_sandbox_session_for_level(crate::WindowsSandboxSessionRequest {
            ...
            deepseek_home: request.deepseek_home.as_path(),
...
fn parse_windows_sandbox_wrapper_args(args: Vec<String>) -> Result<WindowsSandboxWrapperRequest> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            DEEPSEEK_HOME_FLAG => deepseek_home = Some(PathBuf::from(next_flag_value(&mut args, &arg)?)),
            COMMAND_CWD_FLAG => {
    ...
    let deepseek_home = deepseek_home.ok_or_else(|| anyhow!("missing required {DEEPSEEK_HOME_FLAG}"))?;
    if !deepseek_home.is_absolute() {
        bail!(
            "{DEEPSEEK_HOME_FLAG} must be absolute: {}",
