...
pub const SANDBOX_USERS_GROUP: &str = "DeepSeekSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "DeepSeek sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(deepseek_home);
...
pub(super) fn prepare_setup_marker(deepseek_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(deepseek_home).join("setup_marker.json");
