...
pub const OFFLINE_USERNAME: &str = "DeepSeekSandboxOffline";
pub const ONLINE_USERNAME: &str = "DeepSeekSandboxOnline";
...
pub fn sandbox_dir(deepseek_home: &Path) -> PathBuf {
    deepseek_home.join(".sandbox")
}

pub fn sandbox_bin_dir(deepseek_home: &Path) -> PathBuf {
    deepseek_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(deepseek_home: &Path) -> PathBuf {
    deepseek_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(deepseek_home: &Path) -> PathBuf {
    sandbox_dir(deepseek_home).join("setup_marker.json")
}

pub fn sandbox_users_path(deepseek_home: &Path) -> PathBuf {
    sandbox_secrets_dir(deepseek_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, &permissions, env_map, deepseek_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        deepseek_home: request.deepseek_home.to_path_buf(),
    ...
    let sbx_dir = sandbox_dir(request.deepseek_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(request.deepseek_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| request.deepseek_home.to_path_buf());
    log_note(
        &format!(
    ...
    if !status.success() {
        ...
        return Err(report_helper_failure(
            request.deepseek_home,
            cleared_report,
            status.code(),
        ));
    }
    if let Err(err) = clear_setup_error_report(request.deepseek_home) {
        log_note(
...
fn gather_helper_read_roots(deepseek_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(deepseek_home);
...
fn gather_full_read_roots_for_permissions(
    ...
    let mut roots = gather_helper_read_roots(deepseek_home);
    roots.extend(
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, deepseek_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<deepseek_otel::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
    #[serde(default)]
    refresh_only: bool,
...
fn report_helper_failure(
    ...
    match read_setup_error_report(deepseek_home) {
...
fn verify_setup_completed(deepseek_home: &Path) -> Result<()> {
    if sandbox_setup_is_complete(deepseek_home) {
        Ok(())
    } else {
        Err(failure(
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(deepseek_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
                ...
                Some(&sandbox_dir(deepseek_home)),
            );
            false
        }
    };
    ...
    if !needs_elevation {
        ...
        verify_setup_completed(deepseek_home)?;
        if let Err(err) = clear_setup_error_report(deepseek_home) {
            log_note(
                &format!(
...
fn run_elevated_setup_inner(
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: deepseek_otel::global_statsig_metrics_settings(),
        mode: SetupMode::Full,
        refresh_only: false,
    };
    ...
    run_setup_exe(&payload, needs_elevation, request.deepseek_home)
}

pub fn run_elevated_provisioning_setup(deepseek_home: &Path, real_user: &str) -> Result<()> {
    let sbx_dir = sandbox_dir(deepseek_home);
    std::fs::create_dir_all(&sbx_dir).map_err(|err| {
        failure(
            SetupErrorCode::OrchestratorSandboxDirCreateFailed,
            format!("failed to create sandbox dir {}: {err}", sbx_dir.display()),
    ...
    let payload = ElevationPayload {
        ...
        command_cwd: deepseek_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
    ...
    run_setup_exe(&payload, /*needs_elevation*/ false, deepseek_home)
}
...
fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.deepseek_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, deepseek_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to DEEPSEEK_HOME or anything under DEEPSEEK_HOME/.sandbox,
    // DEEPSEEK_HOME/.sandbox-bin, or DEEPSEEK_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let deepseek_home_key = canonical_path_key(deepseek_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(deepseek_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(deepseek_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(deepseek_home));
    let secrets_dir_prefix = format!("{}/", secrets_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = canonical_path_key(root);
        key != deepseek_home_key
            && key != sbx_dir_key
...
mod tests {
    ...
    fn setup_completion_requires_ready_artifacts() {
        let deepseek_home = TempDir::new().expect("tempdir");
        let err = verify_setup_completed(deepseek_home.path())
    ...
    fn report_helper_failure_uses_setup_error_report_when_clear_succeeded() {
        let tmp = TempDir::new().expect("tempdir");
        let deepseek_home = tmp.path().join("deepseek-home");
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_deepseek_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let deepseek_home = user_profile.join("DeepSeekHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&deepseek_home).expect("create codex home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &deepseek_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        ...
        let roots = gather_read_roots(&command_cwd, &permissions, &HashMap::new(), &deepseek_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&deepseek_home)).expect("canonical helper dir");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&deepseek_home);
        fs::create_dir_all(&deepseek_home).expect("create codex home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_deepseek_home = dunce::canonicalize(&deepseek_home).expect("canonical codex home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_deepseek_home));
