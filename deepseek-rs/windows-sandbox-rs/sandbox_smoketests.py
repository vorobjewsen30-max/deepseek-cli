...
# Run a suite of smoke tests against the Windows sandbox via the DeepSeek CLI
...
def _resolve_codex_cmd() -> List[str]:
    """Resolve the DeepSeek CLI to invoke `codex sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run DeepSeek.
    """
    ...
    raise FileNotFoundError(
        "DeepSeek CLI not found. Build it first, e.g.\n"
        "  cargo build -p deepseek-cli --release\n"
        "or for debug:\n"
        "  cargo build -p deepseek-cli\n"
    )
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"DEEPSEEK_HOME": str(proxy_home)},
            )
    ...
    deepseek_home = Path(os.environ["USERPROFILE"]) / ".codex"
    cap_sid_target = deepseek_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
