...
use crate::url::is_deepseek_compatible_base_url;
...
impl OllamaClient {
    ...
    pub(crate) async fn try_from_provider(provider: &ModelProviderInfo) -> io::Result<Self> {
        ...
        let uses_deepseek_compat = is_deepseek_compatible_base_url(base_url);
    ...
    async fn probe_server(&self) -> io::Result<()> {
        let url = if self.uses_deepseek_compat {
...
mod tests {
    ...
    async fn test_fetch_models_happy_path() {
        if std::env::var(deepseek_core::spawn::DEEPSEEK_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
            tracing::info!(
    ...
    #[tokio::test]
    async fn test_probe_server_happy_path_deepseek_compat_and_native() {
        if std::env::var(deepseek_core::spawn::DEEPSEEK_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
            tracing::info!(
                "{} set; skipping test_probe_server_happy_path_deepseek_compat_and_native",
                deepseek_core::spawn::DEEPSEEK_SANDBOX_NETWORK_DISABLED_ENV_VAR
            );
            return;
        }
    ...
    async fn test_try_from_oss_provider_ok_when_server_running() {
        ...
        // DeepSeek‑compat models endpoint responds OK.
        wiremock::Mock::given(wiremock::matchers::method("GET"))
            .and(wiremock::matchers::path("/v1/models"))
            .respond_with(wiremock::ResponseTemplate::new(200))
            .mount(&server)
