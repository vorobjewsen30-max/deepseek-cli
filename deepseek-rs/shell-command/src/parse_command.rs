...
/// The easiest way to iterate is to add unit tests and have DeepSeek fix the implementation.
...
/// Tests are at the top to encourage using TDD + DeepSeek to fix the implementation.
mod tests {
    use super::*;
    ...
    fn supports_grep_recursive_current_dir() {
        assert_parsed(
            &vec_str(&["grep", "-R", "DEEPSEEK_SANDBOX_ENV_VAR", "-n", "."]),
            vec![ParsedCommand::Search {
                cmd: "grep -R DEEPSEEK_SANDBOX_ENV_VAR -n .".to_string(),
                query: Some("DEEPSEEK_SANDBOX_ENV_VAR".to_string()),
                path: Some(".".to_string()),
            }],
        );
    }
    ...
    fn supports_grep_recursive_specific_file() {
        assert_parsed(
            &vec_str(&[
                "grep",
                "-R",
                "DEEPSEEK_SANDBOX_ENV_VAR",
                "-n",
                "core/src/spawn.rs",
            ]),
            vec![ParsedCommand::Search {
                cmd: "grep -R DEEPSEEK_SANDBOX_ENV_VAR -n core/src/spawn.rs".to_string(),
                query: Some("DEEPSEEK_SANDBOX_ENV_VAR".to_string()),
    ...
    fn supports_single_string_script_with_cd_and_pipe() {
        let inner = r#"cd /Users/pakrym/code/codex && rg -n "deepseek_api" codex-rs -S | head -n 50"#;
        assert_parsed(
            &vec_str(&["bash", "-lc", inner]),
            vec![ParsedCommand::Search {
                cmd: "rg -n deepseek_api codex-rs -S".to_string(),
                query: Some("deepseek_api".to_string()),
