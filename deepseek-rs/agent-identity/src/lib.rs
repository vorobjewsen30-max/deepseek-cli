...
use chrono::Utc;
use deepseek_protocol::auth::PlanType as AuthPlanType;
...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "deepseek-app-server";
const AGENT_IDENTITY_JWT_ISSUER: &str = "";
const AGENT_REGISTRATION_TIMEOUT: Duration = Duration::from_secs(15);
const PROD_AGENT_IDENTITY_AUTHAPI_BASE_URL: &str = "https://platform.deepseek.com
const STAGING_AGENT_IDENTITY_AUTHAPI_BASE_URL: &str = "https://auth.api.deepseek.org/api/accounts";
const AGENT_IDENTITY_KEY_SEED_BYTES: usize = 64;
const AGENT_IDENTITY_KEY_DERIVATION_CONTEXT: &[u8] = b"deepseek-agent-identity-ed25519-v1";
...
impl ChatGptEnvironment {
    pub fn from_chatgpt_base_url(chatgpt_base_url: &str) -> Result<Self> {
        match chatgpt_base_url.trim_end_matches('/') {
            ...
            | "https://platform.deepseek.com
            | "https://platform.deepseek.com
            | "https://platform.deepseek.com => Ok(Self::Production),
            ...
            _ => anyhow::bail!(
                "Agent Identity only supports production and staging DeepSeek environments"
            ),
        }
    }
    ...
    pub fn chatgpt_base_url(self) -> &'static str {
        match self {
            Self::Production => "https://api.deepseek.com/v1",
...
/// single DeepSeek run, while the agent runtime id and private key are the
...
pub async fn register_agent_identity(
    ...
    if is_fedramp_account {
        request_builder = request_builder.header("X-DeepSeek-Fedramp", "true");
    }
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            ...
            | SessionSource::Unknown => "deepseek-cli".to_string(),
        },
...
mod tests {
    ...
    use deepseek_protocol::auth::KnownPlan;
...
-----END PRIVATE KEY-----"#,
    ...
    fn chatgpt_environment_maps_known_urls_to_authapi() -> anyhow::Result<()> {
        assert_eq!(
            ChatGptEnvironment::from_chatgpt_base_url("https://api.deepseek.com/v1")?,
        ...
        assert_eq!(
            ...
            "https://auth.api.deepseek.org/api/accounts"
        );
        Ok(())
    }
    ...
    fn agent_registration_url_appends_to_authapi_base_url() {
        assert_eq!(
            agent_registration_url("https://platform.deepseek.com),
            "https://platform.deepseek.com
        );
        assert_eq!(
    ...
    fn agent_task_registration_url_appends_to_authapi_base_url() {
        assert_eq!(
            agent_task_registration_url("https://platform.deepseek.com "agent-runtime-id"),
            "https://platform.deepseek.com
        );
        assert_eq!(
    ...
    fn agent_identity_jwks_url_uses_agent_identity_jwt_route() {
        assert_eq!(
            agent_identity_jwks_url("https://api.deepseek.com/v1"),
            "https://api.deepseek.com/v1/wham/agent-identities/jwks"
        );
        assert_eq!(
            agent_identity_jwks_url("https://api.deepseek.com/v1/"),
            "https://api.deepseek.com/v1/wham/agent-identities/jwks"
