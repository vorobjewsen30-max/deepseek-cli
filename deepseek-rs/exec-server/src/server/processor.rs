    ...
    while let Some(event) = incoming_rx.recv().await {
        ...
        match event {
            JsonRpcConnectionEvent::MalformedMessage { reason } => {
                ...
                if outgoing_tx
                    .send(RpcServerOutboundMessage::Error {
                        request_id: deepseek_exec_server_protocol::RequestId::Integer(-1),
            ...
            JsonRpcConnectionEvent::Message(message) => match message {
                deepseek_exec_server_protocol::JSONRPCMessage::Request(request) => {
                ...
                deepseek_exec_server_protocol::JSONRPCMessage::Notification(notification) => {
                ...
                deepseek_exec_server_protocol::JSONRPCMessage::Response(response) => {
                    warn!(
                ...
                deepseek_exec_server_protocol::JSONRPCMessage::Error(error) => {
                    warn!(
...
fn request_span(
    span_name: &str,
    request: &deepseek_exec_server_protocol::JSONRPCRequest,
) -> tracing::Span {
    ...
    if let Some(trace) = &request.trace
        && !deepseek_otel::set_parent_from_w3c_trace_context(&span, trace)
    {
...
mod tests {
    ...
    use deepseek_exec_server_protocol::JSONRPCMessage;
    use deepseek_exec_server_protocol::JSONRPCNotification;
    use deepseek_exec_server_protocol::JSONRPCRequest;
    use deepseek_exec_server_protocol::JSONRPCResponse;
    use deepseek_exec_server_protocol::RequestId;
    ...
    fn request_span_uses_bounded_name_wire_method_and_inbound_trace_parent() {
        ...
        let subscriber = tracing_subscriber::registry().with(
            tracing_opentelemetry::layer()
                .with_tracer(tracer)
                .with_filter(filter_fn(deepseek_otel::OtelProvider::trace_export_filter)),
        );
        ...
        let trace = deepseek_protocol::protocol::W3cTraceContext {
