...
use deepseek_exec_server_protocol::JSONRPCErrorError;
...
use deepseek_sandboxing::SandboxDirectSpawnTransformRequest;
use deepseek_sandboxing::SandboxExecRequest;
...
use deepseek_utils_absolute_path::canonicalize_preserving_symlinks;
        ...
        sandbox_manager
            .transform_for_direct_spawn(SandboxDirectSpawnTransformRequest {
                ...
                windows_sandbox_proxy_settings_mode:
                    deepseek_sandboxing::WindowsSandboxProxySettingsMode::Preserve,
                transform: SandboxTransformRequest {
                    ...
                    deepseek_linux_sandbox_exe: self.runtime_paths.deepseek_linux_sandbox_exe.as_deref(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    ...
    for path in std::iter::once(runtime_paths.codex_self_exe.as_path())
        .chain(runtime_paths.deepseek_linux_sandbox_exe.as_deref())
    {
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        ...
        let runtime_paths =
            ExecServerRuntimePaths::new(codex_self_exe, /*deepseek_linux_sandbox_exe*/ None)
    ...
    fn sandbox_cwd_rejects_cwd_dependent_profile_without_context_cwd() {
        ...
        let sandbox_context = deepseek_file_system::FileSystemSandboxContext::from_permission_profile(
    ...
    fn helper_permissions_include_linux_sandbox_alias_parent() {
        ...
        let deepseek_linux_sandbox_exe = root.path().join("aliases").join("deepseek-linux-sandbox");
        let runtime_paths =
            ExecServerRuntimePaths::new(codex_self_exe, Some(deepseek_linux_sandbox_exe))
                .expect("runtime paths");
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
    ...
    fn sandbox_context_with_cwd(
        ...
        deepseek_file_system::FileSystemSandboxContext::from_permission_profile_with_cwd(
            PermissionProfile::from_runtime_permissions(policy, NetworkSandboxPolicy::Restricted),
            cwd,
        )
    }
