...
use crate::environment_toml::environment_provider_from_deepseek_home;
...
/// Owns the execution/filesystem environments available to the DeepSeek runtime.
///
...
impl EnvironmentManager {
    ...
    /// Builds a manager from `DEEPSEEK_HOME` and local runtime paths used when
    /// creating local filesystem helpers.
    ///
    /// If `DEEPSEEK_HOME/environments.toml` is present, it defines the configured
    /// environments. Otherwise this preserves the legacy
    /// `CODEX_EXEC_SERVER_URL` behavior.
    pub async fn from_deepseek_home(
        deepseek_home: impl AsRef<std::path::Path>,
        local_runtime_paths: Option<ExecServerRuntimePaths>,
        ...
        let provider = environment_provider_from_deepseek_home(deepseek_home.as_ref())?;
    ...
    /// reading user config files from `DEEPSEEK_HOME`.
...
mod tests {
    ...
    async fn local_environment_passes_runtime_paths_to_exec_backend() {
        ...
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile_with_cwd(
            deepseek_protocol::models::PermissionProfile::workspace_write(),
            sandbox_cwd.clone(),
        );
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = deepseek_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
            std::env::current_exe().expect("current exe").as_path(),
        )
        .expect("absolute current exe");
        let path = deepseek_utils_path_uri::PathUri::from_abs_path(&path);
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            deepseek_protocol::models::PermissionProfile::from_runtime_permissions(
                &deepseek_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
                deepseek_protocol::permissions::NetworkSandboxPolicy::Restricted,
            ),
        );
