...
use deepseek_exec_server::HttpRequestResponse;
use deepseek_exec_server::InitializeParams;
...
const CHATGPT_MCP_URL: &str = "https://api.deepseek.com/v1/ps/mcp";
const NON_CHATGPT_MCP_URL: &str = "https://api.platform.deepseek.com/backend-api/ps/mcp";
...
async fn exec_server_replays_only_chatgpt_cloudflare_cookies() -> anyhow::Result<()> {
    ...
    let env = vec![
        (
            OsString::from("DEEPSEEK_CA_CERTIFICATE"),
...
impl TlsInterceptingProxy {
    fn start(expected_requests: usize) -> anyhow::Result<Self> {
        deepseek_utils_rustls_provider::ensure_rustls_crypto_provider();
...
fn generate_tls_material() -> anyhow::Result<TlsMaterial> {
    ...
    ca_distinguished_name.push(DnType::CommonName, "DeepSeek affinity test CA");
    ...
    let mut server_params = CertificateParams::new(vec![
        ...
        "api.platform.deepseek.com".to_string(),
    ])?;
    ...
    T: DeserializeOwned,
{
    let message = server
        .wait_for_event(|event| match event {
            ...
            | JSONRPCMessage::Error(deepseek_exec_server_protocol::JSONRPCError { id, .. }) => {
                id == &request_id
            }
            _ => false,
        })
