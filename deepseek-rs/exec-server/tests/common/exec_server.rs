...
pub(crate) struct ExecServerHarness {
    ...
    _helper_paths: TestDeepSeekHelperPaths,
    child: Child,
...
pub(crate) struct TestDeepSeekHelperPaths {
    pub(crate) codex_exe: PathBuf,
    pub(crate) deepseek_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_codex_helper_paths() -> anyhow::Result<TestDeepSeekHelperPaths> {
    let (helper_binary, deepseek_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestDeepSeekHelperPaths {
    ...
    V: AsRef<std::ffi::OsStr>,
{
    ...
    child.env("DEEPSEEK_HOME", deepseek_home.path());
    child.envs(env);
