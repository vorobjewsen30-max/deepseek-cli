...
use deepseek_exec_server::ExecResponse;
use deepseek_exec_server::ExecServerClient;
use deepseek_exec_server::ExecServerRuntimePaths;
use deepseek_exec_server::FsReadFileParams;
use deepseek_exec_server::NoiseChannelIdentity;
use deepseek_exec_server::NoiseChannelPublicKey;
use deepseek_exec_server::NoiseRendezvousConnectArgs;
use deepseek_exec_server::NoiseRendezvousConnectBundle;
use deepseek_exec_server::ProcessId;
use deepseek_exec_server::RemoteEnvironmentConfig;
...
fn static_registry_auth_provider() -> deepseek_api::SharedAuthProvider {
...
async fn remote_environment_routes_encrypted_exec_server_rpc() -> Result<()> {
    ...
    let (codex_exe, deepseek_linux_sandbox_exe) = common::current_test_binary_helper_paths()?;
    let runtime_paths = ExecServerRuntimePaths::new(codex_exe, deepseek_linux_sandbox_exe)?;
    let config = RemoteEnvironmentConfig::new(
        registry.uri(),
        ENVIRONMENT_ID.to_string(),
        static_registry_auth_provider(),
    )?;
    let remote_environment = tokio::spawn(deepseek_exec_server::run_remote_environment(
        config,
        runtime_paths,
    ));
