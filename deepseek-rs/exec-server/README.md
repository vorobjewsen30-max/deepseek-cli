...
`deepseek-exec-server` is the library backing `codex exec-server`, a small
...
filesystem operations and `deepseek-linux-sandbox`.

## Transport

The server speaks the exec-specific `deepseek-exec-server-protocol` message
...
must support it.
It uses the standard DeepSeek sign-in state; run `deepseek login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `DEEPSEEK_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; DeepSeek then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `DEEPSEEK_API_KEY`;
DeepSeek sends it as a bearer token on the registration request. For example:
...
- `arg0`: optional argv0 override forwarded to `deepseek-utils-pty`.
...
remote registration should use; the CLI builds that provider from DeepSeek auth
