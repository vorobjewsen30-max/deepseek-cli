...
cargo build -p deepseek-cli --bin codex

# 2) Start websocket app-server in background
cargo run -p deepseek-app-server-test-client -- \
  --codex-bin ./target/debug/codex \
  serve --listen ws://127.0.0.1:4222 --kill

# 3) Call app-server (defaults to ws://127.0.0.1:4222)
cargo run -p deepseek-app-server-test-client -- model-list
```

`send-message` and `send-message-v2` handle `request_user_input` server requests interactively.
When DeepSeek asks a question, choose a numbered option (or `o` for a free-form answer when offered)
...
active DeepSeek profile must be authenticated. On a fresh local cache, the command
retries ephemeral turns while the installed remote bundle finishes syncing.

```bash
# Build a debug DeepSeek binary; analytics capture is unavailable in release builds.
cargo build -p deepseek-cli --bin codex

cargo run -p deepseek-app-server-test-client -- \
  --codex-bin ./target/debug/codex \
  plugin-analytics-smoke \
  --plugin-id linear@deepseek-curated-remote
```

Use `--capture-file /tmp/plugin-analytics.jsonl` to select the output path.
The command validates one `deepseek_plugin_disabled`, `deepseek_plugin_enabled`, and
`deepseek_plugin_used` event with the expected local and remote plugin identities
...
installed, installs it, validates `deepseek_plugin_installed`, uninstalls it, and
validates `deepseek_plugin_uninstalled`, and verifies that the original
uninstalled state was restored.

The mutation events include the local DeepSeek ID in `plugin_id` and the backend ID
in `remote_plugin_id`.
...
Cleanup does not require analytics capture or a debug DeepSeek binary. When the
...
```bash
cargo run -p deepseek-app-server-test-client -- watch
```

## Testing Thread Rejoin Behavior

Build and start an app server using commands above. The app-server log is written to `/tmp/deepseek-app-server-test-client/app-server.log`
...
```bash
cargo run -p deepseek-app-server-test-client -- send-message-v2 "seed thread for rejoin test"
cargo run -p deepseek-app-server-test-client -- thread-list --limit 5
```
...
```bash
cargo run --bin deepseek-app-server-test-client -- \
...
```bash
cargo run --bin deepseek-app-server-test-client -- thread-resume <THREAD_ID>
```
