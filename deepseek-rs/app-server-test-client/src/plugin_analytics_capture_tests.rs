...
fn reads_and_validates_remote_plugin_mutation_events() {
    ...
    let installed = mutation_event("deepseek_plugin_installed");
    let uninstalled = mutation_event("deepseek_plugin_uninstalled");
    let unrelated = json!({
        "event_type": "deepseek_plugin_installed",
        "event_params": {
            "plugin_id": "other@deepseek-curated-remote",
...
fn rejects_missing_capability_metadata() {
    let mut installed = mutation_event("deepseek_plugin_installed");
...
fn mutation_event(event_type: &str) -> Value {
    json!({
        ...
        "event_params": {
            "plugin_id": "sample@deepseek-curated-remote",
...
fn unique_capture_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "deepseek-plugin-analytics-capture-{name}-{}-{nonce}.jsonl",
        process::id()
    ))
}
