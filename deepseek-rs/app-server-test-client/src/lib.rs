...
use deepseek_app_server_protocol::SandboxPolicy;
...
use deepseek_otel::OtelProvider;
use deepseek_otel::current_span_w3c_trace_context;
use deepseek_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const OTEL_SERVICE_NAME: &str = "deepseek-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $DEEPSEEK_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the DeepSeek app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "DeepSeek", version, about = "Bootstrap DeepSeek app-server", long_about = None)]
struct Cli {
...
enum CliCommand {
    ...
    ///
    /// Logs are written to:
    ///   `/tmp/deepseek-app-server-test-client/`
    Serve {
    ...
    /// Send a user message through the DeepSeek app-server.
    SendMessage {
        /// User message to send to DeepSeek.
        user_message: String,
    },
    ...
    /// Trigger the вход через DeepSeek flow and wait for completion.
    TestLogin {
        /// Use the device-code login flow instead of the browser callback flow.
        #[arg(long, default_value_t = false)]
        device_code: bool,
    },
    /// Fetch the current account rate limits from the DeepSeek app-server.
    GetAccountRateLimits,
    /// List the available models from the DeepSeek app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the DeepSeek app-server.
    ...
    PluginAnalyticsSmoke {
        /// Installed local plugin id, such as `linear@deepseek-curated-remote`.
        #[arg(long)]
        plugin_id: String,
...
fn resolve_endpoint(codex_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    ...
    if let Some(codex_bin) = codex_bin {
        return Ok(Endpoint::SpawnDeepSeek(codex_bin));
    }
    if let Some(url) = url {
...
fn serve(codex_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/deepseek-app-server-test-client");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnDeepSeek(codex_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut DeepSeekClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = DeepSeekClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = DeepSeekClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve deepseek-app-server-test-client binary path")?;
...
impl DeepSeekClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnDeepSeek(codex_bin) => Self::spawn_stdio(codex_bin, config_overrides),
    ...
    fn spawn_stdio_with_env(
        ...
        let mut deepseek_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
        ...
        let stdin = deepseek_app_server
            .stdin
            .take()
            .context("codex app-server stdin unavailable")?;
        let stdout = deepseek_app_server
            .stdout
            .take()
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `deepseek-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    ...
                    title: Some("DeepSeek Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: deepseek_app_server_protocol::LoginAccountParams::Chatgpt {
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: deepseek_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = deepseek_core::otel_init::build_provider(
            &config,
