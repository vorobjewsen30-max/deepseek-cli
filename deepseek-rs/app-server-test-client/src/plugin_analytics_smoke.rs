...
pub(super) fn run(
    ...
    let capture_path = capture_file.unwrap_or_else(|| {
        std::env::temp_dir().join(format!("deepseek-plugin-analytics-{}.jsonl", process::id()))
    });
    ...
    let mut client = DeepSeekClient::spawn_stdio_with_env(codex_bin, &overrides, &child_environment)?;
...
fn run_plugin_turn(client: &mut DeepSeekClient, expected: &ExpectedPlugin) -> Result<String> {
...
fn wait_for_plugin_usage(
    ...
    loop {
        ...
        if events.iter().any(|event| {
            event["event_type"] == "deepseek_plugin_used"
...
fn plugin_installed(client: &mut DeepSeekClient) -> Result<PluginInstalledResponse> {
...
fn smoke_config_overrides(responses_base_url: &str) -> Result<Vec<String>> {
    ...
    Ok(vec![
        ...
        format!("model_providers.{MOCK_PROVIDER_ID}.requires_deepseek_auth=false"),
...
pub(super) fn wait_until_capture_is_ready(path: &Path) -> Result<()> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            bail!(
                "analytics capture did not become ready at {}; use a debug DeepSeek binary",
                path.display()
            );
        }
...
fn validate_plugin_events(events: Vec<Value>, expected: &ExpectedPlugin) -> Result<Vec<Value>> {
    ...
    for event_type in required_event_types() {
        ...
        if event_type == "deepseek_plugin_used" {
...
fn required_event_types() -> [&'static str; 3] {
    [
        "deepseek_plugin_disabled",
        "deepseek_plugin_enabled",
        "deepseek_plugin_used",
    ]
}
...
fn validate_used_metadata(event: &Value) -> Result<()> {
    ...
    for field in [
        ...
        if params.get(field).is_none_or(Value::is_null) {
            bail!("deepseek_plugin_used event has null or missing `{field}`");
        }
    }
...
impl TemporaryConfigFile {
    fn create() -> Result<Self> {
        let path = std::env::temp_dir().join(format!(
            "deepseek-plugin-analytics-config-{}.toml",
            process::id()
        ));
        fs::write(&path, "")
