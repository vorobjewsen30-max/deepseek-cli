...
const REMOTE_MARKETPLACE_HINT: &str = "deepseek-curated-remote";
...
pub(super) fn run(
    ...
    let capture_path = capture_file.unwrap_or_else(|| {
        std::env::temp_dir().join(format!(
            "deepseek-plugin-analytics-mutation-{}.jsonl",
            process::id()
        ))
    });
...
pub(super) fn run_cleanup(
    ...
    let mut client = DeepSeekClient::spawn_stdio(codex_bin, &overrides)?;
    client.initialize()?;
...
fn spawn_client(
    ...
    DeepSeekClient::spawn_stdio_with_env(codex_bin, &overrides, &environment)
}
...
fn install_remote_plugin(client: &mut DeepSeekClient, plugin: &RemotePluginExpectation) -> Result<()> {
    let request_id = client.request_id();
...
fn uninstall_remote_plugin(client: &mut DeepSeekClient, remote_plugin_id: &str) -> Result<()> {
    let request_id = client.request_id();
...
fn print_recovery_command(codex_bin: &Path, config_overrides: &[String], remote_plugin_id: &str) {
    let test_client = std::env::current_exe()
        ...
        .unwrap_or_else(|_| "deepseek-app-server-test-client".to_string());
