//! Schema-heavy configuration TOML types used by DeepSeek.
...
use deepseek_protocol::config_types::WebSearchToolConfig;
...
/// Backward-compatible shape for DeepSeek workspace login restrictions in config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by DeepSeek will likely degrade model performance.
    ...
    /// file: Use a file in the DeepSeek home directory.
    ...
    /// Defaults to `$CODEX_SQLITE_HOME` when set. Otherwise uses `$DEEPSEEK_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where DeepSeek writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$DEEPSEEK_HOME/log`.
    ...
    /// Optional product SKU forwarded on host-owned DeepSeek Apps MCP requests.
...
pub struct DebugConfigLockToml {
    ...
    /// Allow replaying a lock generated by a different DeepSeek version.
...
pub use deepseek_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use deepseek_protocol::protocol::RealtimeVoice;
...
Built-in providers cannot be overridden. Rename your custom provider (for example, `deepseek-custom`).",
            conflicts.join(", ")
        ))
    }
}
