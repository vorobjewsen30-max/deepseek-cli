...
use deepseek_file_system::ExecutorFileSystem;
...
///   `%ProgramData%\DeepSeek\DeepSeek\requirements.toml` (Windows)
...
///   `%ProgramData%\DeepSeek\DeepSeek\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${DEEPSEEK_HOME}/config.toml`
/// - profile   `${DEEPSEEK_HOME}/<name>.config.toml`, when selected
...
///
/// See https://platform.deepseek.com/docs for details.
///
...
pub async fn load_config_layers_state(
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(deepseek_home)?;
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, deepseek_home, overrides.clone(), strict_config)
            .await?;
    if !ignore_managed_requirements {
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(deepseek_home)?;
    let base_user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, deepseek_home);
        ...
        && let Some(base_user_config) = base_user_layer.config.as_table()
    {
        ...
        if legacy_profile_is_selected || legacy_profile_table_exists {
            return Err(io::Error::new(
                ...
                format!(
                    "--profile `{active_user_profile}` cannot be used while {} contains legacy `profile = \"{active_user_profile}\"` or `[profiles.{active_user_profile}]` config; move those settings into {} and remove the legacy profile selector/table. See https://platform.deepseek.com/docs for more information.",
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(deepseek_home)?;
...
fn windows_codex_system_dir() -> PathBuf {
    ...
    program_data.join("DeepSeek").join("DeepSeek")
}
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for DeepSeek to function correctly.
...
async fn load_project_layers(
    ...
    let deepseek_home_abs = AbsolutePathBuf::from_absolute_path(deepseek_home)?;
    let deepseek_home_normalized =
        normalize_path(deepseek_home_abs.as_path()).unwrap_or_else(|_| deepseek_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        ...
        if dot_codex_abs == deepseek_home_abs || dot_codex_normalized == deepseek_home_normalized {
            continue;
        }
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("DeepSeek").join("DeepSeek").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("DeepSeek").join("DeepSeek").join("config.toml"))
        );
    }
}
