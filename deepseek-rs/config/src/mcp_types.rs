...
/// OAuth client settings used when DeepSeek launches an MCP OAuth flow.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
...
/// Authentication flow DeepSeek attempts after resolving an HTTP MCP server's
/// configured bearer token and authorization headers, which always take
/// precedence. DeepSeek authentication falls back to stored OAuth credentials
...
pub enum McpServerAuth {
    ...
    #[serde(rename = "oauth")]
    OAuth,
    /// Use the current DeepSeek session for servers on the trusted first-party
    /// DeepSeek origin. If no DeepSeek session provider is available, startup can
...
pub struct McpServerConfig {
    ...
    /// Effective environment id for where DeepSeek should start this MCP server.
    pub environment_id: String,

    /// When `false`, DeepSeek skips initializing this MCP server.
