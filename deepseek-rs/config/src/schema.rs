...
use deepseek_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == deepseek_features::Feature::Artifact {
            continue;
        }
        if feature.id == deepseek_features::Feature::CodeMode {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<deepseek_features::FeatureToml<
                    deepseek_features::CodeModeConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == deepseek_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<deepseek_features::FeatureToml<
                    deepseek_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == deepseek_features::Feature::TokenBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<deepseek_features::FeatureToml<
                    deepseek_features::TokenBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == deepseek_features::Feature::RolloutBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<deepseek_features::FeatureToml<
                    deepseek_features::RolloutBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == deepseek_features::Feature::CurrentTimeReminder {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<deepseek_features::FeatureToml<
                    deepseek_features::CurrentTimeReminderConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == deepseek_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
        ...
        if feature.id == deepseek_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<deepseek_features::FeatureToml<
                    deepseek_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
