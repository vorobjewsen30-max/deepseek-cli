//! Types used to define loaded and effective DeepSeek configuration values.
...
use deepseek_protocol::config_types::EnvironmentVariablePattern;
pub use deepseek_protocol::config_types::ModeKind;
pub use deepseek_protocol::config_types::Personality;
pub use deepseek_protocol::config_types::ServiceTier;
use deepseek_protocol::config_types::ShellEnvironmentPolicy;
use deepseek_protocol::config_types::ShellEnvironmentPolicyInherit;
...
/// Determine where DeepSeek should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in DEEPSEEK_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in DEEPSEEK_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Determine where DeepSeek should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum OAuthCredentialsStoreMode {
    ...
    #[default]
    Auto,
    /// DEEPSEEK_HOME/.credentials.json
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across DeepSeek product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across DeepSeek product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in DeepSeek rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, DeepSeek does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
/// Fallback resize-reflow row cap when DeepSeek cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$DEEPSEEK_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against DEEPSEEK_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
    ...
    /// transcript exceeds this cap. Omit to use DeepSeek's terminal-specific default. Set to `0` to
...
/// (primarily the DeepSeek IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of DeepSeek-managed fast defaults.
...
pub struct PluginMcpServerConfig {
    /// When `false`, DeepSeek skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time DeepSeek successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision DeepSeek last successfully activated for this marketplace.
    #[serde(default)]
