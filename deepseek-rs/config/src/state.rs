...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("deepseek-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, deepseek_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
pub struct ConfigLayerStack {
    ...
    /// the base `$DEEPSEEK_HOME/config.toml` layer followed by the profile override
...
impl ConfigLayerStack {
    ...
    /// the base `$DEEPSEEK_HOME/config.toml` layer because the active layer is the
