...
use deepseek_apply_patch::CODEX_CORE_APPLY_PATCH_ARG1;
use deepseek_exec_server::CODEX_FS_HELPER_ARG1;
use deepseek_install_context::InstallContext;
use deepseek_sandboxing::landlock::CODEX_LINUX_SANDBOX_ARG0;
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "deepseek-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";
const TOKIO_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current DeepSeek executable for child re-execs.
    ///
    ...
    /// instead of the real DeepSeek CLI.
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            deepseek_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if exe_name == CODEX_LINUX_SANDBOX_ARG0 {
        ...
        deepseek_linux_sandbox::run_main();
    } else if exe_name == APPLY_PATCH_ARG0 || exe_name == MISSPELLED_APPLY_PATCH_ARG0 {
        deepseek_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    if argv1 == CODEX_FS_HELPER_ARG1 {
        deepseek_exec_server::run_fs_helper_main();
    }
    #[cfg(target_os = "windows")]
    ...
    if argv1 == CODEX_CORE_APPLY_PATCH_ARG1 {
        ...
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                ...
                let cwd = match deepseek_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                let cwd = cwd.into();
                match runtime.block_on(deepseek_apply_patch::apply_patch(
                    &patch_arg,
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    deepseek_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that DeepSeek will proceed successfully even if
...
/// While we want to deploy the DeepSeek CLI as a single executable for simplicity,
...
/// `deepseek-linux-sandbox` we *directly* execute
/// [`deepseek_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
...
///     `deepseek-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        ...
        deepseek_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `deepseek-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().deepseek_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
fn load_dotenv() {
    if let Ok(deepseek_home) = find_deepseek_home()
        && let Ok(iter) = dotenvy::from_path_iter(deepseek_home.join(".env"))
    {
        set_filtered(iter);
    }
}
...
/// user to install a separate executable, simplifying the deployment of DeepSeek
/// CLI.
...
fn prepare_path_entry_for_codex_aliases(
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if deepseek_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (deepseek_home: {deepseek_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a DEEPSEEK_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = deepseek_home.join("tmp").join("arg0");
...
mod tests {
    ...
    fn package_path_test_fixture() -> anyhow::Result<PackagePathTestFixture> {
        ...
        let install_context = InstallContext {
            ...
            package_layout: Some(DeepSeekPackageLayout {
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_deepseek_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("deepseek-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                codex_self_exe: Some(PathBuf::from("/usr/bin/codex")),
                deepseek_linux_sandbox_exe: Some(alias_path.clone()),
