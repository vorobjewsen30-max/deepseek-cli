...
use deepseek_mcp_server::DeepSeekToolCallParam;
use deepseek_mcp_server::ExecApprovalElicitRequestParams;
use deepseek_mcp_server::ExecApprovalResponse;
use deepseek_mcp_server::PatchApprovalElicitRequestParams;
use deepseek_mcp_server::PatchApprovalResponse;
use deepseek_protocol::protocol::FileChange;
use deepseek_protocol::protocol::ReviewDecision;
use deepseek_shell_command::parse_command;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(DEEPSEEK_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    let codex_request_id = mcp_process
        .send_codex_tool_call(DeepSeekToolCallParam {
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    deepseek_mcp_tool_call_id: String,
    codex_event_id: String,
    thread_id: deepseek_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow DeepSeek to run `{}` in `{}`?",
...
async fn codex_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `deepseek mcp` with a specific config.toml.
    let deepseek_home = TempDir::new()?;
    create_config_toml(deepseek_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(deepseek_home.path()).await?;
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow DeepSeek to apply proposed code changes?".to_string());
...
/// Create a DeepSeek config that uses the mock server as the model provider.
