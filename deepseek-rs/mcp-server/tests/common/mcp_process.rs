...
impl McpProcess {
    pub async fn new(deepseek_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(deepseek_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = deepseek_utils_cargo_bin::cargo_bin("deepseek-mcp-server")
            .context("should find binary for deepseek-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = deepseek_login::default_client::originator().value;
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `deepseek-mcp-server` child process.
        //
