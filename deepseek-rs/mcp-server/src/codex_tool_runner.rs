//! Asynchronous worker that executes a **DeepSeek** tool-call inside a spawned
...
use deepseek_core::config::Config as DeepSeekConfig;
...
/// To adhere to MCP `tools/call` response format, include the DeepSeek
...
/// Run a complete DeepSeek session and stream events back to the client.
///
...
pub async fn run_codex_tool_session(
    ...
    let NewThread {
        ...
        Err(e) => {
            let result = CallToolResult::error(vec![Content::text(format!(
                "Failed to start DeepSeek session: {e}"
            ))]);
    ...
    // Use the original MCP request ID as the `sub_id` for the DeepSeek submission so that
    ...
    loop {
        match thread.next_event().await {
            ...
            Err(e) => {
                let result = create_call_tool_result_with_thread_id(
                    thread_id,
                    format!("DeepSeek runtime error: {e}"),
                    Some(true),
                );
                outgoing.send_response(request_id.clone(), result).await;
                break;
