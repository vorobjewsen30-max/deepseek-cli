...
pub struct DeepSeekToolCallParam {
    /// The *initial user prompt* to start the DeepSeek conversation.
    pub prompt: String,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub approval_policy: Option<DeepSeekToolCallApprovalPolicy>,

    /// Sandbox mode: `read-only`, `workspace-write`, or `danger-full-access`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub sandbox: Option<DeepSeekToolCallSandboxMode>,
...
pub enum DeepSeekToolCallApprovalPolicy {
    Untrusted,
    OnRequest,
    Never,
}

impl From<DeepSeekToolCallApprovalPolicy> for AskForApproval {
    fn from(value: DeepSeekToolCallApprovalPolicy) -> Self {
        match value {
            DeepSeekToolCallApprovalPolicy::Untrusted => AskForApproval::UnlessTrusted,
            DeepSeekToolCallApprovalPolicy::OnRequest => AskForApproval::OnRequest,
            DeepSeekToolCallApprovalPolicy::Never => AskForApproval::Never,
        }
    }
}
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq)]
#[serde(rename_all = "kebab-case")]
pub enum DeepSeekToolCallSandboxMode {
    ReadOnly,
    WorkspaceWrite,
    DangerFullAccess,
}

impl From<DeepSeekToolCallSandboxMode> for SandboxMode {
    fn from(value: DeepSeekToolCallSandboxMode) -> Self {
        match value {
            DeepSeekToolCallSandboxMode::ReadOnly => SandboxMode::ReadOnly,
            DeepSeekToolCallSandboxMode::WorkspaceWrite => SandboxMode::WorkspaceWrite,
            DeepSeekToolCallSandboxMode::DangerFullAccess => SandboxMode::DangerFullAccess,
        }
    }
}

/// Builds a `Tool` definition (JSON schema etc.) for the DeepSeek tool-call.
pub(crate) fn create_tool_for_codex_tool_call_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        ...
        .into_generator()
        .into_root_schema_for::<DeepSeekToolCallParam>();

    let input_schema = create_tool_input_schema(schema, "DeepSeek tool schema should serialize");

    Tool::new(
        "codex",
        "Run a DeepSeek session. Accepts configuration parameters matching the DeepSeek Config struct.",
        input_schema,
    )
    .with_title("DeepSeek")
...
impl DeepSeekToolCallParam {
    /// Returns the initial user prompt to start the DeepSeek conversation and the
    ...
    pub async fn into_config(
        ...
        // Build the `ConfigOverrides` recognized by deepseek-core.
...
pub struct DeepSeekToolCallReplyParam {
    /// DEPRECATED: use threadId instead.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    conversation_id: Option<String>,

    /// The thread id for this DeepSeek session.
    ...
    /// The *next user prompt* to continue the DeepSeek conversation.
    pub prompt: String,
}

impl DeepSeekToolCallReplyParam {
...
pub(crate) fn create_tool_for_codex_tool_call_reply_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        ...
        .into_generator()
        .into_root_schema_for::<DeepSeekToolCallReplyParam>();

    let input_schema = create_tool_input_schema(schema, "DeepSeek reply tool schema should serialize");

    Tool::new(
        "codex-reply",
        "Continue a DeepSeek conversation by providing the thread id and prompt.",
        input_schema,
    )
    .with_title("DeepSeek Reply")
    .with_raw_output_schema(codex_tool_output_schema())
}
...
mod tests {
    ...
    fn verify_codex_tool_json_schema() {
        ...
        let expected_tool_json = serde_json::json!({
          "description": "Run a DeepSeek session. Accepts configuration parameters matching the DeepSeek Config struct.",
          "inputSchema": {
            ...
            "properties": {
              ...
              "config": {
                ...
                "description": "Individual config settings that will override what is in DEEPSEEK_HOME/config.toml.",
                "type": "object"
              },
              "cwd": {
              ...
              "prompt": {
                "description": "The *initial user prompt* to start the DeepSeek conversation.",
                "type": "string"
              },
              "sandbox": {
    ...
    fn codex_tool_call_param_rejects_removed_profile_field() {
        let err = serde_json::from_value::<DeepSeekToolCallParam>(serde_json::json!({
            "prompt": "hello",
            "profile": "work"
        }))
    ...
    fn verify_codex_tool_reply_json_schema() {
        ...
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Continue a DeepSeek conversation by providing the thread id and prompt.",
          "inputSchema": {
            "properties": {
              "conversationId": {
              ...
              "prompt": {
                "description": "The *next user prompt* to continue the DeepSeek conversation.",
                "type": "string"
              },
              "threadId": {
                "description": "The thread id for this DeepSeek session. This field is required, but we keep it optional here for backward compatibility for clients that still use conversationId.",
                "type": "string"
              }
            },
            "required": [
