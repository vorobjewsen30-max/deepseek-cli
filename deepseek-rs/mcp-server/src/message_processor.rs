...
use deepseek_login::default_client::USER_AGENT_SUFFIX;
...
use crate::codex_tool_config::DeepSeekToolCallParam;
use crate::codex_tool_config::DeepSeekToolCallReplyParam;
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        let thread_manager = Arc::new(ThreadManager::new(
            ...
            deepseek_core::thread_store_from_config(config.as_ref(), state_db.clone()),
    ...
    async fn handle_initialize(
        ...
        let server_info =
            Implementation::new("deepseek-mcp-server", env!("CARGO_PKG_VERSION")).with_title("DeepSeek");

        // Preserve DeepSeek's existing non-spec `serverInfo.user_agent` field.
    ...
    async fn handle_tool_call_codex(
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<DeepSeekToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.arg0_paths.clone()).await {
                    ...
                    Err(e) => {
                        let result = CallToolResult::error(vec![rmcp::model::Content::text(
                            format!("Failed to load DeepSeek configuration from overrides: {e}"),
                        )]);
                        self.outgoing.send_response(id, result).await;
                        return;
                    }
                ...
                Err(e) => {
                    let result = CallToolResult::error(vec![rmcp::model::Content::text(format!(
                        "Failed to parse configuration for DeepSeek tool: {e}"
                    ))]);
                    self.outgoing.send_response(id, result).await;
                    return;
                }
        ...
        // Spawn an async task to handle the DeepSeek session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the DeepSeek session and stream events back to the client.
    ...
    async fn handle_tool_call_codex_session_reply(
        ...
        // parse arguments
        let codex_tool_call_reply_param: DeepSeekToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<DeepSeekToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse DeepSeek tool call reply parameters: {e}");
                    let result = CallToolResult::error(vec![rmcp::model::Content::text(format!(
                        "Failed to parse configuration for DeepSeek tool: {e}"
                    ))]);
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        // Obtain the DeepSeek thread from the server.
        ...
        // Submit interrupt to DeepSeek.
        if let Err(e) = codex_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: deepseek_protocol::protocol::Op::Interrupt,
                client_user_message_id: None,
                trace: None,
            })
            .await
        {
            tracing::error!("Failed to submit interrupt to DeepSeek: {e}");
            return;
        }
