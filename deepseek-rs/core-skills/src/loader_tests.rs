...
async fn make_config(deepseek_home: &TempDir) -> TestConfig {
    make_config_for_cwd(deepseek_home, deepseek_home.path().to_path_buf()).await
}
...
async fn make_config_for_cwd(deepseek_home: &TempDir, cwd: PathBuf) -> TestConfig {
    let user_config_path = deepseek_home.path().join(CONFIG_TOML_FILE);
    let system_config_path = deepseek_home.path().join("etc/codex/config.toml");
    fs::create_dir_all(
        system_config_path
            .parent()
...
fn write_skill(deepseek_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(&deepseek_home.path().join("skills"), dir, name, description)
}

fn write_system_skill(deepseek_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(
        &deepseek_home.path().join("skills/.system"),
        dir,
        name,
        description,
    )
...
async fn loads_skill_dependencies_metadata_from_yaml() {
    let deepseek_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&deepseek_home, "demo", "dep-skill", "from json");
...
"#,
    ...
    let cfg = make_config(&deepseek_home).await;
...
async fn loads_skill_interface_metadata_from_yaml() {
    let deepseek_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&deepseek_home, "demo", "ui-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_from_yaml() {
    let deepseek_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&deepseek_home, "demo", "policy-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn empty_skill_policy_defaults_to_allow_implicit_invocation() {
    let deepseek_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&deepseek_home, "demo", "policy-empty", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_products_from_yaml() {
    let deepseek_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&deepseek_home, "demo", "policy-products", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    assert_eq!(
        ...
        Some(SkillPolicy {
            allow_implicit_invocation: None,
            products: vec![Product::DeepSeek, Product::Chatgpt, Product::Atlas],
        })
    );
}
...
async fn loads_skills_via_symlinked_subdir_for_user_scope() {
    ...
    fs::create_dir_all(deepseek_home.path().join("skills")).unwrap();
    symlink_dir(shared.path(), &deepseek_home.path().join("skills/shared"));
...
async fn does_not_loop_on_symlink_cycle_for_user_scope() {
    ...
    // Create a cycle:
    //   $DEEPSEEK_HOME/skills/cycle/loop -> $DEEPSEEK_HOME/skills/cycle
    let cycle_dir = deepseek_home.path().join("skills/cycle");
...
async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
    ...
    let cfg = make_config_for_cwd(&deepseek_home, repo_dir.path().to_path_buf()).await;
    let outcome = load_skills_for_test(&cfg).await;
...
async fn system_scope_ignores_symlinked_subdir() {
    ...
    let system_root = deepseek_home.path().join("skills/.system");
...
async fn respects_max_scan_depth_for_user_scope() {
    ...
    let skills_root = deepseek_home.path().join("skills");
    let outcome = load_skills_from_roots(
        [SkillRoot {
...
async fn loads_valid_skill() {
    let deepseek_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&deepseek_home, "demo", "demo-skill", "does things\ncarefully");
    let cfg = make_config(&deepseek_home).await;
...
async fn falls_back_to_directory_name_when_skill_name_is_missing() {
    ...
    let skill_path = write_raw_skill_at(
        &deepseek_home.path().join("skills"),
        "directory-derived",
...
async fn skips_hidden_and_invalid() {
    let deepseek_home = tempfile::tempdir().expect("tempdir");
    let hidden_dir = deepseek_home.path().join("skills/.hidden");
    ...
    let invalid_dir = deepseek_home.path().join("skills/invalid");
...
async fn preserves_overlong_descriptions() {
    ...
    write_skill(&deepseek_home, "max-len", "max-len", &max_desc);
    let cfg = make_config(&deepseek_home).await;
    ...
    write_skill(&deepseek_home, "too-long", "too-long", &too_long_desc);
    let outcome = load_skills_for_test(&cfg).await;
    assert!(
        outcome.errors.is_empty(),
        "unexpected errors: {:?}",
...
async fn loads_skills_from_all_codex_dirs_under_project_root() {
    ...
    let cfg = make_config_for_cwd(&deepseek_home, nested_dir).await;
...
async fn loads_skills_from_codex_dir_when_not_git_repo() {
    ...
    let cfg = make_config_for_cwd(&deepseek_home, work_dir.path().to_path_buf()).await;
...
async fn keeps_duplicate_names_from_repo_and_user() {
    ...
    let user_skill_path = write_skill(&deepseek_home, "user", "dupe-skill", "from user");
...
async fn repo_skills_search_does_not_escape_repo_root() {
    ...
    let cfg = make_config_for_cwd(&deepseek_home, repo_dir).await;
...
async fn loads_skills_when_cwd_is_file_in_repo() {
    ...
    let cfg = make_config_for_cwd(&deepseek_home, file_path).await;
...
async fn loads_skills_from_system_cache_when_present() {
    ...
    let skill_path = write_system_skill(&deepseek_home, "system", "system-skill", "from system");
