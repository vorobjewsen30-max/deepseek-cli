...
async fn loads_plugin_namespace_dependencies_and_policy() {
    ...
    fs::create_dir_all(root.path().join(".deepseek-plugin")).expect("manifest dir");
    fs::create_dir_all(skill_dir.join("agents")).expect("metadata dir");
    fs::write(
        root.path().join(".deepseek-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )
    .expect("manifest");
    fs::write(
    ...
    .expect("skill");
    fs::write(
        skill_dir.join("agents/deepseek.yaml"),
        r#"
dependencies:
  tools:
    - type: mcp
...
"#,
    ...
    let outcome =
        load_environment_skills_from_root(LOCAL_FS.as_ref(), &root_uri, Some(Product::DeepSeek)).await;
