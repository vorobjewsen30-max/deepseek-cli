...
fn rate_limit_reset_contract_uses_expected_paths_and_payloads() {
    assert_eq!(
        test_client("https://example.test", PathStyle::DeepSeekApi).rate_limit_status_url(),
        "https://example.test/api/codex/usage"
    );
    assert_eq!(
        test_client("https://example.test", PathStyle::DeepSeekApi)
            .consume_rate_limit_reset_credit_url(),
        "https://example.test/api/codex/rate-limit-reset-credits/consume"
    );
    assert_eq!(
        test_client("https://api.deepseek.com/v1", PathStyle::ChatGptApi)
            .rate_limit_status_url(),
        "https://api.deepseek.com/v1/wham/usage"
    );
    assert_eq!(
        test_client("https://api.deepseek.com/v1", PathStyle::ChatGptApi)
            .consume_rate_limit_reset_credit_url(),
        "https://api.deepseek.com/v1/wham/rate-limit-reset-credits/consume"
    );
