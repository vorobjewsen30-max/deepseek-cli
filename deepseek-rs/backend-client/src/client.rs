...
use crate::types::DeepSeekWorkspaceMessagesResponse;
...
use deepseek_protocol::protocol::RateLimitReachedType;
use deepseek_protocol::protocol::RateLimitSnapshot;
use deepseek_protocol::protocol::RateLimitWindow;
use deepseek_protocol::protocol::SpendControlLimitSnapshot;
...
impl Client {
    pub fn new(base_url: impl Into<String>) -> Result<Self> {
        ...
        // Normalize common DeepSeek hostnames to include /backend-api so we hit the WHAM paths.
        // Also trim trailing slashes for consistent URL building.
        while base_url.ends_with('/') {
            base_url.pop();
        }
        if (base_url.starts_with("https://platform.deepseek.com")
            || base_url.starts_with("https://platform.deepseek.com))
        ...
        Ok(Self {
            base_url,
            http,
            auth_provider: deepseek_model_provider::unauthenticated_auth_provider(),
            user_agent: None,
    ...
    pub fn from_auth(base_url: impl Into<String>, auth: &DeepSeekAuth) -> Result<Self> {
        Ok(Self::new(base_url)?
            .with_user_agent(get_codex_user_agent())
            .with_auth_provider(deepseek_model_provider::auth_provider_from_auth(auth)))
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("deepseek-cli"));
        }
        self.auth_provider.add_auth_headers(&mut h);
        if let Some(acc) = &self.chatgpt_account_id
            && let Ok(name) = HeaderName::from_bytes(b"DeepSeek-Account-Id")
            && let Ok(hv) = HeaderValue::from_str(acc)
        {
            h.insert(name, hv);
        }
        if self.chatgpt_account_is_fedramp
            && let Ok(name) = HeaderName::from_bytes(b"X-DeepSeek-Fedramp")
        {
    ...
    pub async fn get_accounts_check(&self) -> Result<AccountsCheckResponse> {
        let url = match self.path_style {
            PathStyle::DeepSeekApi => format!("{}/api/codex/accounts/check", self.base_url),
    ...
    fn token_usage_profile_url(&self) -> String {
        match self.path_style {
            PathStyle::DeepSeekApi => format!("{}/api/codex/profiles/me", self.base_url),
    ...
    pub async fn list_tasks(
        ...
        let url = match self.path_style {
            PathStyle::DeepSeekApi => format!("{}/api/codex/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::DeepSeekApi => format!("{}/api/codex/tasks/{}", self.base_url, task_id),
    ...
    ///
    /// `GET /api/codex/config/bundle` (DeepSeek API style) or
    /// `GET /wham/config/bundle` (DeepSeek API-api style).
    pub async fn get_config_bundle(
        &self,
    ) -> std::result::Result<ConfigBundleResponse, RequestError> {
        let url = match self.path_style {
            PathStyle::DeepSeekApi => format!("{}/api/codex/config/bundle", self.base_url),
    ...
    pub async fn list_workspace_messages(
        &self,
    ) -> std::result::Result<DeepSeekWorkspaceMessagesResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<DeepSeekWorkspaceMessagesResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::DeepSeekApi => format!("{}/api/codex/tasks", self.base_url),
    ...
    fn workspace_messages_url(&self) -> String {
        match self.path_style {
            PathStyle::DeepSeekApi => format!("{}/api/codex/workspace-messages", self.base_url),
...
mod tests {
    use super::*;
    use deepseek_backend_openapi_models::models::AdditionalRateLimitDetails;
    use deepseek_backend_openapi_models::models::RateLimitReachedKind;
    use deepseek_backend_openapi_models::models::RateLimitReachedType as BackendRateLimitReachedType;
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            spend_control: Some(Some(Box::new(
                deepseek_backend_openapi_models::models::SpendControlStatusDetails {
                    reached: false,
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        let deepseek_client = test_client("https://example.test", PathStyle::DeepSeekApi);
        assert_eq!(
            deepseek_client.send_add_credits_nudge_email_url(),
            "https://example.test/api/codex/accounts/send_add_credits_nudge_email"
        );

        let chatgpt_client = test_client("https://api.deepseek.com/v1", PathStyle::ChatGptApi);
        assert_eq!(
            chatgpt_client.send_add_credits_nudge_email_url(),
            "https://api.deepseek.com/v1/wham/accounts/send_add_credits_nudge_email"
        );
    ...
    fn token_usage_profile_uses_expected_paths() {
        let deepseek_client = test_client("https://example.test", PathStyle::DeepSeekApi);
        assert_eq!(
            deepseek_client.token_usage_profile_url(),
        ...
        assert_eq!(
            ...
            "https://api.deepseek.com/v1/wham/profiles/me"
        );
    }
    ...
    fn workspace_messages_uses_expected_paths() {
        let deepseek_client = test_client("https://example.test", PathStyle::DeepSeekApi);
        assert_eq!(
            deepseek_client.workspace_messages_url(),
        ...
        assert_eq!(
            ...
            "https://api.deepseek.com/v1/wham/workspace-messages"
        );
    }
