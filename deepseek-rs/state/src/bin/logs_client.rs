...
use clap::Parser;
use clap::ValueEnum;
use deepseek_state::LogQuery;
use deepseek_state::LogRow;
use deepseek_state::StateRuntime;
use dirs::home_dir;
use owo_colors::OwoColorize;

#[derive(Debug, Parser)]
#[command(name = "deepseek-state-logs")]
#[command(about = "Tail DeepSeek logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to DEEPSEEK_HOME. Defaults to $DEEPSEEK_HOME or ~/.codex.
    #[arg(long, env = "DEEPSEEK_HOME")]
    deepseek_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --deepseek-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let deepseek_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(deepseek_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let deepseek_home = args.deepseek_home.clone().unwrap_or_else(default_deepseek_home);
    Ok(deepseek_state::logs_db_path(deepseek_home.as_path()))
}

fn default_deepseek_home() -> PathBuf {
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["deepseek-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["deepseek-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["deepseek-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["deepseek-state-logs", "--level", "WARN"])
