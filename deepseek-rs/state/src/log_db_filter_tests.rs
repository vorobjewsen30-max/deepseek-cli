...
async fn sqlite_sink_drops_low_level_opentelemetry_sdk_logs() {
    let deepseek_home =
        std::env::temp_dir().join(format!("deepseek-state-log-db-filter-{}", Uuid::new_v4()));
    ...
    tracing::trace!(target: "deepseek_state", "retained-trace");
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("TRACE", "deepseek_state", Some("retained-trace")),
        ]
    );
