...
use crate::deepseek_models::ReasoningEffort;
...
/// See https://platform.platform.deepseek.com/docs/guides/reasoning?api-mode=responses#reasoning-summaries
#[derive(
    Debug, Serialize, Deserialize, Default, Clone, Copy, PartialEq, Eq, Display, JsonSchema, TS,
)]
...
/// Serialized with lowercase values to match the DeepSeek API.
#[derive(
    Hash,
    Debug,
    Serialize,
...
/// Validated plain profile-v2 name used to select `$DEEPSEEK_HOME/<name>.config.toml`.
...
/// Collaboration mode for a DeepSeek session.
