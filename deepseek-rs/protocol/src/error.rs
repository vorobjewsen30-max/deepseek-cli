...
use crate::protocol::DeepSeekErrorInfo;
...
use chrono::Local;
use chrono::Utc;
use deepseek_async_utils::CancelErr;
use deepseek_utils_string::truncate_middle_chars;
use deepseek_utils_string::truncate_middle_with_token_budget;
...
pub type Result<T> = std::result::Result<T, DeepSeekErr>;
...
pub enum DeepSeekErr {
    ...
    #[error(
        "DeepSeek ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `DeepSeekError::Spawn` variant.
    ...
    QuotaExceeded,
    #[error(
        "To use DeepSeek with your DeepSeek plan, upgrade to Plus: https://platform.deepseek.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("deepseek-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for DeepSeekErr {
    fn from(_: CancelErr) -> Self {
        DeepSeekErr::TurnAborted
    }
}
...
impl DeepSeekErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            DeepSeekErr::TurnAborted
            | DeepSeekErr::SessionBudgetExceeded
            | DeepSeekErr::Interrupted
            | DeepSeekErr::EnvVar(_)
            | DeepSeekErr::Fatal(_)
            | DeepSeekErr::UsageNotIncluded
            | DeepSeekErr::QuotaExceeded
            | DeepSeekErr::InvalidImageRequest()
            | DeepSeekErr::InvalidRequest(_)
            | DeepSeekErr::RefreshTokenFailed(_)
            | DeepSeekErr::UnsupportedOperation(_)
            | DeepSeekErr::Sandbox(_)
            | DeepSeekErr::LandlockSandboxExecutableNotProvided
            | DeepSeekErr::RetryLimit(_)
            | DeepSeekErr::ContextWindowExceeded
            | DeepSeekErr::ThreadNotFound(_)
            | DeepSeekErr::AgentLimitReached { .. }
            | DeepSeekErr::Spawn
            | DeepSeekErr::SessionConfiguredNotFirstEvent
            | DeepSeekErr::UsageLimitReached(_)
            | DeepSeekErr::ServerOverloaded
            | DeepSeekErr::CyberPolicy { .. } => false,
            DeepSeekErr::Stream(..)
            | DeepSeekErr::Timeout
            | DeepSeekErr::RequestTimeout
            | DeepSeekErr::UnexpectedStatus(_)
            | DeepSeekErr::ResponseStreamFailed(_)
            | DeepSeekErr::ConnectionFailed(_)
            | DeepSeekErr::InternalServerError
            | DeepSeekErr::InternalAgentDied
            | DeepSeekErr::Io(_)
            | DeepSeekErr::Json(_)
            | DeepSeekErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            DeepSeekErr::LandlockRuleset(_) | DeepSeekErr::LandlockPathFd(_) => false,
        }
    }

    /// Minimal shim so that existing `e.downcast_ref::<DeepSeekErr>()` checks continue to compile
    ...
    pub fn to_deepseek_protocol_error(&self) -> DeepSeekErrorInfo {
        match self {
            DeepSeekErr::ContextWindowExceeded => DeepSeekErrorInfo::ContextWindowExceeded,
            DeepSeekErr::SessionBudgetExceeded => DeepSeekErrorInfo::SessionBudgetExceeded,
            DeepSeekErr::UsageLimitReached(_)
            | DeepSeekErr::QuotaExceeded
            | DeepSeekErr::UsageNotIncluded => DeepSeekErrorInfo::UsageLimitExceeded,
            DeepSeekErr::ServerOverloaded => DeepSeekErrorInfo::ServerOverloaded,
            DeepSeekErr::CyberPolicy { .. } => DeepSeekErrorInfo::CyberPolicy,
            DeepSeekErr::RetryLimit(_) => DeepSeekErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            DeepSeekErr::ConnectionFailed(_) => DeepSeekErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            DeepSeekErr::ResponseStreamFailed(_) => DeepSeekErrorInfo::ResponseStreamConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            DeepSeekErr::RefreshTokenFailed(_) => DeepSeekErrorInfo::Unauthorized,
            DeepSeekErr::SessionConfiguredNotFirstEvent
            | DeepSeekErr::InternalServerError
            | DeepSeekErr::InternalAgentDied => DeepSeekErrorInfo::InternalServerError,
            DeepSeekErr::UnsupportedOperation(_)
            | DeepSeekErr::ThreadNotFound(_)
            | DeepSeekErr::AgentLimitReached { .. } => DeepSeekErrorInfo::BadRequest,
            DeepSeekErr::Sandbox(_) => DeepSeekErrorInfo::SandboxError,
            _ => DeepSeekErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            codex_error_info: Some(self.to_deepseek_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            DeepSeekErr::RetryLimit(err) => Some(err.status),
            DeepSeekErr::UnexpectedStatus(err) => Some(err.status),
            DeepSeekErr::ConnectionFailed(err) => err.source.status(),
            DeepSeekErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://platform.deepseek.com/explore/pro), visit https://platform.deepseek.com to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            Some(PlanType::Known(
                KnownPlan::Team
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using DeepSeek (https://platform.deepseek.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
            Some(PlanType::Known(KnownPlan::Pro | KnownPlan::ProLite)) => format!(
                "You've hit your usage limit. Visit https://platform.deepseek.com to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
pub fn get_error_message_ui(e: &DeepSeekErr) -> String {
    let message = match e {
        DeepSeekErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        DeepSeekErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
