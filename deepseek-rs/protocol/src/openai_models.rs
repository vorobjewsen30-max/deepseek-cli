//! Shared model metadata types exchanged between DeepSeek services and clients.
//!
...
/// See https://platform.platform.deepseek.com/docs/guides/reasoning?api-mode=responses#get-started-with-reasoning
...
/// Metadata describing a DeepSeek-supported model.
#[derive(Debug, Clone, Deserialize, Serialize, TS, JsonSchema, PartialEq)]
...
/// Model metadata returned by the DeepSeek backend `/models` endpoint.
...
impl ModelPreset {
    ...
    ///
    /// In DeepSeek mode, all models are visible. Otherwise, only API-supported models are shown.
