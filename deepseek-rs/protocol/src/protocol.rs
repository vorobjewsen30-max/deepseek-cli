//! Defines the protocol for a DeepSeek session between a client and an agent.
//!
...
use crate::deepseek_models::ReasoningEffort as ReasoningEffortConfig;
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for DeepSeek:";
...
pub struct ConversationStartParams {
    /// Whether DeepSeek response handoffs are managed through explicit client append calls.
    pub client_managed_handoffs: bool,
    /// Sends automatic DeepSeek responses as realtime conversation items instead of handoff appends.
    pub codex_responses_as_items: bool,
    /// Optional prefix added to automatic DeepSeek response items when `codex_responses_as_items` is set.
    pub codex_response_item_prefix: Option<String>,
    /// Optional prefix added to automatic V1 DeepSeek commentary sent with
    ...
    /// Whether to append DeepSeek's startup context to the realtime backend prompt.
...
pub enum Op {
    ...
    /// Request DeepSeek to drop the last N user turns from in-memory context.
    ///
...
/// running the command proposed by DeepSeek.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// DeepSeek errors that we expose to clients.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum DeepSeekErrorInfo {
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub codex_error_info: Option<DeepSeekErrorInfo>,
}
...
impl ErrorEvent {
    ...
    pub fn affects_turn_status(&self) -> bool {
        self.codex_error_info
            .as_ref()
            .is_none_or(DeepSeekErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::DeepSeek),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older DeepSeek
...
impl From<crate::deepseek_models::TruncationPolicyConfig> for TruncationPolicy {
    fn from(config: crate::deepseek_models::TruncationPolicyConfig) -> Self {
        match config.mode {
            crate::deepseek_models::TruncationMode::Bytes => Self::Bytes(config.limit as usize),
            crate::deepseek_models::TruncationMode::Tokens => Self::Tokens(config.limit as usize),
        }
    }
}
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(deepseek_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                deepseek_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::DeepSeek])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::DeepSeek]));
        assert!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd = deepseek_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            deepseek_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
    ...
    fn generic_error_affects_turn_status() {
        let event = ErrorEvent {
            ...
            codex_error_info: Some(DeepSeekErrorInfo::Other),
        };
        assert!(event.affects_turn_status());
    }
