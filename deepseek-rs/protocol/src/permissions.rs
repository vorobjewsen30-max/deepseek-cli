...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer DeepSeek degrades to warn-and-ignore
    /// instead of failing to load. DeepSeek 0.112.0 rejected unknown values here,
...
mod tests {
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_deepseek_config = cwd.path().join(".codex").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_deepseek_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_deepseek_config));
    }
