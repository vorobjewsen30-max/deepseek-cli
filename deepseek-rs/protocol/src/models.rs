...
use deepseek_utils_image::data_url_from_bytes;
...
use deepseek_utils_image::ImageProcessingError;
...
pub enum SandboxEnforcement {
    /// DeepSeek owns sandbox construction for this profile.
    #[default]
    Managed,
...
/// Filesystem permissions for profiles where DeepSeek owns sandbox construction.
...
fn local_image_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!(
            "DeepSeek could not read the local image at `{}`: {}",
            path.display(),
            error
        ),
    }
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "DeepSeek cannot attach image at `{}`: unsupported image `{}`.",
            path.display(),
            mime
        ),
    }
...
/// The payload we send back to DeepSeek when reporting a tool call result.
///
...
// (Moved event mapping logic into deepseek-core to avoid coupling protocol to UI-facing events.)
        ...
        for i in 0..200 {
            exec_policy
                .add_prefix_rule(
                    ...
                    deepseek_execpolicy::Decision::Allow,
                )
                .expect("add rule");
        }
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "DeepSeek cannot attach image at `{}`: unsupported image `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
