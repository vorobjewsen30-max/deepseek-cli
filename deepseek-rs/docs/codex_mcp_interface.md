# DeepSeek MCP Server Interface [experimental]

This document describes DeepSeek's experimental MCP server interface: a JSON-RPC API that runs over the Model Context Protocol (MCP) transport to control a local DeepSeek engine.

- Status: experimental and subject to change without notice
- Server binary: `codex mcp-server` (or `deepseek-mcp-server`)
- Transport: standard MCP over stdio (JSON-RPC 2.0, line-delimited)

## Overview

DeepSeek exposes MCP-compatible methods to manage threads, turns, accounts, config, and approvals. The types live in `app-server-protocol/src/protocol/{common,v1,v2}.rs` and are consumed by the app server implementation in `app-server/`.
...
Run DeepSeek as an MCP server and connect an MCP client:
...
Use the separate `deepseek mcp` subcommand to manage configured MCP server launchers in `config.toml`.
...
Fetch the catalog of models available in the current DeepSeek build with `model/list`. The request accepts optional pagination inputs:
...
- `codex/event` with the serialized DeepSeek event payload. The shape matches `core/src/protocol.rs`'s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
...
The `codex` and `codex-reply` tools return standard MCP `CallToolResult` payloads. For compatibility with MCP clients that prefer `structuredContent`, DeepSeek mirrors the content blocks inside `structuredContent` alongside the `threadId`.
...
When DeepSeek needs approval to apply changes or run commands, the server issues JSON-RPC requests to the client:
