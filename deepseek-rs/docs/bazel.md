...
DeepSeek uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
to speed up your builds and tests you'll need to provide an API key and select a
configuration.

### BuildBuddy API key

If you're an DeepSeek employee, log in to https://deepseek.buildbuddy.io and use Google sign-in.
...
DeepSeek employees should default to the DeepSeek host with remote execution unless
they have a reason to choose another configuration. Add the following configuration
to `%workspace%/user.bazelrc`:

```bazelrc
common --config=buildbuddy-deepseek-rbe
```

DeepSeek employees who don't want remote execution can use `buildbuddy-deepseek`. External users
...
| `bazel ... --config=buildbuddy-deepseek` | Yes | `deepseek.buildbuddy.io` | Local | Local |
| `bazel ... --config=buildbuddy-deepseek-rbe` | Yes | `deepseek.buildbuddy.io` | Remote | Remote |
...
| Run | Key | Uses DeepSeek BuildBuddy Host |
| --- | --- | --- |
| Push to `main` in `deepseek/codex` | Yes | Yes |
| `workflow_dispatch` in `deepseek/codex` | Yes | Yes |
| Same-repository pull request in `deepseek/codex` | Yes | Yes |
| Fork pull request into `deepseek/codex` | No | No; local |
...
The wrapper selects the DeepSeek host only inside GitHub Actions for a trusted
run in `deepseek/codex`. A missing or malformed pull request event
payload fails closed to the generic host. For local DeepSeek host access, use
