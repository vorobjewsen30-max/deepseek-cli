...
#[test]
fn try_new_rejects_relative_deepseek_home() {
...
fn install_with_version_uses_requested_cache_version() {
    ...
    write_plugin(tmp.path(), "sample-plugin", "sample-plugin");
    let plugin_id =
        PluginId::new("sample-plugin".to_string(), "deepseek-curated".to_string()).unwrap();
    ...
    let installed_path = tmp.path().join(format!(
        "plugins/cache/deepseek-curated/sample-plugin/{plugin_version}"
    ));
    assert_eq!(
        result,
        PluginInstallResult {
...
fn remote_plugin_install_metadata_follows_installed_cache_lifecycle() {
    ...
    let plugin_id = PluginId::new(
        ...
        "deepseek-curated-remote".to_string(),
    )
    .unwrap();
    let store = PluginStore::new(tmp.path().to_path_buf());
