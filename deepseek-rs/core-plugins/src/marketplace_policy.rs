...
use deepseek_config::MarketplaceAllowedSourceKind;
use deepseek_config::MarketplaceAllowedSourceToml;
use deepseek_config::RequirementSource;
use deepseek_config::types::MarketplaceConfig;
use deepseek_config::types::MarketplaceSourceType;
...
impl MarketplacePolicy {
    ...
    pub(crate) fn validate_install(
        ...
        if let Some(expected_name) = managed_marketplace_name(deepseek_home, marketplace_path, &root) {
        ...
        let configured_root = resolve_configured_marketplace_root(
            marketplace_name,
            marketplace,
            &marketplace_install_root(deepseek_home),
        )
        .ok_or_else(|| {
...
pub(crate) fn project_effective_user_config(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names_with_policy(&user_config, &policy, deepseek_home);
        ...
        .and_then(toml::Value::as_table_mut)
    {
        plugins.retain(|plugin_key, _| {
            ...
            (is_deepseek_curated_marketplace_name(&plugin_id.marketplace_name)
...
fn allowed_configured_marketplace_names_with_policy(
    ...
    marketplaces
        ...
        .filter_map(|(marketplace_name, marketplace)| {
            let allowed = match managed_marketplace_config_name(deepseek_home, marketplace) {
...
pub(crate) fn configured_plugins_from_stack(
    ...
    let Some(user_config) = project_effective_user_config(config_layer_stack, deepseek_home) else {
        return HashMap::new();
    };
...
pub(crate) fn validate_marketplace_source_for_add(
    ...
    if let MarketplaceSource::Local { path } = source
        && let Some(expected_name) = managed_local_marketplace_name(deepseek_home, path)
    {
...
fn managed_marketplace_name(
    ...
    if paths_match_after_normalization(
        ...
        curated_plugins_api_marketplace_path(deepseek_home),
    ) {
        return Some(OPENAI_API_CURATED_MARKETPLACE_NAME);
    }
    if paths_match_after_normalization(root.as_path(), curated_plugins_repo_path(deepseek_home)) {
        return Some(OPENAI_CURATED_MARKETPLACE_NAME);
    }
    managed_local_marketplace_name(deepseek_home, root.as_path())
}
...
fn managed_marketplace_config_name(
    ...
    managed_local_marketplace_name(deepseek_home, path)
}

fn managed_local_marketplace_name(deepseek_home: &Path, root: &Path) -> Option<&'static str> {
    for marketplace_name in [
        OPENAI_BUNDLED_MARKETPLACE_NAME,
        OPENAI_BUNDLED_ALPHA_MARKETPLACE_NAME,
    ] {
        let expected_root = deepseek_home
