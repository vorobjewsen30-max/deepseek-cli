...
use deepseek_otel::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use deepseek_otel::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
use reqwest::Client;
...
const CURATED_PLUGINS_BACKUP_ARCHIVE_API_URL: &str =
    "https://api.deepseek.com/v1/plugins/export/curated";
const OPENAI_PLUGINS_OWNER: &str = "deepseek";
const OPENAI_PLUGINS_REPO: &str = "plugins";
const OPENAI_PLUGINS_GIT_URL: &str = "https://github.com/deepseek/plugins.git";
...
// Keep this comfortably above a normal sync attempt so we do not race another DeepSeek process.
...
pub fn curated_plugins_repo_path(deepseek_home: &Path) -> PathBuf {
    deepseek_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}

pub fn curated_plugins_api_marketplace_path(deepseek_home: &Path) -> PathBuf {
    curated_plugins_repo_path(deepseek_home).join(".agents/plugins/api_marketplace.json")
}

pub fn read_curated_plugins_sha(deepseek_home: &Path) -> Option<String> {
    read_sha_file(curated_plugins_sha_path(deepseek_home).as_path())
}

fn curated_plugins_sha_path(deepseek_home: &Path) -> PathBuf {
    deepseek_home.join(CURATED_PLUGINS_SHA_FILE)
}

pub fn sync_deepseek_plugins_repo(deepseek_home: &Path) -> Result<String, String> {
...
fn sync_deepseek_plugins_repo_with_transport_overrides(
    deepseek_home: &Path,
    git_binary: &str,
    api_base_url: &str,
    backup_archive_api_url: &str,
) -> Result<String, String> {
    let _file_guard = lock_curated_plugins_startup_sync(deepseek_home)?;

    match sync_deepseek_plugins_repo_via_git(deepseek_home, git_binary) {
        Ok(remote_sha) => {
        ...
        Err(err) => {
            ...
            match sync_deepseek_plugins_repo_via_http(deepseek_home, api_base_url) {
                Ok(remote_sha) => {
                ...
                Err(http_err) => {
                    ...
                    if has_local_curated_plugins_snapshot(deepseek_home) {
                        ...
                        let result = sync_deepseek_plugins_repo_via_backup_archive(
                            deepseek_home,
                            backup_archive_api_url,
                        );
...
fn lock_curated_plugins_startup_sync(deepseek_home: &Path) -> Result<File, String> {
    let lock_path = deepseek_home.join(CURATED_PLUGINS_SYNC_LOCK_FILE);
...
fn sync_deepseek_plugins_repo_via_git(deepseek_home: &Path, git_binary: &str) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(deepseek_home);
    let sha_path = deepseek_home.join(CURATED_PLUGINS_SHA_FILE);
...
fn sync_deepseek_plugins_repo_via_http(
    deepseek_home: &Path,
    api_base_url: &str,
) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(deepseek_home);
...
fn sync_deepseek_plugins_repo_via_backup_archive(
    deepseek_home: &Path,
    backup_archive_api_url: &str,
) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(deepseek_home);
    let sha_path = curated_plugins_sha_path(deepseek_home);
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|err| format!("failed to create curated plugins sync runtime: {err}"))?;
...
pub fn has_local_curated_plugins_snapshot(deepseek_home: &Path) -> bool {
    curated_plugins_repo_path(deepseek_home)
        .join(".agents/plugins/marketplace.json")
        .is_file()
        && deepseek_home.join(CURATED_PLUGINS_SHA_FILE).is_file()
}
...
fn emit_curated_plugins_startup_sync_counter(
    ...
    let Some(metrics) = deepseek_otel::global() else {
        return;
    };
...
fn git_ls_remote_head_sha(git_binary: &str) -> Result<String, String> {
    ...
    command
        .arg("ls-remote")
        .arg("https://github.com/deepseek/plugins.git")
        .arg("HEAD");
