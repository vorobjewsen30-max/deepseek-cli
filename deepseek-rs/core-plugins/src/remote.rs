...
use deepseek_app_server_protocol::SkillInterface;
...
pub const REMOTE_GLOBAL_MARKETPLACE_NAME: &str = "deepseek-curated-remote";
...
pub const REMOTE_GLOBAL_MARKETPLACE_DISPLAY_NAME: &str = "DeepSeek Curated Remote";
    ...
    for source in sources {
        match source {
            RemoteMarketplaceSource::Global => {
                ...
                if let Some(deepseek_home) = global_catalog_cache_path
                    && let Some(directory_plugins) =
                        catalog_cache::load_cached_global_directory_plugins(
                            deepseek_home, config, auth,
                        )
                {
                ...
                if let (Some(deepseek_home), Some(directory_plugins)) =
...
pub async fn fetch_and_cache_global_remote_plugin_catalog(
    ...
    catalog_cache::write_cached_global_directory_plugins(deepseek_home, config, auth, &plugins);
    Ok(())
}
...
pub fn has_cached_global_remote_plugin_catalog(
    ...
    catalog_cache::load_cached_global_directory_plugins(deepseek_home, config, auth).is_some()
}
...
pub fn cached_global_remote_discoverable_plugins(
    ...
    catalog_cache::load_cached_global_directory_plugins(deepseek_home, config, auth)
        .unwrap_or_default()
        .into_iter()
...
pub async fn fetch_deepseek_curated_remote_collection_marketplace(
    config: &RemotePluginServiceConfig,
    auth: Option<&DeepSeekAuth>,
...
pub async fn uninstall_remote_plugin(
    ...
    tokio::task::spawn_blocking(move || {
        remove_remote_plugin_cache(deepseek_home, marketplace_name, plugin_name, legacy_plugin_id)
    })
    .await
    .map_err(|err| {
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(deepseek_home.clone())
    ...
    let legacy_remote_plugin_cache_root = deepseek_home
...
fn ensure_chatgpt_auth(auth: Option<&DeepSeekAuth>) -> Result<&DeepSeekAuth, RemotePluginCatalogError> {
...
fn authenticated_request(
    ...
    Ok(request
        ...
        .headers(deepseek_model_provider::auth_provider_from_auth(auth).to_auth_headers())
