...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    let mut request = client
        ...
        .query(&[(
            "platform",
            product.unwrap_or(Product::DeepSeek).to_app_platform(),
        )])
        .timeout(REMOTE_FEATURED_PLUGIN_FETCH_TIMEOUT);

    if let Some(auth) = auth.filter(|auth| auth.uses_codex_backend()) {
        request =
            request.headers(deepseek_model_provider::auth_provider_from_auth(auth).to_auth_headers());
    }
...
fn ensure_codex_backend_auth(
    auth: Option<&DeepSeekAuth>,
) -> Result<&DeepSeekAuth, RemotePluginMutationError> {
