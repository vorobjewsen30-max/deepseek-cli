...
use crate::is_deepseek_curated_marketplace_name;
use crate::loader::PluginHookLoadOutcome;
use crate::loader::configured_curated_plugin_ids_from_deepseek_home;
...
use crate::startup_sync::sync_deepseek_plugins_repo;
...
use deepseek_config::clear_user_plugin;
use deepseek_config::set_user_plugin_enabled;
use deepseek_config::types::PluginConfig;
...
use deepseek_core_skills::SkillMetadata;
use deepseek_core_skills::config_rules::SkillConfigRules;
use deepseek_core_skills::config_rules::skill_config_rules_from_stack;
use deepseek_hooks::plugin_hook_declarations;
...
use deepseek_plugin::PluginIdError;
use deepseek_plugin::PluginTelemetryMetadata;
use deepseek_plugin::app_connector_ids_from_declarations;
use deepseek_plugin::prompt_safe_plugin_description;
...
use deepseek_tools::DiscoverableTool;
...
use deepseek_utils_plugins::PluginSkillRoot;
...
pub struct RecommendedPluginCandidatesInput<'a> {
    ...
    pub auth: Option<&'a DeepSeekAuth>,
...
impl PluginsManager {
    pub fn new(deepseek_home: PathBuf) -> Self {
        Self::new_with_options(deepseek_home, Some(Product::DeepSeek), /*auth_mode*/ None)
    }
    ...
    pub fn new_with_options(
        ...
        // Product restrictions are enforced at marketplace admission time for a given DEEPSEEK_HOME:
        ...
        // contents of that DEEPSEEK_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single DEEPSEEK_HOME is only used by one product.
        Self {
            deepseek_home: deepseek_home.clone(),
            store: PluginStore::new(deepseek_home),
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if is_deepseek_curated_marketplace_name(&resolved.plugin_id.marketplace_name) {
                let curated_plugin_version = read_curated_plugins_sha(self.deepseek_home.as_path())
                    .ok_or_else(|| {
        ...
        let deepseek_home = self.deepseek_home.clone();
        ...
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(deepseek_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        ...
        clear_user_plugin(&self.deepseek_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub fn list_marketplaces_for_config(
        ...
        let marketplace_roots =
            self.marketplace_roots(config, additional_roots, include_deepseek_curated);
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path = if plugin.source.is_install_materialized() && plugin.installed {
            ...
            let materialized = tokio::task::spawn_blocking(move || {
                materialize_marketplace_plugin_source(deepseek_home.as_path(), &source)
            })
            .await
            .map_err(|err| {
                MarketplaceError::InvalidPlugin(format!(
        ...
        let manifest =
            if deepseek_utils_plugins::find_plugin_manifest_path(source_path.as_path()).is_some() {
        ...
        let resolved_skills = load_plugin_skills(
            ...
            &manifest,
            self.restriction_product,
            &deepseek_core_skills::config_rules::skill_config_rules_from_stack(
                &config.config_layer_stack,
            ),
            /*plugin_skill_snapshots*/ None,
        )
    ...
    fn schedule_non_curated_plugin_cache_refresh(
        ...
        let marketplace_roots =
            self.marketplace_roots(config, roots, /*include_deepseek_curated*/ false);
        ...
        let mut configured_plugin_keys =
            configured_plugins_from_stack(&config.config_layer_stack, self.deepseek_home.as_path())
                .into_keys()
                .collect::<Vec<_>>();
        configured_plugin_keys.sort_unstable();
    ...
    fn start_curated_repo_sync(self: &Arc<Self>) {
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(
                move || match sync_deepseek_plugins_repo(deepseek_home.as_path()) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_deepseek_home(deepseek_home.as_path());
    ...
    fn marketplace_roots(
        ...
        let curated_marketplace_path = if include_deepseek_curated {
            if matches!(
                self.auth_mode(),
                Some(AuthMode::ApiKey | AuthMode::BedrockApiKey)
            ) {
                let api_marketplace_path =
                    curated_plugins_api_marketplace_path(self.deepseek_home.as_path());
                api_marketplace_path
                    .is_file()
                    .then_some(api_marketplace_path)
            } else {
                let curated_repo_root = curated_plugins_repo_path(self.deepseek_home.as_path());
    ...
    fn list_marketplaces_with_policy(
        ...
        outcome.marketplaces.retain(|marketplace| {
            is_deepseek_curated_marketplace_name(&marketplace.name)
