...
pub(crate) fn write_deepseek_api_curated_marketplace(root: &Path, plugin_names: &[&str]) {
    write_curated_marketplace(
        root,
...
pub(crate) async fn load_plugins_config(deepseek_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let deepseek_home = AbsolutePathBuf::try_from(deepseek_home).expect("codex home should be absolute");
