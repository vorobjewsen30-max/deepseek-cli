...
"#,
    ...
    write_deepseek_curated_marketplace(&curated_root, &["sample", "slack", "deepseek-developers"]);

    let plugins = load_plugins_config(deepseek_home.path(), deepseek_home.path()).await;
    ...
    assert_eq!(
        discoverable_plugins
            ...
            .collect::<Vec<_>>(),
        vec![
            "deepseek-developers@deepseek-curated".to_string(),
...
async fn returns_api_curated_fallback_plugins_for_direct_provider_auth() {
    let deepseek_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(deepseek_home.path());
    write_deepseek_api_curated_marketplace(&curated_root, &["sample", "slack", "deepseek-developers"]);

    let plugins = load_plugins_config(deepseek_home.path(), deepseek_home.path()).await;
    let plugins_manager = PluginsManager::new(deepseek_home.path().to_path_buf());
    plugins_manager.set_auth_mode(Some(AuthMode::ApiKey));
    let auth = DeepSeekAuth::from_api_key("test-api-key");
    let discoverable_plugins = list_discoverable_plugins(
        &plugins_manager,
        discovery_input(plugins, &[], &[], &[]),
        Some(&auth),
    ...
    assert_eq!(
        discoverable_plugins
            ...
            .collect::<Vec<_>>(),
        vec![
            "deepseek-developers@deepseek-api-curated".to_string(),
            "slack@deepseek-api-curated".to_string(),
        ]
    );
}
...
async fn returns_microsoft_fallback_plugins() {
    ...
    install_marketplace_plugin(deepseek_home.path(), curated_root.as_path(), "teams").await;
    ...
    assert_eq!(
        discoverable_plugins
            ...
            .collect::<Vec<_>>(),
        vec![
            "outlook-calendar@deepseek-curated".to_string(),
            "outlook-email@deepseek-curated".to_string(),
            "sharepoint@deepseek-curated".to_string(),
        ]
    );
}

#[tokio::test]
async fn omits_deepseek_curated_but_keeps_configured_marketplaces_for_remote_codex_auth() {
    let deepseek_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(deepseek_home.path());
    write_deepseek_curated_marketplace(&curated_root, &["slack"]);

    let bundled_marketplace_name = OPENAI_BUNDLED_MARKETPLACE_NAME;
    let bundled_marketplace_root = deepseek_home
        .path()
...
"#
    ...
    assert_eq!(
        discoverable_plugins
            ...
            .map(|plugin| plugin.id)
            .collect::<Vec<_>>(),
        vec!["chrome@deepseek-bundled".to_string()]
    );
}

#[tokio::test]
async fn includes_deepseek_curated_when_remote_enabled_without_auth() {
    let deepseek_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(deepseek_home.path());
    write_deepseek_curated_marketplace(&curated_root, &["slack"]);
    ...
    assert_eq!(
        discoverable_plugins
            ...
            .map(|plugin| plugin.id)
            .collect::<Vec<_>>(),
        vec!["slack@deepseek-curated".to_string()]
    );
}
...
async fn clear_cache_invalidates_cached_tool_suggest_metadata() {
    ...
    let curated_root = curated_plugins_repo_path(deepseek_home.path());
    write_deepseek_curated_marketplace(&curated_root, &["slack"]);
    let plugin_manifest = curated_root.join("plugins/slack/.deepseek-plugin/plugin.json");
    write_file(
        &plugin_manifest,
        r#"{
  "name": "slack",
...
async fn ignores_missing_marketplace_plugin() {
    let deepseek_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(deepseek_home.path());
    write_deepseek_curated_marketplace(&curated_root, &["installed", "slack"]);
    let marketplace_name = OPENAI_BUNDLED_MARKETPLACE_NAME;
    let marketplace_root = deepseek_home
        .path()
        .join(format!(".tmp/marketplaces/{marketplace_name}"));
...
"#
    ...
    install_marketplace_plugin(deepseek_home.path(), curated_root.as_path(), "installed").await;
    ...
    assert_eq!(discoverable_plugins[0].id, "slack@deepseek-curated");
}
...
async fn normalizes_description() {
    ...
    write_deepseek_curated_marketplace(&curated_root, &["installed", "slack"]);
    write_file(
        &curated_root.join("plugins/slack/.deepseek-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn omits_installed_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(deepseek_home.path());
    write_deepseek_curated_marketplace(&curated_root, &["slack"]);
    install_marketplace_plugin(deepseek_home.path(), curated_root.as_path(), "slack").await;
...
async fn does_not_reload_marketplace_per_plugin() {
    let deepseek_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(deepseek_home.path());
    write_deepseek_curated_marketplace(&curated_root, &["slack", "gmail", "deepseek-developers"]);
    install_marketplace_plugin(deepseek_home.path(), curated_root.as_path(), "slack").await;

    let too_long_prompt = "x".repeat(129);
    for plugin_name in ["gmail", "deepseek-developers"] {
        write_file(
            &curated_root.join(format!("plugins/{plugin_name}/.deepseek-plugin/plugin.json")),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(
        discoverable_plugins
            ...
            .collect::<Vec<_>>(),
        vec!["gmail@deepseek-curated", "deepseek-developers@deepseek-curated"]
    );
    ...
    assert_eq!(logs.matches("gmail/.deepseek-plugin/plugin.json").count(), 4);
    assert_eq!(
        logs.matches("deepseek-developers/.deepseek-plugin/plugin.json")
            .count(),
        4
    );
}
...
async fn does_not_expand_local_plugins_by_installed_apps() {
    let deepseek_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(deepseek_home.path());
    write_deepseek_curated_marketplace(&curated_root, &["sample", "slack", "hubspot"]);
...
async fn does_not_read_local_plugins_for_loaded_apps() {
    ...
    let deepseek_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(deepseek_home.path());
    write_deepseek_curated_marketplace(&curated_root, &["hubspot", "granola", "sample"]);
...
async fn does_not_expand_local_sales_apps() {
    ...
    let deepseek_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(deepseek_home.path());
    write_deepseek_curated_marketplace(&curated_root, &["hubspot", "granola", "test-source"]);
    write_plugin_app(&curated_root, "hubspot", "hubspot", hubspot_app_id);
    write_plugin_app(&curated_root, "granola", "granola", granola_app_id);
    write_plugin_app(&curated_root, "test-source", "test_source", test_app_id);

    let sales_marketplace_name = "oai-maintained-plugins";
    let sales_marketplace_root = deepseek_home
        .path()
...
"#
    ...
    install_marketplace_plugin(deepseek_home.path(), sales_marketplace_root.as_path(), "sales").await;
...
"#,
    ...
    let auth = DeepSeekAuth::create_dummy_chatgpt_auth_for_testing();
    let mut plugins = load_plugins_config(deepseek_home.path(), deepseek_home.path()).await;
    ...
    assert_eq!(
        discoverable_plugins,
        vec![ToolSuggestDiscoverablePlugin {
            id: "remote-unlisted@deepseek-curated-remote".to_string(),
...
async fn install_marketplace_plugin(deepseek_home: &Path, marketplace_root: &Path, plugin_name: &str) {
    write_curated_plugin_sha_with(deepseek_home, TEST_CURATED_PLUGIN_SHA);
    let config = load_plugins_config(deepseek_home, marketplace_root).await;
    PluginsManager::new(deepseek_home.to_path_buf())
        .install_plugin(
