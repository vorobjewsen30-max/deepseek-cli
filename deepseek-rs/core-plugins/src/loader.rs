...
use deepseek_config::HooksFile;
use deepseek_config::types::McpServerConfig;
use deepseek_config::types::PluginConfig;
use deepseek_config::types::PluginMcpServerConfig;
use deepseek_connectors::parse_plugin_app_config;
use deepseek_connectors::parse_plugin_app_config_value;
...
use deepseek_core_skills::config_rules::resolve_disabled_skill_paths;
use deepseek_core_skills::config_rules::skill_config_rules_from_stack;
use deepseek_core_skills::loader::SkillRoot;
use deepseek_core_skills::loader::load_skills_from_roots;
use deepseek_exec_server::LOCAL_FS;
use deepseek_mcp::parse_plugin_mcp_config;
use deepseek_plugin::AppDeclaration;
use deepseek_plugin::LoadedPlugin;
use deepseek_plugin::PluginCapabilitySummary;
use deepseek_plugin::PluginHookSource;
...
use deepseek_utils_plugins::find_plugin_manifest_path;
...
async fn load_plugins_from_layer_stack_with_scope(
    ...
    let configured_plugins = merge_configured_plugins_with_remote_installed(
        configured_plugins_from_stack(config_layer_stack, store.deepseek_home().as_path()),
        extra_plugins,
        store,
        remote_global_catalog_active,
    );
...
fn merge_configured_plugins_with_remote_installed(
    ...
    for plugin_key in configured_plugins.keys() {
        ...
        if !is_deepseek_curated_marketplace_name(&plugin_id.marketplace_name)
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(deepseek_home.to_path_buf()).map_err(|err| err.to_string())?;
    let curated_marketplace_paths = curated_marketplace_paths_for_cache_refresh(deepseek_home)?;
...
fn curated_marketplace_paths_for_cache_refresh(
    ...
    let api_marketplace_path = deepseek_home
        .join(".tmp/plugins")
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    let mut configured_non_curated_plugin_ids = configured_plugin_keys
        ...
        .filter_map(|plugin_key| match PluginId::parse(plugin_key) {
            Ok(plugin_id) if !is_deepseek_curated_marketplace_name(&plugin_id.marketplace_name) => {
                Some(plugin_id)
            }
            Ok(_) => None,
            Err(err) => {
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let refresh_result = (|| -> Result<bool, String> {
            let materialized =
                materialize_marketplace_plugin_source(deepseek_home, &source).map_err(|err| {
...
fn configured_plugins_from_deepseek_home(
    deepseek_home: &Path,
...
fn curated_plugin_ids_from_config_keys(
    ...
    .into_iter()
    .filter(|plugin_id| is_deepseek_curated_marketplace_name(&plugin_id.marketplace_name))
    .collect::<Vec<_>>();
    configured_curated_plugin_ids.sort_unstable_by_key(PluginId::as_key);
    configured_curated_plugin_ids
}

pub fn configured_curated_plugin_ids_from_deepseek_home(deepseek_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_deepseek_home(
        deepseek_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".deepseek-plugin/plugin.json"));
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = deepseek_home.join("plugins/.marketplace-plugin-source-staging");
