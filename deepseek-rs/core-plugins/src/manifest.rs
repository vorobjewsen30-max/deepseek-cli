...
pub type PluginManifest = deepseek_plugin::manifest::PluginManifest<AbsolutePathBuf>;
pub type PluginManifestHooks = deepseek_plugin::manifest::PluginManifestHooks<AbsolutePathBuf>;
pub type PluginManifestInterface = deepseek_plugin::manifest::PluginManifestInterface<AbsolutePathBuf>;
pub type PluginManifestMcpServers =
    deepseek_plugin::manifest::PluginManifestMcpServers<AbsolutePathBuf>;
pub type PluginManifestPaths = deepseek_plugin::manifest::PluginManifestPaths<AbsolutePathBuf>;

pub(crate) type UriPluginManifest = deepseek_plugin::manifest::PluginManifest<PathUri>;
...
pub(crate) fn parse_plugin_manifest_uri(
    ...
    let interface = interface.and_then(|interface| {
        ...
        let interface = deepseek_plugin::manifest::PluginManifestInterface {
            display_name,
            short_description,
            long_description,
            developer_name,
    ...
    Ok(deepseek_plugin::manifest::PluginManifest {
        name,
        version,
        description,
        keywords,
        paths: deepseek_plugin::manifest::PluginManifestPaths {
...
fn resolve_manifest_hooks(
    plugin_root: &PathUri,
    hooks: Option<RawPluginManifestHooks>,
) -> Option<deepseek_plugin::manifest::PluginManifestHooks<PathUri>> {
    match hooks? {
        RawPluginManifestHooks::Path(path) => {
            resolve_manifest_path(plugin_root, "hooks", Some(&path))
                .map(|path| deepseek_plugin::manifest::PluginManifestHooks::Paths(vec![path]))
        }
        RawPluginManifestHooks::Paths(paths) => {
            let hooks = paths
                ...
                .collect::<Vec<_>>();
            (!hooks.is_empty()).then_some(deepseek_plugin::manifest::PluginManifestHooks::Paths(hooks))
        }
        RawPluginManifestHooks::Inline(hooks) => {
            Some(deepseek_plugin::manifest::PluginManifestHooks::Inline(vec![
                hooks,
            ]))
        }
        RawPluginManifestHooks::InlineList(hooks) => (!hooks.is_empty())
            .then_some(deepseek_plugin::manifest::PluginManifestHooks::Inline(hooks)),
...
fn resolve_manifest_mcp_servers(
    plugin_root: &PathUri,
    mcp_servers: Option<RawPluginManifestMcpServers>,
) -> Option<deepseek_plugin::manifest::PluginManifestMcpServers<PathUri>> {
    match mcp_servers? {
        RawPluginManifestMcpServers::Path(path) => {
            resolve_manifest_path(plugin_root, "mcpServers", Some(&path))
                .map(deepseek_plugin::manifest::PluginManifestMcpServers::Path)
        }
        RawPluginManifestMcpServers::Object(servers) => match serde_json::to_string(&servers) {
            Ok(servers) => Some(deepseek_plugin::manifest::PluginManifestMcpServers::Object(
                servers,
            )),
            Err(err) => {
...
mod tests {
    ...
    use deepseek_plugin::PluginProvider;
    use deepseek_plugin::ResolvedPlugin;
    use deepseek_plugin::manifest::PluginManifest as GenericPluginManifest;
    use deepseek_plugin::manifest::PluginManifestHooks;
    use deepseek_plugin::manifest::PluginManifestInterface;
    use deepseek_plugin::manifest::PluginManifestMcpServers;
    use deepseek_plugin::manifest::PluginManifestPaths;
    ...
    fn write_manifest(plugin_root: &Path, version: Option<&str>, interface: &str) {
        fs::create_dir_all(plugin_root.join(".deepseek-plugin")).expect("create manifest dir");
        let version = version
  ...
  "interface": {
    ...
    fn parse_uri_composer_icon(plugin_root: &PathUri, composer_icon: &str) -> Option<PathUri> {
        let manifest_path = plugin_root
            .join(".deepseek-plugin/plugin.json")
            .expect("manifest URI");
  ...
  "interface": {{
    ...
    "composerIcon": "./assets/icon.svg"
        ...
        let manifest_contents =
            fs::read_to_string(plugin_root.join(".deepseek-plugin/plugin.json")).expect("manifest");
