...
use crate::test_support::write_deepseek_api_curated_marketplace;
use crate::test_support::write_deepseek_curated_marketplace;
use deepseek_config::AppToolApproval;
...
use deepseek_config::McpServerConfig;
use deepseek_config::McpServerOAuthConfig;
use deepseek_config::McpServerToolConfig;
use deepseek_config::RequirementSource;
use deepseek_config::RequirementsLayerEntry;
use deepseek_config::compose_requirements;
use deepseek_config::types::McpServerTransportConfig;
use deepseek_core_skills::PluginSkillSnapshots;
use deepseek_core_skills::SkillsLoadInput;
use deepseek_core_skills::SkillsService;
use deepseek_core_skills::config_rules::SkillConfigRules;
use deepseek_login::DeepSeekAuth;
use deepseek_plugin::AppDeclaration;
use deepseek_plugin::PluginId;
use deepseek_protocol::auth::AuthMode;
use deepseek_protocol::protocol::HookEventName;
use deepseek_protocol::protocol::Product;
...
fn config_layer_stack_with_requirements(
    ...
    let config_file =
        AbsolutePathBuf::try_from(deepseek_home.join(CONFIG_TOML_FILE)).expect("absolute config path");
...
fn plugins_config_input_with_requirements(
    ...
    PluginsConfigInput::new(
        config_layer_stack_with_requirements(deepseek_home, user_config, requirements),
...
async fn marketplace_policy_projection_disables_installed_plugin_and_invalidates_cache() {
    let deepseek_home = TempDir::new().expect("create DeepSeek home");
    write_plugin(
        &deepseek_home.path().join("plugins/cache/company"),
        "sample/local",
        "sample",
    );
    let user_config = r#"
...
"#,
    ...
    let manager = PluginsManager::new(deepseek_home.path().to_path_buf());
...
async fn plugin_read_rejects_marketplace_blocked_by_requirements() {
    let deepseek_home = TempDir::new().expect("create DeepSeek home");
    let marketplace_root = deepseek_home.path().join("marketplace");
...
"#,
    ...
    let err = PluginsManager::new(deepseek_home.path().to_path_buf())
...
fn marketplace_policy_filters_discovered_marketplaces_by_configured_name() {
    let deepseek_home = TempDir::new().expect("create DeepSeek home");
    let repo_root = deepseek_home.path().join("repo");
...
"#,
    ...
    let allowed_outcome = manager
        .list_marketplaces_for_config(
            ...
            /*include_deepseek_curated*/ false,
        )
...
fn write_auth_projection_plugin(deepseek_home: &Path, name: &str, include_app: bool) {
  ...
  "mcpServers": {{
    ...
    if include_app {
        write_auth_projection_app(deepseek_home, name, name);
    }
}

fn write_auth_projection_app(deepseek_home: &Path, plugin_name: &str, app_name: &str) {
    let plugin_root = deepseek_home
        .join("plugins/cache")
        .join("test")
        .join(plugin_name)
...
async fn auth_projection_config(deepseek_home: &Path) -> PluginsConfigInput {
...
"#
    .to_string();
    write_file(&deepseek_home.join(CONFIG_TOML_FILE), &config_toml);
    load_config(deepseek_home, deepseek_home).await
}
...
async fn plugin_auth_projection_hides_apps_without_chatgpt_auth() {
    let deepseek_home = TempDir::new().unwrap();
    write_auth_projection_plugin(deepseek_home.path(), "sample", /*include_app*/ true);
    write_auth_projection_plugin(deepseek_home.path(), "docs", /*include_app*/ false);
    let config = auth_projection_config(deepseek_home.path()).await;
...
async fn plugin_auth_projection_keeps_non_conflicting_mcp_with_chatgpt_apps_route() {
    let deepseek_home = TempDir::new().unwrap();
    write_auth_projection_plugin(deepseek_home.path(), "sample", /*include_app*/ false);
    write_auth_projection_app(deepseek_home.path(), "sample", "sample_app");
    write_auth_projection_plugin(deepseek_home.path(), "docs", /*include_app*/ false);
    let config = auth_projection_config(deepseek_home.path()).await;
    let manager = PluginsManager::new_with_options(
        deepseek_home.path().to_path_buf(),
...
"#,
    ...
    let config = load_config(deepseek_home.path(), deepseek_home.path()).await;
    let manager = PluginsManager::new_with_options(
        deepseek_home.path().to_path_buf(),
        Some(Product::DeepSeek),
        Some(AuthMode::Chatgpt),
...
fn write_plugin_with_version(
    ...
    fs::create_dir_all(plugin_root.join(".deepseek-plugin")).unwrap();
...
async fn load_plugins_from_config(
    ...
    write_file(&deepseek_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(deepseek_home, deepseek_home).await;
    PluginsManager::new_with_options(deepseek_home.to_path_buf(), Some(Product::DeepSeek), auth_mode)
        .plugins_for_config(&config)
        .await
}

async fn load_config(deepseek_home: &Path, cwd: &Path) -> PluginsConfigInput {
    load_plugins_config_input(deepseek_home, cwd).await
}
...
fn remote_installed_plugin_in_marketplace(
    ...
    RemoteInstalledPlugin {
        ...
        name: name.to_string(),
        enabled: true,
        install_policy: deepseek_app_server_protocol::PluginInstallPolicy::Available,
        auth_policy: deepseek_app_server_protocol::PluginAuthPolicy::OnUse,
        availability: deepseek_app_server_protocol::PluginAvailability::Available,
        interface: None,
        keywords: Vec::new(),
    }
}

fn write_cached_plugin(deepseek_home: &Path, marketplace_name: &str, plugin_name: &str) {
    write_plugin_with_version(
        &deepseek_home
            .join("plugins/cache")
...
"#;
    let outcome =
        load_plugins_from_config(config_toml, deepseek_home.path(), /*auth_mode*/ None).await;
...
async fn installed_plugin_telemetry_metadata_collects_capabilities() {
    let deepseek_home = TempDir::new().unwrap();
    write_cached_plugin(deepseek_home.path(), "test", "sample");
    let manager = PluginsManager::new(deepseek_home.path().to_path_buf());
...
async fn installed_plugin_telemetry_metadata_resolves_persisted_remote_identity() {
    let deepseek_home = TempDir::new().unwrap();
    write_cached_plugin(deepseek_home.path(), "deepseek-curated-remote", "linear");
    let plugin_id =
        PluginId::parse("linear@deepseek-curated-remote").expect("plugin id should parse");
    PluginStore::new(deepseek_home.path().to_path_buf())
    ...
    assert_eq!(
        ...
        PluginTelemetryMetadata {
            ...
            capability_summary: Some(PluginCapabilitySummary {
                config_name: "linear@deepseek-curated-remote".to_string(),
...
[plugins."linear@deepseek-curated"]
enabled = true

[plugins."calendar@deepseek-curated"]
enabled = true
"#,
    );
    write_cached_plugin(deepseek_home.path(), "deepseek-curated", "linear");
    write_cached_plugin(deepseek_home.path(), "deepseek-curated", "calendar");
    write_cached_plugin(deepseek_home.path(), "deepseek-curated-remote", "linear");
    write_cached_plugin(deepseek_home.path(), "deepseek-curated-remote", "remote-only");
    ...
    assert_eq!(
        outcome
            ...
            .collect::<Vec<_>>(),
        vec![
            "calendar@deepseek-curated".to_string(),
            "linear@deepseek-curated".to_string(),
            "remote-only@deepseek-curated-remote".to_string(),
        ]
    );
}
...
[plugins."linear@deepseek-api-curated"]
enabled = true
...
"#,
    ...
    write_cached_plugin(deepseek_home.path(), "deepseek-curated", "linear");
    write_cached_plugin(deepseek_home.path(), "deepseek-api-curated", "linear");
    write_cached_plugin(deepseek_home.path(), "deepseek-curated", "calendar");
    write_cached_plugin(deepseek_home.path(), "deepseek-curated-remote", "linear");
    write_cached_plugin(deepseek_home.path(), "deepseek-curated-remote", "remote-only");
    ...
    assert_eq!(
        outcome
            ...
            .collect::<Vec<_>>(),
        vec![
            "linear@deepseek-api-curated".to_string(),
            "linear@deepseek-curated-remote".to_string(),
            "remote-only@deepseek-curated-remote".to_string(),
        ]
    );
}
...
async fn build_remote_installed_plugin_marketplaces_from_cache_uses_remote_metadata() {
    ...
    plugin.install_policy = deepseek_app_server_protocol::PluginInstallPolicy::InstalledByDefault;
    plugin.auth_policy = deepseek_app_server_protocol::PluginAuthPolicy::OnInstall;
    plugin.interface = Some(deepseek_app_server_protocol::PluginInterface {
    ...
    assert_eq!(marketplaces[0].name, "deepseek-curated-remote");
    assert_eq!(marketplaces[0].display_name, "DeepSeek Curated Remote");
    assert_eq!(marketplaces[0].plugins.len(), 1);
    let plugin = &marketplaces[0].plugins[0];
    assert_eq!(plugin.id, "linear@deepseek-curated-remote");
    ...
    assert_eq!(
        ...
        deepseek_app_server_protocol::PluginInstallPolicy::InstalledByDefault
    );
    assert_eq!(
        plugin.auth_policy,
        deepseek_app_server_protocol::PluginAuthPolicy::OnInstall
    );
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    let deepseek_home = TempDir::new().unwrap();
    let plugin_a_root = deepseek_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_root = deepseek_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-b/local");

    write_file(
        &plugin_a_root.join(".deepseek-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".deepseek-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    let outcome =
        load_plugins_from_config(&config_toml, deepseek_home.path(), Some(AuthMode::Chatgpt)).await;
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(deepseek_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        disabled_skill_paths: HashSet::new(),
        has_enabled_skills: false,
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![deepseek_home.path().join("skills-plugin/skills").abs()],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![deepseek_home.path().join("disabled-plugin/skills").abs()],
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config(deepseek_home.path(), deepseek_home.path()).await;
    let outcome = PluginsManager::new(deepseek_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn plugin_cache_ignores_unrelated_session_overrides() {
    ...
    let plugin_root = deepseek_home
        ...
        .join("test/sample/local");
    write_plugin(
        deepseek_home.path().join("plugins/cache/test").as_path(),
        "sample/local",
        "sample",
    );
    write_file(
  ...
  "mcpServers": {
    ...
    let user_file = deepseek_home.path().join(CONFIG_TOML_FILE).abs();
    ...
    let config = |session_config| {
        PluginsConfigInput::new(
            ...
            /*plugins_enabled*/ true,
            /*remote_plugin_enabled*/ false,
            "https://platform.deepseek.com".to_string(),
        )
    };
    let manager = PluginsManager::new(deepseek_home.path().to_path_buf());
...
async fn skills_service_reuses_skills_parsed_during_plugin_load() {
    let deepseek_home = TempDir::new().unwrap();
    let deepseek_home_abs = deepseek_home.path().to_path_buf().abs();
    let plugin_root = deepseek_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    let skills_input = SkillsLoadInput::new(
        deepseek_home_abs.clone(),
        plugin_outcome.effective_plugin_skill_roots(),
        config.config_layer_stack.clone(),
        /*bundled_skills_enabled*/ false,
    )
    .with_plugin_skill_snapshots(plugin_skill_snapshots);
    let skills_service = SkillsService::new(deepseek_home_abs, /*bundled_skills_enabled*/ false);
...
async fn strict_install_requires_allowed_local_marketplace_to_be_added_first() {
    let deepseek_home = TempDir::new().expect("create DeepSeek home");
    let marketplace_root = deepseek_home.path().join("company-marketplace");
    write_plugin(&marketplace_root, "sample", "sample");
    write_file(
        &marketplace_root.join(".agents/plugins/marketplace.json"),
        r#"{
...
"#
    ...
    let config = config_layer_stack_with_requirements(deepseek_home.path(), "", &requirements);
    let marketplace_path =
        AbsolutePathBuf::try_from(marketplace_root.join(".agents/plugins/marketplace.json"))
            .expect("absolute marketplace path");
    let manager = PluginsManager::new(deepseek_home.path().to_path_buf());
    ...
    assert!(!deepseek_home.path().join(CONFIG_TOML_FILE).exists());
...
"#
    ...
    write_file(&deepseek_home.path().join(CONFIG_TOML_FILE), &user_config);
    let config =
        config_layer_stack_with_requirements(deepseek_home.path(), &user_config, &requirements);
    let outcome = manager
        .install_plugin(
            &config,
            PluginInstallRequest {
...
#[tokio::test]
async fn install_deepseek_curated_plugin_uses_short_sha_cache_version() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
    write_deepseek_curated_marketplace(&curated_root, &["slack"]);
    ...
    let installed_path = tmp.path().join(format!(
        "plugins/cache/deepseek-curated/slack/{TEST_CURATED_PLUGIN_CACHE_VERSION}"
    ));
    assert_eq!(
        result,
        PluginInstallOutcome {
...
async fn install_plugin_writes_marketplace_manifest_fallback_when_missing_plugin_json() {
  ...
  "plugins": [
    ...
    assert!(!plugin_root.join(".deepseek-plugin/plugin.json").exists());
    assert!(
        !tmp.path()
    ...
    let fallback_json: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(installed_path.join(".deepseek-plugin/plugin.json")).unwrap(),
    )
    .unwrap();
    assert_eq!(
...
async fn install_plugin_supports_git_subdir_marketplace_sources() {
  ...
  "plugins": [
    ...
    assert!(installed_path.join(".deepseek-plugin/plugin.json").is_file());
}
...
async fn read_plugin_for_config_filters_mcp_servers_for_codex_backend_auth() {
  ...
  "plugins": [
    ...
    write_file(
        &repo_root.join("sample-plugin/.deepseek-plugin/plugin.json"),
...
async fn read_plugin_for_config_does_not_fallback_from_invalid_plugin_manifest() {
  ...
  "plugins": [
    ...
    write_file(&plugin_root.join(".deepseek-plugin/plugin.json"), "{");
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
plugins = true
...
async fn read_plugin_for_config_installed_git_source_reads_from_cache_without_cloning() {
  ...
  "plugins": [
    ...
    write_file(
        &cached_plugin_root.join(".deepseek-plugin/plugin.json"),
        r#"{
  "name": "toolkit",
...
#[tokio::test]
async fn list_marketplaces_can_skip_deepseek_curated_before_loading() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
...
"#,
    ...
    let config = load_config(tmp.path(), tmp.path()).await;
    let outcome = PluginsManager::new(tmp.path().to_path_buf())
        .list_marketplaces_for_config(&config, &[], /*include_deepseek_curated*/ false)
        .unwrap();
...
"#,
  ...
  "plugins": [
    ...
    write_file(
        ...
        r#"{
  "name": "deepseek-api-curated",
  "interface": {
    "displayName": "DeepSeek Curated"
  },
  "plugins": [
    {
      "name": "api-plugin",
    ...
    assert_eq!(
        curated_marketplace,
        ConfiguredMarketplace {
            name: "deepseek-api-curated".to_string(),
            path: AbsolutePathBuf::try_from(
                curated_root.join(".agents/plugins/api_marketplace.json")
            )
            .unwrap(),
            interface: Some(MarketplaceInterface {
                display_name: Some("DeepSeek Curated".to_string()),
            }),
            plugins: vec![ConfiguredMarketplacePlugin {
                id: "api-plugin@deepseek-api-curated".to_string(),
...
async fn configured_marketplace_upgrade_invalidates_cached_tool_suggest_metadata() {
  ...
  "plugins": [
    ...
    write_file(
        &remote_repo.join("plugins/sample/.deepseek-plugin/plugin.json"),
...
"#,
    ...
    let featured_plugin_ids = manager
        .featured_plugin_ids_for_config(
            &config,
            Some(&DeepSeekAuth::create_dummy_chatgpt_auth_for_testing()),
        )
        .await
        .unwrap();
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["deepseek-plugin"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(featured_plugin_ids, vec!["deepseek-plugin".to_string()]);
}
...
"#,
    ...
    let expected = RecommendedPluginsMode::Endpoint {
        plugins: vec![
            RecommendedPlugin {
                config_id: "github@deepseek-curated-remote".to_string(),
                remote_plugin_id: "plugin_github".to_string(),
                display_name: "GitHub".to_string(),
                app_connector_ids: Vec::new(),
            },
            RecommendedPlugin {
                config_id: "slack@deepseek-curated-remote".to_string(),
...
"#,
    ...
    let disabled_tools = [ToolSuggestDisabledTool::plugin(
        "github@deepseek-curated-remote",
    )];
    ...
    assert_eq!(
        ...
        Some(vec![DiscoverableTool::from(DiscoverablePluginInfo {
            id: "slack@deepseek-curated-remote".to_string(),
...
fn refresh_curated_plugin_cache_replaces_existing_local_version_with_short_sha_version() {
    ...
    .unwrap();
    write_plugin(
        &tmp.path().join("plugins/cache/deepseek-curated"),
        "slack/local",
        "slack",
    );
    ...
    assert!(
        !tmp.path()
            .join("plugins/cache/deepseek-curated/slack/local")
            .exists()
    );
    assert!(
        tmp.path()
...
fn refresh_curated_plugin_cache_reinstalls_missing_api_curated_plugin() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
    write_deepseek_curated_marketplace(&curated_root, &[]);
    write_deepseek_api_curated_marketplace(&curated_root, &["api-only"]);
    write_curated_plugin_sha(tmp.path(), TEST_CURATED_PLUGIN_SHA);
    let plugin_id = PluginId::new(
        "api-only".to_string(),
    ...
    assert!(
        tmp.path()
            .join(format!(
                "plugins/cache/deepseek-api-curated/api-only/{TEST_CURATED_PLUGIN_CACHE_VERSION}"
            ))
            .is_dir()
    );
}
...
fn refresh_curated_plugin_cache_leaves_api_curated_plugin_when_api_manifest_missing() {
    ...
    assert!(
        tmp.path()
            .join("plugins/cache/deepseek-api-curated/api-only/local")
            .is_dir()
    );
}
...
fn refresh_curated_plugin_cache_removes_cache_for_plugin_removed_from_marketplace() {
    ...
    let plugin_cache_root = tmp
        .path()
        .join("plugins/cache/deepseek-curated/google-sheets");
    write_plugin(
        &tmp.path().join("plugins/cache/deepseek-curated"),
...
#[test]
fn curated_plugin_ids_from_config_keys_reads_latest_deepseek_home_user_config() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
[plugins."slack@deepseek-curated"]
enabled = true

[plugins."api-only@deepseek-api-curated"]
enabled = true
...
"#,
    ...
    assert_eq!(
        configured_curated_plugin_ids_from_deepseek_home(tmp.path())
            .into_iter()
            .map(|plugin_id| plugin_id.as_key())
            .collect::<Vec<_>>(),
        vec![
            "api-only@deepseek-api-curated".to_string(),
            "slack@deepseek-curated".to_string(),
        ]
    );
...
fn refresh_curated_plugin_cache_migrates_full_sha_cache_version_to_short_version() {
    ...
    assert!(
        !tmp.path()
            .join(format!(
                "plugins/cache/deepseek-curated/slack/{TEST_CURATED_PLUGIN_SHA}"
            ))
            .exists()
    );
    assert!(
...
async fn load_plugins_ignores_project_config_files() {
    let deepseek_home = TempDir::new().unwrap();
    let project_root = deepseek_home.path().join("project");
    let plugin_root = deepseek_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
