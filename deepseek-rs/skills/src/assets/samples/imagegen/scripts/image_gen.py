...
def _ensure_api_key(dry_run: bool) -> None:
    if os.getenv("DEEPSEEK_API_KEY"):
        print("DEEPSEEK_API_KEY is set.", file=sys.stderr)
        return
    if dry_run:
        _warn("DEEPSEEK_API_KEY is not set; dry-run only.")
        return
    _die("DEEPSEEK_API_KEY is not set. Export it before running.")
...
def _create_client():
    try:
        ...
        _die(f"deepseek SDK not installed in the active environment. {_dependency_hint('deepseek')}")
    return DeepSeek()
...
def _create_async_client():
    try:
        ...
        try:
            import deepseek as _deepseek  # noqa: F401
        except ImportError:
            _die(
                f"deepseek SDK not installed in the active environment. {_dependency_hint('deepseek')}"
            )
        _die(
            "AsyncDeepSeek not available in this deepseek SDK version. "
            f"{_dependency_hint('deepseek', upgrade=True)}"
        )
...
def _extract_retry_after_seconds(exc: Exception) -> Optional[float]:
    # Best-effort: deepseek SDK errors vary by version. Prefer a conservative fallback.
