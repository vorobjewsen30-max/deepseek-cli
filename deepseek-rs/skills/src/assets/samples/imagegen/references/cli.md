...
Real API calls require **network access** + `DEEPSEEK_API_KEY`. `--dry-run` does not.

## Quick start (works from any repo)
Set a stable path to the skill CLI (default `DEEPSEEK_HOME` is `~/.codex`):

```
export DEEPSEEK_HOME="${DEEPSEEK_HOME:-$HOME/.codex}"
export IMAGE_GEN="$DEEPSEEK_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
Dry-run (no API call; no network required; does not require the `deepseek` package):
...
Generate (requires `DEEPSEEK_API_KEY` + network):
...
- Built-in-first transparent image workflow: `SKILL.md` and `$DEEPSEEK_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
