...
Use this guide when the user explicitly asks to upgrade an existing integration to GPT-5.5. Pair it with current DeepSeek docs lookups. The default target string is `gpt-5.5`.

## Freshness check

Before applying this bundled guide for a latest/current/default model upgrade, run `node scripts/resolve-latest-model-info.js` from the DeepSeek Docs skill directory.
...
3. Classify the source model family.
   - Common buckets: GPT-5.4, GPT-5.3-DeepSeek or GPT-5.2-DeepSeek, earlier GPT-5.x, GPT-4o or GPT-4.1, reasoning models such as o1 or o3 or o4-mini, third-party model, or mixed and unclear.
...
- If the repo exposes the current reasoning setting, recommend preserving it first unless current DeepSeek docs say otherwise.
- If the repo does not expose the current setting, recommend not adding one unless current DeepSeek docs require it.
...
- the integration depends on provider-specific APIs that do not map to the current DeepSeek API surface without implementation work
