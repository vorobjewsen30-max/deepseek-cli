...
- the user wants DeepSeek to see the updated plugin without manually editing marketplace files
...
1. Update the plugin manifest to a single DeepSeek cachebuster suffix:
...
outside DeepSeek depends on a specific token:
...
6. Prompt the user to use a new thread to try the updated plugin, so that DeepSeek picks up new skills
   and tools.
...
Replace the existing DeepSeek cachebuster instead of appending another one. Do not keep incrementing
...
- Do not tell the user to run `codex plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by DeepSeek.
