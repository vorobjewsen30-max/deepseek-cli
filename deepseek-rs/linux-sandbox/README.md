# deepseek-linux-sandbox

This crate is responsible for producing:

- a `deepseek-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the DeepSeek CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `deepseek-exec` CLI can check if its arg0 is `deepseek-linux-sandbox` and, if so, execute as if it were `deepseek-linux-sandbox`
  - this should also be true of the `codex` multitool CLI

On Linux, DeepSeek prefers the first `bwrap` found on `PATH`
...
with DeepSeek.
DeepSeek also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. DeepSeek surfaces the same startup warning
...
it cannot create the required user namespaces, so DeepSeek rejects sandboxed shell
...
- If `bwrap` is missing, DeepSeek also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, DeepSeek surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; DeepSeek rejects sandboxed
