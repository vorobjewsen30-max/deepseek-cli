...
use deepseek_core::exec::process_exec_tool_call;
...
fn deepseek_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_deepseek-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> deepseek_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<deepseek_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let deepseek_linux_sandbox_exe = Some(deepseek_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &deepseek_protocol::exec_output::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(DeepSeekErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(DeepSeekErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<deepseek_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> deepseek_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(DeepSeekErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let deepseek_linux_sandbox_exe: Option<PathBuf> = Some(deepseek_linux_sandbox_exe());
...
async fn sandbox_blocks_curl() {
    assert_network_blocked(&["curl", "-I", "http://platform.deepseek.com"]).await;
}

#[tokio::test]
async fn sandbox_blocks_wget() {
    assert_network_blocked(&["wget", "-qO-", "http://platform.deepseek.com"]).await;
}
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = deepseek_linux_sandbox_exe()
        .parent()
...
async fn sandbox_blocks_getent() {
    assert_network_blocked(&["getent", "ahosts", "platform.deepseek.com"]).await;
}
