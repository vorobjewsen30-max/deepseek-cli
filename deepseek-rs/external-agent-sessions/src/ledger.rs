...
pub fn has_current_session_been_imported(
    ...
    load_import_ledger(deepseek_home)?.contains_current_source(source_path)
}
...
pub fn record_completed_session_imports(
    ...
    let mut ledger = load_import_ledger(deepseek_home)?;
    ...
    save_import_ledger(deepseek_home, &ledger)
}
...
pub(super) fn load_import_ledger(
    ...
    let path = import_ledger_path(deepseek_home);
...
fn import_ledger_path(deepseek_home: &Path) -> PathBuf {
    deepseek_home.join(SESSION_IMPORT_LEDGER_FILE)
}
