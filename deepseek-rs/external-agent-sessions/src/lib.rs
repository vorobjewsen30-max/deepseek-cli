...
pub fn prepare_validated_session_import(
    ...
    let has_been_imported = has_current_session_been_imported(deepseek_home, &session.path)?;
...
mod tests {
    ...
    fn skips_session_that_was_already_imported() {
        ...
        let deepseek_home = root.path().join("deepseek-home");
        let source_path = root.path().join("session.jsonl");
        std::fs::write(&source_path, "{}\n").expect("session");
        ledger::record_imported_session(&deepseek_home, &source_path, ThreadId::new())
            .expect("record import");

        let pending =
            prepare_validated_session_import(&deepseek_home, session_migration(&source_path))
