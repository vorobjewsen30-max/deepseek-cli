...
- The legacy rule matcher lives in `deepseek-execpolicy-legacy`.
...
- From the DeepSeek CLI, run `codex execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:
...
```bash
cargo run -p deepseek-execpolicy -- check --rules path/to/policy.rules git status
```
