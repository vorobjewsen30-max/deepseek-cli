...
use deepseek_code_mode::CodeModeSessionDelegate;
...
use deepseek_code_mode::FunctionCallOutputContentItem;
use deepseek_code_mode::NotificationFuture;
use deepseek_code_mode::ProcessOwnedCodeModeSessionProvider;
use deepseek_code_mode::RuntimeResponse;
use deepseek_code_mode::ToolDefinition;
use deepseek_code_mode::ToolInvocationFuture;
use deepseek_code_mode::WaitOutcome;
use deepseek_code_mode::WaitRequest;
use deepseek_code_mode::host::MAX_FRAME_BYTES;
...
async fn remote_session_persists_values_forwards_delegates_and_controls_cells() {
    let provider = ProcessOwnedCodeModeSessionProvider::with_host_program(
        deepseek_utils_cargo_bin::cargo_bin("deepseek-code-mode-host").expect("host binary"),
    );
...
async fn child_process_loss_cleans_up_and_rebuilds_the_shared_host() {
    let host_program =
        deepseek_utils_cargo_bin::cargo_bin("deepseek-code-mode-host").expect("host binary");
    let proxy_dir =
        std::env::temp_dir().join(format!("deepseek-code-mode-host-loss-{}", std::process::id()));
