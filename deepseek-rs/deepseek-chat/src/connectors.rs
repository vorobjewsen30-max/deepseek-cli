...
use deepseek_connectors::DirectoryListResponse;
use deepseek_connectors::merge::merge_connectors;
use deepseek_connectors::merge::merge_plugin_connectors;
use deepseek_core::config::Config;
pub use deepseek_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use deepseek_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use deepseek_core::connectors::list_accessible_connectors_from_mcp_tools_with_mcp_manager;
pub use deepseek_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use deepseek_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use deepseek_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
pub use deepseek_core::connectors::with_app_enabled_state;
...
async fn connector_auth(config: &Config) -> anyhow::Result<DeepSeekAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_deepseek_api_key_env*/ false).await;
    let auth = auth_manager
        .auth()
        .await
        .ok_or_else(|| anyhow::anyhow!("DeepSeek auth not available"))?;
    anyhow::ensure!(
        auth.uses_codex_backend(),
        "DeepSeek connectors require DeepSeek backend auth"
    );
    Ok(auth)
}
...
pub async fn list_cached_all_connectors(
    ...
    let connectors = deepseek_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = deepseek_connectors::list_all_connectors_with_options(
        cache_context,
...
mod tests {
    use super::*;
    use deepseek_connectors::metadata::connector_install_url;
    use deepseek_plugin::AppConnectorId;
