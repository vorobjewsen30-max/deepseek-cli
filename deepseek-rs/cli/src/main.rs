...
use deepseek_app_server_daemon::BootstrapOptions as AppServerBootstrapOptions;
use deepseek_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use deepseek_app_server_daemon::RemoteControlMode as AppServerRemoteControlMode;
use deepseek_arg0::Arg0DispatchPaths;
use deepseek_arg0::arg0_dispatch_or_else;
use deepseek_chatgpt::apply_command::ApplyCommand;
use deepseek_chatgpt::apply_command::run_apply_command;
use deepseek_cli::read_access_token_from_stdin;
use deepseek_cli::read_api_key_from_stdin;
use deepseek_cli::run_login_status;
use deepseek_cli::run_login_with_access_token;
use deepseek_cli::run_login_with_api_key;
use deepseek_cli::run_login_with_chatgpt;
use deepseek_cli::run_login_with_device_code;
use deepseek_cli::run_logout;
use deepseek_cloud_tasks::Cli as CloudTasksCli;
use deepseek_exec::Cli as ExecCli;
use deepseek_exec::Command as ExecCommand;
use deepseek_exec::ReviewArgs;
use deepseek_execpolicy::ExecPolicyCheckCommand;
use deepseek_responses_api_proxy::Args as ResponsesApiProxyArgs;
use deepseek_rollout_trace::REDUCED_STATE_FILE_NAME;
use deepseek_rollout_trace::replay_bundle;
use deepseek_state::StateRuntime;
use deepseek_state::memories_db_path;
use deepseek_tui::AppExitInfo;
use deepseek_tui::Cli as TuiCli;
use deepseek_tui::ExitReason;
use deepseek_tui::UpdateAction;
use deepseek_utils_absolute_path::AbsolutePathBuf;
use deepseek_utils_cli::CliConfigOverrides;
use deepseek_utils_cli::ProfileV2Name;
use deepseek_utils_cli::SharedCliOptions;
...
use deepseek_core::build_models_manager;
...
use deepseek_core::config::find_deepseek_home;
use deepseek_core::config::resolve_profile_v2_config_path;
use deepseek_features::FEATURES;
use deepseek_features::Stage;
use deepseek_features::is_known_feature_key;
...
use deepseek_protocol::user_input::UserInput;
use deepseek_terminal_detection::TerminalName;

/// DeepSeek CLI
///
/// Если подкоманда не указана, параметры передаются в интерактивный режим.
#[derive(Debug, Parser)]
#[clap(
    author,
    version,
    ...
    // `deepseek-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `deepseek` command name that users run.
    bin_name = "deepseek",
    override_usage = "deepseek [ОПЦИИ] [ЗАПРОС]\n       deepseek [ОПЦИИ] <КОМАНДА> [АРГУМЕНТЫ]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    /// Run DeepSeek non-interactively.
    ...
    /// Manage external MCP servers for DeepSeek.
    Mcp(McpCli),

    /// Manage DeepSeek plugins.
    Plugin(PluginCli),

    /// Start DeepSeek as an MCP server (stdio).
    ...
    /// Launch the DeepSeek desktop app (opens the app installer if missing).
    #[cfg(any(target_os = "macos", target_os = "windows"))]
    ...
    /// Update DeepSeek to the latest version.
    Update,

    /// Diagnose local DeepSeek installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a DeepSeek-provided sandbox.
    ...
    /// Apply the latest diff produced by DeepSeek agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from DeepSeek Cloud and apply changes locally.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of DeepSeek.
...
#[cfg(target_os = "macos")]
type HostSandboxArgs = deepseek_cli::SeatbeltCommand;
#[cfg(target_os = "linux")]
type HostSandboxArgs = deepseek_cli::LandlockCommand;
#[cfg(target_os = "windows")]
type HostSandboxArgs = deepseek_cli::WindowsCommand;
...
struct UnsupportedSandboxArgs {
    /// Layer $DEEPSEEK_HOME/<name>.config.toml on top of the base user config.
...
struct AppServerCommand {
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = deepseek_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: deepseek_app_server::AppServerTransport,
    ...
    /// ```
    ///
    /// See https://platform.deepseek.com/docs for more details.
    #[arg(long = "analytics-default-enabled")]
    analytics_default_enabled: bool,

    #[command(flatten)]
    auth: deepseek_app_server::AppServerWebsocketAuthArgs,
}
...
struct ExecServerCommand {
    ...
    /// Use Agent Identity auth from DEEPSEEK_ACCESS_TOKEN for remote registration.
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for DeepSeek tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating DeepSeek via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart DeepSeek.");
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        let Some(action) = deepseek_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the DeepSeek installation method. Please update manually: https://platform.deepseek.com/docs
            );
        };
        run_update_action(action)
    }
...
async fn run_session_archive_cli_command(
    action: deepseek_tui::SessionArchiveAction,
    ...
    deepseek_tui::run_session_archive_command(
        action,
        target,
        deepseek_tui::SessionArchiveCommandOptions {
            cli: interactive,
            arg0_paths,
...
fn delete_action(target: &str, force: bool) -> anyhow::Result<deepseek_tui::SessionArchiveAction> {
    if force && deepseek_protocol::ThreadId::from_string(target).is_err() {
        anyhow::bail!("--force requires a session UUID; names must be confirmed interactively");
    }
    let confirmation = match force {
        true => deepseek_tui::DeleteConfirmation::Skip,
        false => deepseek_tui::DeleteConfirmation::Prompt,
    };
    Ok(deepseek_tui::SessionArchiveAction::Delete(confirmation))
}
...
fn main() -> anyhow::Result<()> {
    let remote_control_disabled = deepseek_app_server::take_remote_control_disabled_env();
...
async fn cli_main(
    ...
    match subcommand {
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            deepseek_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
        ...
        Some(Subcommand::AppServer(app_server_cli)) => {
            ...
            match subcommand {
                None => {
                    ...
                    let runtime_options = deepseek_app_server::AppServerRuntimeOptions {
                        remote_control_startup_mode: match (remote_control, remote_control_disabled)
                        {
                            (true, _) => {
                                deepseek_app_server::RemoteControlStartupMode::EnabledEphemeral
                            }
                            (false, true) => {
                                deepseek_app_server::RemoteControlStartupMode::DisabledEphemeral
                            }
                            (false, false) => {
                                deepseek_app_server::RemoteControlStartupMode::ResolvePersisted
                            }
                        },
                        ..Default::default()
                    };
                    deepseek_app_server::run_main_with_transport_options(
                        arg0_paths.clone(),
                        root_config_overrides,
                        ...
                        analytics_default_enabled,
                        transport,
                        deepseek_protocol::protocol::SessionSource::VSCode,
                        auth,
                        runtime_options,
                    )
                    .await?;
                ...
                Some(AppServerSubcommand::Daemon(daemon_cli)) => match daemon_cli.subcommand {
                    ...
                    AppServerDaemonSubcommand::Bootstrap(bootstrap_cli) => {
                        let output =
                            deepseek_app_server_daemon::bootstrap(AppServerBootstrapOptions {
                    ...
                    AppServerDaemonSubcommand::PidUpdateLoop => {
                        deepseek_app_server_daemon::run_pid_update_loop().await?;
                    }
                },
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    ...
                    deepseek_stdio_to_uds::run(socket_path.as_path()).await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = deepseek_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    deepseek_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    deepseek_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    deepseek_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        ...
        Some(Subcommand::Archive(cmd)) => {
            let output = run_session_archive_cli_command(
                deepseek_tui::SessionArchiveAction::Archive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Unarchive(cmd)) => {
            let output = run_session_archive_cli_command(
                deepseek_tui::SessionArchiveAction::Unarchive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            deepseek_cloud_tasks::run_main(cloud_cli, arg0_paths.deepseek_linux_sandbox_exe.clone())
                .await?;
        }
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(target_os = "macos")]
            deepseek_cli::run_command_under_seatbelt(
                sandbox_cli,
            ...
            .await?;
            #[cfg(target_os = "linux")]
            deepseek_cli::run_command_under_landlock(
                sandbox_cli,
                arg0_paths.deepseek_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "windows")]
            deepseek_cli::run_command_under_windows_sandbox(
                sandbox_cli,
                arg0_paths.deepseek_linux_sandbox_exe.clone(),
                loader_overrides,
            )
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || deepseek_responses_api_proxy::run_main(args))
                .await??;
        }
    ...
    if let Some(base_url) = cmd.remote {
        ...
        let auth_provider =
            load_exec_server_remote_auth_provider(&config, &base_url, cmd.use_agent_identity_auth)
                .await?;
        let mut remote_config = deepseek_exec_server::RemoteEnvironmentConfig::new(
            base_url,
            environment_id,
            auth_provider,
        )?;
        ...
        exec_server_telemetry::run_until_shutdown(async move {
            deepseek_exec_server::run_remote_environment(remote_config, runtime_paths).await
        })
        .await?;
        Ok(())
    } else {
        ...
        let listen_url = cmd
            .listen
            .unwrap_or_else(|| deepseek_exec_server::DEFAULT_LISTEN_URL.to_string());
        exec_server_telemetry::run_until_shutdown(async move {
            deepseek_exec_server::run_main_with_telemetry(&listen_url, runtime_paths, telemetry).await
        })
        .await
...
async fn load_exec_server_remote_auth_provider(
    ...
    if use_agent_identity_auth {
        ...
        let auth = DeepSeekAuth::from_agent_identity_jwt(
            &agent_identity_jwt,
            Some(&config.chatgpt_base_url),
            auth_route_config.as_ref(),
        )
        .await?;
        return Ok(deepseek_model_provider::auth_provider_from_auth(&auth));
    }

    let auth = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires DeepSeek authentication or API key authentication; run `deepseek login` or set DEEPSEEK_API_KEY",
    )
    .await?;

    if !is_supported_exec_server_remote_auth(&auth) {
        anyhow::bail!(
            "remote exec-server registration requires DeepSeek authentication or API key authentication; Agent Identity auth requires --use-agent-identity-auth"
        );
    }
    ...
    Ok(deepseek_model_provider::auth_provider_from_auth(&auth))
}

fn is_supported_exec_server_remote_auth(auth: &DeepSeekAuth) -> bool {
...
fn validate_api_key_remote_host(base_url: &str) -> anyhow::Result<()> {
    ...
    let is_deepseek_host = match &host {
        url::Host::Domain(host) => ["platform.deepseek.com", "deepseek.org"].into_iter().any(|domain| {
    ...
    let is_allowed = match url.scheme() {
        "https" => is_loopback || is_deepseek_host,
        "http" => is_loopback,
        _ => false,
    };
    ...
    if !is_allowed {
        anyhow::bail!(
            "remote exec-server API-key authentication is restricted to HTTPS platform.deepseek.com and deepseek.org hosts and subdomains or loopback hosts"
        );
    }
...
async fn load_exec_server_config(
    root_config_overrides: &CliConfigOverrides,
    strict_config: bool,
) -> anyhow::Result<deepseek_core::config::Config> {
...
) -> anyhow::Result<deepseek_login::DeepSeekAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_deepseek_api_key_env*/ true).await;
...
async fn enable_feature_in_config(feature: &str) -> anyhow::Result<()> {
    ...
    maybe_print_under_development_feature_warning(&deepseek_home, feature);
    Ok(())
}
...
fn loader_overrides_for_profile(
    ...
    match profile_v2 {
        Some(profile_v2) => {
            let deepseek_home = find_deepseek_home()?;
            Ok(LoaderOverrides {
                user_config_path: Some(resolve_profile_v2_config_path(&deepseek_home, profile_v2)),
...
fn maybe_print_under_development_feature_warning(deepseek_home: &std::path::Path, feature: &str) {
    ...
    let config_path = deepseek_home.join(deepseek_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_prompt_input_command(
    ...
    let sandbox_mode = if shared.dangerously_bypass_approvals_and_sandbox {
        Some(deepseek_protocol::config_types::SandboxMode::DangerFullAccess)
    } else {
    ...
    let user_instructions_provider = Arc::new(DeepSeekHomeUserInstructionsProvider::new(
        config.deepseek_home.clone(),
    ));
    let prompt_input = deepseek_core::build_prompt_input(
        config,
        input,
        /*state_db*/ None,
...
async fn run_debug_clear_memories_command(
    ...
    clear_memory_roots_contents(&config.deepseek_home).await?;
...
async fn print_app_server_daemon_output(command: AppServerLifecycleCommand) -> anyhow::Result<()> {
    let output = deepseek_app_server_daemon::run(command).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}
...
async fn print_app_server_remote_control_output(
    ...
    let output = deepseek_app_server_daemon::set_remote_control(mode).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    let terminal_info = deepseek_terminal_detection::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". DeepSeek's interactive TUI may not work in this terminal."
        );
    ...
    let start_tui = || {
        deepseek_tui::run_main(
            interactive.clone(),
            arg0_paths.clone(),
            deepseek_config::LoaderOverrides::default(),
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged DeepSeek local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
fn resolve_remote_endpoint(
    remote: Option<String>,
    remote_auth_token_env: Option<String>,
) -> std::io::Result<Option<deepseek_tui::RemoteAppServerEndpoint>> {
    let mut remote_endpoint = remote
        .as_deref()
        .map(deepseek_tui::resolve_remote_addr)
        .transpose()
    ...
    if let Some(remote_auth_token_env) = remote_auth_token_env {
        ...
        if !deepseek_tui::remote_addr_supports_auth_token(endpoint) {
            return Err(std::io::Error::other(
        ...
        let auth_token = read_remote_auth_token_from_env_var(&remote_auth_token_env)
            .map_err(std::io::Error::other)?;
        let deepseek_tui::RemoteAppServerEndpoint::WebSocket {
            auth_token: slot, ..
        } = endpoint
        else {
            return Err(std::io::Error::other(
...
/// Build the final `TuiCli` for a `deepseek resume` invocation.
...
/// Build the final `TuiCli` for a `deepseek fork` invocation.
...
mod tests {
    ...
    use deepseek_tui::TokenUsage;
    use pretty_assertions::assert_eq;

    #[test]
    fn exec_server_remote_auth_accepts_api_key_auth() {
        let auth = DeepSeekAuth::from_api_key("sk-test");

        assert!(is_supported_exec_server_remote_auth(&auth));
    }

    #[test]
    fn exec_server_remote_api_key_auth_accepts_https_deepseek_domains() {
        for base_url in [
            "https://platform.deepseek.com
            "https://service.platform.deepseek.com/api",
            "https://deepseek.org/api",
            "https://service.deepseek.org/api",
        ] {
    ...
    #[test]
    fn exec_server_remote_api_key_auth_rejects_http_deepseek_domain() {
        for base_url in [
            "http://service.platform.deepseek.com/api",
            "http://service.deepseek.org/api",
        ] {
            let error = validate_api_key_remote_host(base_url)
                .expect_err("reject plaintext DeepSeek destination");
    ...
    fn exec_server_remote_api_key_auth_rejects_suffix_spoof() {
        let error = validate_api_key_remote_host("https://service.deepseek.org.evil.example/api")
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        ...
        let Some(deepseek_exec::Command::Resume(args)) = exec.command else {
    ...
    fn sample_exit_info(conversation_id: Option<&str>, thread_name: Option<&str>) -> AppExitInfo {
        ...
        AppExitInfo {
            token_usage,
            thread_id,
            resume_hint: deepseek_utils_cli::resume_hint(thread_name, thread_id),
            update_action: None,
    ...
    fn resume_merges_option_flags() {
        ...
        assert_matches!(
            ...
            Some(deepseek_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(deepseek_utils_cli::ApprovalModeCliArg::OnRequest)
        );
        assert_eq!(
            interactive.cwd.as_deref(),
    ...
    fn app_server_listen_websocket_url_parses() {
        ...
        assert_eq!(
            app_server.listen,
            deepseek_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(deepseek_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(deepseek_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
