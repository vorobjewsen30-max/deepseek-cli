...
//! interactive-session layers. Direct `deepseek login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `deepseek-login.log` artifact that
...
use deepseek_login::CLIENT_ID;
use deepseek_login::DeepSeekAuth;
use deepseek_login::ServerOptions;
use deepseek_login::login_with_access_token;
use deepseek_login::login_with_api_key;
use deepseek_login::logout_with_revoke;
use deepseek_login::run_device_code_login;
...
const CHATGPT_LOGIN_DISABLED_MESSAGE: &str =
    "Вход через DeepSeek отключён. Используйте вход по API-ключу.";
const API_KEY_LOGIN_DISABLED_MESSAGE: &str =
    "Вход по API-ключу отключён.";
const ACCESS_TOKEN_LOGIN_DISABLED_MESSAGE: &str =
    "Вход по токену доступа отключён. Используйте вход по API-ключу.";
const LOGIN_SUCCESS_MESSAGE: &str = "Вход выполнен успешно!";

/// Installs a small file-backed tracing layer for direct `deepseek login` flows.
///
...
/// command produce a durable `deepseek-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match deepseek_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("deepseek-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("deepseek_cli=info,deepseek_core=info,deepseek_login=info"));
    ...
    // Direct `deepseek login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `deepseek login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv DEEPSEEK_API_KEY | deepseek login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv DEEPSEEK_ACCESS_TOKEN | deepseek login --with-access-token`.",
...
/// `deepseek login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match DeepSeekAuth::from_auth_storage(
        &config.deepseek_home,
        config.cli_auth_credentials_store_mode,
        Some(&config.chatgpt_base_url),
        config.auth_keyring_backend_kind(),
    ...
    .await
    {
        Ok(Some(auth)) => match auth.auth_mode() {
            ...
            AuthMode::Chatgpt | AuthMode::ChatgptAuthTokens => {
                eprintln!("Logged in using DeepSeek");
                std::process::exit(0);
            }
...
mod tests {
    ...
    async fn clears_existing_auth_before_login() {
        let deepseek_home = tempdir().expect("create temporary DeepSeek home");
        login_with_api_key(
            deepseek_home.path(),
            "sk-existing",
