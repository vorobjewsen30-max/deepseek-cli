...
use deepseek_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use deepseek_core_plugins::marketplace::marketplace_root_dir;
use deepseek_core_plugins::marketplace_add::MarketplaceAddOutcome;
use deepseek_core_plugins::marketplace_add::MarketplaceAddRequest;
use deepseek_core_plugins::marketplace_add::add_marketplace;
use deepseek_core_plugins::marketplace_remove::MarketplaceRemoveOutcome;
use deepseek_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
#[derive(Debug, Parser)]
#[command(bin_name = "deepseek plugin marketplace")]
...
enum MarketplaceSubcommand {
    ...
    /// List plugin marketplaces DeepSeek is currently considering and their roots.
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "deepseek plugin marketplace add",
    after_help = "Examples:\n  deepseek plugin marketplace add ./path/to/marketplace\n  deepseek plugin marketplace add owner/repo --ref main\n  deepseek plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(bin_name = "deepseek plugin marketplace list")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "deepseek plugin marketplace upgrade",
    after_help = "Examples:\n  deepseek plugin marketplace upgrade\n  deepseek plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "deepseek plugin marketplace remove",
    after_help = "Example:\n  deepseek plugin marketplace remove debug"
)]
...
async fn run_list(overrides: Vec<(String, toml::Value)>, args: ListMarketplaceArgs) -> Result<()> {
    let config = Config::load_with_cli_overrides(overrides)
        .await
        .context("failed to load configuration")?;
    let manager = PluginsManager::new(config.deepseek_home.to_path_buf());
    ...
    if args.json {
        let marketplace_sources =
            configured_marketplace_sources_by_root(config.deepseek_home.as_path(), &plugins_input);
        let output =
...
impl JsonMarketplaceListOutput {
    fn from_marketplaces(
        marketplaces: Vec<deepseek_core_plugins::marketplace::Marketplace>,
...
fn configured_marketplace_sources_by_root(
    ...
    let marketplace_sources = configured_marketplace_sources(plugins_input, deepseek_home);
