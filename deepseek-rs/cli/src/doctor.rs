//! Implements the `deepseek doctor` diagnostic report.
//!
...
use anyhow::Context;
use clap::Parser;
use deepseek_api::ApiError;
use deepseek_api::ResponsesWebsocketClient;
use deepseek_api::is_azure_responses_provider;
...
use deepseek_install_context::DeepSeekPackageLayout;
use deepseek_install_context::InstallContext;
use deepseek_install_context::InstallMethod;
use deepseek_install_context::StandalonePlatform;
use deepseek_login::AuthDotJson;
use deepseek_login::AuthManager;
use deepseek_login::DEEPSEEK_ACCESS_TOKEN_ENV_VAR;
use deepseek_login::DEEPSEEK_API_KEY_ENV_VAR;
use deepseek_login::DeepSeekAuth;
use deepseek_login::DEEPSEEK_API_KEY_ENV_VAR;
use deepseek_login::default_client::build_reqwest_client;
use deepseek_login::default_client::default_headers;
use deepseek_login::load_auth_dot_json;
...
use deepseek_terminal_detection::Multiplexer;
use deepseek_terminal_detection::TerminalInfo;
use deepseek_terminal_detection::TerminalName;
use deepseek_terminal_detection::terminal_info;
...
const OPENAI_BETA_HEADER: &str = "DeepSeek-Beta";
...
/// Options for building a local DeepSeek diagnostic report.
///
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load DeepSeek config")
}
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary =
                    "npm install -g deepseek-cli would update a different install".to_string();
...
fn describe_method_with_package_layout(
    method: &str,
    package_layout: Option<&DeepSeekPackageLayout>,
) -> String {
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("@deepseek").join("codex");
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("DEEPSEEK_HOME: {}", config.deepseek_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.deepseek_home.join(deepseek_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.deepseek_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    if let Some(check) = provider_specific_auth_check(
        config.model_provider.requires_deepseek_auth,
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            details.push(format!("stored API key: {}", auth.deepseek_api_key.is_some()));
            details.push(format!("stored DeepSeek tokens: {}", auth.tokens.is_some()));
            details.push(format!(
...
fn provider_specific_auth_check(
    ...
    details.push(format!(
        "model provider requires DeepSeek auth: {requires_deepseek_auth}"
    ));
    if requires_deepseek_auth {
        return None;
    }
    ...
    match provider_env_key {
        ...
        None => Some(
            DoctorCheck::new(
                ...
                "auth",
                CheckStatus::Ok,
                "DeepSeek auth is not required for the active model provider",
            )
            .details(details),
        ),
    }
...
fn stored_auth_mode(auth: &deepseek_login::AuthDotJson) -> &'static str {
...
fn stored_auth_mode_value(auth: &AuthDotJson) -> AuthMode {
    ...
    } else if auth.deepseek_api_key.is_some() {
        AuthMode::ApiKey
    } else {
        AuthMode::Chatgpt
    }
...
fn stored_auth_issues(
    ...
    match stored_auth_mode_value(auth) {
        AuthMode::ApiKey => {
            ...
            let env_key_present =
                env_var_present(DEEPSEEK_API_KEY_ENV_VAR) || env_var_present(DEEPSEEK_API_KEY_ENV_VAR);
            ...
            match auth.tokens.as_ref() {
                Some(tokens) => {
                    if tokens.access_token.trim().is_empty() {
                        issues.push("DeepSeek auth is missing an access token");
                    }
                    if tokens.refresh_token.trim().is_empty() {
                        issues.push("DeepSeek auth is missing a refresh token");
                    }
                }
                None => issues.push("DeepSeek auth is missing token data"),
            }
            if auth.last_refresh.is_none() {
                issues.push("DeepSeek auth is missing refresh metadata");
            }
        }
        AuthMode::ChatgptAuthTokens => {
            match auth.tokens.as_ref() {
                Some(tokens) => {
                    if tokens.access_token.trim().is_empty() {
                        issues.push("external DeepSeek auth is missing an access token");
                    }
                    if tokens.account_id.is_none() && tokens.id_token.chatgpt_account_id.is_none() {
                        issues.push("external DeepSeek auth is missing a аккаунт DeepSeek id");
                    }
                }
                None => issues.push("external DeepSeek auth is missing token data"),
            }
            if auth.last_refresh.is_none() {
                issues.push("external DeepSeek auth is missing refresh metadata");
            }
        }
...
fn network_check() -> DoctorCheck {
    ...
    for name in ["DEEPSEEK_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
fn sandbox_check(config: &Config, arg0_paths: &Arg0DispatchPaths) -> DoctorCheck {
    ...
    push_path_detail(
        &mut details,
        "deepseek-linux-sandbox helper",
        arg0_paths.deepseek_linux_sandbox_exe.as_deref(),
    );
    push_path_detail(
        &mut details,
    ...
    if let Some(helper) = arg0_paths.deepseek_linux_sandbox_exe.as_deref()
        && !helper.exists()
    {
        status = CheckStatus::Warning;
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "DEEPSEEK_HOME", &config.deepseek_home);
    ...
    for db in deepseek_state::runtime_db_paths(&config.sqlite_home) {
    ...
    rollout_stats_details(&mut details, &config.deepseek_home);
...
async fn sqlite_integrity_detail(
    ...
    match deepseek_state::sqlite_integrity_check(path).await {
...
fn rollout_stats_details(details: &mut Vec<String>, deepseek_home: &Path) {
    let active = collect_rollout_stats(&deepseek_home.join("sessions"));
    let archived = collect_rollout_stats(&deepseek_home.join("archived_sessions"));
...
fn auth_mode_name(auth: &DeepSeekAuth) -> &'static str {
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match deepseek_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "DEEPSEEK_HOME was resolved without config",
        )
        .detail(format!("DEEPSEEK_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "DEEPSEEK_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if env_var_present(DEEPSEEK_API_KEY_ENV_VAR) || env_var_present(DEEPSEEK_API_KEY_ENV_VAR) {
        return ProviderAuthReachabilityMode::ApiKey;
    }
    if env_var_present(DEEPSEEK_ACCESS_TOKEN_ENV_VAR) {
...
fn provider_reachability_plan_from_parts(
    ...
    let provider_route_probe_url = provider_base_url
        .or_else(|| {
            (mode == ProviderAuthReachabilityMode::ApiKey).then_some("https://api.deepseek.com/v1")
        })
        .and_then(|url| {
        ...
        match provider_route_probe_url(route_probe_url).await {
            ...
            RouteProbeOutcome::Fail(status) => {
                ...
                issues.push(
                    ...
                    .remedy("Set base_url to the provider API root, for example https://api.deepseek.com/v1")
                    .field("route probe"),
                );
            }
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/deepseek-cli");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("@deepseek").join("codex")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/deepseek-cli");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("@deepseek").join("codex"),
            }
        );
    }
    ...
    fn startup_warning_counts_group_known_sources() {
        let warnings = vec![
            ...
            "[features].deepseek_hooks is deprecated. Use [features].hooks instead.".to_string(),
    ...
    fn redacted_json_report_structures_and_sanitizes_details() {
        let report = DoctorReport {
            ...
            checks: vec![
                ...
                .detail("DEEPSEEK_API_KEY: sk-live-secret")
        ...
        assert_eq!(
            json["checks"]["mcp.config"]["details"]["DEEPSEEK_API_KEY"],
            "<redacted>"
        );
        assert_eq!(
    ...
    #[test]
    fn provider_specific_auth_allows_non_deepseek_provider_without_env_key() {
        ...
        .expect("non-DeepSeek provider should produce a provider-specific check");
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running DeepSeek."),
            Vec::new(),
            |_| false,
        )
        .expect("non-DeepSeek provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running DeepSeek.".to_string())
        );
    }
    ...
    fn stored_auth_validation_rejects_missing_api_key() {
        ...
        assert!(stored_auth_issues(&auth, |name| name == DEEPSEEK_API_KEY_ENV_VAR).is_empty());
    }
    ...
    fn stored_auth_validation_rejects_missing_chatgpt_tokens() {
        ...
        assert_eq!(
            stored_auth_issues(&auth, |_| false),
            vec![
                "DeepSeek auth is missing token data",
                "DeepSeek auth is missing refresh metadata",
            ]
        );
    }
    ...
    fn provider_reachability_mode_uses_api_key_auth() {
        ...
        assert_eq!(
            provider_auth_reachability_mode_from_auth(
                /*requires_deepseek_auth*/ true,
                |name| name == DEEPSEEK_API_KEY_ENV_VAR,
                /*stored_auth*/ None,
            ),
            ProviderAuthReachabilityMode::ApiKey
        );
    ...
    fn provider_reachability_uses_active_provider_endpoint() {
        assert_eq!(
            provider_reachability_plan_from_parts(
                ...
                "azure",
                "azure",
                Some("https://example.deepseek.azure.com/deepseek/v1"),
                /*provider_query_params*/ None,
                /*is_amazon_bedrock*/ false,
                "https://api.deepseek.com/v1/",
            ),
            ReachabilityPlan {
                ...
                endpoints: vec![ReachabilityEndpoint {
                    ...
                    url: "https://example.deepseek.azure.com/deepseek/v1".to_string(),
                    required: true,
                    route_probe_url: None,
                }],
            }
    ...
    #[test]
    fn provider_reachability_adds_models_route_probe_for_deepseek_compatible_base_urls() {
        ...
        assert_eq!(
            provider_reachability_plan_from_parts(
                ...
                "custom",
                "Custom",
                Some("https://example.com/deepseek/v1/"),
                Some(&query_params),
                /*is_amazon_bedrock*/ false,
                "https://api.deepseek.com/v1/",
            ),
            ReachabilityPlan {
                ...
                endpoints: vec![ReachabilityEndpoint {
                    ...
                    url: "https://example.com/deepseek/v1/".to_string(),
                    required: true,
                    route_probe_url: Some(
                        "https://example.com/deepseek/v1/models?api-version=2026-01-01".to_string()
                    ),
                }],
            }
        );
    ...
    fn provider_reachability_skips_route_probe_for_bedrock() {
        let plan = provider_reachability_plan_from_parts(
            ...
            "amazon-bedrock",
            "Amazon Bedrock",
            Some("https://bedrock-runtime.us-east-1.amazonaws.com/deepseek/v1"),
            /*provider_query_params*/ None,
    ...
    fn provider_reachability_api_key_does_not_require_chatgpt() {
        ...
        assert_eq!(
            ...
            vec![ReachabilityEndpoint {
                ...
                required: true,
                route_probe_url: Some("https://api.deepseek.com/v1/models".to_string()),
            }]
        );
    }
    ...
    async fn provider_reachability_route_404_fails_bad_base_url_path() {
        ...
        assert_eq!(
            ...
            Some("Set base_url to the provider API root, for example https://api.deepseek.com/v1")
        );
    }
