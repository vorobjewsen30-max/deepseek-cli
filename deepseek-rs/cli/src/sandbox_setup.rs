...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run DeepSeek after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
        conflicts_with = "current_user",
        requires = "deepseek_home"
    )]
    user: Option<String>,

    /// Use the current Windows user as the DeepSeek user.
    #[arg(
        long = "current-user",
    ...
    /// DEEPSEEK_HOME for the DeepSeek user. Required with --user.
    #[arg(long = "deepseek-home", value_name = "DIR")]
...
async fn run_elevated(cmd: SandboxSetupCommand) -> anyhow::Result<()> {
    ...
    deepseek_core::windows_sandbox::run_elevated_provisioning_setup(
        identity.deepseek_home.as_path(),
        identity.real_user.as_str(),
    )?;
    ConfigEditsBuilder::new(identity.deepseek_home.as_path())
    ...
    println!(
        ...
        identity.real_user,
        identity.deepseek_home.display()
    );
    Ok(())
}
...
fn resolve_sandbox_setup_identity(
    ...
    if cmd.current_user {
        ...
        let deepseek_home = match cmd.deepseek_home.clone() {
            Some(deepseek_home) => deepseek_home,
            None => find_deepseek_home()?.to_path_buf(),
        };
    ...
    let deepseek_home = cmd
        .deepseek_home
        .clone()
        .ok_or_else(|| anyhow::anyhow!("--deepseek-home is required with --user"))?;
...
mod tests {
    ...
    #[test]
    fn requires_deepseek_home_for_managed_user() {
        let err =
