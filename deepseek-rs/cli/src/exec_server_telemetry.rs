...
const OTEL_SERVICE_NAME: &str = "deepseek-exec-server";

pub(crate) fn init(
    config: Option<&deepseek_core::config::Config>,
) -> (impl Send + Sync, deepseek_exec_server::ExecServerTelemetry) {
    let fmt_layer = tracing_subscriber::fmt::layer()
        .with_writer(std::io::stderr)
        .with_filter(stderr_env_filter());
    let otel = match config {
        Some(config) => deepseek_core::otel_init::build_provider(
            config,
    ...
    deepseek_core::otel_init::record_process_start(provider, OTEL_SERVICE_NAME);
    ...
    let telemetry = provider
        ...
        .cloned()
        .map(deepseek_exec_server::ExecServerTelemetry::new)
        .unwrap_or_default();
