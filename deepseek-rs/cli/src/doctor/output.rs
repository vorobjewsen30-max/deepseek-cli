...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run deepseek doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
fn header_suffix(report: &DoctorReport) -> String {
    let version = format!("v{}", report.deepseek_version);
    report
        .checks
        .iter()
...
fn update_note(check: &DoctorCheck, report: &DoctorReport) -> Option<DoctorNote> {
    ...
    let mut parenthetical = format!("current {}", report.deepseek_version);
...
fn auth_reachability_note(report: &DoctorReport) -> Option<DoctorNote> {
    ...
    if auth_mode_lower.contains("chatgpt") && reachability_mode_lower.contains("api key") {
        return Some(DoctorNote {
            status: DisplayStatus::Warning,
            name: "auth".to_string(),
            summary: "mixed auth signals: вход через DeepSeek plus API key env var; HTTP reachability uses API-key mode".to_string(),
        });
    }
    None
}
...
mod tests {
    ...
    fn sample_report() -> DoctorReport {
        let checks = vec![
            ...
            .detail("terminal title project value: deepseek"),
            DoctorCheck::new(
                "state.paths",
                "state",
                CheckStatus::Ok,
            ...
            .detail("DEEPSEEK_API_KEY: present")
            .remediation("Run `deepseek login`."),
            DoctorCheck::new(
                "updates.status",
                "updates",
                CheckStatus::Ok,
    ...
    fn render_human_report_includes_details_by_default_without_color() {
        ...
        let expected = format!(
            "\
DeepSeek Doctor v0.0.0

Notes
   ⚠ terminal     narrow terminal
   ✗ auth         token expired - Run `deepseek login`.
─────────────────────────────────────────────────────────────
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `deepseek login`.
-------------------------------------------------------------
...
Environment
  ...
  [!!] terminal     narrow terminal
  [ok] title        default | project deepseek
...
",
    ...
    fn render_human_report_includes_redacted_details() {
        ...
        assert!(rendered.contains("      DEEPSEEK_API_KEY           present"));
    }
    ...
    fn render_human_report_promotes_notes_without_changing_statuses() {
        ...
        assert!(rendered.contains(
            "⚠ auth         mixed auth signals: вход через DeepSeek plus API key env var; HTTP reachability uses API-key mode"
        ));
        ...
        let report = DoctorReport {
            ...
            checks: vec![
                DoctorCheck::new("config.load", "config", CheckStatus::Ok, "config loaded")
                    ...
                    .detail("model provider: deepseek")
    ...
    fn detail_value_colors_inline_statuses_and_low_signal_values() {
        let rendered = detail_value(
            "npm: no · commit unknown · integrity ok · ~/deepseek/deepseek-cli/target/debug/deepseek-cli · <redacted>",
        ...
        assert!(rendered.contains("\u{1b}[38;5;117m~/deepseek/deepseek-cli/target/debug/deepseek-cli"));
    ...
    fn redact_detail_preserves_env_var_names() {
        assert_eq!(
            redact_detail("auth env vars present: DEEPSEEK_API_KEY, DEEPSEEK_API_KEY"),
            "auth env vars present: DEEPSEEK_API_KEY, DEEPSEEK_API_KEY"
        );
    }
    ...
    fn redact_detail_preserves_secret_presence_booleans() {
        assert_eq!(
            redact_detail("stored DeepSeek tokens: true"),
            "stored DeepSeek tokens: true"
        );
        assert_eq!(
            redact_detail("stored DeepSeek tokens: false"),
            "stored DeepSeek tokens: false"
        );
    }
