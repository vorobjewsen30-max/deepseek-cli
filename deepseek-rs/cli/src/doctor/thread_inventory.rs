...
use deepseek_state::ThreadStateAuditRow;
...
async fn thread_inventory_check_for_roots(
    ...
    let scan = scan_rollout_files(deepseek_home).await;
    let state_db_path = deepseek_state::state_db_path(sqlite_home);
    ...
    let rows = match deepseek_state::read_thread_state_audit_rows(&state_db_path).await {
        Ok(rows) => rows,
        Err(err) => {
    ...
    parity_check_from_scan_and_rows(deepseek_home, scan, rows, details)
}
...
fn missing_state_db_check(scan: RolloutScan, details: Vec<String>) -> DoctorCheck {
    ...
    if !scan.files.is_empty() {
        check = check
            .issue(
                ...
                .remedy("Start DeepSeek with no state DB present so startup backfill can create it from rollout files."),
        )
            .remediation(
                "Start DeepSeek with no state DB present so startup backfill can create it from rollout files.",
            );
    }
    if !scan.scan_errors.is_empty() || !scan.malformed_names.is_empty() || scan.reached_scan_cap {
        check = check.issue(
            ...
            .remedy("Check file permissions and unexpected files under DEEPSEEK_HOME sessions."),
        );
    }
    check
}
            ...
            .filter_map(|row| {
                let expected_archived = rollout_by_key
                    ...
                    .or_else(|| {
                        row.rollout_path
                            .is_file()
                            .then(|| archived_from_rollout_path(deepseek_home, &row.rollout_path))
                            .flatten()
                    })?;
...
async fn scan_rollout_files(deepseek_home: &Path) -> RolloutScan {
    let mut scan = RolloutScan::default();
    scan_rollout_root(
        &deepseek_home.join("sessions"),
        /*archived*/ false,
        &mut scan,
    )
    .await;
    scan_rollout_root(
        &deepseek_home.join("archived_sessions"),
        /*archived*/ true,
        &mut scan,
    )
    .await;
...
async fn thread_id_from_rollout(path: &Path) -> RolloutThreadId {
    ...
    deepseek_rollout::builder_from_items(items.as_slice(), path)
...
fn archived_from_rollout_path(deepseek_home: &Path, path: &Path) -> Option<bool> {
    let key = path_key(path);
    if key.starts_with(path_key(&deepseek_home.join("archived_sessions"))) {
        return Some(true);
    }
    if key.starts_with(path_key(&deepseek_home.join("sessions"))) {
        return Some(false);
    }
    None
}
    ...
    async fn thread_inventory_check_warns_for_missing_stale_and_mismatched_rows() {
        ...
        assert!(check.issues.iter().all(|issue| {
            !issue
                .remedy
                .as_deref()
                .is_some_and(|remedy| remedy.starts_with("Restart DeepSeek"))
        }));
        assert!(
            check
                .details
    ...
    impl Fixture {
        ...
        fn write_rollout(&self, archived: bool, timestamp: &str, thread_id: &str) -> PathBuf {
            let root = if archived {
                self.deepseek_home.path().join("archived_sessions")
            } else {
                self.deepseek_home.path().join("sessions/2025/01/02")
            };
            ...
            let rollout_line = RolloutLine {
                ...
                item: RolloutItem::SessionMeta(deepseek_protocol::protocol::SessionMetaLine {
                    meta: deepseek_protocol::protocol::SessionMeta {
                        session_id: parsed_thread_id.into(),
                        id: parsed_thread_id,
                        timestamp: timestamp.to_string(),
                        cwd: self.deepseek_home.path().to_path_buf(),
        ...
        async fn insert_thread_row(&self, id: &str, rollout_path: &Path, archived: bool) {
            let state_db_path = deepseek_state::state_db_path(self.sqlite_home.path());
...
INSERT INTO threads (
    ...
    archived_at
            ...
            .bind("cli")
            .bind("test-provider")
            .bind(self.deepseek_home.path().display().to_string())
            .bind("test title")
            .bind("read-only")
            .bind("on-request")
