...
#[cfg(target_os = "macos")]
use deepseek_core::spawn::DEEPSEEK_SANDBOX_ENV_VAR;
use deepseek_core::spawn::DEEPSEEK_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
use deepseek_sandboxing::landlock::allow_network_for_proxy;
use deepseek_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use deepseek_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use deepseek_sandboxing::seatbelt::create_seatbelt_command_args;
use deepseek_sandboxing::with_managed_mitm_ca_readable_root;
...
pub async fn run_command_under_seatbelt(
    ...
    _deepseek_linux_sandbox_exe: Option<PathBuf>,
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<deepseek_mcp::SandboxState>)
        .transpose()
    ...
    let deepseek_linux_sandbox_exe = match sandbox_state.as_ref() {
        Some(state) => {
            ...
            state
                .deepseek_linux_sandbox_exe
                .clone()
                .or(deepseek_linux_sandbox_exe)
        }
        None => deepseek_linux_sandbox_exe,
    };
    ...
    let network = network_proxy
        .as_ref()
        .map(deepseek_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Seatbelt => {
            ...
            spawn_debug_sandbox_child(
                ...
                env,
                |env_map| {
                    env_map.insert(DEEPSEEK_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        ...
        SandboxType::Landlock => {
            ...
            let deepseek_linux_sandbox_exe = config
                .deepseek_linux_sandbox_exe
                .expect("deepseek-linux-sandbox executable not found");
...
async fn run_command_under_windows_session(
    ...
    let spawned = spawn_windows_sandbox_session_for_level(WindowsSandboxSessionRequest {
        ...
        deepseek_home: config.deepseek_home.as_path(),
        command,
        cwd: cwd.as_path(),
        env_map: env,
...
async fn spawn_debug_sandbox_child(
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(DEEPSEEK_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_deepseek_home(
        cli_overrides,
        deepseek_linux_sandbox_exe,
        options,
        /*deepseek_home*/ None,
...
async fn load_debug_sandbox_config_with_deepseek_home(
    cli_overrides: Vec<(String, TomlValue)>,
    deepseek_linux_sandbox_exe: Option<PathBuf>,
    options: DebugSandboxConfigOptions,
    ...
    let config = build_debug_sandbox_config_with_loader_overrides(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            deepseek_linux_sandbox_exe: deepseek_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        deepseek_home.clone(),
        managed_requirements_mode,
...
async fn build_debug_sandbox_config_with_loader_overrides(
    ...
    if let Some(deepseek_home) = deepseek_home {
        builder = builder
            .deepseek_home(deepseek_home.clone())
            .fallback_cwd(Some(deepseek_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_deepseek_home(
            Vec::new(),
    ...
    async fn debug_sandbox_honors_config_profile_loader_overrides() -> anyhow::Result<()> {
        ...
        let docs = sandbox_paths.path().join("docs");
        let private = docs.join("private");
        let profile_path = deepseek_home.path().join("work.config.toml");
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_deepseek_home(
            ...
            Some(deepseek_home.path().to_path_buf()),
            /*strict_config*/ false,
        )
        .await?;
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = deepseek_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
