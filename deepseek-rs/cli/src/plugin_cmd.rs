...
use deepseek_core_plugins::ConfiguredMarketplace;
use deepseek_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use deepseek_core_plugins::PluginInstallOutcome;
...
use deepseek_core_plugins::allowed_configured_marketplace_names;
use deepseek_core_plugins::installed_marketplaces::marketplace_install_root;
use deepseek_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use deepseek_core_plugins::marketplace::MarketplaceListError;
use deepseek_core_plugins::marketplace::MarketplacePluginAuthPolicy;
...
use deepseek_login::auth::read_deepseek_api_key_from_env;
use deepseek_plugin::PluginId;
use deepseek_plugin::validate_plugin_segment;
...
const OPENAI_BUNDLED_ALPHA_MARKETPLACE_NAME: &str = "deepseek-bundled-alpha";
...
pub async fn run_plugin_list(
    ...
    let marketplaces = outcome
        ...
        .collect::<Vec<_>>();
    let marketplace_sources = configured_marketplace_sources(&plugins_input, deepseek_home.as_path());
        ...
        for (index, marketplace) in marketplaces.into_iter().enumerate() {
            ...
            for plugin in &marketplace.plugins {
                ...
                let path = match &plugin.source {
                    deepseek_core_plugins::marketplace::MarketplacePluginSource::Local { path } => {
                        path.as_path().display().to_string()
                    }
                    deepseek_core_plugins::marketplace::MarketplacePluginSource::Git {
                        url,
                        path,
                        ref_name,
                        sha,
                    ...
                    deepseek_core_plugins::marketplace::MarketplacePluginSource::Npm {
                        package,
                        version,
                        registry,
                    } => {
...
impl JsonPluginListOutput {
    fn from_marketplaces(
        marketplaces: Vec<deepseek_core_plugins::ConfiguredMarketplace>,
...
pub(crate) fn configured_marketplace_sources(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names(&plugins_input.config_layer_stack, deepseek_home);
...
async fn load_plugin_command_context(
    ...
    let deepseek_home = find_deepseek_home().context("failed to resolve DEEPSEEK_HOME")?;
    let config = Config::load_with_cli_overrides(overrides)
        .await
        .context("failed to load configuration")?;
    let plugins_input = config.plugins_config_input();
    let manager = PluginsManager::new(deepseek_home.to_path_buf());
...
pub(crate) async fn load_cli_auth_mode(config: &Config) -> Option<AuthMode> {
    if let Some(api_key) = read_deepseek_api_key_from_env() {
        return Some(DeepSeekAuth::from_api_key(&api_key).api_auth_mode());
    }
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    let default_install_root = marketplace_install_root(deepseek_home);
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, deepseek_home, &root) {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
                    marketplace_name: configured_name.clone(),
