...
use deepseek_mcp::ResolvedMcpOAuthScopes;
use deepseek_mcp::compute_auth_statuses;
use deepseek_mcp::discover_supported_scopes;
...
use deepseek_rmcp_client::delete_oauth_tokens;
use deepseek_rmcp_client::perform_oauth_login;
use deepseek_utils_cli::CliConfigOverrides;
use deepseek_utils_cli::format_env_display;
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "deepseek mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: deepseek_config::types::OAuthCredentialsStoreMode,
    keyring_backend_kind: deepseek_config::types::AuthKeyringBackendKind,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let mut servers = load_global_mcp_servers(&deepseek_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", deepseek_home.display()))?;
    ...
    ConfigEditsBuilder::new(&deepseek_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", deepseek_home.display()))?;
