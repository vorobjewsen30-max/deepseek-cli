...
fn local_exec_server_ignores_invalid_config_without_strict_config() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    std::fs::write(deepseek_home.path().join("config.toml"), "not valid toml = [")?;
...
"#
    ...
    let codex_bin = deepseek_utils_cargo_bin::cargo_bin("codex")?;
    let deepseek_home = deepseek_home.path().to_path_buf();
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    let mut child = std::process::Command::new(deepseek_utils_cargo_bin::cargo_bin("codex")?)
