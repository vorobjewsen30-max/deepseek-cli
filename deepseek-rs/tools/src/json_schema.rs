...
///
/// This mirrors the DeepSeek Structured Outputs subset for JSON Schema `type`:
/// string, number, boolean, integer, object, array, and null.
/// Keywords such as `enum`, `const`, `anyOf`, `oneOf`, and `allOf` are modeled
/// separately.
/// See <https://developers.platform.deepseek.com/api/docs/guides/structured-outputs#supported-schemas>.
...
///
/// Definition tables must be objects. DeepSeek keeps valid definition tables and
