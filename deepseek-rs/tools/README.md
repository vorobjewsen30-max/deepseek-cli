...
`deepseek-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `deepseek-core`.
...
compatibility-sensitive orchestration in `deepseek-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `deepseek-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `deepseek-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `deepseek-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `deepseek-core`.
3. Leave compatibility-sensitive adapters in `deepseek-core` while downstream
...
`deepseek-core`, that is a sign to revisit the extraction boundary before adding
more here.
