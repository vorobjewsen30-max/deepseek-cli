...
use deepseek_app_server_client::EnvironmentManager;
use deepseek_app_server_client::ExecServerRuntimePaths;
use deepseek_app_server_client::InProcessAppServerClient;
use deepseek_app_server_client::InProcessClientStartArgs;
use deepseek_app_server_client::InProcessServerEvent;
...
use deepseek_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use deepseek_config::ConfigLoadOptions;
use deepseek_config::LoaderOverrides;
use deepseek_config::format_config_error_with_source;
use deepseek_core::StateDbHandle;
use deepseek_core::check_execpolicy_for_warnings;
use deepseek_core::config::Config;
use deepseek_core::config::ConfigBuilder;
use deepseek_core::config::ConfigOverrides;
use deepseek_core::config::ConfigTomlLoadResult;
use deepseek_core::config::find_deepseek_home;
use deepseek_core::config::load_config_toml_with_layer_stack;
use deepseek_core::config::resolve_bootstrap_auth_keyring_backend_kind;
use deepseek_core::config::resolve_bootstrap_auth_route_config;
use deepseek_core::config::resolve_oss_provider;
use deepseek_core::config::resolve_profile_v2_config_path;
use deepseek_core::find_thread_meta_by_name_str;
use deepseek_core::format_exec_policy_error_with_source;
...
use deepseek_login::default_client::set_default_originator;
...
use deepseek_otel::set_parent_from_context;
use deepseek_otel::traceparent_context_from_env;
...
use deepseek_protocol::protocol::ReviewTarget;
...
pub use event_processor_with_jsonl_output::DeepSeekStatus;
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    if let Err(err) = set_default_originator("deepseek_exec".to_string()) {
    ...
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&deepseek_home, profile_v2));
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        deepseek_core::otel_init::build_provider(
            &config,
    ...
    deepseek_core::otel_init::record_process_start(otel.as_ref(), "deepseek_exec");
    deepseek_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "deepseek_exec");
    ...
    let state_db = deepseek_core::init_state_db(&config).await;
    let environment_manager = if run_loader_overrides.ignore_user_config {
        ...
        EnvironmentManager::from_deepseek_home(config.deepseek_home.clone(), Some(local_runtime_paths))
            .await?
    };
    let in_process_start_args = InProcessClientStartArgs {
        ...
        client_name: "deepseek_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = deepseek_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what DeepSeek
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            deepseek_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("DeepSeek initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        deepseek_app_server_protocol::TurnStatus::Failed
                            | deepseek_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
                ...
                if should_process_notification(
                    ...
                    match event_processor.process_server_notification(notification) {
                        DeepSeekStatus::Running => {}
                        DeepSeekStatus::InitiateShutdown => {
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<deepseek_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<deepseek_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.deepseek_home, session_id, Some(state_db.as_ref()))
                .await?
