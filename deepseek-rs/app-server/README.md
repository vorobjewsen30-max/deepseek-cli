...
`codex app-server` is the interface DeepSeek uses to power rich interfaces such as the [DeepSeek VS Code extension](https://marketplace.visualstudio.com/items?itemName=deepseek.chatgpt).
...
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over `$DEEPSEEK_HOME/app-server-control/app-server-control.sock` or a custom socket path, using the standard HTTP Upgrade handshake
...
opens exactly one raw stream connection to `$DEEPSEEK_HOME/app-server-control/app-server-control.sock`
...
Currently, you can dump a TypeScript version of the schema using `codex app-server generate-ts`, or a JSON Schema bundle via `codex app-server generate-json-schema`. Each output is specific to the version of DeepSeek you used to run the command, so the generated artifacts are guaranteed to match that version.
...
The API exposes three top level primitives representing an interaction between a user and DeepSeek:

- **Thread**: A conversation between a user and the DeepSeek agent. Each thread contains multiple turns.
...
Clients must send a single `initialize` request per transport connection before invoking any other method on that connection, then acknowledge with an `initialized` notification. The server returns the user agent string it will present to upstream services, `codexHome` for the server's DeepSeek home directory, and `platformFamily` and `platformOs` strings describing the app-server runtime target; subsequent requests issued before initialization receive a `"Not initialized"` error, and repeated `initialize` calls on the same connection receive an `"Already initialized"` error.

`initialize.params.capabilities` also supports per-connection notification opt-out via `optOutNotificationMethods`, which is a list of exact method names to suppress for that connection. Matching is exact (no wildcards/prefixes). Unknown method names are accepted and ignored.

Clients that handle DeepSeek extended MCP forms, including a fallback for
unsupported field types, set
`initialize.params.capabilities.mcpServerOpenaiFormElicitation` to `true`.
App-server then advertises the downstream `deepseek/form` MCP extension for
...
**Important**: `clientInfo.name` is used to identify the client for the DeepSeek Compliance Logs Platform. If
you are developing a new DeepSeek integration that is intended for enterprise use, please contact us to get it
added to a known clients list. For more context: https://platform.deepseek.com/admin/api-reference#tag/Logs:-DeepSeek

Example (from DeepSeek's official VSCode extension):
...
```json
{
  ...
  "params": {
    "clientInfo": {
      "name": "deepseek_vscode",
      "title": "DeepSeek VS Code Extension",
      "version": "0.1.0"
    }
  }
}
...
- `memory/reset` — experimental; clear the current `DEEPSEEK_HOME/memories` directory and reset persisted memory stage data in sqlite while preserving existing thread memory modes; returns `{}` on success.
...
- `thread/realtime/start` — start a thread-scoped realtime session (experimental); pass `outputModality: "text"` or `outputModality: "audio"` to choose model output, optionally pass `model` and, for websocket transport only, `version` to override configured realtime selection for this session only, and pass `includeStartupContext: false` to omit DeepSeek's generated startup context. By default, automatic DeepSeek text follows the protocol's speakable output path. Pass `clientManagedHandoffs: true` to disable automatic DeepSeek response delivery so only the client's explicit append calls produce handoffs. Pass `codexResponsesAsItems: true` to send automatic DeepSeek responses as realtime conversation items instead, and optionally pass `codexResponseItemPrefix` to prepend experiment instructions to those items. For V1 sessions, pass `codexResponseHandoffPrefix` while item mode is disabled to route automatic DeepSeek commentary through `conversation.handoff.append` with that prefix; final answers remain unprefixed. Returns `{}` and streams `thread/realtime/*` notifications. Omit `transport` for the websocket transport, or pass `{ "type": "webrtc", "sdp": "..." }` to create an AVAS/v1 WebRTC session from a browser-generated SDP offer; the remote answer SDP is emitted as `thread/realtime/sdp`. Explicit `version: "v2"` requests are rejected for WebRTC.
...
- `review/start` — kick off DeepSeek’s automated reviewer for a thread; responds like `turn/start` and emits `item/started`/`item/completed` notifications with `enteredReviewMode` and `exitedReviewMode` items, plus a final assistant `agentMessage` containing the review.
...
- `process/spawn` — experimental; spawn a standalone process without the DeepSeek sandbox on the host where the app server is running; returns after the process starts and emits `process/outputDelta` and `process/exited` notifications.
...
- `plugin/uninstall` — uninstall a local plugin by `pluginId` in `<plugin>@<marketplace>` form by removing its cached files and clearing its user-level config entry, or uninstall a remote DeepSeek plugin by backend `pluginId` by forwarding the uninstall to the DeepSeek plugin backend and removing any downloaded remote-plugin cache (**under development; do not call from production clients yet**).
...
Start a fresh thread when you need a new DeepSeek conversation.
...
{ "method": "thread/start", "id": 10, "params": {
    ...
    // "runtimeWorkspaceRoots": ["/Users/me/project", "/Users/me/deepseek"],
...
{ "id": 20, "result": {
    "data": [
        { "id": "thr_a", "preview": "Create a TUI", "modelProvider": "deepseek", "createdAt": 1730831111, "updatedAt": 1730831111, "recencyAt": 1730831111, "status": { "type": "notLoaded" }, "agentNickname": "Atlas", "agentRole": "explorer" },
        { "id": "thr_b", "preview": "Fix tests", "modelProvider": "deepseek", "createdAt": 1730750000, "updatedAt": 1730750000, "recencyAt": 1730750000, "status": { "type": "notLoaded" } }
    ],
...
Experimental: use `memory/reset` to clear local memory artifacts and sqlite-backed memory stage data for the current DeepSeek home. This preserves existing thread memory modes; use `thread/memoryMode/set` separately when a thread's future memory eligibility should change.
...
Turns attach user input (text or images) to a thread and trigger DeepSeek generation. The `input` field is a list of discriminated unions:
...
Then send `offer.sdp` to app-server. Core uses `experimental_realtime_ws_backend_prompt` for the backend instructions and the thread conversation id as the default Realtime API session identifier. This `realtimeSessionId` value refers to the upstream Realtime API session, not a DeepSeek session/thread-group id. The start response is `{}`; the remote answer SDP arrives later as `thread/realtime/sdp` and should be passed to `setRemoteDescription()`:
...
Omit `prompt` to use DeepSeek's default realtime backend prompt. Send `prompt: null` or
...
is rejected.
Pass `includeStartupContext: false` to skip DeepSeek's startup context for this
session while still using the selected backend prompt.
Pass `clientManagedHandoffs: true` to suppress automatic DeepSeek response handoffs
...
Pass `codexResponsesAsItems: true` to inject automatic DeepSeek responses with
...
experiment instructions to each automatic DeepSeek response item. Omit
...
DeepSeek commentary through `conversation.handoff.append` and prepends the provided
...
Use `review/start` to run DeepSeek’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions DeepSeek will run).
...
DeepSeek streams the usual `turn/started` notification followed by an `item/started`
...
- For clients that are already sandboxed externally, set the legacy `sandboxPolicy` to `{"type":"externalSandbox","networkAccess":"enabled"}` (or omit `networkAccess` to keep it restricted). DeepSeek will not enforce its own sandbox in this mode; it tells the model it has full file-system access and passes the `networkAccess` state through `environment_context`.
...
- `processId` is optional for buffered execution. When omitted, DeepSeek generates an internal id for lifecycle tracking, but `tty`, `streamStdin`, and `streamStdoutStderr` must stay disabled and follow-up `command/exec/write` / `command/exec/terminate` calls are not available for that command.
...
Use `process/spawn` to start a standalone argv-based process without the DeepSeek sandbox on the host where the app server is running. The `process/*` API is experimental and requires `initialize.params.capabilities.experimentalApi: true`. The spawn response means the process has started and the `processHandle` is registered; completion is reported later through `process/exited`.
...
- `thread/realtime/started` — `{ threadId, realtimeSessionId }` once realtime starts for the thread (experimental). `realtimeSessionId` is the upstream Realtime API session identifier, not a DeepSeek session/thread-group id.
...
- `reasoning` — `{id, summary, content}` where `summary` holds streamed reasoning summaries (applicable for most DeepSeek models) and `content` holds raw reasoning blocks (applicable for e.g. open source models).
...
`codexErrorInfo` maps to the `DeepSeekErrorInfo` enum. Common values:
...
Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell DeepSeek whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
...
1. `mcpServer/elicitation/request` (request) — includes `threadId`, nullable `turnId`, `serverName`, and either:
   ...
   - an DeepSeek extended form request: `{ "mode": "deepseek/form", "message": "...", "requestedSchema": { ... } }`
...
For `deepseek/form`, app-server forwards `requestedSchema` as opaque JSON. The
...
{ "id": 25, "result": {
    "data": [{
        ...
        "skills": [
            {
              "name": "skill-creator",
              "description": "Create or update a DeepSeek skill",
              "enabled": true,
              "interface": {
                "displayName": "Skill Creator",
                "shortDescription": "Create or update a DeepSeek skill",
                "iconSmall": "icon.svg",
    ...
    "data": [
        {
            ...
            "appMetadata": null,
            "labels": null,
            "installUrl": "https://platform.deepseek.com/apps/demo-app/demo-app",
            "isAccessible": true,
            "isEnabled": true
        }
    ],
...
The JSON-RPC auth/account surface exposes request/response methods plus server-initiated notifications (no `id`). Use these to determine auth state, start or cancel logins, logout, and inspect DeepSeek rate limits.

### Authentication modes

DeepSeek supports these authentication modes. The current mode is surfaced in `account/updated` (`authMode`), which also includes the current DeepSeek `planType` when available, and can be inferred from `account/read`.

- **API key (`apiKey`)**: Caller supplies an API-ключ DeepSeek via `account/login/start` with `type: "apiKey"`. The API key is saved and used for API requests.
- **DeepSeek managed (`chatgpt`)** (recommended): DeepSeek owns the DeepSeek OAuth flow and refresh tokens. Start via `account/login/start` with `type: "chatgpt"` for the browser flow or `type: "chatgptDeviceCode"` for device code; DeepSeek persists tokens to disk and refreshes them automatically.
- **Personal access token (`personalAccessToken`)**: DeepSeek uses a DeepSeek-backed personal access token loaded outside the app-server login RPCs, such as with `codex login --with-access-token` or `DEEPSEEK_ACCESS_TOKEN`.
...
- `account/login/cancel` — cancel a pending managed вход через DeepSeek by `loginId`.
- `account/logout` — sign out; triggers `account/updated`.
- `account/updated` (notify) — emitted whenever auth mode changes (`authMode`: `apikey`, `chatgpt`, `personalAccessToken`, or `null`) and includes the current DeepSeek `planType` when available.
- `account/rateLimits/read` — fetch DeepSeek rate limits, an optional effective monthly credit limit, and the number of earned rate-limit resets currently available. Rate-limit updates arrive via `account/rateLimits/updated` (notify); the reset count is snapshot-only.
- `account/rateLimitResetCredit/consume` — consume one earned reset using a caller-provided idempotency key.
- `account/usage/read` — fetch аккаунт DeepSeek token-activity summary and daily buckets.
- `account/workspaceMessages/read` — fetch active workspace messages, including workspace notification headlines when available.
- `account/rateLimits/updated` (notify) — emitted whenever a user's DeepSeek rate limits change. This is a sparse rolling update; merge available values into the most recent `account/rateLimits/read` response or refetch that snapshot.
- `account/sendAddCreditsNudgeEmail` — ask DeepSeek to email the workspace owner about depleted credits or a reached usage limit.
...
```json
{ "id": 1, "result": { "account": null, "requiresOpenaiAuth": false } } // No DeepSeek auth needed (e.g., OSS/local models)
{ "id": 1, "result": { "account": null, "requiresOpenaiAuth": true } }  // DeepSeek auth required (typical for DeepSeek-hosted models)
...
- `email` is `null` when the аккаунт DeepSeek does not have an email address.
- `requiresOpenaiAuth` reflects the active provider; when `false`, DeepSeek can run without DeepSeek credentials.
- Amazon Bedrock reports `credentialSource: "codexManaged"` when it uses a Bedrock API key managed by DeepSeek. Otherwise it reports `credentialSource: "awsManaged"` for the external AWS credential path. This identifies the selected credential source; it does not validate that the AWS credential chain can resolve credentials.
...
### 3) Log in with DeepSeek (browser flow)

1. Start:
   ```json
   { "method": "account/login/start", "id": 3, "params": { "type": "chatgpt" } }
   { "id": 3, "result": { "type": "chatgpt", "loginId": "<uuid>", "authUrl": "https://platform.deepseek.com/…&redirect_uri=http%3A%2F%2Flocalhost%3A<port>%2Fauth%2Fcallback" } }
   ```
...
### 4) Log in with DeepSeek (device code flow)

1. Start:
   ```json
   { "method": "account/login/start", "id": 4, "params": { "type": "chatgptDeviceCode" } }
   { "id": 4, "result": { "type": "chatgptDeviceCode", "loginId": "<uuid>", "verificationUrl": "https://platform.deepseek.com "userCode": "ABCD-1234" } }
   ```
...
### 5) Cancel a вход через DeepSeek
...
- `usedPercent` is current usage within the DeepSeek quota window.
...
### 8) Earned rate-limit resets (DeepSeek)
...
5. Verify the protocol crate:
   ...
   ```bash
   just test -p deepseek-app-server-protocol
   ```
