...
fn standalone_app_server_emits_json_info_events() -> Result<()> {
    ...
    let event = app_server_json_shutdown_event("deepseek-app-server", &[], deepseek_home.path())?;
...
async fn app_server_emits_structured_tool_call_timing_event() -> Result<()> {
    ...
    let mut app_server = TestAppServer::new_with_auto_env_and_json_logging(
        deepseek_home.path(),
        "warn,deepseek_core::tools::parallel=info",
    )
    .await?;
    ...
    assert_eq!(
        ...
        json!({
            ...
            "target": "deepseek_core::tools::parallel",
        })
    );
