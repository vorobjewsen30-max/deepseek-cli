...
fn create_config_toml_custom_provider(
    ...
    let requires_line = if requires_deepseek_auth {
...
fn create_config_toml(deepseek_home: &Path) -> std::io::Result<()> {
    let config_toml = deepseek_home.join("config.toml");
    std::fs::write(
        config_toml,
        r#"
...
fn create_config_toml_forced_login(deepseek_home: &Path, forced_method: &str) -> std::io::Result<()> {
    let config_toml = deepseek_home.join("config.toml");
    let contents = format!(
        r#"
model = "mock-model"
...
async fn get_auth_status_no_auth() -> Result<()> {
    ...
    create_config_toml(deepseek_home.path())?;
...
async fn personal_access_token_without_email_supports_auth_status_and_account_read() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_with_env(
        ...
        &[
            ...
            ("DEEPSEEK_ACCESS_TOKEN", Some("at-test-token")),
...
async fn get_auth_status_with_api_key_when_auth_not_required() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    create_config_toml_custom_provider(deepseek_home.path(), /*requires_deepseek_auth*/ false)?;
    ...
    assert_eq!(status.auth_token, None, "expected no token");
    assert_eq!(
        status.requires_deepseek_auth,
        Some(false),
        "requires_deepseek_auth should be false",
    );
    Ok(())
}
...
async fn login_api_key_rejected_when_forced_chatgpt() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    create_config_toml_forced_login(deepseek_home.path(), "chatgpt")?;
