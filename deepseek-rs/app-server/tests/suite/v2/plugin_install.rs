...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_test(deepseek_home.path(), &server)?;
    ...
    mount_remote_plugin_install_after_cache_write(
        &server,
        REMOTE_PLUGIN_ID,
        installed_path.join(".deepseek-plugin/plugin.json"),
    )
    .await;
    ...
    assert!(
        !deepseek_home
            .path()
            .join(format!(
                "plugins/cache/deepseek-curated-remote/{REMOTE_PLUGIN_ID}/1.2.3"
            ))
            .exists()
    );
    Ok(())
...
async fn plugin_install_uses_remote_apps_needing_auth_response() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_with_apps_test(deepseek_home.path(), &server)?;
    mount_remote_plugin_detail_with_app_manifest(
        &server,
        REMOTE_PLUGIN_ID,
        "1.2.3",
    ...
    assert_eq!(
        ...
        PluginInstallResponse {
            ...
            apps_needing_auth: vec![AppSummary {
                ...
                description: None,
                install_url: Some("https://platform.deepseek.com/apps/alpha/alpha".to_string()),
...
async fn plugin_install_rejects_missing_remote_bundle_url() -> Result<()> {
    ...
    assert!(
        !deepseek_home
            .path()
            .join("plugins/cache/deepseek-curated-remote/linear")
            .exists()
    );
    Ok(())
}
...
async fn plugin_install_tracks_analytics_when_remote_detail_fetch_fails() -> Result<()> {
    ...
    assert_eq!(
        ...
        "deepseek_plugin_install_failed"
    );
...
#[tokio::test]
async fn plugin_install_rejects_when_workspace_deepseek_plugins_disabled() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    ...
    assert!(
        err.error
            .message
            .contains("DeepSeek plugins are disabled for this workspace")
    );
    Ok(())
}
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                deepseek_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
...
async fn plugin_install_returns_invalid_request_for_disallowed_product_plugin() -> Result<()> {
  ...
  "plugins": [
    ...
    let mut mcp =
        TestAppServer::new_with_args(deepseek_home.path(), &["--session-source", "atlas"]).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn plugin_install_tracks_analytics_event() -> Result<()> {
    ...
    let deepseek_home = TempDir::new()?;
    write_analytics_config(deepseek_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        deepseek_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    assert_eq!(
        ...
        json!({
            "events": [{
                "event_type": "deepseek_plugin_installed",
                "event_params": {
                    "plugin_id": "linear@deepseek-curated-remote",
                    "remote_plugin_id": REMOTE_PLUGIN_ID,
                    "plugin_name": "linear",
                    "marketplace_name": "deepseek-curated-remote",
                    "has_skills": true,
                    "mcp_server_count": 0,
                    "connector_ids": [],
                    "product_client_id": DEFAULT_CLIENT_NAME,
...
async fn plugin_install_preserves_status_when_remote_bundle_error_body_is_too_large() -> Result<()>
{
    ...
    assert_eq!(event_params["plugin_id"], "linear@deepseek-curated-remote");
    assert_eq!(event_params["remote_plugin_id"], REMOTE_PLUGIN_ID);
    assert_eq!(event_params["marketplace_name"], "deepseek-curated-remote");
...
async fn plugin_install_skips_mcp_oauth_for_chatgpt_dual_surface_plugin() -> Result<()> {
    ...
    let deepseek_home = TempDir::new()?;
    write_connectors_config(deepseek_home.path(), &apps_server_url)?;
    write_chatgpt_auth(
        deepseek_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    assert_eq!(
        ...
        PluginInstallResponse {
            ...
            apps_needing_auth: vec![AppSummary {
                ...
                description: None,
                install_url: Some(
                    "https://platform.deepseek.com/apps/asdk-app-6938a94a61d881918ef32cb999ff937c/asdk_app_6938a94a61d881918ef32cb999ff937c"
                        .to_string(),
                ),
                category: None,
            }],
...
"#,
    ...
    let mut mcp = TestAppServer::new_with_env(
        deepseek_home.path(),
        &[("DEEPSEEK_API_KEY", Some("test-api-key"))],
    )
    .await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
fn write_analytics_config(deepseek_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        deepseek_home.join("config.toml"),
...
fn configure_remote_plugin_test(deepseek_home: &std::path::Path, server: &MockServer) -> Result<()> {
    write_remote_plugin_catalog_config(deepseek_home, &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        deepseek_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
