...
fn external_agent_home(deepseek_home: &Path) -> PathBuf {
    deepseek_home.join(concat!(".", "cl", "aude"))
}
...
-> Result<()> {
    ...
    let home_dir = deepseek_home.path().display().to_string();
    ...
    let state_db =
        deepseek_state::StateRuntime::init(sqlite_home.path().to_path_buf(), "mock_provider".into())
            .await?;
...
async fn external_agent_config_import_reports_failed_sync_import_in_completion() -> Result<()> {
    ...
    let source_home = external_agent_home(deepseek_home.path());
    std::fs::create_dir_all(&source_home)?;
    std::fs::write(
        source_home.join("settings.json"),
        r#"{"env":{"FOO":"bar"}}"#,
    )?;
    std::fs::write(deepseek_home.path().join("config.toml"), "invalid = [")?;
    let home_dir = deepseek_home.path().display().to_string();
    let analytics_capture_file = deepseek_home.path().join("analytics-events.jsonl");
...
async fn external_agent_config_import_completed_tracks_analytics_event() -> Result<()> {
    ...
    let missing_session_path =
        external_agent_home(deepseek_home.path()).join("projects/repo/missing.jsonl");
    let project_root = deepseek_home.path().join("repo");
    let home_dir = deepseek_home.path().display().to_string();
    let mut mcp =
        TestAppServer::new_with_env(deepseek_home.path(), &[("HOME", Some(home_dir.as_str()))])
            .await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn external_agent_config_import_creates_session_rollouts() -> Result<()> {
    ...
    let session_dir = external_agent_home(deepseek_home.path()).join("projects/repo");
...
async fn external_agent_config_import_does_not_initialize_required_mcp() -> Result<()> {
    ...
    let deepseek_home = TempDir::new()?;
    create_config_toml(deepseek_home.path(), &server.uri())?;
    let mut config = std::fs::read_to_string(deepseek_home.path().join("config.toml"))?;
    config.push_str(
        r#"
...
"#,
    ...
    std::fs::write(deepseek_home.path().join("config.toml"), config)?;
    let project_root = deepseek_home.path().join("repo");
    let recent_timestamp = chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, true);
    let session_dir = external_agent_home(deepseek_home.path()).join("projects/repo");
    let session_path = session_dir.join("session.jsonl");
