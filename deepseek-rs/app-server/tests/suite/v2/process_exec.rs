...
async fn process_spawn_returns_before_exit_and_emits_exit_notification() -> Result<()> {
    ...
    let (_server, mut mcp) = initialized_mcp(deepseek_home.path()).await?;

    let process_handle = "one-shot-1".to_string();
    let probe_file = deepseek_home.path().join("process-created");
    let release_file = deepseek_home.path().join("process-release");
    ...
    let spawn_request_id = mcp
        .send_process_spawn_request(ProcessSpawnParams {
            ...
            ..process_spawn_params(process_handle.clone(), deepseek_home.path(), command)?
        })
        .await?;
...
async fn initialized_mcp(deepseek_home: &Path) -> Result<(MockServer, TestAppServer)> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    create_config_toml(deepseek_home, &server.uri(), "never")?;
