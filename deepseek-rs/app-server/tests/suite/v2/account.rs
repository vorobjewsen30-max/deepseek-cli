...
use deepseek_app_server_protocol::ChatgptAuthTokensRefreshReason;
use deepseek_app_server_protocol::ChatgptAuthTokensRefreshResponse;
...
use deepseek_login::CLIENT_ID_OVERRIDE_ENV_VAR;
use deepseek_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
use deepseek_login::login_with_api_key;
use deepseek_login::login_with_bedrock_api_key;
...
fn create_config_toml(deepseek_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
    ...
    let requires_line = match params.requires_deepseek_auth {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(deepseek_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(deepseek_home.path().join("auth.json").exists());

    let mut mcp =
        TestAppServer::new_with_env(deepseek_home.path(), &[("DEEPSEEK_API_KEY", None)]).await?;
...
async fn set_auth_token_updates_account_and_notifies() -> Result<()> {
    ...
    create_config_toml(
        ...
        CreateConfigTomlParams {
            requires_deepseek_auth: Some(true),
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(deepseek_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<deepseek_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(deepseek_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            client_user_message_id: None,
            input: vec![deepseek_app_server_protocol::UserInput::Text {
...
async fn login_account_api_key_rejected_when_forced_chatgpt() -> Result<()> {
    ...
    assert_eq!(
        err.error.message,
        "API key login is disabled. Use вход через DeepSeek instead."
    );
    Ok(())
}
...
async fn login_account_chatgpt_rejected_when_forced_api() -> Result<()> {
    ...
    assert_eq!(
        err.error.message,
        "вход через DeepSeek is disabled. Use API key login instead."
    );
    Ok(())
}
...
async fn set_auth_token_cancels_active_chatgpt_login() -> Result<()> {
    ...
    // Initiate the вход через DeepSeek flow
    let request_id = mcp.send_login_account_chatgpt_request().await?;
    let resp: JSONRPCResponse = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_response_message(RequestId::Integer(request_id)),
    ...
    // Set an external auth token instead of completing the вход через DeepSeek flow.
...
async fn get_account_no_auth() -> Result<()> {
    ...
    assert_eq!(account.requires_deepseek_auth, true);
    Ok(())
}
...
async fn get_account_when_auth_not_required() -> Result<()> {
    ...
    create_config_toml(
        deepseek_home.path(),
        CreateConfigTomlParams {
            requires_deepseek_auth: Some(false),
            ..Default::default()
        },
    )?;
