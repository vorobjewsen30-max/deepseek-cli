...
use deepseek_app_server_protocol::HookHandlerType;
use deepseek_app_server_protocol::HookMetadata;
use deepseek_app_server_protocol::HookSource;
use deepseek_app_server_protocol::HookTrustStatus;
...
fn command_hook_hash(
    ...
    let Ok(value) = deepseek_config::TomlValue::try_from(identity) else {
        unreachable!("normalized hook identity should serialize to TOML");
    };
    deepseek_config::version_for_toml(&value)
}

fn write_user_hook_config(deepseek_home: &std::path::Path) -> Result<()> {
    std::fs::write(
...
fn write_plugin_hook_config(deepseek_home: &std::path::Path, hooks_json: &str) -> Result<()> {
    let plugin_root = deepseek_home.join("plugins/cache/test/demo/local");
...
async fn hooks_list_shows_discovered_hook() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_user_hook_config(deepseek_home.path())?;
...
async fn hooks_list_warms_plugin_capabilities_for_thread_start() -> Result<()> {
  ...
  "hooks": {
    ...
    let plugin_mcp_path = deepseek_home
        .path()
...
async fn hooks_list_shows_plugin_hook_load_warnings() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_plugin_hook_config(deepseek_home.path(), "{ not-json")?;
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    set_project_trust_level(deepseek_home.path(), &repo_root, TrustLevel::Trusted)?;
    ...
    .await??;
    let _: deepseek_app_server_protocol::ConfigWriteResponse = to_response(response)?;
...
async fn config_batch_write_updates_hook_trust_for_loaded_session() -> Result<()> {
    ...
    let deepseek_home = TempDir::new()?;
    let hook_script_path = deepseek_home.path().join("user_prompt_submit_hook.py");
    let hook_log_path = deepseek_home.path().join("user_prompt_submit_hook_log.jsonl");
    std::fs::write(
        &hook_script_path,
        format!(
            r#"import json
...
"#,
    ...
    let hook_list_id = mcp
        .send_hooks_list_request(HooksListParams {
            cwds: vec![deepseek_home.path().to_path_buf()],
        })
        .await?;
    let response: JSONRPCResponse = timeout(
        DEFAULT_TIMEOUT,
