...
async fn auto_env_rejects_explicit_environment_config() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    std::fs::write(deepseek_home.path().join("environments.toml"), "")?;

    let result = TestAppServer::new_with_auto_env(deepseek_home.path()).await;
    ...
    assert_eq!(
        ...
        format!(
            ...
            deepseek_home.path().join("environments.toml").display()
        )
    );
