...
use deepseek_app_server::AppServerRuntimeOptions;
use deepseek_app_server::AppServerTransport;
use deepseek_app_server::AppServerWebsocketAuthSettings;
use deepseek_app_server::PluginStartupTasks;
use deepseek_app_server::RemoteControlStartupMode;
use deepseek_app_server::run_main_with_transport_options;
use deepseek_app_server_protocol::JSONRPCError;
use deepseek_app_server_protocol::JSONRPCResponse;
use deepseek_app_server_protocol::RemoteControlClient;
use deepseek_app_server_protocol::RemoteControlClientsListOrder;
use deepseek_app_server_protocol::RemoteControlClientsListParams;
use deepseek_app_server_protocol::RemoteControlClientsListResponse;
use deepseek_app_server_protocol::RemoteControlClientsRevokeParams;
use deepseek_app_server_protocol::RemoteControlClientsRevokeResponse;
use deepseek_app_server_protocol::RemoteControlConnectionStatus;
use deepseek_app_server_protocol::RemoteControlDisableResponse;
use deepseek_app_server_protocol::RemoteControlEnableResponse;
use deepseek_app_server_protocol::RemoteControlPairingStartParams;
use deepseek_app_server_protocol::RemoteControlPairingStartResponse;
use deepseek_app_server_protocol::RemoteControlPairingStatusParams;
use deepseek_app_server_protocol::RemoteControlPairingStatusResponse;
use deepseek_app_server_protocol::RemoteControlStatusChangedNotification;
use deepseek_app_server_protocol::RemoteControlStatusReadResponse;
...
use deepseek_state::RemoteControlEnrollmentRecord;
...
async fn managed_requirements_reject_all_remote_control_rpcs() -> Result<()> {
    ...
    std::fs::write(
        deepseek_home.path().join("requirements.toml"),
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = deepseek_home.path().join("managed_config.toml");
    let socket_path = deepseek_home.path().join("app-server.sock");
    let transport =
        AppServerTransport::from_listen_url(&format!("unix://{}", socket_path.display()))?;
    let _deepseek_home_guard = EnvVarGuard::set("DEEPSEEK_HOME", deepseek_home.path().as_os_str());
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    let listener = configured_remote_control_listener(deepseek_home.path()).await?;
    let websocket_url = format!(
        "ws://{}/backend-api/wham/remote/control/server",
        listener.local_addr()?
    );
    let state_db =
        StateRuntime::init(deepseek_home.path().to_path_buf(), "test-provider".to_string()).await?;
    state_db
    ...
    let _app_server = TestAppServer::new_with_args(deepseek_home.path(), &["--listen", "off"]).await?;
...
async fn listen_off_ignores_persisted_enable_when_disabled_by_requirements() -> Result<()> {
    ...
    let mut app_server =
        TestAppServer::new_with_args(deepseek_home.path(), &["--listen", "off"]).await?;
...
async fn listen_off_exits_without_persisted_remote_control_enable() -> Result<()> {
    for persisted_preference in [None, Some(false)] {
        ...
        if let Some(remote_control_enabled) = persisted_preference {
            ...
            let state_db =
                StateRuntime::init(deepseek_home.path().to_path_buf(), "test-provider".to_string())
                    .await?;
            state_db
                .upsert_remote_control_enrollment(&RemoteControlEnrollmentRecord {
                    websocket_url,
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(deepseek_home.path()).await?;
    let mut mcp = TestAppServer::new(deepseek_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(deepseek_home.path()).await?;
    let mut mcp = TestAppServer::new(deepseek_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(deepseek_home.path()).await?;
    let mut mcp = TestAppServer::new(deepseek_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(deepseek_home.path()).await?;
    let mut mcp = TestAppServer::new(deepseek_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
impl ClientManagementRemoteControlBackend {
    async fn start(deepseek_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(deepseek_home).await?;
...
async fn configured_remote_control_listener(deepseek_home: &std::path::Path) -> Result<TcpListener> {
