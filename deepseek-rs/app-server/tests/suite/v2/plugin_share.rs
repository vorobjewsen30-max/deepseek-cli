...
async fn plugin_share_save_uploads_local_plugin() -> Result<()> {
    ...
    write_remote_plugin_config(deepseek_home.path(), &format!("{}/backend-api", server.uri()))?;
    write_chatgpt_auth(
        deepseek_home.path(),
    ...
    write_corrupt_plugin_share_local_path_mapping(deepseek_home.path())?;
    ...
    let request_id = mcp
        .send_plugin_list_request(PluginListParams {
            ...
            marketplace_kinds: Some(vec![
                deepseek_app_server_protocol::PluginListMarketplaceKind::Local,
            ]),
        })
        .await?;
    let response: JSONRPCResponse = timeout(
...
async fn plugin_share_update_targets_updates_share_targets() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareUpdateTargetsResponse {
            ...
            discoverability: deepseek_app_server_protocol::PluginShareDiscoverability::Unlisted,
        }
    );
    Ok(())
}
...
async fn plugin_share_delete_removes_created_workspace_plugin() -> Result<()> {
    ...
    let local_plugin_path = AbsolutePathBuf::try_from(deepseek_home.path().join("local-plugin"))?;
    write_plugin_share_local_path_mapping(deepseek_home.path(), "plugins_123", &local_plugin_path)?;
...
fn write_remote_plugin_config(deepseek_home: &Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
...
fn write_test_plugin(root: &Path, plugin_name: &str) -> std::io::Result<PathBuf> {
    ...
    write_file(
        &plugin_path.join(".deepseek-plugin/plugin.json"),
...
fn write_corrupt_plugin_share_local_path_mapping(deepseek_home: &Path) -> std::io::Result<()> {
    write_file(
