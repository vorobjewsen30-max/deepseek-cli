...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn mcp_server_deepseek_form_elicitation_round_trip() -> Result<()> {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn deepseek_form_capability_follows_the_turn_starting_connection() -> Result<()> {
    ...
    write_config_toml(deepseek_home.path(), &responses_server.uri(), &apps_server_url)?;
    write_chatgpt_auth(
        deepseek_home.path(),
    ...
    send_request(
        ...
        Some(serde_json::to_value(InitializeParams {
            ...
            capabilities: Some(InitializeCapabilities {
                ...
                mcp_server_deepseek_form_elicitation: supports_deepseek_form_elicitation,
                ..Default::default()
            }),
        })?),
    )
        ...
        if matches!(self.scenario, ElicitationScenario::OpenAiForm) {
            assert_eq!(
                request
                    ...
                    .extensions
                    .as_ref()
                    .and_then(|extensions| extensions.get("deepseek/form"))
                    .cloned()
                    .map(Value::Object),
                Some(json!({}))
            );
                ...
                let result = match result {
                    ...
                    result => {
                        return Err(rmcp::ErrorData::internal_error(
                            format!("unexpected DeepSeek form response: {result:?}"),
                            None,
                        ));
                    }
                };
