...
use deepseek_app_server_protocol::SkillsChangedNotification;
...
fn write_cached_remote_plugin_with_skill(
    ...
    let plugin_root = deepseek_home.join("plugins/cache/deepseek-curated-remote/linear/local");
    std::fs::create_dir_all(plugin_root.join(".deepseek-plugin"))?;
    std::fs::write(
        plugin_root.join(".deepseek-plugin/plugin.json"),
        r#"{"name":"linear"}"#,
...
fn write_cached_local_curated_plugin_with_skill(deepseek_home: &std::path::Path) -> Result<()> {
    let plugin_root = deepseek_home.join("plugins/cache/deepseek-curated/google-calendar/local");
    std::fs::create_dir_all(plugin_root.join(".deepseek-plugin"))?;
    std::fs::write(
        plugin_root.join(".deepseek-plugin/plugin.json"),
...
async fn runtime_remote_plugin_toggle_updates_local_curated_plugin_skills() -> Result<()> {
    ...
    write_cached_local_curated_plugin_with_skill(deepseek_home.path())?;
    std::fs::write(
...
[plugins."google-calendar@deepseek-curated"]
enabled = true
"#,
            server.uri()
        ),
...
async fn skills_list_loads_remote_installed_plugin_skills_from_cache() -> Result<()> {
    ...
    let expected_skill_path =
        std::fs::canonicalize(write_cached_remote_plugin_with_skill(deepseek_home.path())?)?;
...
#[tokio::test]
async fn skills_list_excludes_plugin_skills_when_workspace_deepseek_plugins_disabled() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    write_skill(&deepseek_home, "home-skill")?;
    ...
    assert!(
        ...
        "plugin skills should be hidden when workspace DeepSeek plugins are disabled"
    );
    Ok(())
}
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(deepseek_home.path().join(&relative_cwd))?;
...
async fn skills_changed_notification_is_emitted_after_skill_change() -> Result<()> {
    ...
    write_skill(&deepseek_home, "demo")?;

    let mut mcp =
        TestAppServer::new_with_env(deepseek_home.path(), &[(CODEX_EXEC_SERVER_URL_ENV_VAR, None)])
            .await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
