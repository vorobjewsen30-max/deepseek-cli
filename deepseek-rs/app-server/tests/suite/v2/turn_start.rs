...
use deepseek_app_server::INPUT_TOO_LARGE_ERROR_CODE;
use deepseek_app_server::INVALID_PARAMS_ERROR_CODE;
use deepseek_app_server_protocol::AdditionalContextEntry;
use deepseek_app_server_protocol::AdditionalContextKind;
use deepseek_app_server_protocol::ByteRange;
...
use deepseek_app_server_protocol::FileChangePatchUpdatedNotification;
...
use deepseek_app_server_protocol::TextElement;
...
use deepseek_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
use deepseek_core::test_support::all_model_presets;
...
use deepseek_protocol::protocol::MULTI_AGENT_MODE_OPEN_TAG;
use deepseek_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS;
use deepseek_utils_absolute_path::test_support::PathExt;
...
const TEST_ORIGINATOR: &str = "deepseek_vscode";
...
async fn run_local_image_turn(detail: Option<ImageDetail>) -> Result<Vec<Value>> {
    // Two DeepSeek turns hit the mock model (session start + turn/start).
    let responses = vec![
    ...
    let image_path = deepseek_home.path().join("image.png");
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("DeepSeek VS Code Extension".to_string()),
...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    write_models_cache(deepseek_home.path())?;
    let cache_path = deepseek_home.path().join("models_cache.json");
    ...
    write_test_skill(deepseek_home.path(), "alpha-skill")?;
    write_test_skill(deepseek_home.path(), "beta-skill")?;

    let isolated_home = deepseek_home.path().to_string_lossy();
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: deepseek_utils_absolute_path::AbsolutePathBuf::try_from(
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three DeepSeek turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ...
    assert_eq!(
        started.turn.status,
        deepseek_app_server_protocol::TurnStatus::InProgress
    );
    assert_eq!(started.turn.id, turn.id);
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let deepseek_home = tmp.path().to_path_buf();
    ...
    create_config_toml(
        deepseek_home.as_path(),
        &server.uri(),
        "untrusted",
        &BTreeMap::default(),
    )?;

    let mut mcp = TestAppServer::new(deepseek_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(deepseek_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(deepseek_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new(&deepseek_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            sandbox_policy: Some(deepseek_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
async fn turn_start_permission_profile_rebinds_runtime_workspace_roots_between_turns() -> Result<()>
{
    ...
    let old_root = deepseek_utils_absolute_path::AbsolutePathBuf::from_absolute_path(old_root)?;
    let new_root = deepseek_utils_absolute_path::AbsolutePathBuf::from_absolute_path(new_root)?;
...
async fn turn_start_resolves_sticky_thread_local_environment_and_turn_overrides() -> Result<()> {
    ...
    let server = create_mock_responses_server_repeating_assistant("done").await;
    create_config_toml(&deepseek_home, &server.uri(), "never", &BTreeMap::default())?;
    std::fs::write(
        deepseek_home.join("environments.toml"),
        r#"
[[environments]]
id = "remote"
url = "ws://127.0.0.1:1"
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![deepseek_app_server_protocol::FileUpdateChange {
...
async fn turn_start_streams_apply_patch_change_updates_v2() -> Result<()> {
    ...
    write_models_cache(&deepseek_home)?;
    let cache_path = deepseek_home.join("models_cache.json");
    let mut cache: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&cache_path)?)?;
    let models = cache["models"]
        .as_array_mut()
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    std::fs::write(
        deepseek_home.path().join("custom-role.toml"),
...
async fn turn_start_with_elevated_override_does_not_persist_project_trust() -> Result<()> {
    ...
    let config_toml = std::fs::read_to_string(deepseek_home.path().join("config.toml"))?;
...
fn write_test_skill(deepseek_home: &Path, name: &str) -> std::io::Result<()> {
    let skill_dir = deepseek_home.join("skills").join(name);
