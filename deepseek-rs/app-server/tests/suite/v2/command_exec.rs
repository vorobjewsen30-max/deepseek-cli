...
async fn command_exec_without_streams_can_be_terminated() -> Result<()> {
    ...
    create_config_toml(deepseek_home.path(), &server.uri(), "never")?;
...
async fn command_exec_env_overrides_merge_with_server_environment_and_support_unset() -> Result<()>
{
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "/bin/sh".to_string(),
                "-lc".to_string(),
                "printf '%s|%s|%s|%s' \"$COMMAND_EXEC_BASELINE\" \"$COMMAND_EXEC_EXTRA\" \"${RUST_LOG-unset}\" \"$DEEPSEEK_HOME\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
    ...
    assert_eq!(
        ...
        CommandExecResponse {
            exit_code: 0,
            stdout: format!("request|added|unset|{}", deepseek_home.path().display()),
            stderr: String::new(),
        }
    );
...
async fn command_exec_permission_profile_does_not_reuse_default_network_proxy() -> Result<()> {
    ...
    let deepseek_home = TempDir::new()?;
    create_config_toml(deepseek_home.path(), &server.uri(), "never")?;
    insert_networked_permission_profile_config(deepseek_home.path(), Some("networked"))?;
    let mut mcp = TestAppServer::new(deepseek_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn command_exec_permission_profile_project_roots_use_command_cwd() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let deepseek_home = TempDir::new()?;
    let command_dir = deepseek_home.path().join("command-cwd");
...
"#,
    ...
    assert!(
        !deepseek_home.path().join("parent.txt").exists(),
...
-> Result<()> {
    ...
    let (mut process, bind_addr) = spawn_websocket_server(deepseek_home.path()).await?;
...
"#
    ...
    insert_command_exec_config(deepseek_home, &inserted_config)?;
    Ok(())
}

fn insert_command_exec_config(deepseek_home: &Path, inserted_config: &str) -> Result<()> {
    let config_path = deepseek_home.join("config.toml");
