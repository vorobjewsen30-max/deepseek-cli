...
use deepseek_app_server_protocol::ThreadStatusChangedNotification;
...
"#,
    ...
    assert_eq!(response.model, "deepseek.gpt-5.5");
    Ok(())
}
...
"#,
    ...
    assert_eq!(
        ...
        vec!["deepseek.gpt-5.5", "deepseek.gpt-5.4", "gpt-5.4-mini"]
    );
    Ok(())
}
...
async fn thread_start_provider_model_fallback_ignores_dynamic_catalog() -> Result<()> {
    ...
    let deepseek_home = TempDir::new()?;
    create_config_toml_without_approval_policy(deepseek_home.path(), &server.uri())?;
    let mut mcp = TestAppServer::new_with_auto_env(deepseek_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let deepseek_home = TempDir::new()?;
    create_config_toml_without_approval_policy(deepseek_home.path(), &server.uri())?;
    let global_agents_path = deepseek_home.path().join("AGENTS.md");
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let deepseek_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(deepseek_home.path(), &server.uri())?;
...
"#,
    ...
    set_project_trust_level(deepseek_home.path(), workspace.path(), TrustLevel::Trusted)?;
    let config_before = std::fs::read_to_string(deepseek_home.path().join("config.toml"))?;
    ...
    let config_after = std::fs::read_to_string(deepseek_home.path().join("config.toml"))?;
    assert_eq!(config_after, config_before);
...
fn create_config_toml_without_approval_policy(
    ...
    create_config_toml_with_optional_approval_policy(
        deepseek_home, server_uri, /*approval_policy*/ None,
    )
}
