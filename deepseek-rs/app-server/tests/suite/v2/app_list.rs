...
use deepseek_app_server_protocol::AppBranding;
...
use deepseek_app_server_protocol::AppReview;
use deepseek_app_server_protocol::AppScreenshot;
...
async fn list_apps_returns_empty_with_api_key_auth() -> Result<()> {
    ...
    save_auth(
        ...
        &AuthDotJson {
            ...
            deepseek_api_key: Some("test-api-key".to_string()),
            tokens: None,
            last_refresh: None,
            agent_identity: None,
...
#[tokio::test]
async fn list_apps_returns_empty_when_workspace_deepseek_plugins_disabled() -> Result<()> {
    let connectors = vec![AppInfo {
        id: "beta".to_string(),
        name: "Beta".to_string(),
        description: Some("Beta connector".to_string()),
...
async fn list_apps_includes_plugin_apps_for_chatgpt_auth() -> Result<()> {
    ...
    let deepseek_home = TempDir::new()?;
    write_connectors_and_plugins_config(deepseek_home.path(), &server_url)?;
    write_plugin_app_fixture(deepseek_home.path(), "sample", "connector_sample")?;
    write_chatgpt_auth(
        deepseek_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn list_apps_emits_updates_and_returns_after_both_lists_load() -> Result<()> {
    ...
    let expected_accessible = vec![AppInfo {
        ...
        app_metadata: None,
        labels: None,
        install_url: Some("https://platform.deepseek.com/apps/beta-app/beta".to_string()),
        is_accessible: true,
        is_enabled: true,
        plugin_display_names: Vec::new(),
    }];
    ...
    let expected_merged = vec![
        AppInfo {
            ...
            app_metadata: None,
            labels: None,
            install_url: Some("https://platform.deepseek.com/apps/beta/beta".to_string()),
            is_accessible: true,
            is_enabled: true,
            plugin_display_names: Vec::new(),
        },
...
fn write_connectors_and_plugins_config(deepseek_home: &Path, base_url: &str) -> std::io::Result<()> {
    let config_toml = deepseek_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
...
fn write_plugin_app_fixture(deepseek_home: &Path, plugin_name: &str, app_id: &str) -> Result<()> {
