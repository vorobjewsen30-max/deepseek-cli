...
async fn standalone_image_generation_returns_saved_path_hint_to_model() -> Result<()> {
    ...
    create_config_toml(deepseek_home.path(), &server.uri(), ImagegenTestMode::Direct)?;
    write_chatgpt_auth(
        deepseek_home.path(),
...
async fn standalone_image_edit_uses_attached_model_visible_image() -> Result<()> {
    let edit_request = run_image_edit_test(|deepseek_home| {
        let image_path = deepseek_home.join("attached.png");
...
async fn run_image_edit_test(
    ...
    let deepseek_home = TempDir::new()?;
    let (arguments, input) = input(deepseek_home.path())?;
    let response_mock = responses::mount_sse_sequence(
        &server,
        vec![
            responses::sse(vec![
