...
use deepseek_app_server_protocol::Model;
use deepseek_app_server_protocol::ModelListParams;
use deepseek_app_server_protocol::ModelListResponse;
use deepseek_app_server_protocol::ModelServiceTier;
use deepseek_app_server_protocol::ModelUpgradeInfo;
use deepseek_app_server_protocol::ReasoningEffortOption;
...
fn expected_visible_models() -> Vec<Model> {
    // Filter by supported_in_api to support testing with both DeepSeek and non-DeepSeek auth modes.
    let mut presets = ModelPreset::filter_by_auth(
        deepseek_core::test_support::all_model_presets().clone(),
        /*chatgpt_mode*/ false,
    );
...
async fn list_models_uses_chatgpt_remote_catalog_as_source_of_truth() -> Result<()> {
    ...
    let remote_model: ModelInfo = serde_json::from_value(json!({
        ...
        "display_name": "DeepSeek Remote Only",
...
deepseek_base_url = "{server_uri}/v1"
"#
        ),
    )?;
    write_chatgpt_auth(
