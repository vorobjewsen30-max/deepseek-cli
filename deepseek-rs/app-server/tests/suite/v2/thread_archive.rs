...
async fn thread_archive_requires_materialized_rollout() -> Result<()> {
    ...
    // Verify file moved.
    let archived_directory = deepseek_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
...
async fn thread_archive_succeeds_when_descendant_archive_fails() -> Result<()> {
    ...
    let child_rollout_path =
        find_thread_path_by_id_str(deepseek_home.path(), &child_id, /*state_db_ctx*/ None)
            .await?
            .expect("child rollout path");
    let archived_child_path = deepseek_home
        .path()
...
async fn thread_archive_succeeds_when_spawned_descendant_is_missing() -> Result<()> {
    ...
    assert!(
        find_thread_path_by_id_str(deepseek_home.path(), &parent_id, /*state_db_ctx*/ None)
            .await?
            .is_none(),
...
async fn thread_archive_clears_stale_subscriptions_before_resume() -> Result<()> {
    ...
    let mut primary = TestAppServer::new_with_auto_env(deepseek_home.path()).await?;
    ...
    let mut secondary = TestAppServer::new_with_auto_env(deepseek_home.path()).await?;
