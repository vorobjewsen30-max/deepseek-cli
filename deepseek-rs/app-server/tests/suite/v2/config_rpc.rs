...
use deepseek_app_server_protocol::AppConfig;
use deepseek_app_server_protocol::AppToolApproval;
use deepseek_app_server_protocol::ApprovalsReviewer;
use deepseek_app_server_protocol::AppsConfig;
use deepseek_app_server_protocol::AppsDefaultConfig;
use deepseek_app_server_protocol::AskForApproval;
use deepseek_app_server_protocol::ConfigBatchWriteParams;
use deepseek_app_server_protocol::ConfigEdit;
use deepseek_app_server_protocol::ConfigLayerSource;
...
use deepseek_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use deepseek_app_server_protocol::ToolsV2;
...
use deepseek_protocol::config_types::WebSearchLocation;
...
fn write_config(deepseek_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let deepseek_home_path = deepseek_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(deepseek_home_path.join("config.toml"))?;
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    write_config(&deepseek_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    set_project_trust_level(deepseek_home.path(), workspace.path(), TrustLevel::Trusted)?;
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let deepseek_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", deepseek_home);
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(deepseek_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let deepseek_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(deepseek_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
"#,
    ...
    let config: toml::Value =
        toml::from_str(&std::fs::read_to_string(deepseek_home.join("config.toml"))?)?;
    assert_eq!(
...
fn assert_layers_user_then_optional_system(
    layers: &[deepseek_app_server_protocol::ConfigLayer],
