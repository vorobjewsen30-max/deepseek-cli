...
#[tokio::test]
async fn deepseek_model_header_mismatch_emits_model_rerouted_notification_v2() -> Result<()> {
    ...
    let response = responses::sse_response(body).insert_header("DeepSeek-Model", SERVER_MODEL);
...
async fn cyber_policy_response_emits_typed_error_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        error,
        ErrorNotification {
            error: deepseek_app_server_protocol::TurnError {
...
-> Result<()> {
    ...
    let response = responses::sse_response(body).insert_header("DeepSeek-Model", REQUESTED_MODEL);
    let _response_mock = responses::mount_response_once(&server, response).await;
    ...
    loop {
        ...
        match notification.method.as_str() {
            "error" => {
                ...
                if payload.error.codex_error_info == Some(DeepSeekErrorInfo::CyberPolicy) {
                    error = Some(payload);
                }
            }
            "model/rerouted" => {
