...
use deepseek_app_server_protocol::PluginAuthPolicy;
use deepseek_app_server_protocol::PluginInstallPolicy;
...
use deepseek_core::config::set_project_trust_level;
use deepseek_login::AuthKeyringBackendKind;
...
fn write_plugins_enabled_config(deepseek_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
        deepseek_home.join("config.toml"),
        r#"[features]
plugins = true
"#,
    )
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    let home = deepseek_home.path().to_string_lossy().into_owned();
...
{
    let deepseek_home = TempDir::new()?;
    write_deepseek_curated_marketplace(
        deepseek_home.path(),
        &["linear", "computer-use", "not-mentioned"],
    )?;
    write_installed_plugin(&deepseek_home, "deepseek-curated", "linear")?;
    std::fs::write(
...
"#,
    ...
    let mut mcp = TestAppServer::new(deepseek_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
    ...
    assert_eq!(response.marketplaces[0].name, "deepseek-curated");
    assert_eq!(
        response.marketplaces[0]
            ...
            .collect::<Vec<_>>(),
        vec![
            ("linear@deepseek-curated".to_string(), true, true),
            ("computer-use@deepseek-curated".to_string(), false, false),
        ]
    );
...
async fn plugin_installed_prefers_remote_curated_conflicts_when_remote_plugin_enabled() -> Result<()>
{
    ...
    write_deepseek_curated_marketplace(deepseek_home.path(), &["linear", "calendar"])?;
    write_installed_plugin(&deepseek_home, "deepseek-curated", "linear")?;
    write_installed_plugin(&deepseek_home, "deepseek-curated", "calendar")?;
    std::fs::write(
        deepseek_home.path().join("config.toml"),
        format!(
...
"#,
    ...
    let mut app_server = TestAppServer::new(deepseek_home.path()).await?;
    ...
    let local_marketplace = response
        ...
        .expect("expected deepseek-curated marketplace entry");
    assert_eq!(
        local_marketplace
            .plugins
            .iter()
            ...
            .collect::<Vec<_>>(),
        vec!["calendar@deepseek-curated".to_string()]
    );
    let remote_marketplace = response
        .marketplaces
        .iter()
        .find(|marketplace| marketplace.name == "deepseek-curated-remote")
        .expect("expected deepseek-curated-remote marketplace entry");
    assert_eq!(
        remote_marketplace
            .plugins
            .iter()
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.deepseek-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.deepseek-plugin/plugin.json"),
...
#[tokio::test]
async fn plugin_list_returns_empty_when_workspace_deepseek_plugins_disabled() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
...
#[tokio::test]
async fn plugin_list_reuses_cached_workspace_deepseek_plugins_setting() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.deepseek-plugin"))?;
    write_plugins_enabled_config_with_base_url(
        deepseek_home.path(),
        &format!("{}/backend-api/", server.uri()),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.deepseek-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )?;
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    auth_policy: PluginAuthPolicy::OnInstall,
                    availability: deepseek_app_server_protocol::PluginAvailability::Available,
                    interface: Some(deepseek_app_server_protocol::PluginInterface {
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    std::fs::create_dir_all(deepseek_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(deepseek_home.path())?;
    std::fs::write(
        deepseek_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "codex-curated",
  "plugins": [
    {
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&deepseek_home, "codex-curated", "enabled-plugin")?;
    write_installed_plugin(&deepseek_home, "codex-curated", "disabled-plugin")?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "codex-curated",
...
async fn plugin_list_uses_home_config_for_enabled_state() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    std::fs::create_dir_all(deepseek_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&deepseek_home, "codex-curated", "shared-plugin")?;
    std::fs::write(
        deepseek_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "codex-curated",
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    "privacyPolicyURL": "https://platform.deepseek.com
    "termsOfServiceURL": "https://platform.deepseek.com
    "defaultPrompt": [
...
async fn plugin_list_returns_installed_git_source_interface_from_cache() -> Result<()> {
  ...
  "plugins": [
    ...
    let cached_plugin_root = deepseek_home.path().join("plugins/cache/debug/toolkit/local");
    std::fs::create_dir_all(cached_plugin_root.join(".deepseek-plugin"))?;
    std::fs::write(
...
async fn app_server_startup_sync_downloads_remote_installed_plugin_bundles() -> Result<()> {
    ...
    let installed_path = deepseek_home
        .path()
        .join("plugins/cache/deepseek-curated-remote/linear/1.2.3");
    ...
    wait_for_path_exists(&installed_path.join(".deepseek-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(installed_path.join(".deepseek-plugin/plugin.json"))?,
    )?;
    assert_eq!(
    ...
    let config = std::fs::read_to_string(deepseek_home.path().join("config.toml"))?;
    assert!(!config.contains("linear@deepseek-curated-remote"));
    Ok(())
}
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    write_installed_plugin_with_version(&deepseek_home, "deepseek-curated-remote", "linear", "1.0.0")?;
    write_installed_plugin_with_version(&deepseek_home, "deepseek-curated-remote", "stale", "1.0.0")?;
    ...
    let old_path = deepseek_home
        .path()
        .join("plugins/cache/deepseek-curated-remote/linear/1.0.0");
    let new_path = deepseek_home
        .path()
        .join("plugins/cache/deepseek-curated-remote/linear/1.2.3");
    let stale_path = deepseek_home
        .path()
        .join("plugins/cache/deepseek-curated-remote/stale");
    ...
    assert_eq!(
        remote_marketplace
            ...
            .collect::<Vec<_>>(),
        vec![("linear@deepseek-curated-remote".to_string(), true, true)]
    );

    wait_for_path_exists(&new_path.join(".deepseek-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(new_path.join(".deepseek-plugin/plugin.json"))?,
    )?;
    assert_eq!(
        installed_plugin_manifest["version"],
        serde_json::json!("1.2.3")
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled() -> Result<()> {
    ...
    write_installed_plugin_with_version(&deepseek_home, "deepseek-curated-remote", "linear", "1.2.3")?;
  ...
  "pagination": {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(200).set_body_string(r#"["linear@deepseek-curated-remote"]"#),
        )
        .mount(&server)
        .await;
    ...
    let remote_marketplace = response
        ...
        .into_iter()
        .find(|marketplace| marketplace.name == "deepseek-curated-remote")
        .expect("expected deepseek-curated remote marketplace");
    ...
    assert_eq!(
        ...
        deepseek_app_server_protocol::PluginAvailability::Available
    );
    assert_eq!(
    ...
    let cache_files = std::fs::read_dir(deepseek_home.path().join("cache/remote_plugin_catalog"))?
    ...
    assert_eq!(
        ...
        vec!["linear@deepseek-curated-remote".to_string()]
    );
    assert!(
        !server
            .received_requests()
...
async fn plugin_list_uses_cached_global_remote_catalog_and_refreshes_it() -> Result<()> {
    ...
    wait_for_cached_remote_catalog_plugin_ids(deepseek_home.path(), &[cached_remote_plugin_id])
        .await?;
    ...
    wait_for_remote_plugin_request_count(&server, "/ps/plugins/list", /*expected_count*/ 1).await?;
    wait_for_cached_remote_catalog_plugin_ids(deepseek_home.path(), &[refreshed_remote_plugin_id])
        .await?;
...
#[tokio::test]
async fn plugin_list_includes_deepseek_curated_remote_collection_when_remote_plugin_disabled_and_requested()
-> Result<()> {
    let deepseek_home = TempDir::new()?;
    let server = MockServer::start().await;
  ...
  "pagination": {
    ...
    mount_deepseek_curated_remote_collection_plugin_list(&server, collection_body).await;
    mount_remote_installed_plugins(&server, "GLOBAL", empty_remote_installed_plugins_body()).await;
    mount_remote_installed_plugins(&server, "WORKSPACE", empty_remote_installed_plugins_body())
        .await;
    mount_empty_user_installed_plugins(&server).await;
...
#[tokio::test]
async fn plugin_list_propagates_deepseek_curated_remote_collection_errors_when_remote_plugin_disabled()
-> Result<()> {
    let deepseek_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_remote_plugins_disabled_config_with_base_url(
    ...
    assert!(
        err.error
            .message
            .contains("list DeepSeek Curated remote plugin catalog")
    );
    Ok(())
}

#[tokio::test]
async fn plugin_list_skips_deepseek_curated_remote_collection_for_api_auth_when_remote_plugin_disabled()
-> Result<()> {
    let deepseek_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_remote_plugins_disabled_config_with_base_url(
...
-> Result<()> {
    ...
    write_deepseek_api_curated_marketplace(deepseek_home.path(), &["api-plugin"])?;
    login_with_api_key(
        deepseek_home.path(),
        "sk-test-key",
        AuthCredentialsStoreMode::File,
    ...
    let api_curated_marketplace = response
        .marketplaces
        .iter()
        .find(|marketplace| marketplace.name == "deepseek-api-curated")
...
#[tokio::test]
async fn plugin_list_does_not_query_deepseek_curated_remote_collection_by_default() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugins_enabled_config_with_base_url(
        deepseek_home.path(),
    ...
    assert!(
        response
            .marketplaces
            .iter()
            .all(|marketplace| marketplace.name != "deepseek-curated-remote")
    );
    assert!(
        server
            .received_requests()
...
"#,
    ...
    wait_for_path_exists(
        &deepseek_home.path().join(
            "plugins/cache/created-by-me-remote/private-linear/1.2.3/.deepseek-plugin/plugin.json",
        ),
    )
    .await?;
...
"#,
    ...
    assert_eq!(response.marketplaces.len(), 1);
    assert_eq!(response.marketplaces[0].name, "deepseek-curated-remote");
    assert_eq!(
        response.marketplaces[0]
            .plugins
            .iter()
    ...
    let installed_path = deepseek_home
        .path()
        .join("plugins/cache/deepseek-curated-remote/linear/1.2.3/.deepseek-plugin/plugin.json");
...
async fn plugin_list_marks_remote_plugin_disabled_by_admin() -> Result<()> {
  ...
  "pagination": {
    ...
    let remote_marketplace = response
        ...
        .into_iter()
        .find(|marketplace| marketplace.name == "deepseek-curated-remote")
        .expect("expected DeepSeek remote marketplace");
    ...
    assert_eq!(
        plugin.availability,
        deepseek_app_server_protocol::PluginAvailability::DisabledByAdmin
    );
    Ok(())
}
...
async fn plugin_list_fetches_featured_plugin_ids_without_chatgpt_auth() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugin_sync_config(deepseek_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_deepseek_curated_marketplace(deepseek_home.path(), &["linear", "gmail"])?;

    Mock::given(method("GET"))
        .and(path("/backend-api/plugins/featured"))
        .and(query_param("platform", "codex"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["linear@deepseek-curated"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(
        response.featured_plugin_ids,
        vec!["linear@deepseek-curated".to_string()]
    );
    Ok(())
}
...
async fn plugin_list_uses_warmed_featured_plugin_ids_cache_on_first_request() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_with_plugin_startup_tasks(deepseek_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn wait_for_cached_remote_catalog_plugin_ids(
    ...
    timeout(DEFAULT_TIMEOUT, async {
        loop {
            let plugin_ids = cached_remote_catalog_plugin_ids(deepseek_home)?;
...
fn cached_remote_catalog_plugin_ids(deepseek_home: &std::path::Path) -> Result<Vec<String>> {
    let cache_dir = deepseek_home.join("cache/remote_plugin_catalog");
    if !cache_dir.exists() {
        return Ok(Vec::new());
    }
...
async fn mount_deepseek_curated_remote_collection_plugin_list(server: &MockServer, body: &str) {
    Mock::given(method("GET"))
        .and(path("/backend-api/ps/plugins/list"))
        .and(query_param("scope", "GLOBAL"))
        .and(query_param("limit", "200"))
...
fn write_installed_plugin(
    ...
    write_installed_plugin_with_version(deepseek_home, marketplace_name, plugin_name, "local")
}
...
fn write_plugin_sync_config(deepseek_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        deepseek_home.join("config.toml"),
        format!(
            r#"
...
[plugins."gmail@deepseek-curated"]
enabled = false
...
fn write_deepseek_curated_marketplace(
    deepseek_home: &std::path::Path,
    plugin_names: &[&str],
) -> std::io::Result<()> {
...
fn write_deepseek_api_curated_marketplace(
    deepseek_home: &std::path::Path,
    plugin_names: &[&str],
) -> std::io::Result<()> {
    write_curated_marketplace(
...
fn write_curated_marketplace(
    ...
    let curated_root = deepseek_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.deepseek-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(deepseek_home.join(".tmp"))?;
    std::fs::write(
        deepseek_home.join(".tmp/plugins.sha"),
...
fn write_plugin_share_local_path_mapping(
    ...
    std::fs::create_dir_all(deepseek_home.join(".tmp"))?;
    std::fs::write(
        deepseek_home.join(".tmp/plugin-share-local-paths-v1.json"),
        format!("{contents}\n"),
    )
}
