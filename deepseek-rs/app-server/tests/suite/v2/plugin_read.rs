...
use deepseek_app_server_protocol::AppInfo;
use deepseek_app_server_protocol::AppMetadata;
use deepseek_app_server_protocol::AppTemplateSummary;
use deepseek_app_server_protocol::AppTemplateUnavailableReason;
use deepseek_app_server_protocol::HookEventName;
    ...
    let request_id = mcp
        .send_plugin_read_request(PluginReadParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                deepseek_home.path().join("marketplace.json"),
            )?),
...
"#,
  ...
  "pagination": {
    ...
    assert_eq!(response.plugin.marketplace_name, "deepseek-curated-remote");
    assert_eq!(
        response.plugin.summary.id,
        "example-plugin@deepseek-curated-remote"
    );
    assert_eq!(
...
async fn plugin_read_rejects_invalid_remote_plugin_name() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    write_remote_plugin_catalog_config(deepseek_home.path(), "https://example.invalid/backend-api/")?;
    let mut mcp = TestAppServer::new(deepseek_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
#[tokio::test]
async fn plugin_read_returns_canonical_deepseek_curated_marketplace_name() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    ...
    std::fs::write(
        ...
        r#"{
  "name": "demo-plugin",
  "description": "DeepSeek curated plugin"
}"#,
    )?;
    std::fs::write(
        deepseek_home.path().join("config.toml"),
...
[plugins."demo-plugin@deepseek-curated"]
enabled = true
"#,
    )?;
    write_installed_plugin(&deepseek_home, "deepseek-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "deepseek-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@deepseek-curated");
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(deepseek_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let deepseek_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&deepseek_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "codex-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(deepseek_home.path().join(".tmp"))?;
    std::fs::write(
        deepseek_home
            .path()
...
"#,
    ...
    std::fs::write(
        ...
        r#"---
name: chatgpt-only
description: Visible only for DeepSeek
---
...
"#,
    ...
    std::fs::write(
        plugin_root.join("skills/thread-summarizer/agents/deepseek.yaml"),
        r#"policy:
  products:
    - CODEX
"#,
    ...
    std::fs::write(
        plugin_root.join("skills/chatgpt-only/agents/deepseek.yaml"),
        r#"policy:
  products:
    - CHATGPT
"#,
...
"#,
    ...
    write_installed_plugin(&deepseek_home, "codex-curated", "demo-plugin")?;
    ...
    assert_eq!(
        ...
        Some("https://platform.deepseek.com/apps/gmail/gmail")
    );
    assert_eq!(
...
async fn plugin_read_returns_app_metadata_category() -> Result<()> {
    ...
    let deepseek_home = TempDir::new()?;
    write_connectors_config(deepseek_home.path(), &server_url)?;
    write_chatgpt_auth(
        deepseek_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_read_hides_apps_for_api_key_auth() -> Result<()> {
    ...
    std::fs::write(
        ...
        r#"{"DEEPSEEK_API_KEY":"sk-test-key","tokens":null,"last_refresh":null}"#,
    )?;
...
fn write_plugins_enabled_config(deepseek_home: &TempDir) -> Result<()> {
    std::fs::write(
        deepseek_home.path().join("config.toml"),
        r#"[features]
plugins = true
...
fn write_connectors_config(deepseek_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
