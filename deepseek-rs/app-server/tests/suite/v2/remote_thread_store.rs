...
//! the temporary `deepseek_home` must not contain rollout session files or sqlite
...
use deepseek_app_server::in_process::InProcessClientHandle;
use deepseek_app_server::in_process::InProcessServerEvent;
...
use deepseek_thread_store::CreateThreadParams as StoreCreateThreadParams;
...
async fn thread_delete_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    // Plugin startup warmups may create `.tmp` under deepseek_home. Disable them
    // here so this regression stays focused on thread persistence artifacts.
    create_config_toml_with_thread_store(deepseek_home.path(), &server.uri(), &store_id)?;

    let thread_store = InMemoryThreadStore::for_id(store_id.clone());
    let _in_memory_store = InMemoryThreadStoreId { store_id };

    let mut client = start_in_process_server(deepseek_home.path()).await?;
    ...
    assert_no_local_persistence_artifacts(deepseek_home.path())?;
...
async fn start_in_process_server(deepseek_home: &Path) -> Result<InProcessClientHandle> {
    let loader_overrides = LoaderOverrides::without_managed_config_for_tests();
    let config = Arc::new(
        ConfigBuilder::default()
...
fn assert_no_local_persistence_artifacts(deepseek_home: &Path) -> Result<()> {
    ...
    // the isolated test deepseek_home.
    assert!(
        !deepseek_home.join("sessions").exists(),
        "non-local thread persistence should not create local rollout sessions"
    );
    assert!(
        !deepseek_home.join("archived_sessions").exists(),
        "non-local thread persistence should not create archived rollout sessions"
    );
    assert!(
        !deepseek_state::state_db_path(deepseek_home).exists(),
        "non-local thread persistence should not create local thread sqlite"
    );

    let sqlite_artifacts = std::fs::read_dir(deepseek_home)?
    ...
    let mut entries = deepseek_home_entries(deepseek_home)?;
    // Bazel test runs may initialize shell snapshot storage under deepseek_home.
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in deepseek_home"
    );

    Ok(())
}

fn deepseek_home_entries(deepseek_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(deepseek_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;
