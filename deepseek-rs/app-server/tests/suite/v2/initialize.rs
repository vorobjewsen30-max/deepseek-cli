...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_deepseek_home = AbsolutePathBuf::try_from(deepseek_home.path().canonicalize()?)?;
    ...
    let InitializeResponse {
        user_agent,
        deepseek_home: response_deepseek_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("deepseek_vscode/"));
    assert_eq!(response_deepseek_home, expected_deepseek_home);
...
async fn initialize_probe_does_not_override_originator() -> Result<()> {
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "deepseek_app_server_daemon".to_string(),
            title: Some("DeepSeek App Server Daemon".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
    ...
    assert!(user_agent.starts_with("deepseek_cli_rs/"));
    Ok(())
}
...
async fn initialize_codex_backend_does_not_override_originator() -> Result<()> {
    ...
    let message = timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("DeepSeek Backend".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_with_env(
        deepseek_home.path(),
        &[(
            "DEEPSEEK_INTERNAL_ORIGINATOR_OVERRIDE",
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_with_env(
        deepseek_home.path(),
        &[("DEEPSEEK_INTERNAL_ORIGINATOR_OVERRIDE", None)],
    )
    .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let deepseek_home = TempDir::new()?;
    let notify_file = deepseek_home.path().join("notify.json");
    let notify_capture = cargo_bin("deepseek-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(deepseek_home, server_uri, approval_policy, "")
}
