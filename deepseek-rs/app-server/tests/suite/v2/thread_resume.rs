...
use deepseek_app_server_protocol::McpToolCallAppContext;
...
use deepseek_core::ARCHIVED_SESSIONS_SUBDIR;
use deepseek_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
use deepseek_protocol::protocol::SessionSource as RolloutSessionSource;
...
use deepseek_rollout::append_rollout_item_to_path;
use deepseek_rollout::read_session_meta_line;
...
const CODEX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are DeepSeek, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn thread_resume_rejects_unmaterialized_thread() -> Result<()> {
    ...
    create_config_toml(deepseek_home.path(), &server.uri())?;
...
async fn thread_goal_get_rejects_unmaterialized_thread() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_without_managed_config(deepseek_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let deepseek_home = TempDir::new()?;
    let deepseek_home_path = normalized_existing_path(deepseek_home.path())?;
    create_config_toml(&deepseek_home_path, &server.uri())?;
    let config_path = deepseek_home_path.join("config.toml");
    let config = std::fs::read_to_string(&config_path)?;
    std::fs::write(
        &config_path,
        config.replace("personality = true\n", "personality = true\ngoals = true\n"),
    ...
    let sqlite_home = deepseek_home_path
        .as_path()
        .to_str()
...
async fn thread_resume_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    let deepseek_home = TempDir::new()?;
    create_config_toml_with_chatgpt_base_url(deepseek_home.path(), &server.uri(), &server.uri())?;
    mount_analytics_capture(&server, deepseek_home.path()).await?;
...
fn set_session_meta_on_fake_rollout(
    ...
    let path = rollout_path(deepseek_home, filename_ts, thread_id);
...
async fn thread_resume_redacts_payloads_for_chatgpt_remote_clients() -> Result<()> {
    for client_name in ["deepseek_chatgpt_android_remote", "deepseek_chatgpt_ios_remote"] {
...
fn append_resume_redaction_history(
    ...
    let rollout_file_path = rollout_path(deepseek_home, filename_ts, conversation_id);
...
async fn thread_resume_rejects_archived_session_by_id() -> Result<()> {
    ...
    let active_rollout_path = rollout_path(deepseek_home.path(), filename_ts, &conversation_id);
    let archived_dir = deepseek_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
...
async fn thread_goal_set_edits_objective_without_resetting_usage() -> Result<()> {
    ...
    let state_db =
        StateRuntime::init(deepseek_home.path().to_path_buf(), "mock_provider".into()).await?;
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            deepseek_state::GoalAccountingMode::ActiveOnly,
...
async fn thread_goal_lifecycle_emits_analytics_and_clear_deletes_goal() -> Result<()> {
    ...
    .await??;
    let get: deepseek_app_server_protocol::ThreadGoalGetResponse = to_response(get_resp)?;
...
async fn thread_resume_token_usage_replay_ignores_stale_interrupted_tail_turn() -> Result<()> {
    ...
    let rollout_file_path = rollout_path(deepseek_home.path(), filename_ts, &conversation_id);
    let persisted_rollout = std::fs::read_to_string(&rollout_file_path)?;
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let deepseek_home = TempDir::new()?;
    let config_toml = deepseek_home.path().join("config.toml");
    std::fs::write(
        &config_toml,
        format!(
            r#"
...
"#,
    ...
    let repo_path = deepseek_home.path().join("repo");
    ...
    let rollout_path = rollout_path(deepseek_home.path(), "2025-01-05T12-00-00", &thread_id);
...
async fn thread_resume_defers_updated_at_until_turn_start() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let deepseek_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(deepseek_home.path(), &server.uri()).await?;
    ...
    let resume_id = mcp
        .send_thread_resume_request(ThreadResumeParams {
            ...
            cwd: Some(deepseek_home.path().to_string_lossy().to_string()),
            ..Default::default()
        })
        .await?;
    let resume_resp: JSONRPCResponse = timeout(
    ...
    assert_eq!(cwd, AbsolutePathBuf::from_absolute_path(deepseek_home.path())?);
...
async fn thread_resume_keeps_in_flight_turn_streaming() -> Result<()> {
    ...
    let mut primary = TestAppServer::new(deepseek_home.path()).await?;
    ...
    let mut secondary = TestAppServer::new(deepseek_home.path()).await?;
...
async fn thread_resume_rejects_mismatched_path_for_running_thread_id() -> Result<()> {
    ...
    let stale_path = rollout_path(deepseek_home.path(), "2025-01-01T00-00-00", &stale_thread_id);
...
async fn thread_resume_replays_pending_file_change_request_approval() -> Result<()> {
    let tmp = TempDir::new()?;
    let deepseek_home = tmp.path().join("deepseek_home");
    std::fs::create_dir(&deepseek_home)?;
...
"#;
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    create_config_toml(&deepseek_home, &server.uri())?;

    let mut primary = TestAppServer::new(&deepseek_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, primary.initialize()).await??;
    ...
    let expected_file_change = ThreadItem::FileChange {
        ...
        changes: vec![deepseek_app_server_protocol::FileUpdateChange {
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        rollout_file_path,
        updated_at,
    } = start_materialized_thread_and_restart(deepseek_home.path(), "materialize").await?;
...
async fn thread_resume_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let deepseek_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(deepseek_home.path(), &server.uri()).await?;
    create_config_toml_with_required_broken_mcp(deepseek_home.path(), &server.uri())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(deepseek_home.path(), "seed history").await?;
...
async fn start_materialized_thread_and_restart(
    ...
    let mut first_mcp = TestAppServer::new(deepseek_home).await?;
    ...
    let mut second_mcp = TestAppServer::new(deepseek_home).await?;
...
fn create_config_toml(deepseek_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
...
async fn setup_rollout_fixture(deepseek_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(deepseek_home, server_uri)?;
