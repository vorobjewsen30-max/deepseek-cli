...
use deepseek_rollout::append_thread_name;
...
async fn thread_fork_creates_new_thread_and_emits_started() -> Result<()> {
    ...
    let original_path = deepseek_home
        .path()
        .join("sessions")
        .join("2025")
        .join("01")
...
async fn thread_fork_inherits_explicit_source_name_from_session_index() -> Result<()> {
    ...
    append_thread_name(deepseek_home.path(), source_thread_id, source_name).await?;
...
#[tokio::test]
async fn pathless_ephemeral_thread_rejects_deepseek_home_path_after_reload() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let deepseek_home = TempDir::new()?;
    create_config_toml(deepseek_home.path(), &server.uri())?;
    ...
    let resume_id = app_server
        .send_thread_resume_request(ThreadResumeParams {
            ...
            path: Some(deepseek_home_path.clone()),
            ..Default::default()
        })
        .await?;
