...
use deepseek_app_server_protocol::ConfigReadParams;
use deepseek_app_server_protocol::ConfigValueWriteParams;
...
use deepseek_app_server_protocol::FsCopyParams;
use deepseek_app_server_protocol::FsCreateDirectoryParams;
use deepseek_app_server_protocol::FsGetMetadataParams;
use deepseek_app_server_protocol::FsReadDirectoryParams;
use deepseek_app_server_protocol::FsReadFileParams;
use deepseek_app_server_protocol::FsRemoveParams;
use deepseek_app_server_protocol::FsUnwatchParams;
use deepseek_app_server_protocol::FsWatchParams;
use deepseek_app_server_protocol::FsWriteFileParams;
...
use deepseek_app_server_protocol::JSONRPCNotification;
...
use deepseek_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use deepseek_app_server_protocol::ProcessKillParams;
use deepseek_app_server_protocol::ProcessResizePtyParams;
use deepseek_app_server_protocol::ProcessSpawnParams;
use deepseek_app_server_protocol::ProcessWriteStdinParams;
use deepseek_app_server_protocol::RemoteControlClientsListParams;
use deepseek_app_server_protocol::RemoteControlClientsRevokeParams;
use deepseek_app_server_protocol::RemoteControlPairingStartParams;
use deepseek_app_server_protocol::RemoteControlPairingStatusParams;
...
use deepseek_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use deepseek_exec_server::CODEX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use deepseek_exec_server::CODEX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
use deepseek_exec_server::CODEX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR;
use deepseek_exec_server::CODEX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR;
use deepseek_exec_server::CODEX_EXEC_SERVER_URL_ENV_VAR;
...
pub const DEFAULT_CLIENT_NAME: &str = "deepseek-app-server-tests";
...
impl TestAppServer {
    ...
    pub async fn new(deepseek_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(deepseek_home, &[], &[DISABLE_PLUGIN_STARTUP_TASKS_ARG]).await
    }
    ...
    /// URL-based configuration, this helper rejects a `deepseek_home` containing
    /// that file.
    pub async fn new_with_auto_env(deepseek_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_auto_env_and_env(deepseek_home, &[]).await
    }
    ...
    async fn new_with_auto_env_and_env(
        ...
        let environments_toml = deepseek_home.join("environments.toml");
        ensure!(
            !environments_toml
                .try_exists()
        ...
        let mut app_server = Self::new_with_env(deepseek_home, &env_overrides).await?;
    ...
    pub async fn new_without_managed_config(deepseek_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(deepseek_home, &[(DISABLE_MANAGED_CONFIG_ENV_VAR, Some("1"))]).await
    }
    ...
    pub async fn new_without_managed_config_with_env(
        ...
        Self::new_with_env(deepseek_home, &all_env_overrides).await
    }

    pub async fn new_with_plugin_startup_tasks(deepseek_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(deepseek_home, &[], &[]).await
    }
    ...
    pub async fn new_with_env_and_plugin_startup_tasks(
        ...
        Self::new_with_env_and_args(deepseek_home, env_overrides, &[]).await
    }

    pub async fn new_with_args(deepseek_home: &Path, args: &[&str]) -> anyhow::Result<Self> {
        let mut all_args = vec![DISABLE_PLUGIN_STARTUP_TASKS_ARG];
        all_args.extend_from_slice(args);
        Self::new_with_env_and_args(deepseek_home, &[], &all_args).await
    }
    ...
    async fn new_with_env_and_args(
        ...
        let program = deepseek_utils_cargo_bin::cargo_bin("deepseek-app-server")
            .context("should find binary for deepseek-app-server")?;
        Self::new_with_program_env_and_args(deepseek_home, &program, env_overrides, args).await
    }
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.current_dir(deepseek_home);
        cmd.env("DEEPSEEK_HOME", deepseek_home);
        cmd.env("RUST_LOG", "warn");
        // Keep integration tests isolated from host managed configuration.
        cmd.env(
            "CODEX_APP_SERVER_MANAGED_CONFIG_PATH",
            deepseek_home.join("managed_config.toml"),
        );
        cmd.env_remove(DEEPSEEK_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("deepseek-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    /// Send an `account/login/start` JSON-RPC request with DeepSeek auth tokens.
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: deepseek_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
    ...
    /// Send an `account/login/start` JSON-RPC request for вход через DeepSeek.
    ...
    /// Send an `account/login/start` JSON-RPC request for DeepSeek device code login.
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `deepseek-app-server` child process.
        //
