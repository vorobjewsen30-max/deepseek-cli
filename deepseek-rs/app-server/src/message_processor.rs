...
use deepseek_app_server_protocol::ChatgptAuthTokensRefreshParams;
...
use deepseek_app_server_protocol::ExperimentalApi;
...
use deepseek_app_server_protocol::experimental_required_message;
...
use deepseek_login::auth::ExternalAuth;
use deepseek_login::auth::ExternalAuthRefreshContext;
use deepseek_login::auth::ExternalAuthRefreshReason;
use deepseek_login::auth::ExternalAuthTokens;
use deepseek_protocol::ThreadId;
use deepseek_protocol::auth::AuthMode as LoginAuthMode;
...
impl ConnectionSessionState {
    ...
    pub(crate) fn supports_deepseek_form_elicitation(&self) -> bool {
        self.initialized
            .get()
            .is_some_and(|session| session.supports_deepseek_form_elicitation)
    }
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) feedback: DeepSeekFeedback,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_store = deepseek_core::thread_store_from_config(config.as_ref(), state_db.clone());
        let environment_manager_for_requests = Arc::clone(&environment_manager);
        let environment_manager_for_extensions = Arc::clone(&environment_manager);
        let restriction_product = session_source.restriction_product();
        let executor_skill_provider: Arc<dyn deepseek_skills_extension::SkillProvider> = Arc::new(
            deepseek_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
    ...
    async fn dispatch_initialized_client_request(
        ...
        let supports_deepseek_form_elicitation = session.supports_deepseek_form_elicitation();
