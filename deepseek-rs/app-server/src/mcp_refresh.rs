...
async fn build_refresh_config(
    ...
    let mcp_servers = deepseek_mcp::configured_mcp_servers(&mcp_config);
...
mod tests {
    ...
    use deepseek_config::ThreadConfigContext;
    use deepseek_config::ThreadConfigLoadError;
    use deepseek_config::ThreadConfigLoadErrorCode;
    ...
    use deepseek_extension_api::NoopExtensionEventSink;
    ...
    async fn refresh_config_uses_latest_auth_keyring_backend() -> anyhow::Result<()> {
        ...
        std::fs::write(
            temp_dir.path().join(deepseek_config::CONFIG_TOML_FILE),
    ...
    async fn refresh_test_state() -> anyhow::Result<(
        ...
        let auth_manager = AuthManager::from_auth_for_testing(DeepSeekAuth::from_api_key("dummy"));
            ...
            ThreadManager::new(
                ...
                thread_extensions(
                    ...
                    ThreadExtensionDependencies {
                        ...
                        analytics_events_client: deepseek_analytics::AnalyticsEventsClient::disabled(),
                        thread_manager: thread_manager.clone(),
                        goal_service: Arc::new(deepseek_goal_extension::GoalService::new()),
                ...
                Arc::new(DeepSeekHomeUserInstructionsProvider::new(
                    good_config.deepseek_home.clone(),
                )),
                /*analytics_events_client*/ None,
                Arc::clone(&thread_store),
                deepseek_core::local_agent_graph_store_from_state_db(Some(&state_db)),
    ...
    impl ThreadConfigLoader for CountingThreadConfigLoader {
        fn load(
            &self,
            context: ThreadConfigContext,
        ) -> deepseek_config::ThreadConfigLoaderFuture<'_, Vec<ThreadConfigSource>> {
