...
use deepseek_analytics::AppServerRpcTransport;
...
use deepseek_app_server_protocol::TextPosition as AppTextPosition;
use deepseek_app_server_protocol::TextRange as AppTextRange;
use deepseek_config::ConfigLayerSource;
use deepseek_config::ConfigLoadError;
use deepseek_config::TextRange as CoreTextRange;
use deepseek_core::ExecPolicyError;
...
use deepseek_rollout::state_db as rollout_state_db;
...
const SQLITE_RECOVERY_CONFIG_WARNING_SUMMARY: &str = "DeepSeek rebuilt its local database.";
...
const OTEL_SERVICE_NAME: &str = "deepseek-app-server";
#[cfg(debug_assertions)]
...
pub async fn run_main_with_transport_options(
    ...
    let otel = deepseek_core::otel_init::build_provider(
        &config,
    ...
    deepseek_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    deepseek_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let unix_socket_startup_lock = match &transport {
        AppServerTransport::UnixSocket { socket_path } => {
            let startup_lock_path = app_server_startup_lock_path(&deepseek_home)?;
    ...
    if should_run_personality_migration {
        ...
        match effective_toml.try_into() {
            Ok(config_toml) => {
                match deepseek_core::personality_migration::maybe_migrate_personality(
                ...
                .await
                {
                    Ok(deepseek_core::personality_migration::PersonalityMigrationStatus::Applied) => {
                    ...
                    Ok(
                        deepseek_core::personality_migration::PersonalityMigrationStatus::SkippedMarker
                        | deepseek_core::personality_migration::PersonalityMigrationStatus::SkippedExplicitPersonality
                        | deepseek_core::personality_migration::PersonalityMigrationStatus::SkippedNoSessions,
                    ) => {}
                    Err(err) => {
    ...
    let feedback = DeepSeekFeedback::new();
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        if !deepseek_state::is_sqlite_corruption_error(&err)
        ...
        emit_state_db_backup_warning(&format!(
            "DeepSeek local database at {} appears damaged. Moving it into a backup folder so the app server can rebuild it from saved data.",
            database_path.display()
        ));
        let backups = deepseek_state::backup_runtime_db_for_fresh_start(database_path.as_path())
            .await
            .map_err(|backup_err| {
                anyhow::anyhow!(
        ...
        for backup in &backups {
            emit_state_db_backup_warning(&format!(
                "Moved damaged DeepSeek local database file {} to {}",
...
mod tests {
    ...
    fn debug_test_user_config_file_overrides_loader_path() {
        let path = std::env::temp_dir().join("deepseek-app-server-test-config.toml");
        let loader_overrides = loader_overrides_with_test_user_config_file(
