...
use deepseek_cloud_config::cloud_config_bundle_loader;
...
use deepseek_utils_json_to_toml::json_to_toml;
...
/// Shared app-server entry point for loading effective DeepSeek configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) fn deepseek_home(&self) -> &Path {
        self.deepseek_home.as_path()
    }

    pub(crate) fn user_config_path(&self) -> std::io::Result<AbsolutePathBuf> {
        self.loader_overrides.user_config_path(self.deepseek_home())
    }
    ...
    pub(crate) fn replace_cloud_config_bundle_loader(
        ...
        let loader =
            cloud_config_bundle_loader(auth_manager, chatgpt_base_url, self.deepseek_home.clone());
    ...
    fn current_thread_config_loader(&self) -> Arc<dyn ThreadConfigLoader> {
        self.thread_config_loader
            ...
            .unwrap_or_else(|_| Arc::new(deepseek_config::NoopThreadConfigLoader))
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        let mut config = Config::load_default_with_cli_overrides_for_deepseek_home(
        ...
        {
            let user_config_path = self.loader_overrides.user_config_path(self.deepseek_home())?;
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let mut config = deepseek_core::config::ConfigBuilder::default()
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        ...
        config.deepseek_linux_sandbox_exe = self.arg0_paths.deepseek_linux_sandbox_exe.clone();
    ...
    pub(crate) fn new_for_tests(
        ...
        Self::new(
            ...
            Arc::new(deepseek_config::NoopThreadConfigLoader),
        )
    }

    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(deepseek_home: PathBuf) -> Self {
        Self::new_for_tests(
            deepseek_home,
            Vec::new(),
