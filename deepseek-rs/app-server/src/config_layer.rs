use deepseek_app_server_protocol::ConfigLayer as ApiConfigLayer;
use deepseek_app_server_protocol::ConfigLayerMetadata as ApiConfigLayerMetadata;
use deepseek_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
use deepseek_config::ConfigLayer;
use deepseek_config::ConfigLayerMetadata;
use deepseek_config::ConfigLayerSource;

/// Converts a config-layer source owned by `deepseek-config` into the app-server wire type owned by
/// `deepseek-app-server-protocol`.
///
...
/// Converts config-layer metadata owned by `deepseek-config` into the app-server wire type owned by
/// `deepseek-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `deepseek-config` into the app-server wire type owned by
/// `deepseek-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
