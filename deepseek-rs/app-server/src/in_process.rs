...
//!
//! # Relationship to `deepseek-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `deepseek-app-server-client`,
...
use deepseek_app_server_protocol::Result;
...
pub use deepseek_rollout::StateDbHandle;
pub use deepseek_state::log_db::LogDbLayer;
use tokio::sync::mpsc;
...
/// through `deepseek-app-server-client`, which adds worker-task buffering,
...
pub struct InProcessClientHandle {
    ...
    #[cfg(test)]
    _test_deepseek_home: Option<tempfile::TempDir>,
}
...
async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    let installation_id = resolve_installation_id(&args.config.deepseek_home).await?;
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let auth_manager =
            AuthManager::shared_from_config(args.config.as_ref(), args.enable_deepseek_api_key_env)
                .await;
        ...
        let config_manager = ConfigManager::new(
            args.config.deepseek_home.to_path_buf(),
            args.cli_overrides,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(deepseek_home.path()).await);
        let state_db = deepseek_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        client._test_deepseek_home = Some(deepseek_home);
        client
    }
