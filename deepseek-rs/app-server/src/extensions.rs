...
use deepseek_extension_api::AgentSpawnFuture;
use deepseek_extension_api::AgentSpawner;
...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn deepseek_skills_extension::SkillProvider>,
...
where
    S: AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = DeepSeekErr> + 'static,
{
    ...
    if let Some(state_db) = state_db {
        deepseek_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            deepseek_otel::global(),
            thread_manager,
            goal_service,
            |config: &Config| config.features.enabled(deepseek_features::Feature::Goals),
        );
    }
    deepseek_guardian::install(&mut builder, guardian_agent_spawner);
    deepseek_memories_extension::install(&mut builder, deepseek_otel::global());
    deepseek_mcp_extension::install(&mut builder);
    deepseek_mcp_extension::install_executor_plugins(&mut builder, environment_manager);
    deepseek_web_search_extension::install(&mut builder, auth_manager.clone());
    deepseek_image_generation_extension::install(&mut builder, auth_manager, |config: &Config| {
        Some(config.deepseek_home.clone())
    });
    let skill_providers = deepseek_skills_extension::SkillProviders::new()
        .with_executor_provider(executor_skill_provider)
        .with_orchestrator_provider(Arc::new(
            deepseek_skills_extension::OrchestratorSkillProvider::new(),
        ));
    deepseek_skills_extension::install_with_providers(
        &mut builder,
        skill_providers,
        |config: &Config| deepseek_skills_extension::SkillsExtensionConfig {
...
) -> impl AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = DeepSeekErr> {
    move |forked_from_thread_id: ThreadId,
          options: StartThreadOptions|
          -> AgentSpawnFuture<'static, NewThread, DeepSeekErr> {
        let thread_manager = thread_manager.clone();
        Box::pin(async move {
            let thread_manager = thread_manager.upgrade().ok_or_else(|| {
                DeepSeekErr::UnsupportedOperation("thread manager dropped".to_string())
            })?;
            thread_manager
...
mod tests {
    ...
    use deepseek_protocol::protocol::ThreadGoal as CoreThreadGoal;
