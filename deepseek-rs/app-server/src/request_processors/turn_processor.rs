...
use deepseek_protocol::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use deepseek_protocol::protocol::AdditionalContextKind as CoreAdditionalContextKind;
...
struct ThreadSettingsBuildParams {
    ...
    sandbox_policy: Option<deepseek_app_server_protocol::SandboxPolicy>,
...
impl TurnRequestProcessor {
    ...
    pub(crate) async fn turn_start(
        ...
        self.turn_start_inner(
            ...
            /*supports_deepseek_form_elicitation*/ supports_deepseek_form_elicitation,
        )
        .await
    ...
    fn review_request_from_target(
        ...
        let hint = deepseek_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        thread
            .set_deepseek_form_elicitation_support(supports_deepseek_form_elicitation)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to update DeepSeek form elicitation support: {err}"
                ))
            })?;
        ...
        if turn_has_input {
            ...
            deepseek_memories_write::start_memories_startup_task(
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<deepseek_protocol::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        let approval_policy =
            approval_policy.map(deepseek_app_server_protocol::AskForApproval::to_core);
        let approvals_reviewer =
            approvals_reviewer.map(deepseek_app_server_protocol::ApprovalsReviewer::to_core);
        ...
        if has_any_overrides {
            thread
                .preview_thread_settings_overrides(DeepSeekThreadSettingsOverrides {
        ...
        Ok(deepseek_protocol::protocol::ThreadSettingsOverrides {
            environments,
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != deepseek_protocol::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
                ...
                let (message, data, error_type) = match err {
                    ...
                    SteerInputError::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            deepseek_protocol::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            deepseek_protocol::protocol::NonSteerableTurnKind::Compact => (
    ...
    async fn prepare_realtime_conversation_thread(
        ...
        request_id: &ConnectionRequestId,
        thread_id: &str,
    ) -> Result<Option<(ThreadId, Arc<DeepSeekThread>)>, JSONRPCErrorError> {
    ...
    async fn start_inline_review(
        &self,
        request_id: &ConnectionRequestId,
        parent_thread: Arc<DeepSeekThread>,
