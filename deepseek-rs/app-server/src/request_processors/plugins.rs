...
use deepseek_app_server_protocol::PluginAvailability;
use deepseek_app_server_protocol::PluginInstallPolicy;
use deepseek_app_server_protocol::PluginSharePrincipalRole;
use deepseek_app_server_protocol::PluginShareTargetRole;
use deepseek_config::types::McpServerConfig;
use deepseek_core_plugins::OPENAI_CURATED_MARKETPLACE_NAME;
use deepseek_core_plugins::PluginListBackgroundTaskOptions;
use deepseek_core_plugins::is_deepseek_curated_marketplace_name;
use deepseek_core_plugins::remote::REMOTE_CREATED_BY_ME_MARKETPLACE_NAME;
use deepseek_core_plugins::remote::REMOTE_GLOBAL_MARKETPLACE_NAME;
use deepseek_core_plugins::remote::REMOTE_WORKSPACE_MARKETPLACE_NAME;
use deepseek_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_MARKETPLACE_NAME;
use deepseek_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_PRIVATE_MARKETPLACE_NAME;
use deepseek_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_UNLISTED_MARKETPLACE_NAME;
use deepseek_core_plugins::remote::RemoteAppTemplateUnavailableReason;
use deepseek_core_plugins::remote::is_valid_remote_plugin_id;
use deepseek_core_plugins::remote::validate_remote_plugin_id;
use deepseek_core_plugins::remote_bundle::RemotePluginBundleInstallError;
use deepseek_mcp::McpOAuthLoginSupport;
use deepseek_mcp::oauth_login_support;
use deepseek_mcp::should_retry_without_scopes;
use deepseek_plugin::PluginId;
use deepseek_plugin::PluginTelemetryMetadata;
use deepseek_protocol::auth::AuthMode as DomainAuthMode;
use deepseek_rmcp_client::perform_oauth_login_silent;
...
fn plugin_skills_to_info(
    skills: &[deepseek_core::skills::SkillMetadata],
    ...
    skills
        ...
        .map(|skill| SkillSummary {
            ...
            interface: skill.interface.clone().map(|interface| {
                deepseek_app_server_protocol::SkillInterface {
...
fn load_shared_plugin_ids_by_local_path(
    ...
    deepseek_core_plugins::remote::load_plugin_share_remote_ids_by_local_path(
        config.deepseek_home.as_path(),
    )
    .map_err(|err| {
...
fn convert_configured_marketplace_plugin_to_plugin_summary(
    plugin: deepseek_core_plugins::ConfiguredMarketplacePlugin,
    shared_plugin_ids_by_local_path: &std::collections::BTreeMap<AbsolutePathBuf, String>,
) -> PluginSummary {
...
fn filter_deepseek_curated_installed_conflicts(
    ...
    let local_installed_plugin_names = marketplaces
        .iter()
        .filter(|marketplace| is_deepseek_curated_marketplace_name(&marketplace.name))
    ...
    for marketplace in marketplaces.iter_mut() {
        if prefer_remote_curated_conflicts {
            if !is_deepseek_curated_marketplace_name(&marketplace.name) {
                continue;
            }
...
) -> deepseek_core_plugins::remote::RemotePluginShareDiscoverability {
    match discoverability {
        PluginShareDiscoverability::Listed => {
            deepseek_core_plugins::remote::RemotePluginShareDiscoverability::Listed
        }
        PluginShareDiscoverability::Unlisted => {
            deepseek_core_plugins::remote::RemotePluginShareDiscoverability::Unlisted
        }
        PluginShareDiscoverability::Private => {
            deepseek_core_plugins::remote::RemotePluginShareDiscoverability::Private
        }
    }
}
...
) -> deepseek_core_plugins::remote::RemotePluginShareUpdateDiscoverability {
    match discoverability {
        PluginShareUpdateDiscoverability::Unlisted => {
            deepseek_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Unlisted
        }
        PluginShareUpdateDiscoverability::Private => {
            deepseek_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Private
        }
    }
}
...
) -> deepseek_core_plugins::remote::RemotePluginShareTargetRole {
    match role {
        PluginShareTargetRole::Reader => {
            deepseek_core_plugins::remote::RemotePluginShareTargetRole::Reader
        }
        PluginShareTargetRole::Editor => {
            deepseek_core_plugins::remote::RemotePluginShareTargetRole::Editor
        }
    }
}

fn plugin_share_principal_role_from_remote(
    role: deepseek_core_plugins::remote::RemotePluginSharePrincipalRole,
) -> PluginSharePrincipalRole {
    match role {
        deepseek_core_plugins::remote::RemotePluginSharePrincipalRole::Reader => {
            PluginSharePrincipalRole::Reader
        }
        deepseek_core_plugins::remote::RemotePluginSharePrincipalRole::Editor => {
            PluginSharePrincipalRole::Editor
        }
        deepseek_core_plugins::remote::RemotePluginSharePrincipalRole::Owner => {
...
) -> Vec<deepseek_core_plugins::remote::RemotePluginShareTarget> {
    targets
        .into_iter()
        .map(
            |target| deepseek_core_plugins::remote::RemotePluginShareTarget {
                principal_type: match target.principal_type {
                    PluginSharePrincipalType::User => {
                        deepseek_core_plugins::remote::RemotePluginSharePrincipalType::User
                    }
                    PluginSharePrincipalType::Group => {
                        deepseek_core_plugins::remote::RemotePluginSharePrincipalType::Group
                    }
                    PluginSharePrincipalType::Workspace => {
                        deepseek_core_plugins::remote::RemotePluginSharePrincipalType::Workspace
                    }
                },
...
fn plugin_share_principal_from_remote(
    principal: deepseek_core_plugins::remote::RemotePluginSharePrincipal,
...
impl PluginRequestProcessor {
    ...
    async fn workspace_deepseek_plugins_enabled(
        &self,
        config: &Config,
        auth: Option<&DeepSeekAuth>,
    ) -> bool {
        match workspace_settings::deepseek_plugins_enabled_for_workspace(
            config,
            auth,
        ...
        .await
        {
            ...
            Err(err) => {
                warn!(
                    "failed to fetch workspace DeepSeek plugins setting; allowing DeepSeek plugins: {err:#}"
                );
                true
            }
        }
    ...
    async fn plugin_list_response(
        ...
        if !self
            .workspace_deepseek_plugins_enabled(&config, auth.as_ref())
            .await
        {
            return Ok(empty_response());
        }
        let auth_mode = auth.as_ref().map(DeepSeekAuth::api_auth_mode);
        ...
        let refresh_global_remote_catalog_cache = use_remote_global_catalog
            && deepseek_core_plugins::remote::has_cached_global_remote_plugin_catalog(
                config.deepseek_home.as_path(),
            ...
            match tokio::task::spawn_blocking(move || {
                let outcome = plugins_manager_for_marketplace_listing
                    .list_marketplaces_for_config(
                        ...
                        /*include_deepseek_curated*/ !use_remote_global_catalog,
                    )?;
                Ok::<
                    (
                        Vec<PluginMarketplaceEntry>,
                        Vec<deepseek_app_server_protocol::MarketplaceLoadErrorInfo>,
                    ),
                    MarketplaceError,
                >((
                    outcome
                    ...
                    outcome
                        .errors
                        .into_iter()
                        .map(|err| deepseek_app_server_protocol::MarketplaceLoadErrorInfo {
        ...
        if include_vertical && !config.features.enabled(Feature::RemotePlugin) {
            match deepseek_core_plugins::remote::fetch_deepseek_curated_remote_collection_marketplace(
                &remote_plugin_service_config,
                auth.as_ref(),
            )
            .await
            {
                ...
                Err(err) if explicit_marketplace_kinds => {
                    return Err(remote_plugin_catalog_error_to_jsonrpc(
                        err,
                        "list DeepSeek Curated remote plugin catalog",
                    ));
                }
                ...
                Err(err) => {
                    warn!(
                        error = %err,
                        "plugin/list deepseek-curated-remote collection fetch failed; returning local marketplaces only"
                    );
                }
            }
        }
        ...
        if !remote_sources.is_empty() {
            match deepseek_core_plugins::remote::fetch_remote_marketplaces(
                &remote_plugin_service_config,
                auth.as_ref(),
                &remote_sources,
                /*global_catalog_cache_path*/ Some(config.deepseek_home.as_path()),
            )
            .await
            {
    ...
    async fn plugin_installed_response(
        ...
        plugins_manager.set_auth_mode(auth.as_ref().map(DeepSeekAuth::api_auth_mode));
        ...
        filter_deepseek_curated_installed_conflicts(
            &mut data,
            config.features.enabled(Feature::RemotePlugin),
        );
    ...
    async fn load_local_installed_and_suggested_plugins(
        &self,
        plugins_manager: Arc<deepseek_core_plugins::PluginsManager>,
        config: &Config,
        plugins_input: &deepseek_core_plugins::PluginsConfigInput,
    ...
    async fn plugin_read_response(
        ...
        let plugin = match read_source {
            Ok(marketplace_path) => {
                ...
                let share_context = match share_context {
                    Some(context) => {
                        ...
                        match deepseek_core_plugins::remote::fetch_remote_plugin_share_context(
                            &remote_plugin_service_config,
                            auth.as_ref(),
                ...
                PluginDetail {
                    ...
                    hooks: outcome
                        ...
                        .hooks
                        .into_iter()
                        .map(|hook| deepseek_app_server_protocol::PluginHookSummary {
                            key: hook.key,
            ...
            Err(remote_marketplace_name) => {
                ...
                let remote_detail = deepseek_core_plugins::remote::fetch_remote_plugin_detail(
                    &remote_plugin_service_config,
                    auth.as_ref(),
                    &remote_marketplace_name,
                    &plugin_name,
                ...
                let plugin_apps = remote_detail
                    ...
                    .iter()
                    .cloned()
                    .map(deepseek_plugin::AppConnectorId)
                    .collect::<Vec<_>>();
    ...
    async fn plugin_skill_read_response(
        ...
        let remote_skill_detail = deepseek_core_plugins::remote::fetch_remote_plugin_skill_detail(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_marketplace_name,
            &remote_plugin_id,
    ...
    async fn plugin_share_save_response(
        ...
        let access_policy = deepseek_core_plugins::remote::RemotePluginShareAccessPolicy {
        ...
        let result = deepseek_core_plugins::remote::save_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.deepseek_home.as_path(),
            &plugin_path,
    ...
    async fn plugin_share_update_targets_response(
        ...
        let result = deepseek_core_plugins::remote::update_remote_plugin_share_targets(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_plugin_id,
    ...
    async fn plugin_share_list_response(
        ...
        let data = deepseek_core_plugins::remote::list_remote_plugin_shares(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.deepseek_home.as_path(),
        )
    ...
    async fn plugin_share_checkout_response(
        ...
        let result = deepseek_core_plugins::remote::checkout_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.deepseek_home.as_path(),
            &remote_plugin_id,
    ...
    async fn plugin_share_delete_response(
        ...
        deepseek_core_plugins::remote::delete_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.deepseek_home.as_path(),
            &remote_plugin_id,
    ...
    async fn load_plugin_share_config_and_auth(
        &self,
    ) -> Result<(Config, Option<DeepSeekAuth>), JSONRPCErrorError> {
        let config = self.load_latest_config(/*fallback_cwd*/ None).await?;
        if !config.features.enabled(Feature::Plugins) {
    ...
    async fn plugin_install_response(
        ...
        {
            return Err(invalid_request(
                "DeepSeek plugins are disabled for this workspace",
            ));
        }
        ...
        let plugin_mcp_servers = load_plugin_mcp_servers(
            ...
            auth.as_ref().map(DeepSeekAuth::auth_mode),
        )
        .await;
        ...
        let plugin_apps =
            deepseek_plugin::app_connector_ids_from_declarations(&plugin_app_declarations);
        let apps_needing_auth = self
            .plugin_apps_needing_auth_for_install(
                &config,
                auth.as_ref().is_some_and(DeepSeekAuth::is_chatgpt_auth),
    ...
    async fn remote_plugin_install_response(
        ...
        let remote_detail =
            deepseek_core_plugins::remote::fetch_remote_plugin_detail_with_download_urls(
                &remote_plugin_service_config,
                auth.as_ref(),
                &remote_marketplace_name,
                &remote_plugin_id,
        ...
        let _remote_plugin_cache_mutation =
            deepseek_core_plugins::remote::mark_remote_plugin_cache_mutation_in_flight(
                config.deepseek_home.as_path(),
                &actual_remote_marketplace_name,
                &remote_plugin_name,
            );
        let validated_bundle = deepseek_core_plugins::remote_bundle::validate_remote_plugin_bundle(
            &remote_plugin_id,
            &actual_remote_marketplace_name,
            &remote_plugin_name,
        ...
        let result = deepseek_core_plugins::remote_bundle::download_and_install_remote_plugin_bundle(
        ...
        let install_result = deepseek_core_plugins::remote::install_remote_plugin(
            &remote_plugin_service_config,
            auth.as_ref(),
            &actual_remote_marketplace_name,
            &remote_plugin_id,
        ...
        let is_chatgpt_auth = auth.as_ref().is_some_and(DeepSeekAuth::is_chatgpt_auth);
    ...
    async fn plugin_apps_needing_auth_for_install(
        ...
        plugin_id: &str,
        plugin_apps: &[deepseek_plugin::AppConnectorId],
    ) -> Vec<AppSummary> {
    ...
    async fn plugin_uninstall_response(
        ...
        if deepseek_plugin::PluginId::parse(&plugin_id).is_err()
    ...
    async fn remote_plugin_uninstall_response(
        ...
        let uninstall_target = deepseek_core_plugins::remote::resolve_remote_plugin_uninstall_target(
            &remote_plugin_service_config,
            auth.as_ref(),
            &plugin_id,
        )
        ...
        let uninstall_result = deepseek_core_plugins::remote::uninstall_remote_plugin(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.deepseek_home.to_path_buf(),
            uninstall_target,
...
fn plugin_app_category_by_id_from_value(value: &serde_json::Value) -> HashMap<String, String> {
    deepseek_core_plugins::loader::plugin_app_declarations_from_value(value)
        .into_iter()
...
fn remote_plugin_share_discoverability_to_info(
    discoverability: deepseek_core_plugins::remote::RemotePluginShareDiscoverability,
...
fn remote_plugin_bundle_install_error_to_jsonrpc(
    err: deepseek_core_plugins::remote_bundle::RemotePluginBundleInstallError,
) -> JSONRPCErrorError {
