...
use deepseek_external_agent_sessions::CompletedExternalAgentSessionImport;
use deepseek_external_agent_sessions::ExternalAgentSessionMigration;
use deepseek_external_agent_sessions::ImportedExternalAgentSession;
use deepseek_external_agent_sessions::PendingSessionImport;
use deepseek_external_agent_sessions::prepare_validated_session_import;
use deepseek_external_agent_sessions::record_completed_session_imports;
...
impl ExternalAgentSessionImporter {
    ...
    pub(super) async fn import_sessions(
        ...
        if let Err(err) = record_completed_session_imports(&self.deepseek_home, completed_imports) {
            record_import_error(
                &mut item_result,
    ...
    async fn prepare_session_import(
        ...
        tokio::task::spawn_blocking(move || prepare_validated_session_import(&deepseek_home, session))
            .await
    ...
    async fn persist_session(
        ...
        let create_params = CreateThreadParams {
            ...
            source: source.clone(),
            thread_source: None,
            originator: deepseek_login::default_client::originator().value,
