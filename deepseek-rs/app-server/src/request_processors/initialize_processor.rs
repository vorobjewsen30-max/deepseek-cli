...
use deepseek_login::default_client::SetOriginatorError;
...
const NON_ORIGINATING_CLIENT_NAMES: &[&str] = &["deepseek_app_server_daemon", "codex-backend"];
...
impl InitializeRequestProcessor {
    ...
    pub(crate) async fn initialize(
        ...
        let supports_deepseek_form_elicitation = capabilities.mcp_server_deepseek_form_elicitation;
        ...
        let deepseek_home = self.config.deepseek_home.clone();
        if session
            ...
            if let Err(error) = set_default_originator(originator.clone()) {
                match error {
                    ...
                    SetOriginatorError::AlreadyInitialized => {
                        ...
                        // TODO(owen): Once we remove support for DEEPSEEK_INTERNAL_ORIGINATOR_OVERRIDE,
