...
use deepseek_app_server_protocol::SelectedCapabilityRoot;
...
const CODEX_TUI_CLIENT_NAME: &str = "deepseek-tui";
...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: deepseek_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                deepseek_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = deepseek_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<DeepSeekThread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            deepseek_home: self.config.deepseek_home.to_path_buf(),
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<deepseek_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<deepseek_app_server_protocol::ThreadStartSource>,
            ...
            && (requested_permissions_trust_project || effective_permissions_trust_project)
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                deepseek_core::config::set_project_trust_level(
                    &listener_task_context.deepseek_home,
        ...
        let NewThread {
            ...
            .start_thread_with_options(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(deepseek_app_server_protocol::ThreadStartSource::Startup)
                {
                    deepseek_app_server_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    deepseek_app_server_protocol::ThreadStartSource::Clear => InitialHistory::Cleared,
                },
                history_mode,
                session_source: None,
                thread_source,
            ...
            .map_err(|err| match err {
                ...
                DeepSeekErr::UnsupportedOperation(message) => method_not_found(message),
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<deepseek_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<deepseek_app_server_protocol::ApprovalsReviewer>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(deepseek_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(deepseek_app_server_protocol::ApprovalsReviewer::to_core),
            sandbox_mode: sandbox.map(SandboxMode::to_core),
            deepseek_linux_sandbox_exe: self.arg0_paths.deepseek_linux_sandbox_exe.clone(),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = deepseek_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.deepseek_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.deepseek_home.display()
                ))
            })?;
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(deepseek_core::util::normalize_thread_name);
        let history_items = source_thread
            .history
            .take()
            .map(|history| history.items)
        ...
        let NewThread {
            ...
            .await
            .map_err(|err| match err {
                DeepSeekErr::Io(_) | DeepSeekErr::Json(_) => {
...
) -> Result<deepseek_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    build_thread_turns_page_response(
        items,
        loaded_status,
        has_live_running_thread,
...
pub(super) fn core_thread_write_error(operation: &str, err: DeepSeekErr) -> JSONRPCErrorError {
    match err {
...
) -> (Thread, Option<deepseek_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<deepseek_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(deepseek_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match deepseek_core::parse_turn_item(item) {
                Some(deepseek_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn requested_permissions_trust_project(overrides: &ConfigOverrides, cwd: &Path) -> bool {
    if matches!(
        ...
        Some(
            deepseek_protocol::config_types::SandboxMode::WorkspaceWrite
                | deepseek_protocol::config_types::SandboxMode::DangerFullAccess
        )
    ) {
        return true;
    }
...
fn permission_profile_trusts_project(
    profile: &deepseek_protocol::models::PermissionProfile,
    cwd: &Path,
) -> bool {
    match profile {
        deepseek_protocol::models::PermissionProfile::Disabled
        | deepseek_protocol::models::PermissionProfile::External { .. } => true,
        deepseek_protocol::models::PermissionProfile::Managed { .. } => profile
