...
// Duration before a browser вход через DeepSeek attempt is abandoned.
...
impl AccountRequestProcessor {
    ...
    fn current_account_updated_notification(&self) -> AccountUpdatedNotification {
        ...
        AccountUpdatedNotification {
            auth_mode: auth
                .as_ref()
                .map(DeepSeekAuth::api_auth_mode)
                .map(auth_mode_to_api),
            plan_type: auth.as_ref().and_then(DeepSeekAuth::account_plan_type),
        }
    }
    ...
    async fn maybe_refresh_plugin_caches_for_current_config(
        ...
        thread_manager
            .plugins_manager()
            .set_auth_mode(auth.as_ref().map(DeepSeekAuth::api_auth_mode));
        thread_manager
            .plugins_manager()
    ...
    // Build options for a вход через DeepSeek attempt; performs validation.
    ...
    async fn login_chatgpt_auth_tokens_response(
        ...
        if matches!(
            ...
            return Err(invalid_request(
                "External DeepSeek auth is disabled. Use API key login instead.",
            ));
        }
    ...
    async fn get_auth_status_response(
        ...
        // If a custom provider is configured with `requires_deepseek_auth == false`,
        // then no auth step is required; otherwise, default to requiring auth.
        let requires_deepseek_auth = self.config.model_provider.requires_deepseek_auth;

        let response = if !requires_deepseek_auth {
    ...
    async fn get_account_response(
        ...
        Ok(GetAccountResponse {
            account,
            requires_deepseek_auth: account_state.requires_deepseek_auth,
        })
    }
...
mod tests {
    use super::*;
    use deepseek_backend_client::TokenUsageProfileDailyBucket;
    use deepseek_backend_client::TokenUsageProfileStats;
