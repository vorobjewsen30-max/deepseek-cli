...
use deepseek_app_server_protocol::ComputerUseRequirements;
use deepseek_app_server_protocol::ConfigBatchWriteParams;
use deepseek_app_server_protocol::ConfigReadParams;
use deepseek_app_server_protocol::ConfigReadResponse;
use deepseek_app_server_protocol::ConfigRequirements;
use deepseek_app_server_protocol::ConfigRequirementsReadResponse;
use deepseek_app_server_protocol::ConfigValueWriteParams;
use deepseek_app_server_protocol::ConfigWriteErrorCode;
use deepseek_app_server_protocol::ConfigWriteResponse;
use deepseek_app_server_protocol::ConfiguredHookHandler;
use deepseek_app_server_protocol::ConfiguredHookMatcherGroup;
use deepseek_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use deepseek_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use deepseek_app_server_protocol::JSONRPCErrorError;
use deepseek_app_server_protocol::ManagedHooksRequirements;
use deepseek_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use deepseek_app_server_protocol::ModelsRequirements;
use deepseek_app_server_protocol::NetworkDomainPermission;
use deepseek_app_server_protocol::NetworkRequirements;
use deepseek_app_server_protocol::NetworkUnixSocketPermission;
...
use deepseek_config::HookEventsToml;
use deepseek_config::HookHandlerConfig as CoreHookHandlerConfig;
use deepseek_config::ManagedHooksRequirementsToml;
use deepseek_config::MatcherGroup as CoreMatcherGroup;
use deepseek_config::ResidencyRequirement as CoreResidencyRequirement;
use deepseek_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use deepseek_core::ThreadManager;
use deepseek_features::canonical_feature_for_key;
use deepseek_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<deepseek_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = deepseek_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(deepseek_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(deepseek_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            deepseek_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            deepseek_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: deepseek_config::ComputerUseRequirementsToml,
...
) -> deepseek_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => deepseek_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: deepseek_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(deepseek_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(deepseek_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(deepseek_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: deepseek_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        deepseek_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        deepseek_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: deepseek_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        deepseek_config::NetworkUnixSocketPermissionToml::Allow => NetworkUnixSocketPermission::Allow,
        deepseek_config::NetworkUnixSocketPermissionToml::Deny => NetworkUnixSocketPermission::Deny,
    }
}
...
mod tests {
    ...
    use deepseek_app_server_protocol::WindowsSandboxSetupMode;
    use deepseek_config::ComputerUseRequirementsToml;
    use deepseek_config::ConfigRequirementsToml;
    use deepseek_config::ModelsRequirementsToml;
    use deepseek_config::NewThreadModelDefaultsToml;
    use deepseek_config::WindowsRequirementsToml;
