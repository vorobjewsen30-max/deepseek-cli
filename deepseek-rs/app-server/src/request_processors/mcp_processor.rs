...
impl McpRequestProcessor {
    ...
    async fn mcp_server_oauth_login_response(
        ...
        let effective_servers = deepseek_mcp::effective_mcp_servers(&mcp_config, auth.as_ref());
        let Some(server) = effective_servers
            .get(&name)
            .and_then(deepseek_mcp::EffectiveMcpServer::configured_config)
        else {
    ...
    async fn list_mcp_server_status_task(
        ...
        request_id: ConnectionRequestId,
        params: ListMcpServerStatusParams,
        mcp_config: deepseek_mcp::McpConfig,
        auth: Option<DeepSeekAuth>,
        runtime_context: McpRuntimeContext,
        codex_apps_tools_cache: deepseek_mcp::DeepSeekAppsToolsCache,
    ) {
