...
async fn write_value_preserves_comments_and_order() -> Result<()> {
    ...
    let original = r#"# DeepSeek user configuration
model = "gpt-5.2"
...
"#;
    ...
    let expected = r#"# DeepSeek user configuration
model = "gpt-5.2"
approval_policy = "on-request"
...
async fn batch_write_rejects_legacy_profile_selector() -> Result<()> {
    ...
    let error = service
        .batch_write(ConfigBatchWriteParams {
            edits: vec![
                deepseek_app_server_protocol::ConfigEdit {
...
async fn reserved_builtin_provider_override_rejected() {
    ...
    let error = service
        .write_value(ConfigValueWriteParams {
            file_path: Some(tmp.path().join(CONFIG_TOML_FILE).display().to_string()),
            key_path: "model_providers.deepseek.name".to_string(),
            value: serde_json::json!("DeepSeek Override"),
            merge_strategy: MergeStrategy::Replace,
            expected_version: None,
        })
        .await
