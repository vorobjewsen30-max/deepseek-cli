...
fn fixture_paths() -> (TempDir, PathBuf, PathBuf) {
    ...
    let deepseek_home = root.path().join(".codex");
    (root, external_agent_home, deepseek_home)
}
...
fn service_for_paths(
    ...
    ExternalAgentConfigService::new_for_test(deepseek_home, external_agent_home)
}
...
async fn detect_home_lists_config_skills_and_agents_md() {
    let (_root, external_agent_home, deepseek_home) = fixture_paths();
    let agents_skills = deepseek_home
        .parent()
    ...
    let items = service_for_paths(external_agent_home.clone(), deepseek_home.clone())
    ...
    let expected = vec![
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                deepseek_home.join("config.toml").display()
            ),
            cwd: None,
            details: None,
        },
        ...
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                deepseek_home.join("AGENTS.md").display()
            ),
            cwd: None,
            details: None,
        },
...
async fn detect_home_lists_recent_sessions() {
    let (root, external_agent_home, deepseek_home) = fixture_paths();
    ...
    let items = service_for_paths(external_agent_home.clone(), deepseek_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            cwds: None,
        })
...
async fn detect_repo_still_reports_non_plugin_items_when_home_config_is_invalid() {
    ...
    fs::write(deepseek_home.join("config.toml"), "this is not valid = [toml")
    ...
    let items = service_for_paths(root.path().join(EXTERNAL_AGENT_DIR), deepseek_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
...
"#,
    ...
    let _supported_mcp_config: std::collections::HashMap<
        String,
        deepseek_config::types::McpServerConfig,
    > = mcp_servers
        .try_into()
    ...
    .expect("parse hooks");
    let _supported_hooks: deepseek_config::HooksFile =
...
Research with DeepSeek carefully."""
"#,
    )
...
async fn import_home_migrates_supported_config_fields_skills_and_agents_md() {
    ...
    service_for_paths(external_agent_home, deepseek_home.clone())
        .import(vec![
            ExternalAgentConfigMigrationItem {
                item_type: ExternalAgentConfigMigrationItemType::AgentsMd,
                description: String::new(),
    ...
    assert_eq!(
        fs::read_to_string(deepseek_home.join("AGENTS.md")).expect("read agents"),
        "DeepSeek guidance"
    );

    let config: TomlValue =
        toml::from_str(&fs::read_to_string(deepseek_home.join("config.toml")).expect("read config"))
            .expect("parse config");
...
"#,
    ...
    assert_eq!(
        ...
        "Use DeepSeek and DeepSeek utilities."
    );
}
...
async fn import_home_config_ignores_invalid_local_settings() {
    ...
    assert_eq!(
        fs::read_to_string(deepseek_home.join("config.toml")).expect("read config"),
        "[shell_environment_policy]\ninherit = \"core\"\n\n[shell_environment_policy.set]\nFOO = \"project\"\n"
    );
}
...
async fn import_home_skips_empty_config_migration() {
    ...
    let outcome = service_for_paths(external_agent_home, deepseek_home.clone())
        .import(vec![ExternalAgentConfigMigrationItem {
            item_type: ExternalAgentConfigMigrationItemType::Config,
            description: String::new(),
            cwd: None,
    ...
    assert!(!deepseek_home.join("config.toml").exists());
}
...
async fn import_local_plugins_returns_completed_status() {
    ...
    fs::create_dir_all(plugin_root.join(".deepseek-plugin")).expect("create plugin manifest dir");
    fs::create_dir_all(&deepseek_home).expect("create codex home");
    ...
    let config = fs::read_to_string(deepseek_home.join("config.toml")).expect("read config");
...
async fn detect_home_skips_config_when_target_already_has_supported_fields() {
    ...
    let items = service_for_paths(external_agent_home, deepseek_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            cwds: None,
        })
...
#[tokio::test]
async fn import_repo_hooks_preserves_disabled_deepseek_hooks_feature() {
    let root = TempDir::new().expect("create tempdir");
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create external agent dir");
...
async fn import_plugins_requires_details() {
    ...
    let err = service_for_paths(external_agent_home, deepseek_home)
...
async fn detect_repo_filters_plugins_against_installed_marketplace() {
    ...
    let deepseek_home = root.path().join(".codex");
    let repo_root = root.path().join("repo");
    let marketplace_root = deepseek_home.join(".tmp").join("marketplaces").join("debug");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
...
async fn import_plugins_requires_source_marketplace_details() {
    ...
    let outcome = service_for_paths(external_agent_home, deepseek_home)
        .import_plugins(
            /*cwd*/ None,
...
fn import_skills_returns_only_new_skill_directory_names() {
    ...
    let copied_names = service_for_paths(external_agent_home, deepseek_home)
