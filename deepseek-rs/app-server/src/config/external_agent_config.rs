...
use deepseek_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use deepseek_core_plugins::marketplace::find_marketplace_manifest_path;
use deepseek_core_plugins::marketplace_add::MarketplaceAddRequest;
use deepseek_core_plugins::marketplace_add::add_marketplace;
use deepseek_core_plugins::marketplace_add::is_local_marketplace_source;
use deepseek_external_agent_migration::build_mcp_config_from_external;
use deepseek_external_agent_migration::count_missing_commands;
use deepseek_external_agent_migration::count_missing_subagents;
use deepseek_external_agent_migration::hook_migration_event_names;
use deepseek_external_agent_migration::import_commands;
use deepseek_external_agent_migration::import_hooks;
use deepseek_external_agent_migration::import_subagents;
use deepseek_external_agent_migration::missing_command_names;
use deepseek_external_agent_migration::missing_subagent_names;
use deepseek_external_agent_sessions::ExternalAgentSessionMigration;
use deepseek_external_agent_sessions::detect_recent_sessions;
...
impl ExternalAgentConfigService {
    pub(crate) fn new(deepseek_home: PathBuf) -> Self {
    ...
    #[cfg(test)]
    fn new_for_test(deepseek_home: PathBuf, external_agent_home: PathBuf) -> Self {
        Self {
            deepseek_home,
            external_agent_home,
        }
    ...
    async fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            || self.deepseek_home.join("config.toml"),
        ...
        let target_hooks = repo_root.map_or_else(
            || self.deepseek_home.join("hooks.json"),
        ...
        let target_subagents = repo_root.map_or_else(
            || self.deepseek_home.join("agents"),
        ...
        let target_agents_md = repo_root.map_or_else(
            || self.deepseek_home.join("AGENTS.md"),
        ...
        if let Some(settings) = settings.as_ref() {
            match ConfigBuilder::default()
                .deepseek_home(self.deepseek_home.clone())
                .fallback_cwd(Some(self.deepseek_home.clone()))
                .build()
                .await
            {
                Ok(config) => {
                    ...
                    let configured_marketplace_plugins = configured_marketplace_plugins(
                        &config,
                        &PluginsManager::new(self.deepseek_home.clone()),
                    )?;
        ...
        if repo_root.is_none() {
            let sessions = detect_recent_sessions(&self.external_agent_home, &self.deepseek_home)?;
    ...
    pub(crate) async fn import_plugins(
        ...
        let config = ConfigBuilder::default()
            ...
            .fallback_cwd(Some(
                cwd.map(Path::to_path_buf)
                    .unwrap_or_else(|| self.deepseek_home.clone()),
            ))
            .build()
            .await
        ...
        let plugins_manager = PluginsManager::new(self.deepseek_home.clone());
        for plugin_group in plugins {
            ...
            let add_marketplace_outcome =
                add_marketplace(self.deepseek_home.clone(), requirements.clone(), request).await;
...
fn configured_marketplace_plugins(
    ...
    let marketplaces = plugins_manager
        .list_marketplaces_for_config(&plugins_input, &[], /*include_deepseek_curated*/ true)
        .map_err(|err| {
        ...
        let plugins = marketplace
            ...
            .filter(|plugin| {
                plugin
                    ...
                    .products
                    .as_deref()
                    .is_none_or(|products| Product::DeepSeek.matches_product_restriction(products))
            })
...
fn rewrite_external_agent_terms(content: &str) -> String {
    ...
    for from in [
        ...
        rewritten = replace_case_insensitive_with_boundaries(&rewritten, from, "DeepSeek");
    }
    rewritten
}
