...
fn init_test_tracing() -> &'static TestTracing {
    ...
    TEST_TRACING.get_or_init(|| {
        ...
        let provider = SdkTracerProvider::builder()
            ...
            .build();
        let tracer = provider.tracer("deepseek-app-server-message-processor-tests");
...
impl TracingHarness {
    async fn new() -> Result<Self> {
        ...
        let config = Arc::new(build_test_config(deepseek_home.path(), &server.uri()).await?);
...
async fn build_test_config(deepseek_home: &Path, server_uri: &str) -> Result<Config> {
...
async fn build_test_processor(
    ...
    let auth_manager =
        AuthManager::shared_from_config(config.as_ref(), /*enable_deepseek_api_key_env*/ false).await;
    ...
    loop {
        ...
        match envelope {
            crate::outgoing_message::OutgoingEnvelope::ToConnection {
                ...
                if matches!(
                    notification,
                    deepseek_app_server_protocol::ServerNotification::ThreadStarted(_)
                ) {
                    return;
                }
            }
