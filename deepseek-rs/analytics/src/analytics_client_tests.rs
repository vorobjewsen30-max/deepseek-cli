...
use crate::events::DeepSeekAcceptedLineFingerprintsEventParams;
use crate::events::DeepSeekAcceptedLineFingerprintsEventRequest;
use crate::events::DeepSeekAppMentionedEventRequest;
use crate::events::DeepSeekAppServerClientMetadata;
use crate::events::DeepSeekAppUsedEventRequest;
use crate::events::DeepSeekCommandExecutionEventParams;
use crate::events::DeepSeekCommandExecutionEventRequest;
use crate::events::DeepSeekCompactionEventRequest;
use crate::events::DeepSeekHookRunEventRequest;
use crate::events::DeepSeekOnboardingExternalAgentImportFailureEventRequest;
use crate::events::DeepSeekOnboardingExternalAgentImportFailureMetadata;
use crate::events::DeepSeekPluginEventRequest;
use crate::events::DeepSeekPluginInstallFailedEventRequest;
use crate::events::DeepSeekPluginInstallFailedMetadata;
use crate::events::DeepSeekPluginUsedEventRequest;
use crate::events::DeepSeekReviewEventParams;
use crate::events::DeepSeekReviewEventRequest;
use crate::events::DeepSeekRuntimeMetadata;
use crate::events::DeepSeekToolItemEventBase;
use crate::events::DeepSeekTurnEventRequest;
...
use crate::events::deepseek_plugin_metadata;
use crate::events::deepseek_plugin_used_metadata;
...
use crate::facts::TurnDeepSeekErrorFact;
...
use deepseek_app_server_protocol::ApprovalsReviewer as AppServerApprovalsReviewer;
use deepseek_app_server_protocol::AskForApproval as AppServerAskForApproval;
...
use deepseek_app_server_protocol::CollabAgentTool;
use deepseek_app_server_protocol::CollabAgentToolCallStatus;
use deepseek_app_server_protocol::CommandAction;
...
use deepseek_app_server_protocol::GuardianApprovalReview;
use deepseek_app_server_protocol::GuardianApprovalReviewAction;
use deepseek_app_server_protocol::GuardianApprovalReviewStatus;
use deepseek_app_server_protocol::GuardianCommandSource as AppServerGuardianCommandSource;
use deepseek_app_server_protocol::InitializeCapabilities;
use deepseek_app_server_protocol::InitializeParams;
use deepseek_app_server_protocol::ItemCompletedNotification;
use deepseek_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification;
use deepseek_app_server_protocol::ItemStartedNotification;
use deepseek_app_server_protocol::JSONRPCErrorError;
use deepseek_app_server_protocol::McpToolCallStatus;
use deepseek_app_server_protocol::NonSteerableTurnKind;
use deepseek_app_server_protocol::PatchApplyStatus;
use deepseek_app_server_protocol::PermissionsRequestApprovalParams;
use deepseek_app_server_protocol::RequestId;
use deepseek_app_server_protocol::RequestPermissionProfile;
use deepseek_app_server_protocol::SandboxPolicy as AppServerSandboxPolicy;
...
use deepseek_app_server_protocol::SessionSource as AppServerSessionSource;
use deepseek_app_server_protocol::SubAgentActivityKind;
...
use deepseek_app_server_protocol::ThreadSource as AppServerThreadSource;
use deepseek_app_server_protocol::ThreadStartResponse;
use deepseek_app_server_protocol::ThreadStatus as AppServerThreadStatus;
...
use deepseek_app_server_protocol::TurnError as AppServerTurnError;
use deepseek_app_server_protocol::TurnStartParams;
use deepseek_app_server_protocol::TurnStartedNotification;
use deepseek_app_server_protocol::TurnStatus as AppServerTurnStatus;
...
use deepseek_login::default_client::DEFAULT_ORIGINATOR;
use deepseek_login::default_client::originator;
use deepseek_plugin::AppConnectorId;
use deepseek_plugin::PluginCapabilitySummary;
...
use deepseek_protocol::models::PermissionProfile as CorePermissionProfile;
...
fn sample_app_server_client_metadata() -> DeepSeekAppServerClientMetadata {
    DeepSeekAppServerClientMetadata {
...
fn sample_runtime_metadata() -> DeepSeekRuntimeMetadata {
...
fn sample_turn_start_response(turn_id: &str) -> ClientResponsePayload {
    ClientResponsePayload::TurnStart(deepseek_app_server_protocol::TurnStartResponse {
        turn: Turn {
            id: turn_id.to_string(),
            items_view: deepseek_app_server_protocol::TurnItemsView::Full,
            items: vec![],
...
fn sample_turn_completed_notification(
    ...
    codex_error_info: Option<deepseek_app_server_protocol::DeepSeekErrorInfo>,
) -> ServerNotification {
    ...
    reducer
        .ingest(
            AnalyticsFact::Initialize {
                ...
                product_client_id: "deepseek-tui".to_string(),
                runtime: sample_runtime_metadata(),
                rpc_transport: AppServerRpcTransport::Stdio,
            },
            out,
    ...
    AnalyticsFact::Initialize {
        ...
        params: InitializeParams {
            ...
            capabilities: Some(InitializeCapabilities {
                ...
                mcp_server_deepseek_form_elicitation: false,
            }),
        },
        product_client_id: DEFAULT_ORIGINATOR.to_string(),
    ...
    ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: RequestPermissionProfile {
                network: Some(deepseek_app_server_protocol::AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: None,
            },
...
fn sample_guardian_review_completed(
    ...
    ServerNotification::ItemGuardianApprovalReviewCompleted(
        ItemGuardianApprovalReviewCompletedNotification {
            ...
            decision_source: deepseek_app_server_protocol::AutoReviewDecisionSource::Agent,
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(DeepSeekAppMentionedEventRequest {
...
fn app_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppUsed(DeepSeekAppUsedEventRequest {
...
fn accepted_line_fingerprints_event_serializes_expected_shape() {
    let event = TrackEventRequest::AcceptedLineFingerprints(Box::new(
        DeepSeekAcceptedLineFingerprintsEventRequest {
            event_type: "codex_accepted_line_fingerprints",
            event_params: DeepSeekAcceptedLineFingerprintsEventParams {
...
fn compaction_event_serializes_expected_shape() {
    let event = TrackEventRequest::Compaction(Box::new(DeepSeekCompactionEventRequest {
        event_type: "codex_compaction_event",
        event_params: crate::events::codex_compaction_event_params(
            DeepSeekCompactionEvent {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
...
fn command_execution_event_serializes_expected_shape() {
    let event = TrackEventRequest::CommandExecution(DeepSeekCommandExecutionEventRequest {
        event_type: "codex_command_execution_event",
        event_params: DeepSeekCommandExecutionEventParams {
            base: DeepSeekToolItemEventBase {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                item_id: "item-1".to_string(),
                app_server_client: DeepSeekAppServerClientMetadata {
                    product_client_id: "deepseek_tui".to_string(),
                    client_name: Some("deepseek-tui".to_string()),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "item_id": "item-1",
                "app_server_client": {
                    "product_client_id": "deepseek_tui",
                    "client_name": "deepseek-tui",
...
fn review_event_serializes_expected_shape() {
    let event = TrackEventRequest::ReviewEvent(DeepSeekReviewEventRequest {
        event_type: "codex_review_event",
        event_params: DeepSeekReviewEventParams {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            item_id: None,
...
async fn compaction_event_ingests_custom_fact() {
    ...
    let parent_thread_id =
        deepseek_protocol::ThreadId::from_string("22222222-2222-2222-2222-222222222222")
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(Box::new(
                DeepSeekCompactionEvent {
                    ...
                    codex_error_kind: Some(DeepSeekErrKind::ContextWindowExceeded),
                    codex_error_http_status_code: None,
                    active_context_tokens_before: 131_000,
...
async fn guardian_review_event_ingests_custom_fact_with_optional_target_item() {
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::GuardianReview(Box::new(
                GuardianReviewEventParams {
                    ...
                    guardian_model_provider_id: Some("deepseek".to_string()),
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "deepseek_guardian_review");
    assert_eq!(
...
fn subagent_thread_started_review_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            ...
            forked_from_thread_id: None,
            product_client_id: "deepseek-tui".to_string(),
            client_name: "deepseek-tui".to_string(),
...
fn subagent_thread_started_thread_spawn_serializes_thread_lineage() {
    let parent_thread_id =
        deepseek_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
            .expect("valid thread id");
    let forked_from_thread_id =
        deepseek_protocol::ThreadId::from_string("22222222-2222-4222-8222-222222222222")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
fn subagent_thread_started_other_serializes_explicit_parent_thread_id() {
    let parent_thread_id =
        deepseek_protocol::ThreadId::from_string("33333333-3333-4333-8333-333333333333")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
async fn subagent_events_keep_thread_originator_with_explicit_turn_connection() {
    ...
    let mut events = Vec::new();
    let parent_thread_id =
        deepseek_protocol::ThreadId::from_string("44444444-4444-4444-4444-444444444444")
            .expect("valid parent thread id");
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(DeepSeekPluginUsedEventRequest {
        event_type: "deepseek_plugin_used",
        event_params: deepseek_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(DeepSeekPluginEventRequest {
        event_type: "deepseek_plugin_installed",
        event_params: deepseek_plugin_metadata(sample_plugin_metadata()),
    });
...
fn plugin_install_failed_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstallFailed(DeepSeekPluginInstallFailedEventRequest {
        event_type: "deepseek_plugin_install_failed",
        event_params: DeepSeekPluginInstallFailedMetadata {
            plugin: deepseek_plugin_metadata(sample_plugin_metadata()),
...
fn plugin_management_event_keeps_plugin_id_local_when_remote_id_exists() {
    ...
    let event = TrackEventRequest::PluginInstalled(DeepSeekPluginEventRequest {
        event_type: "deepseek_plugin_installed",
        event_params: deepseek_plugin_metadata(plugin),
    });
...
fn hook_run_event_serializes_expected_shape() {
    let tracking = test_tracking_context("thread-3", "turn-3");
    let event = TrackEventRequest::HookRun(DeepSeekHookRunEventRequest {
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    skill_scope: deepseek_protocol::protocol::SkillScope::User,
                    skill_path,
                    plugin_id: None,
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[2]["event_type"], "deepseek_plugin_used");
    assert_eq!(
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "deepseek_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "remote_plugin_id": null,
                "plugin_name": "sample",
...
async fn reducer_ingests_plugin_install_requested_fact() {
    ...
    let request = PluginInstallRequested {
        ...
        plugins: vec![
            PluginInstallRequestedPlugin {
                plugin_id: "calendar@deepseek-curated-remote".to_string(),
            ...
            PluginInstallRequestedPlugin {
                plugin_id: "github@deepseek-curated-remote".to_string(),
    ...
    assert_eq!(
        ...
        json!([{
            "event_type": "deepseek_plugin_install_requested",
            "event_params": {
                "suggestion_id": "request_plugin_install_call-1",
                "plugins": [{
                    "plugin_id": "calendar@deepseek-curated-remote",
                    "remote_plugin_id": "plugin_calendar",
                    "plugin_name": "Calendar",
                    "connector_ids": ["connector_calendar"],
                }, {
                    "plugin_id": "github@deepseek-curated-remote",
                    "remote_plugin_id": null,
                    "plugin_name": "GitHub",
...
fn external_agent_config_import_failure_event_serializes_expected_shape() {
    let event = TrackEventRequest::ExternalAgentConfigImportFailure(
        DeepSeekOnboardingExternalAgentImportFailureEventRequest {
            event_type: "codex_onboarding_external_agent_import_failure",
            event_params: DeepSeekOnboardingExternalAgentImportFailureMetadata {
                import_id: "import-1".to_string(),
                source: "app_server".to_string(),
...
fn turn_event_serializes_expected_shape() {
    let event = TrackEventRequest::TurnEvent(Box::new(DeepSeekTurnEventRequest {
        event_type: "codex_turn_event",
        event_params: crate::events::DeepSeekTurnEventParams {
            thread_id: "thread-2".to_string(),
    ...
    let expected = serde_json::from_str::<serde_json::Value>(
        r#"{
            ...
            "event_params": {
                ...
                "app_server_client": {
                    "product_client_id": "deepseek_cli_rs",
                    "client_name": "deepseek-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
...
async fn turn_lifecycle_emits_turn_event() {
    ...
    assert_eq!(
        ...
        json!({
            "product_client_id": "deepseek-tui",
            "client_name": "deepseek-tui",
            "client_version": "1.0.0",
            "rpc_transport": "stdio",
...
async fn turn_lifecycle_emits_failed_turn_event() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::TurnDeepSeekError(Box::new(
                TurnDeepSeekErrorFact::from_codex_err(
                    "thread-2".to_string(),
                    "turn-2".to_string(),
                    &DeepSeekErr::InvalidRequest("unknown turn environment id `env-2`".to_string()),
                ),
            ))),
            &mut out,
        )
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(deepseek_app_server_protocol::DeepSeekErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
