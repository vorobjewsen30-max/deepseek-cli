...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::DeepSeekCompactionEvent) {
        self.record_fact(AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(
            Box::new(event),
        )));
    }

    pub fn track_goal_event(&self, event: DeepSeekGoalEvent) {
    ...
    pub fn track_turn_codex_error(&self, fact: TurnDeepSeekErrorFact) {
        self.record_fact(AnalyticsFact::Custom(CustomAnalyticsFact::TurnDeepSeekError(
            Box::new(fact),
        )));
    }
