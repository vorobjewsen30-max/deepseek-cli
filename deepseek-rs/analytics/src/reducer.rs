...
use crate::events::DeepSeekCollabAgentToolCallEventParams;
use crate::events::DeepSeekCollabAgentToolCallEventRequest;
...
use crate::events::DeepSeekDynamicToolCallEventParams;
use crate::events::DeepSeekDynamicToolCallEventRequest;
use crate::events::DeepSeekFileChangeEventParams;
use crate::events::DeepSeekFileChangeEventRequest;
use crate::events::DeepSeekGoalEventRequest;
use crate::events::DeepSeekHookRunEventRequest;
use crate::events::DeepSeekImageGenerationEventParams;
use crate::events::DeepSeekImageGenerationEventRequest;
use crate::events::DeepSeekMcpToolCallEventParams;
use crate::events::DeepSeekMcpToolCallEventRequest;
use crate::events::DeepSeekOnboardingExternalAgentImportCompleteEventRequest;
use crate::events::DeepSeekOnboardingExternalAgentImportCompleteMetadata;
...
use crate::events::DeepSeekPluginInstallRequestedEventRequest;
...
use crate::events::DeepSeekTurnEventParams;
use crate::events::DeepSeekTurnEventRequest;
use crate::events::DeepSeekTurnSteerEventParams;
use crate::events::DeepSeekTurnSteerEventRequest;
use crate::events::DeepSeekWebSearchEventParams;
use crate::events::DeepSeekWebSearchEventRequest;
...
use crate::events::deepseek_plugin_install_requested_metadata;
...
use crate::facts::TurnDeepSeekError;
...
use deepseek_app_server_protocol::ClientResponse;
use deepseek_app_server_protocol::DeepSeekErrorInfo;
use deepseek_app_server_protocol::CollabAgentStatus;
...
use deepseek_app_server_protocol::DynamicToolCallOutputContentItem;
...
use deepseek_app_server_protocol::NetworkPolicyRuleAction;
use deepseek_app_server_protocol::PatchApplyStatus;
use deepseek_app_server_protocol::PatchChangeKind;
...
use deepseek_app_server_protocol::WebSearchAction;
use deepseek_git_utils::collect_git_info;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn compaction(input: &'a DeepSeekCompactionEvent) -> Self {
        Self {
    ...
    fn goal(input: &'a DeepSeekGoalEvent) -> Self {
        Self {
            event_name: "goal",
            thread_id: &input.thread_id,
    ...
    fn tool_item(
        notification: &'a deepseek_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
struct CompletedTurnState {
    ...
    turn_error: Option<DeepSeekErrorInfo>,
    completed_at: u64,
...
struct TurnState {
    ...
    codex_error: Option<TurnDeepSeekError>,
...
impl AnalyticsReducer {
    pub(crate) async fn ingest(&mut self, input: AnalyticsFact, out: &mut Vec<TrackEventRequest>) {
        match input {
            ...
            AnalyticsFact::Custom(input) => match input {
                ...
                CustomAnalyticsFact::TurnDeepSeekError(input) => {
    ...
    fn ingest_guardian_review(
        ...
        out.push(TrackEventRequest::GuardianReview(Box::new(
            GuardianReviewEventRequest {
                event_type: "deepseek_guardian_review",
    ...
    fn ingest_turn_codex_error(&mut self, input: TurnDeepSeekErrorFact) {
        let TurnDeepSeekErrorFact {
            turn_id,
            thread_id,
            error,
        } = input;
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            ...
            TrackEventRequest::AppMentioned(DeepSeekAppMentionedEventRequest {
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::AppUsed(DeepSeekAppUsedEventRequest {
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(DeepSeekHookRunEventRequest {
    ...
    fn ingest_plugin_used(&mut self, input: PluginUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::PluginUsed(DeepSeekPluginUsedEventRequest {
            event_type: "deepseek_plugin_used",
            event_params: deepseek_plugin_used_metadata(&tracking, plugin),
        }));
    }
    ...
    fn ingest_plugin_install_requested(
        ...
        out.push(TrackEventRequest::PluginInstallRequested(
            DeepSeekPluginInstallRequestedEventRequest {
                event_type: "deepseek_plugin_install_requested",
                event_params: deepseek_plugin_install_requested_metadata(&tracking, request),
            },
        ));
    }
    ...
    fn ingest_plugin_state_changed(
        ...
        let event = DeepSeekPluginEventRequest {
    ...
    fn ingest_external_agent_config_import_completed(
        ...
        out.push(TrackEventRequest::ExternalAgentConfigImportCompleted(
            DeepSeekOnboardingExternalAgentImportCompleteEventRequest {
                event_type: "codex_onboarding_external_agent_import_complete",
                event_params: DeepSeekOnboardingExternalAgentImportCompleteMetadata {
    ...
    fn emit_thread_initialized(
        &mut self,
        connection_id: u64,
        thread: deepseek_app_server_protocol::Thread,
        model: String,
        initialization_mode: ThreadInitializationMode,
        thread_originator: Option<String>,
        out: &mut Vec<TrackEventRequest>,
    ...
    fn ingest_compaction(&mut self, input: DeepSeekCompactionEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            DeepSeekCompactionEventRequest {
    ...
    fn ingest_goal(&mut self, input: DeepSeekGoalEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
            self.thread_context_or_warn(AnalyticsDropSite::goal(&input))
        else {
            return;
        };
        out.push(TrackEventRequest::Goal(Box::new(DeepSeekGoalEventRequest {
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: deepseek_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    fn emit_turn_steer_event(
        ...
        out.push(TrackEventRequest::TurnSteer(DeepSeekTurnSteerEventRequest {
            event_type: "codex_turn_steer_event",
            event_params: DeepSeekTurnSteerEventParams {
    ...
    fn emit_review_event(
        ...
        out.push(TrackEventRequest::ReviewEvent(DeepSeekReviewEventRequest {
    ...
    async fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        let turn_event = TrackEventRequest::TurnEvent(Box::new(DeepSeekTurnEventRequest {
    ...
    match item {
        ThreadItem::CommandExecution {
            ...
            Some(TrackEventRequest::CommandExecution(
                DeepSeekCommandExecutionEventRequest {
        ...
        ThreadItem::FileChange {
            ...
            Some(TrackEventRequest::FileChange(DeepSeekFileChangeEventRequest {
                event_type: "codex_file_change_event",
                event_params: DeepSeekFileChangeEventParams {
                    base,
        ...
        ThreadItem::McpToolCall {
            ...
            Some(TrackEventRequest::McpToolCall(
                DeepSeekMcpToolCallEventRequest {
                    event_type: "deepseek_mcp_tool_call_event",
                    event_params: DeepSeekMcpToolCallEventParams {
                        base,
        ...
        ThreadItem::DynamicToolCall {
            ...
            Some(TrackEventRequest::DynamicToolCall(
                DeepSeekDynamicToolCallEventRequest {
                    event_type: "codex_dynamic_tool_call_event",
                    event_params: DeepSeekDynamicToolCallEventParams {
                        base,
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            Some(TrackEventRequest::CollabAgentToolCall(
                DeepSeekCollabAgentToolCallEventRequest {
                    event_type: "codex_collab_agent_tool_call_event",
                    event_params: DeepSeekCollabAgentToolCallEventParams {
                        base,
        ...
        ThreadItem::WebSearch { id, query, action } => {
            ...
            Some(TrackEventRequest::WebSearch(DeepSeekWebSearchEventRequest {
                event_type: "codex_web_search_event",
                event_params: DeepSeekWebSearchEventParams {
                    base,
        ...
        ThreadItem::ImageGeneration {
            ...
            Some(TrackEventRequest::ImageGeneration(
                DeepSeekImageGenerationEventRequest {
                    event_type: "codex_image_generation_event",
                    event_params: DeepSeekImageGenerationEventParams {
                        base,
...
fn file_change_counts(changes: &[deepseek_app_server_protocol::FileUpdateChange]) -> FileChangeCounts {
...
fn analytics_turn_status(status: deepseek_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        deepseek_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        deepseek_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        deepseek_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        deepseek_app_server_protocol::TurnStatus::InProgress => None,
    }
}
