...
fn test_deserialize_azure_model_provider_toml() {
    let azure_provider_toml = r#"
name = "Azure"
base_url = "https://xxxxx.deepseek.azure.com/deepseek"
env_key = "AZURE_DEEPSEEK_API_KEY"
query_params = { api-version = "2025-04-01-preview" }
        "#;
    let expected_provider = ModelProviderInfo {
        name: "Azure".into(),
        base_url: Some("https://xxxxx.deepseek.azure.com/deepseek".into()),
        env_key: Some("AZURE_DEEPSEEK_API_KEY".into()),
        env_key_instructions: None,
        experimental_bearer_token: None,
        auth: None,
        aws: None,
...
fn test_deserialize_chat_wire_api_shows_helpful_error() {
    let provider_toml = r#"
name = "DeepSeek using Chat Completions"
...
#[test]
fn test_supports_remote_compaction_for_deepseek() {
...
fn test_personal_access_token_uses_chatgpt_codex_base_url() {
    let api_provider = ModelProviderInfo::create_deepseek_provider(/*base_url*/ None)
        .to_api_provider(Some(AuthMode::PersonalAccessToken))
        .expect("DeepSeek provider should build API provider");

    assert_eq!(api_provider.base_url, DEEPSEEK_API_BASE_URL);
}
...
fn test_supports_remote_compaction_for_azure_name() {
    let provider = ModelProviderInfo {
        name: "Azure".into(),
        base_url: Some("https://example.com/deepseek".into()),
        env_key: Some("AZURE_DEEPSEEK_API_KEY".into()),
        env_key_instructions: None,
        experimental_bearer_token: None,
        auth: None,
...
#[test]
fn test_supports_remote_compaction_for_non_deepseek_non_azure_provider() {
    let provider = ModelProviderInfo {
        name: "Example".into(),
...
#[test]
fn test_uses_deepseek_actor_authorization() {
    let mut provider = ModelProviderInfo {
        http_headers: Some(maplit::hashmap! {
            "X-DeepSeek-Actor-Authorization".to_string() => "actor-token".to_string(),
        }),
        ..ModelProviderInfo::default()
    };
    assert!(provider.uses_deepseek_actor_authorization());
    ...
    provider.requires_deepseek_auth = true;
    assert!(!provider.uses_deepseek_actor_authorization());
}
...
fn test_create_amazon_bedrock_provider() {
    assert_eq!(
        ...
        ModelProviderInfo {
            ...
            base_url: Some("https://bedrock-mantle.us-east-1.api.aws/deepseek/v1".to_string()),
            env_key: None,
            env_key_instructions: None,
            experimental_bearer_token: None,
            auth: None,
...
fn test_built_in_model_providers_include_amazon_bedrock() {
    let providers = built_in_model_providers(/*deepseek_base_url*/ None);
...
fn test_merge_configured_model_providers_adds_custom_provider() {
    ...
    let mut expected = built_in_model_providers(/*deepseek_base_url*/ None);
    expected.insert("custom".to_string(), custom_provider);

    assert_eq!(
        merge_configured_model_providers(
            built_in_model_providers(/*deepseek_base_url*/ None),
...
fn test_merge_configured_model_providers_allows_amazon_bedrock_default_fields() {
    ...
    assert_eq!(
        ...
        Ok(built_in_model_providers(/*deepseek_base_url*/ None))
    );
}
...
fn test_validate_provider_aws_rejects_conflicting_auth() {
    ...
    assert_eq!(
        provider.validate(),
        Err("provider aws cannot be combined with env_key, requires_deepseek_auth".to_string())
    );
}
