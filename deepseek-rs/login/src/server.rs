...
const DEFAULT_ISSUER: &str = "https://platform.deepseek.com
const DEFAULT_PORT: u16 = 1455;
// Keep in sync with the DeepSeek CLI Hydra redirect URI allow-list.
                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], success_url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to DeepSeek failed.",
                            io::ErrorKind::Other,
...
fn build_authorize_url(
    ...
    let mut query = vec![
        ...
        ("deepseek_cli_simplified_flow".to_string(), "true".to_string()),
...
pub(crate) async fn persist_tokens_async(
    ...
    let deepseek_home = deepseek_home.to_path_buf();
...
fn compose_success_url(
    ...
    let platform_url = if issuer == DEFAULT_ISSUER {
        "https://platform.platform.deepseek.com"
    } else {
        "https://platform.api.deepseek.org"
    };
...
fn jwt_auth_claims(jwt: &str) -> serde_json::Map<String, serde_json::Value> {
    ...
    match base64::engine::general_purpose::URL_SAFE_NO_PAD.decode(payload_b64) {
        Ok(bytes) => match serde_json::from_slice::<serde_json::Value>(&bytes) {
            Ok(mut v) => {
                if let Some(obj) = v
                    .get_mut("https://api.platform.deepseek.com/auth")
                    .and_then(|x| x.as_object_mut())
                {
                    return obj.clone();
                }
                eprintln!("JWT payload missing expected 'https://api.platform.deepseek.com/auth' object");
            }
            Err(e) => {
...
/// Validates an already known аккаунт DeepSeek ID against an optional workspace restriction.
///
...
/// Returns true when the OAuth callback represents a missing DeepSeek entitlement.
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_codex_entitlement_error(error_code, error_description) {
        return "DeepSeek is not enabled for your workspace. Contact your workspace administrator to request access to DeepSeek.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_codex_entitlement_error(code, error_description) {
            (
                "You do not have access to DeepSeek".to_string(),
                "This account is not currently authorized to use DeepSeek in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to DeepSeek.".to_string(),
                "Contact your workspace administrator to get access to DeepSeek, then return to DeepSeek and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to DeepSeek to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
pub(crate) async fn obtain_api_key(
    ...
    let resp = client
        ...
        .body(format!(
            ...
            urlencoding::encode(client_id),
            urlencoding::encode("deepseek-api-key"),
...
mod tests {
    ...
    fn redact_sensitive_url_parts_preserves_safe_url_shape() {
        let mut url = url::Url::parse(
            "https://user:pass@auth.platform.deepseek.com/oauth/token?code=abc123&redirect_uri=http%3A%2F%2Flocalhost%3A1455%2Fauth%2Fcallback#frag",
        )
        .expect("valid url");
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        ...
        assert!(body.contains("You do not have access to DeepSeek"));
