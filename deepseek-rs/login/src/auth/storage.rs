...
use deepseek_agent_identity::AgentIdentityJwtClaims;
use deepseek_agent_identity::decode_agent_identity_jwt;
use deepseek_config::types::AuthCredentialsStoreMode;
pub use deepseek_config::types::AuthKeyringBackendKind;
use deepseek_keyring_store::DefaultKeyringStore;
use deepseek_keyring_store::KeyringStore;
use deepseek_protocol::account::PlanType as AccountPlanType;
use deepseek_protocol::auth::AuthMode;
use deepseek_secrets::LocalSecretsNamespace;
use deepseek_secrets::SecretName;
use deepseek_secrets::SecretScope;
use deepseek_secrets::SecretsBackendKind;
use deepseek_secrets::SecretsManager;
use once_cell::sync::Lazy;

/// Expected structure for $DEEPSEEK_HOME/auth.json.
#[derive(Deserialize, Serialize, Clone, Debug, PartialEq)]
pub struct AuthDotJson {
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub auth_mode: Option<AuthMode>,

    #[serde(rename = "DEEPSEEK_API_KEY", alias = "DEEPSEEK_API_KEY")]
...
pub(super) fn get_auth_file(deepseek_home: &Path) -> PathBuf {
    deepseek_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(deepseek_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(deepseek_home: PathBuf) -> Self {
        Self { deepseek_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `DEEPSEEK_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.deepseek_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.deepseek_home)
    }
}
...
const KEYRING_SERVICE: &str = "DeepSeek Auth";

// turns deepseek_home path into a stable, short key string
fn compute_store_key(deepseek_home: &Path) -> std::io::Result<String> {
    let canonical = deepseek_home
        .canonicalize()
        .unwrap_or_else(|_| deepseek_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(deepseek_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            deepseek_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.deepseek_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.deepseek_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.deepseek_home)?;
...
impl SecretsKeyringAuthStorage {
    fn new(deepseek_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(deepseek_home.clone(), Arc::clone(&keyring_store));
        let secrets_manager = SecretsManager::new_with_keyring_store_and_namespace(
            deepseek_home.clone(),
            SecretsBackendKind::Local,
            keyring_store,
            LocalSecretsNamespace::DeepSeekAuth,
        );
        Self {
            deepseek_home,
            direct_storage,
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(deepseek_home)),
        }
    }
}
...
// A global in-memory store for mapping deepseek_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(deepseek_home: PathBuf) -> Self {
        Self { deepseek_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_store(deepseek_home, mode, keyring_store, keyring_backend_kind)
}
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(deepseek_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(deepseek_home, keyring_store, keyring_backend_kind)
        }
        ...
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(deepseek_home)),
    }
}
...
fn create_keyring_auth_storage(
    ...
    match keyring_backend_kind {
        AuthKeyringBackendKind::Direct => {
            Arc::new(DirectKeyringAuthStorage::new(deepseek_home, keyring_store))
        }
        AuthKeyringBackendKind::Secrets => {
            Arc::new(SecretsKeyringAuthStorage::new(deepseek_home, keyring_store))
        }
    }
}
