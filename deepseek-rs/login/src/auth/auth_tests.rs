...
use deepseek_protocol::auth::KnownPlan as InternalKnownPlan;
...
async fn refresh_without_id_token() {
    let deepseek_home = tempdir().unwrap();
...
fn login_with_api_key_overwrites_existing_auth_json() {
    ...
    assert_eq!(auth.deepseek_api_key.as_deref(), Some("sk-new"));
...
async fn login_with_access_token_writes_agent_identity_jwt() {
    ...
    assert!(auth.tokens.is_none(), "tokens should be cleared");
    assert!(auth.deepseek_api_key.is_none(), "API key should be cleared");
    server.verify().await;
}
...
async fn stored_agent_identity_jwt_keeps_auth_json_unchanged() -> anyhow::Result<()> {
    ...
    let DeepSeekAuth::AgentIdentity(agent_identity_auth) = auth else {
        panic!("stored JWT should load as agent identity auth");
    };
    assert_eq!(agent_identity_auth.run_task_id(), "task-id");
    let storage = FileAuthStorage::new(deepseek_home.path().to_path_buf());
    let auth = storage
        .try_read_auth_json(&get_auth_file(deepseek_home.path()))
        .expect("auth.json should parse");
    assert_eq!(
        auth.agent_identity,
        Some(AgentIdentityStorage::Jwt(agent_identity))
...
async fn chatgpt_auth_task_registration_retry_exhaustion_is_fallback_eligible() -> anyhow::Result<()>
{
    ...
    let storage = FileAuthStorage::new(deepseek_home.path().to_path_buf());
    let auth_path = get_auth_file(deepseek_home.path());
...
async fn agent_identity_jwt_task_registration_retry_exhaustion_is_strict() -> anyhow::Result<()> {
    ...
    let err = DeepSeekAuth::from_agent_identity_jwt_with_authapi_base_url(
        &agent_identity,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        /*auth_route_config*/ None,
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = DeepSeekAuth::from_auth_dot_json(
        deepseek_home.path(),
...
fn external_auth_tokens_without_chatgpt_metadata_cannot_seed_chatgpt_auth() {
    ...
    .expect_err("bearer-only external auth should not seed DeepSeek auth");
...
fn write_auth_file(params: AuthFileParams, deepseek_home: &Path) -> std::io::Result<String> {
    let fake_jwt = fake_jwt_for_auth_file_params(&params)?;
    let auth_file = get_auth_file(deepseek_home);
    let auth_json_data = json!({
        "DEEPSEEK_API_KEY": params.deepseek_api_key,
        "tokens": {
            "id_token": fake_jwt,
...
fn fake_jwt_for_auth_file_params(params: &AuthFileParams) -> std::io::Result<String> {
    ...
    let payload = serde_json::json!({
        ...
        "email_verified": true,
        "https://api.platform.deepseek.com/auth": auth_payload,
    });
...
fn remove_access_token_env_var() -> EnvVarGuard {
    EnvVarGuard::remove(DEEPSEEK_ACCESS_TOKEN_ENV_VAR)
}
...
async fn load_auth_reads_access_token_from_env() {
    ...
    let _access_token_guard = EnvVarGuard::set(DEEPSEEK_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    ...
    let DeepSeekAuth::AgentIdentity(agent_identity) = auth else {
    ...
    assert!(
        !get_auth_file(deepseek_home.path()).exists(),
...
async fn load_auth_reads_personal_access_token_from_env() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    let _authapi_guard = EnvVarGuard::set("CODEX_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard = EnvVarGuard::set(DEEPSEEK_ACCESS_TOKEN_ENV_VAR, "at-env-test");
...
async fn auth_manager_rejects_env_personal_access_token_workspace_mismatch() {
    ...
    let _access_token_guard =
        EnvVarGuard::set(DEEPSEEK_ACCESS_TOKEN_ENV_VAR, "at-env-workspace-mismatch");
...
async fn personal_access_token_does_not_offer_unauthorized_recovery() {
    ...
    let _authapi_guard = EnvVarGuard::set("CODEX_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard =
        EnvVarGuard::set(DEEPSEEK_ACCESS_TOKEN_ENV_VAR, "at-no-unauthorized-recovery");
    let manager = Arc::new(
        AuthManager::new(
            deepseek_home.path().to_path_buf(),
            /*enable_deepseek_api_key_env*/ false,
...
#[tokio::test]
#[serial(codex_auth_env)]
async fn load_auth_keeps_deepseek_api_key_env_precedence() {
    let deepseek_home = tempdir().unwrap();
    let record = agent_identity_record(WORKSPACE_ID_ALLOWED);
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(DEEPSEEK_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(DEEPSEEK_API_KEY_ENV_VAR, "sk-env");
...
async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
    ...
    assert!(err.to_string().contains("вход через DeepSeek is required"));
    assert!(
        !deepseek_home.path().join("auth.json").exists(),
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(DEEPSEEK_API_KEY_ENV_VAR, "sk-env");
    let _access_token_guard = remove_access_token_env_var();
    let deepseek_home = tempdir().unwrap();
    ...
    let err = super::enforce_login_restrictions(&config)
        .await
        .expect_err("environment API key should not satisfy forced вход через DeepSeek");
    assert!(
        err.to_string()
            .contains("вход через DeepSeek is required, but an API key is currently being used.")
    );
}

fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material =
        deepseek_agent_identity::generate_agent_key_material().expect("generate agent key material");
...
async fn assert_agent_identity_plan_alias(
    ...
    let authapi_base_url = server.uri();
    let chatgpt_base_url = format!("{authapi_base_url}/backend-api");
    let auth = DeepSeekAuth::from_agent_identity_jwt_with_authapi_base_url(
        &jwt,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        /*auth_route_config*/ None,
