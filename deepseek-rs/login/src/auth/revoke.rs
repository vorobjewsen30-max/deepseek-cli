...
//!
//! Managed DeepSeek auth stores OAuth tokens locally. Logout attempts to revoke the
...
fn resolved_auth_mode(auth_dot_json: &AuthDotJson) -> AuthMode {
    ...
    if auth_dot_json.deepseek_api_key.is_some() {
...
mod tests {
    ...
    async fn revoke_request_times_out() {
        ...
        let client = DeepSeekHttpClient::new(reqwest::Client::new());
