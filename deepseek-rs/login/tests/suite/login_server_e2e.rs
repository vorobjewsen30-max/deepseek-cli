...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = deepseek_home.clone();

    let opts = ServerOptions {
        deepseek_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: None,
        client_id: deepseek_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = deepseek_home.join("auth.json");
    ...
    assert_eq!(json["DEEPSEEK_API_KEY"], "access-123");
...
#[tokio::test]
async fn creates_missing_deepseek_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let deepseek_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to DeepSeek"),
        "error body should clearly explain the DeepSeek access denial"
    );
    assert!(
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to DeepSeek to retry"),
    ...
    assert!(
        !body.contains("get access to DeepSeek"),
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let mut opts = ServerOptions::new(
        ...
        deepseek_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_deepseek_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        deepseek_home: first_deepseek_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: None,
        client_id: deepseek_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
    ...
    let second_deepseek_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        deepseek_home: second_deepseek_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: None,
        client_id: deepseek_login::CLIENT_ID.to_string(),
        issuer,
