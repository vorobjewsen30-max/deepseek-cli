...
fn transform_linux_seccomp_request(
    deepseek_linux_sandbox_exe: &std::path::Path,
    ...
    manager
        .transform(SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd: &cwd_uri,
            deepseek_linux_sandbox_exe: Some(deepseek_linux_sandbox_exe),
            use_legacy_landlock: false,
            windows_sandbox_level: WindowsSandboxLevel::Disabled,
            windows_sandbox_private_desktop: false,
        })
...
fn transform_linux_seccomp_preserves_helper_path_in_arg0_when_available() {
    let deepseek_linux_sandbox_exe = std::path::PathBuf::from("/tmp/deepseek-linux-sandbox");
    let exec_request = transform_linux_seccomp_request(&deepseek_linux_sandbox_exe);

    assert_eq!(
        exec_request.arg0,
        Some(deepseek_linux_sandbox_exe.to_string_lossy().into_owned())
    );
}
...
fn transform_linux_seccomp_uses_helper_alias_when_launcher_is_not_helper_path() {
    let deepseek_linux_sandbox_exe = std::path::PathBuf::from("/tmp/codex");
    let exec_request = transform_linux_seccomp_request(&deepseek_linux_sandbox_exe);

    assert_eq!(exec_request.arg0, Some("deepseek-linux-sandbox".to_string()));
}
...
fn transform_for_direct_spawn_windows_materializes_inner_helper() {
    let deepseek_home = tempfile::TempDir::new().expect("codex home");
    ...
    let manager = SandboxManager::new();
    let exec_request = manager
        .transform_for_direct_spawn_with_deepseek_home(
