...
use deepseek_core_skills::SKILLS_INTRO_WITH_ABSOLUTE_PATHS;
use deepseek_core_skills::SkillLoadOutcome;
use deepseek_core_skills::SkillMetadata;
use deepseek_core_skills::injection::InjectedHostSkillPrompts;
use deepseek_extension_api::ConversationHistory;
...
use deepseek_extension_api::PreviousWorldStateSection;
...
use deepseek_extension_api::WorldStateContributionInput;
...
use deepseek_protocol::protocol::SKILLS_INSTRUCTIONS_CLOSE_TAG;
use deepseek_protocol::protocol::SKILLS_INSTRUCTIONS_OPEN_TAG;
...
use deepseek_skills_extension::SkillProviders;
use deepseek_skills_extension::SkillsExtensionConfig;
use deepseek_skills_extension::catalog::SkillAuthority;
use deepseek_skills_extension::catalog::SkillCatalog;
use deepseek_skills_extension::catalog::SkillCatalogEntry;
use deepseek_skills_extension::catalog::SkillPackageId;
use deepseek_skills_extension::catalog::SkillProviderError;
use deepseek_skills_extension::catalog::SkillReadResult;
use deepseek_skills_extension::catalog::SkillResourceId;
use deepseek_skills_extension::catalog::SkillSearchResult;
use deepseek_skills_extension::catalog::SkillSourceKind;
use deepseek_skills_extension::install;
use deepseek_skills_extension::install_with_providers;
use deepseek_skills_extension::provider::SkillListQuery;
use deepseek_skills_extension::provider::SkillProvider;
use deepseek_skills_extension::provider::SkillProviderFuture;
use deepseek_skills_extension::provider::SkillReadRequest;
use deepseek_skills_extension::provider::SkillSearchRequest;
...
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    ...
    let skill_path = deepseek_home.join("skills").join("demo").join("SKILL.md");
    ...
    std::fs::remove_dir_all(deepseek_home)?;
    Ok(())
}
...
async fn selected_executor_catalog_follows_step_availability_and_reuses_its_cache() -> TestResult {
    ...
    let available_sections = registry.context_contributors()[0]
        .contribute_world_state(WorldStateContributionInput {
            thread_id: deepseek_protocol::ThreadId::new(),
            turn_id: "turn-1",
...
fn test_deepseek_home() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "deepseek-skills-extension-test-{}-{id}",
        std::process::id(),
    ))
}
