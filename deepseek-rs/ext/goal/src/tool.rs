...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<deepseek_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> deepseek_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => deepseek_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => deepseek_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: deepseek_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            deepseek_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            deepseek_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<deepseek_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &deepseek_state::StateRuntime,
    thread_id: ThreadId,
...
pub(crate) fn protocol_goal_from_state(goal: deepseek_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: deepseek_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> deepseek_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => deepseek_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => deepseek_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => deepseek_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => deepseek_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => deepseek_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => deepseek_state::ThreadGoalStatus::Complete,
    }
}
