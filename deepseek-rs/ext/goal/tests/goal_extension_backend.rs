...
use deepseek_extension_api::FunctionCallError;
use deepseek_extension_api::NoopTurnItemEmitter;
use deepseek_extension_api::ThreadResumeInput;
use deepseek_extension_api::ThreadStartInput;
use deepseek_extension_api::ThreadStopInput;
use deepseek_extension_api::ToolCall;
use deepseek_extension_api::ToolCallOutcome;
use deepseek_extension_api::ToolCallSource;
use deepseek_extension_api::ToolExecutor;
use deepseek_extension_api::ToolFinishInput;
use deepseek_extension_api::ToolPayload;
use deepseek_extension_api::TurnErrorInput;
use deepseek_extension_api::TurnStartInput;
use deepseek_extension_api::TurnStopInput;
use deepseek_goal_extension::GoalObjectiveUpdate;
use deepseek_goal_extension::GoalRuntimeHandle;
use deepseek_goal_extension::GoalService;
use deepseek_goal_extension::GoalSetRequest;
use deepseek_goal_extension::GoalTokenBudgetUpdate;
use deepseek_goal_extension::install_with_backend;
...
use deepseek_protocol::protocol::ThreadGoalStatus;
use deepseek_protocol::protocol::TokenUsage;
use deepseek_protocol::protocol::TokenUsageInfo;
use deepseek_protocol::protocol::TruncationPolicy;
...
async fn budget_limited_goal_keeps_accruing_until_turn_stop() -> anyhow::Result<()> {
    ...
    assert_eq!(deepseek_state::ThreadGoalStatus::BudgetLimited, goal.status);
...
async fn turn_error_usage_limit_accounts_progress_and_clears_accounting() -> anyhow::Result<()> {
    ...
    harness
        ...
        .await;
    harness
        .notify_turn_error("turn-1", DeepSeekErrorInfo::UsageLimitExceeded)
        .await;
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(deepseek_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(
        vec![
            CapturedGoalEvent {
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    harness
        .notify_turn_error("turn-1", DeepSeekErrorInfo::Other)
        .await;
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(deepseek_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
...
async fn usage_limit_plan_turn_does_not_stop_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(deepseek_state::ThreadGoalStatus::Active, goal.status);
...
async fn thread_resume_rehydrates_active_goal_idle_accounting() -> anyhow::Result<()> {
    ...
    runtime
        ...
        .replace_thread_goal(
            ...
            deepseek_state::ThreadGoalStatus::Active,
            /*token_budget*/ None,
        )
        .await?;
    let harness = GoalExtensionHarness::new(runtime.clone(), thread_id).await?;
...
async fn installed_tools(
    runtime: Arc<deepseek_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: deepseek_extension_api::ExtensionRegistry<()>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish(&self, turn_id: &str, call_id: &str, tool_name: &str) {
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = deepseek_extension_api::ToolName::plain(tool_name);
    ...
    async fn notify_turn_error(&self, turn_id: &str, error: DeepSeekErrorInfo) {
        let turn_store = ExtensionData::new(turn_id);
        for contributor in self.registry.turn_lifecycle_contributors() {
            contributor
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall {
    ToolCall {
        ...
        tool_name: deepseek_extension_api::ToolName::plain(tool_name),
        model: "gpt-test".to_string(),
        truncation_policy: TruncationPolicy::Bytes(1024),
        conversation_history: deepseek_extension_api::ConversationHistory::default(),
...
async fn test_runtime() -> anyhow::Result<Arc<deepseek_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    deepseek_state::StateRuntime::init(tempdir.keep(), "test-provider".to_string()).await
}
...
async fn seed_thread_metadata(
    runtime: &deepseek_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = deepseek_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .deepseek_home()
...
fn protocol_status(status: deepseek_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
        deepseek_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        deepseek_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        deepseek_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        deepseek_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        deepseek_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        deepseek_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}
