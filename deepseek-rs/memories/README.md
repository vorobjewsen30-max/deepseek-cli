...
Runtime orchestration for Phase 1 and Phase 2 still lives in `deepseek-core` under
`codex-rs/core/src/memories/`.

## Crates

- `codex-rs/memories/read` (`deepseek-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `codex-rs/memories/write` (`deepseek-memories-write`) owns the write path:
...
- The dated snapshot-copy workflow is used in the separate `deepseek/project/agent_memory/write` harness repo, not here.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.codex/memories/.git` by `deepseek-git-utils`
