...
use deepseek_core::content_items_to_text;
use deepseek_core::detached_memory_responses_metadata;
...
pub(crate) struct SpawnedConsolidationAgent {
    ...
    pub(crate) thread: Arc<DeepSeekThread>,
}
...
impl StageOneRequestContext {
    pub(crate) fn start_timer(&self, name: &str) -> Option<deepseek_otel::Timer> {
        ...
        while let Some(message) = stream.next().await.transpose()? {
            match message {
                ...
                ResponseEvent::OutputItemDone(item) => {
                    if result.is_empty()
                        && let deepseek_protocol::models::ResponseItem::Message { content, .. } = item
