...
use deepseek_git_utils::diff_since_latest_init;
use deepseek_git_utils::reset_git_repository;
use deepseek_login::AuthManager;
use deepseek_login::DeepSeekAuth;
use deepseek_model_provider::ModelProvider;
use deepseek_model_provider::ModelProviderFuture;
use deepseek_model_provider::ProviderAccountResult;
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<deepseek_state::StateRuntime>> {
    let db =
        deepseek_state::StateRuntime::init(home.path().to_path_buf(), "test-provider".into()).await?;
    db.mark_backfill_complete(/*last_watermark*/ None).await?;
    Ok(db)
}

async fn trigger_memories_startup(test: &TestDeepSeek) {
...
) -> (Arc<MemoryStartupContext>, Arc<deepseek_core::config::Config>) {
    let config_snapshot = test.codex.config_snapshot().await;
    let mut config = test.config.clone();
    config
        .features
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    ) -> deepseek_models_manager::manager::SharedModelsManager {
        self.delegate
            .models_manager(deepseek_home, config_model_catalog)
    }
}

async fn seed_stage1_output(
    db: &deepseek_state::StateRuntime,
    deepseek_home: &Path,
    ...
    let mut metadata_builder = deepseek_state::ThreadMetadataBuilder::new(
        thread_id,
        deepseek_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = deepseek_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_candidate(
    ...
    let thread_id = ThreadId::new();
    let rollout_path = deepseek_home.join(format!("rollout-{thread_id}.jsonl"));
...
) -> anyhow::Result<deepseek_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.codex.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        deepseek_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_codex(test: &TestDeepSeek) -> anyhow::Result<()> {
