...
pub enum Command {
    /// Submit a new DeepSeek Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a DeepSeek Cloud task.
    Status(StatusCommand),
    /// List DeepSeek Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a DeepSeek Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a DeepSeek Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in DeepSeek Cloud.
    ...
    /// Git branch to run in DeepSeek Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// DeepSeek Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// DeepSeek Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// DeepSeek Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
