...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = deepseek_login::default_client::USER_AGENT_SUFFIX.lock() {
...
/// - trims trailing '/'
/// - appends '/backend-api' for DeepSeek hosts when missing
...
/// Build headers for DeepSeek-backed requests: `User-Agent`, optional `Authorization`,
/// and optional `DeepSeek-Account-Id`.
pub async fn build_chatgpt_headers() -> HeaderMap {
    use reqwest::header::HeaderValue;
    use reqwest::header::USER_AGENT;

    set_user_agent_suffix("deepseek_cloud_tasks_tui");
    let ua = deepseek_login::default_client::get_codex_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("deepseek-cli")),
    );
    if let Some(am) = load_auth_manager(/*chatgpt_base_url*/ None).await
        && let Some(auth) = am.auth().await
        && auth.uses_codex_backend()
    {
        headers.extend(deepseek_model_provider::auth_provider_from_auth(&auth).to_auth_headers());
    }
    headers
}
