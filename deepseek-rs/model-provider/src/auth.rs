...
use deepseek_agent_identity::AgentIdentityKey;
use deepseek_agent_identity::authorization_header_for_agent_task;
...
use deepseek_login::auth::AgentIdentityAuth;
use deepseek_login::auth::AgentIdentityAuthError;
...
// providers and custom test providers with `requires_deepseek_auth = false`.
#[derive(Clone, Debug)]
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> deepseek_protocol::error::Result<SharedAuthProvider> {
    if matches!(auth, Some(DeepSeekAuth::BedrockApiKey(_))) {
        return Err(DeepSeekErr::UnsupportedOperation(
...
) -> deepseek_protocol::error::Result<ResolvedProviderAuth> {
    let ProviderAuthScope {
        agent_identity_policy,
        session_source,
        agent_identity_session_fallback,
    } = scope;
    if let Some(DeepSeekAuth::AgentIdentity(agent_identity_auth)) = auth {
        ...
        Err(err) => {
                ...
                .and_then(|source| source.downcast_ref::<AgentIdentityAuthError>())
            {
                ...
                tracing::warn!(
                    ...
                    error = %message,
                    newly_engaged,
                    "agent identity bootstrap unavailable; using DeepSeek bearer auth for this session"
                );
...
fn should_bootstrap_chatgpt_agent_identity(
    ...
    agent_identity_policy == AgentIdentityAuthPolicy::ChatGptAuth
        && matches!(auth, Some(DeepSeekAuth::Chatgpt(_)))
}

fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> deepseek_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party DeepSeek auth snapshot.
pub fn auth_provider_from_auth(auth: &DeepSeekAuth) -> SharedAuthProvider {
    match auth {
        DeepSeekAuth::AgentIdentity(auth) => {
            Arc::new(AgentIdentityAuthProvider { auth: auth.clone() })
        }
        DeepSeekAuth::BedrockApiKey(_) => unreachable!("{BEDROCK_API_KEY_UNSUPPORTED_MESSAGE}"),
        DeepSeekAuth::ApiKey(_)
        | DeepSeekAuth::Chatgpt(_)
        | DeepSeekAuth::ChatgptAuthTokens(_)
        | DeepSeekAuth::PersonalAccessToken(_) => Arc::new(BearerAuthProvider {
...
#[cfg(test)]
mod tests {
    use deepseek_agent_identity::generate_agent_key_material;
    use deepseek_login::AuthCredentialsStoreMode;
    use deepseek_login::AuthKeyringBackendKind;
    use deepseek_login::auth::AgentIdentityAuthRecord;
    ...
    static NEXT_DEEPSEEK_HOME_ID: AtomicUsize = AtomicUsize::new(0);
    ...
    fn test_deepseek_home() -> PathBuf {
        let id = NEXT_DEEPSEEK_HOME_ID.fetch_add(1, Ordering::Relaxed);
        let path = std::env::temp_dir().join(format!(
            "deepseek-model-provider-agent-identity-{pid}-{id}",
            pid = std::process::id()
        ));
    ...
    fn write_chatgpt_auth_json(deepseek_home: &Path) {
        let auth_json = json!({
            "tokens": {
    ...
    async fn chatgpt_auth_manager(
    ...
    ) -> (PathBuf, Arc<AuthManager>, DeepSeekAuth) {
        let deepseek_home = test_deepseek_home();
        write_chatgpt_auth_json(&deepseek_home);
        ...
        (deepseek_home, auth_manager, auth)
    }
    ...
    #[test]
    fn deepseek_provider_rejects_bedrock_api_key_auth() {
        let provider = ModelProviderInfo::create_deepseek_provider(/*base_url*/ None);
        let auth = DeepSeekAuth::BedrockApiKey(BedrockApiKeyAuth {
            api_key: "bedrock-api-key-test".to_string(),
            region: "us-east-1".to_string(),
        });

        match resolve_provider_auth(Some(&auth), &provider) {
            Err(DeepSeekErr::UnsupportedOperation(message)) => {
    ...
    async fn first_party_run_scope_uses_agent_assertion_and_exposes_telemetry() {
        let auth = DeepSeekAuth::AgentIdentity(
    ...
    async fn agent_identity_auth_provider_preserves_account_routing_headers() {
        ...
        let provider = auth_provider_from_auth(&DeepSeekAuth::AgentIdentity(auth));
    ...
    async fn chatgpt_bootstrap_unavailable_uses_session_bearer_fallback() {
        ...
        .await;
        let (_deepseek_home, auth_manager, auth) = chatgpt_auth_manager(server.uri()).await;
        let provider = ModelProviderInfo::create_deepseek_provider(/*base_url*/ None);
