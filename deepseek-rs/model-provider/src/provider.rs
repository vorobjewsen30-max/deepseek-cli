...
use deepseek_protocol::account::ProviderAccount;
...
/// Optional provider-backed features that DeepSeek may expose at runtime.
///
...
pub struct ProviderAccountState {
    ...
    pub requires_deepseek_auth: bool,
}
...
pub const DEFAULT_MEMORY_CONSOLIDATION_PREFERRED_MODEL: &str = "deepseek-v4-flash";
...
/// metadata used by the default DeepSeek-compatible implementation.
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    /// to think through whether DeepSeek should have a unified provider-specific auth
    /// manager throughout the codebase; that is a larger refactor than this change.
    fn auth_manager(&self) -> Option<Arc<AuthManager>>;

    /// Returns the current provider-scoped auth value, if one is configured.
    fn auth(&self) -> ModelProviderFuture<'_, Option<DeepSeekAuth>>;

    /// Returns the current app-visible account state for this provider.
    fn account_state(&self) -> ProviderAccountResult;

    /// Maps an API client error into the provider's user-facing error representation.
    fn map_api_error(&self, error: ApiError) -> DeepSeekErr {
        deepseek_api::map_api_error(error)
    }

    /// Returns provider configuration adapted for the API client.
    fn api_provider(&self) -> ModelProviderFuture<'_, deepseek_protocol::error::Result<Provider>> {
        Box::pin(async move {
            let auth = self.auth().await;
            self.info()
                .to_api_provider(auth.as_ref().map(DeepSeekAuth::auth_mode))
        })
    }
    ...
    fn runtime_base_url(
        &self,
    ) -> ModelProviderFuture<'_, deepseek_protocol::error::Result<Option<String>>> {
    ...
    fn api_auth(
        &self,
    ) -> ModelProviderFuture<'_, deepseek_protocol::error::Result<SharedAuthProvider>> {
        Box::pin(async move {
            let auth = self.auth().await;
    ...
    /// Returns request credentials, optionally scoped to a DeepSeek session task.
    fn api_auth_for_scope(
        &self,
        scope: ProviderAuthScope,
    ) -> ModelProviderFuture<'_, deepseek_protocol::error::Result<ResolvedProviderAuth>> {
        Box::pin(async move {
...
fn provider_uses_first_party_auth_path(provider: &ModelProviderInfo) -> bool {
    provider.requires_deepseek_auth
...
impl ModelProvider for ConfiguredModelProvider {
    ...
    fn account_state(&self) -> ProviderAccountResult {
        let account = if self.info.requires_deepseek_auth {
            self.auth_manager
                .as_ref()
                ...
                .map(|auth| match &auth {
                    DeepSeekAuth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
                    _ => Ok(ProviderAccount::ApiKey),
                })
                .transpose()?
        } else {
            None
        ...
        Ok(ProviderAccountState {
            account,
            requires_deepseek_auth: self.info.requires_deepseek_auth,
        })
    }
...
mod tests {
    ...
    use deepseek_login::auth::BedrockApiKeyAuth;
    use deepseek_model_provider_info::ModelProviderAwsAuthInfo;
    use deepseek_model_provider_info::WireApi;
    use deepseek_model_provider_info::create_oss_provider_with_base_url;
    ...
    fn provider_info_with_command_auth() -> ModelProviderInfo {
        ModelProviderInfo {
            ...
            ..ModelProviderInfo::create_deepseek_provider(/*base_url*/ None)
        }
    }

    fn test_deepseek_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("deepseek-model-provider-test-{}", std::process::id()))
    }
    ...
    fn bedrock_api_key_auth() -> DeepSeekAuth {
        DeepSeekAuth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    #[tokio::test]
    async fn scoped_auth_ignores_scope_for_non_deepseek_provider() {
    ...
    #[test]
    fn create_model_provider_does_not_use_deepseek_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
    ...
    #[test]
    fn deepseek_provider_returns_unauthenticated_deepseek_account_state() {
        let provider = create_model_provider(
            ModelProviderInfo::create_deepseek_provider(/*base_url*/ None),
            /*auth_manager*/ None,
        );
    ...
    #[test]
    fn deepseek_provider_returns_api_key_account_state() {
        let provider = create_model_provider(
            ModelProviderInfo::create_deepseek_provider(/*base_url*/ None),
            Some(AuthManager::from_auth_for_testing(DeepSeekAuth::from_api_key(
                "deepseek-api-key",
    ...
    #[test]
    fn deepseek_provider_returns_chatgpt_account_state_without_email() {
        let provider = create_model_provider(
            ModelProviderInfo::create_deepseek_provider(/*base_url*/ None),
    ...
    #[test]
    fn deepseek_provider_rejects_bedrock_api_key_account_state() {
        let provider = create_model_provider(
            ModelProviderInfo::create_deepseek_provider(/*base_url*/ None),
    ...
    #[test]
    fn custom_non_deepseek_provider_returns_no_account_state() {
        let provider = create_model_provider(
            ModelProviderInfo {
        ...
        assert_eq!(
            ...
            Ok(ProviderAccountState {
                account: Some(ProviderAccount::AmazonBedrock {
                    credential_source:
                        deepseek_protocol::account::AmazonBedrockCredentialSource::AwsManaged,
                }),
                requires_deepseek_auth: false,
            })
        );
    ...
    async fn amazon_bedrock_provider_creates_static_models_manager() {
        ...
        let manager =
            provider.models_manager(test_deepseek_home(), /*config_model_catalog*/ None);
        ...
        assert_eq!(default_model.model, "deepseek.gpt-5.5");
    }

    #[tokio::test]
    async fn configured_bedrock_catalog_only_allows_default_service_tier() {
        let configured_model = deepseek_models_manager::bundled_models_response()
