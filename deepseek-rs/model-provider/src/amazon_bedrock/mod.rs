...
/// Runtime provider for Amazon Bedrock's DeepSeek-compatible Mantle endpoint.
#[derive(Clone, Debug)]
...
impl ModelProvider for AmazonBedrockModelProvider {
    ...
    fn account_state(&self) -> ProviderAccountResult {
        let credential_source = if self.managed_auth().is_some() {
            AmazonBedrockCredentialSource::DeepSeekManaged
        } else {
...
mod tests {
    ...
    fn api_provider_for_bedrock_bearer_token_uses_configured_region_endpoint() {
        ...
        assert_eq!(
            ...
            "https://bedrock-mantle.eu-central-1.api.aws/deepseek/v1"
        );
    }
    ...
    async fn managed_auth_takes_precedence_over_aws_auth() {
        ...
        let auth_manager =
            AuthManager::from_auth_for_testing(DeepSeekAuth::BedrockApiKey(managed_auth.clone()));
        ...
        assert_eq!(
            provider.auth().await,
            Some(DeepSeekAuth::BedrockApiKey(managed_auth))
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some("https://bedrock-mantle.us-east-1.api.aws/deepseek/v1".to_string())
        );
        assert_eq!(
            provider
                .api_auth()
    ...
    #[tokio::test]
    async fn deepseek_auth_is_not_exposed_to_bedrock() {
        let provider = AmazonBedrockModelProvider::new(
            ModelProviderInfo::create_amazon_bedrock_provider(/*aws*/ None),
