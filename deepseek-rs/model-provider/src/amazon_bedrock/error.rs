...
pub(super) const BEDROCK_EXPIRED_SIGNATURE_MESSAGE: &str = concat!(
    ...
    "update or unset it, then restart DeepSeek",
);

pub(super) fn map_api_error(error: ApiError) -> DeepSeekErr {
    let mut error = deepseek_api::map_api_error(error);
    if let DeepSeekErr::UnexpectedStatus(response) = &mut error
