...
use deepseek_aws_auth::AwsAuthContext;
use deepseek_aws_auth::AwsAuthError;
use deepseek_aws_auth::AwsRequestToSign;
use deepseek_client::Request;
use deepseek_client::RequestBody;
use deepseek_client::RequestCompression;
...
fn aws_auth_error_to_codex_error(error: AwsAuthError) -> DeepSeekErr {
    DeepSeekErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    // The Bedrock Mantle front door does not preserve legacy DeepSeek
    ...
    // richer DeepSeek agent requests fail even though raw Responses requests work.
...
/// AWS SigV4 auth provider for Bedrock Mantle DeepSeek-compatible requests.
#[derive(Debug)]
