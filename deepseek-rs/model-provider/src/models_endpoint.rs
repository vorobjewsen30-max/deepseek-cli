...
use deepseek_login::AuthEnvTelemetry;
use deepseek_login::AuthManager;
use deepseek_login::DeepSeekAuth;
use deepseek_login::collect_auth_env_telemetry;
...
/// Provider-owned DeepSeek-compatible `/models` endpoint.
#[derive(Debug)]
...
impl OpenAiModelsEndpoint {
    ...
    async fn auth(&self) -> Option<DeepSeekAuth> {
    ...
    async fn list_models(
        ...
        let _timer =
            deepseek_otel::start_global_timer("codex.remote_models.fetch_update.duration_ms", &[]);
        let auth = self.auth().await;
        let auth_mode = auth.as_ref().map(DeepSeekAuth::auth_mode);
        let api_provider = self.provider_info.to_api_provider(auth_mode)?;
        let api_auth = resolve_provider_auth(auth.as_ref(), &self.provider_info)?;
        let transport = ReqwestTransport::new(build_reqwest_client());
        let auth_telemetry = auth_header_telemetry(api_auth.as_ref());
        let agent_identity_telemetry = if let Some(DeepSeekAuth::AgentIdentity(auth)) = auth.as_ref() {
        ...
        .await
        .map_err(|_| DeepSeekErr::Timeout)?
        .map_err(map_api_error)
    }

    fn auth_env(&self) -> AuthEnvTelemetry {
        let deepseek_api_key_env_enabled = self
            .auth_manager
            .as_ref()
            .is_some_and(|auth_manager| auth_manager.deepseek_api_key_env_enabled());
        collect_auth_env_telemetry(&self.provider_info, deepseek_api_key_env_enabled)
    }
}
...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            ...
            auth.env_deepseek_api_key_present = self.auth_env.deepseek_api_key_env_present,
            auth.env_deepseek_api_key_present = self.auth_env.deepseek_api_key_env_present,
            auth.env_deepseek_api_key_enabled = self.auth_env.deepseek_api_key_env_enabled,
        ...
        tracing::event!(
            target: "deepseek_otel.trace_safe",
            tracing::Level::INFO,
            event.name = "codex.api_request",
            duration_ms = %duration.as_millis(),
            http.response.status_code = status,
