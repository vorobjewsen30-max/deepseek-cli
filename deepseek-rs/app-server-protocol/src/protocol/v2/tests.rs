...
use deepseek_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use deepseek_protocol::config_types::MultiAgentMode;
use deepseek_protocol::items::AgentMessageContent;
use deepseek_protocol::items::AgentMessageItem;
use deepseek_protocol::items::CollabAgentTool as CoreCollabAgentTool;
use deepseek_protocol::items::CollabAgentToolCallItem;
use deepseek_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use deepseek_protocol::items::CommandExecutionItem;
use deepseek_protocol::items::CommandExecutionStatus as CoreCommandExecutionStatus;
use deepseek_protocol::items::DynamicToolCallItem;
use deepseek_protocol::items::DynamicToolCallStatus as CoreDynamicToolCallStatus;
use deepseek_protocol::items::FileChangeItem;
use deepseek_protocol::items::ImageViewItem;
use deepseek_protocol::items::McpToolCallItem;
use deepseek_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use deepseek_protocol::items::ReasoningItem;
use deepseek_protocol::items::SubAgentActivityItem;
use deepseek_protocol::items::TurnItem;
use deepseek_protocol::items::UserMessageItem;
use deepseek_protocol::items::WebSearchItem;
use deepseek_protocol::mcp::CallToolResult;
use deepseek_protocol::mcp::McpServerInfo;
use deepseek_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use deepseek_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use deepseek_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use deepseek_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use deepseek_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use deepseek_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use deepseek_protocol::protocol::AgentStatus as CoreAgentStatus;
use deepseek_protocol::protocol::AskForApproval as CoreAskForApproval;
use deepseek_protocol::protocol::ConversationTextRole;
use deepseek_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use deepseek_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use deepseek_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use deepseek_protocol::protocol::SubAgentActivityKind as CoreSubAgentActivityKind;
use deepseek_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use deepseek_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: deepseek_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        deepseek_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        deepseek_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
#[test]
fn mcp_server_elicitation_request_from_core_deepseek_form_request() {
    let requested_schema = json!({
        "type": "object",
        "properties": {
            "template": {
                "type": "deepseek/imagePicker",
                "title": "Template",
                "items": [{
                    "id": "monthly-review",
    ...
    .expect("DeepSeek form request should convert");
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        deepseek_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn network_requirements_deserializes_legacy_fields() {
    let requirements: NetworkRequirements = serde_json::from_value(json!({
        "allowedDomains": ["api.platform.deepseek.com"],
    ...
    assert_eq!(
        ...
        NetworkRequirements {
            ...
            allowed_domains: Some(vec!["api.platform.deepseek.com".to_string()]),
...
fn network_requirements_serializes_canonical_and_legacy_fields() {
    let requirements = NetworkRequirements {
        ...
        domains: Some(BTreeMap::from([
            ("api.platform.deepseek.com".to_string(), NetworkDomainPermission::Allow),
            (
    ...
    assert_eq!(
        ...
        json!({
            ...
            "domains": {
                "api.platform.deepseek.com": "allow",
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![deepseek_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let dynamic_tool_call_item = TurnItem::DynamicToolCall(DynamicToolCallItem {
        ...
        content_items: Some(vec![
            deepseek_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text: "ok".to_string(),
            },
        ]),
        success: Some(true),
    ...
    let sender_thread_id = deepseek_protocol::ThreadId::default();
    let receiver_thread_id = deepseek_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: deepseek_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(deepseek_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn plugin_source_serializes_local_git_npm_and_remote_variants() {
    ...
    assert_eq!(
        serde_json::to_value(PluginSource::Git {
            url: "https://github.com/deepseek/example.git".to_string(),
        ...
        json!({
            "type": "git",
            "url": "https://github.com/deepseek/example.git",
...
fn plugin_marketplace_entry_serializes_remote_only_path_as_null() {
    assert_eq!(
        serde_json::to_value(PluginMarketplaceEntry {
            name: "deepseek-curated-remote".to_string(),
            path: None,
            interface: None,
            plugins: Vec::new(),
        })
...
fn plugin_read_params_serialization_uses_install_source_fields() {
    ...
    assert_eq!(
        serde_json::from_value::<PluginReadParams>(json!({
            "remoteMarketplaceName": "deepseek-curated-remote",
            "pluginName": "gmail",
        }))
        .unwrap(),
        PluginReadParams {
            marketplace_path: None,
            remote_marketplace_name: Some("deepseek-curated-remote".to_string()),
            plugin_name: "gmail".to_string(),
        },
    );
}
...
fn plugin_skill_read_params_serialization_uses_remote_plugin_id() {
    assert_eq!(
        serde_json::to_value(PluginSkillReadParams {
            remote_marketplace_name: "deepseek-curated-remote".to_string(),
...
fn plugin_share_list_response_serializes_share_items() {
    assert_eq!(
        serde_json::to_value(PluginShareListResponse {
            data: vec![PluginShareListItem {
                plugin: PluginSummary {
                    id: "gmail@deepseek-curated-remote".to_string(),
        ...
        json!({
            "data": [{
                "plugin": {
                    "id": "gmail@deepseek-curated-remote",
                    "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
                    "version": null,
                    "localVersion": null,
                    "name": "gmail",
...
fn plugin_uninstall_params_serialization_omits_force_remote_sync() {
    assert_eq!(
        serde_json::to_value(PluginUninstallParams {
            plugin_id: "gmail@deepseek-curated".to_string(),
        })
        .unwrap(),
        json!({
            "pluginId": "gmail@deepseek-curated",
        }),
    );
...
fn codex_error_info_serializes_http_status_code_in_camel_case() {
    let value = DeepSeekErrorInfo::ResponseTooManyFailedAttempts {
...
fn codex_error_info_serializes_cyber_policy_in_camel_case() {
    assert_eq!(
        serde_json::to_value(DeepSeekErrorInfo::CyberPolicy).unwrap(),
        json!("cyberPolicy")
    );
}
...
fn codex_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = DeepSeekErrorInfo::ActiveTurnNotSteerable {
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(deepseek_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        ...
        collaboration_mode: Some(deepseek_protocol::config_types::CollaborationMode {
            mode: deepseek_protocol::config_types::ModeKind::Plan,
