...
use deepseek_protocol::protocol::RealtimeAudioFrame as CoreRealtimeAudioFrame;
...
pub struct ThreadRealtimeStartParams {
    ...
    /// Leaves DeepSeek response handoffs to the client's explicit append calls instead of forwarding
    ...
    /// Optional prefix added to automatic DeepSeek response items when `codexResponsesAsItems` is true.
    #[ts(optional = nullable)]
    ...
    /// Set to false to start without DeepSeek's startup context. Omitted or null includes it.
    #[ts(optional = nullable)]
