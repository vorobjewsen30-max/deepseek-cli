...
use deepseek_protocol::config_types::ApprovalsReviewer as CoreApprovalsReviewer;
use deepseek_protocol::config_types::SandboxMode as CoreSandboxMode;
...
use deepseek_protocol::protocol::NonSteerableTurnKind as CoreNonSteerableTurnKind;
...
impl From<CoreDeepSeekErrorInfo> for DeepSeekErrorInfo {
    fn from(value: CoreDeepSeekErrorInfo) -> Self {
        match value {
            CoreDeepSeekErrorInfo::ContextWindowExceeded => DeepSeekErrorInfo::ContextWindowExceeded,
            CoreDeepSeekErrorInfo::SessionBudgetExceeded => DeepSeekErrorInfo::SessionBudgetExceeded,
            CoreDeepSeekErrorInfo::UsageLimitExceeded => DeepSeekErrorInfo::UsageLimitExceeded,
            CoreDeepSeekErrorInfo::ServerOverloaded => DeepSeekErrorInfo::ServerOverloaded,
            CoreDeepSeekErrorInfo::CyberPolicy => DeepSeekErrorInfo::CyberPolicy,
            CoreDeepSeekErrorInfo::HttpConnectionFailed { http_status_code } => {
                DeepSeekErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreDeepSeekErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                DeepSeekErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            CoreDeepSeekErrorInfo::InternalServerError => DeepSeekErrorInfo::InternalServerError,
            CoreDeepSeekErrorInfo::Unauthorized => DeepSeekErrorInfo::Unauthorized,
            CoreDeepSeekErrorInfo::BadRequest => DeepSeekErrorInfo::BadRequest,
            CoreDeepSeekErrorInfo::ThreadRollbackFailed => DeepSeekErrorInfo::ThreadRollbackFailed,
            CoreDeepSeekErrorInfo::SandboxError => DeepSeekErrorInfo::SandboxError,
            CoreDeepSeekErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                DeepSeekErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreDeepSeekErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                DeepSeekErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreDeepSeekErrorInfo::ActiveTurnNotSteerable { turn_kind } => {
                DeepSeekErrorInfo::ActiveTurnNotSteerable {
                    turn_kind: turn_kind.into(),
                }
            }
            CoreDeepSeekErrorInfo::Other => DeepSeekErrorInfo::Other,
        }
    }
}
