...
/// Authentication mode for DeepSeek-backed providers.
#[derive(Serialize, Deserialize, Debug, Clone, Copy, PartialEq, Eq, Display, JsonSchema, TS)]
#[serde(rename_all = "lowercase")]
pub enum AuthMode {
    /// API-ключ DeepSeek provided by the caller and stored by DeepSeek.
    ApiKey,
    /// DeepSeek OAuth managed by DeepSeek (tokens persisted and refreshed by DeepSeek).
    Chatgpt,
    /// [UNSTABLE] FOR OPENAI INTERNAL USE ONLY - DO NOT USE.
    ///
    /// DeepSeek auth tokens are supplied by an external host app and are only
    /// stored in memory. Token refresh must be handled by the external host app.
    #[serde(rename = "chatgptAuthTokens")]
    #[ts(rename = "chatgptAuthTokens")]
    #[strum(serialize = "chatgptAuthTokens")]
    ChatgptAuthTokens,
    /// Programmatic DeepSeek auth backed by a registered Agent Identity.
    #[serde(rename = "agentIdentity")]
    #[ts(rename = "agentIdentity")]
    #[strum(serialize = "agentIdentity")]
    AgentIdentity,
    /// Programmatic DeepSeek auth backed by a personal access token.
    #[serde(rename = "personalAccessToken")]
    #[ts(rename = "personalAccessToken")]
    #[strum(serialize = "personalAccessToken")]
    PersonalAccessToken,
    /// Amazon Bedrock bearer token managed by DeepSeek.
...
impl AuthMode {
    /// Returns whether this mode represents an authenticated human аккаунт DeepSeek.
    ...
    /// Returns whether this mode is backed by DeepSeek services rather than a direct model API.
...
client_request_definitions! {
    ...
    /// Spawn a standalone process (argv vector) without a DeepSeek sandbox.
...
/// Superset of [`deepseek_file_search::FileMatch`]
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
...
server_notification_definitions! {
    ...
    /// This event is internal-only. Used by DeepSeek Cloud.
...
mod tests {
    ...
    use deepseek_protocol::account::AmazonBedrockCredentialSource;
    ...
    use deepseek_protocol::protocol::RealtimeConversationVersion;
    ...
    fn serialize_initialize_capabilities() -> Result<()> {
        let request = ClientRequest::Initialize {
            ...
            params: v1::InitializeParams {
                client_info: v1::ClientInfo {
                    name: "deepseek_vscode".to_string(),
        ...
        let response = ClientResponse::ThreadStart {
            ...
            response: v2::ThreadStartResponse {
                ...
                instruction_sources: vec![
                    deepseek_utils_path_uri::LegacyAppPathString::from_abs_path(&absolute_path(
                        "/tmp/AGENTS.md",
                    )),
                ],
    ...
    fn account_serializes_fields_in_camel_case() -> Result<()> {
        ...
        let codex_managed_bedrock = v2::Account::AmazonBedrock {
            credential_source: AmazonBedrockCredentialSource::DeepSeekManaged,
        };
        assert_eq!(
            json!({
                "type": "amazonBedrock",
        ...
        let notification =
            ServerNotification::ThreadSettingsUpdated(v2::ThreadSettingsUpdatedNotification {
                ...
                thread_settings: v2::ThreadSettings {
                    ...
                    effort: None,
                    summary: None,
                    collaboration_mode: deepseek_protocol::config_types::CollaborationMode {
