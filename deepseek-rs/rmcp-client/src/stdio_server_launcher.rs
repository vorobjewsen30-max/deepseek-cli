...
use deepseek_exec_server::ExecEnvPolicy;
...
#[cfg(unix)]
use deepseek_utils_pty::process_group::kill_process_group;
#[cfg(unix)]
use deepseek_utils_pty::process_group::terminate_process_group;
...
impl ExecutorStdioServerLauncher {
    ...
    fn remote_env_policy(remote_env_vars: &[String]) -> ExecEnvPolicy {
        let include_only = if remote_env_vars.is_empty() {
            ...
            // environment, not copied from DeepSeek. Start from `All` only so the
