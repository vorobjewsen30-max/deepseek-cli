...
const OPENAI_FORM_METHOD: &str = "deepseek/form";
...
impl ElicitationClientService {
    pub(crate) fn new(
        ...
        let supports_deepseek_form = client_info
            .capabilities
            .extensions
            .as_ref()
    ...
    async fn handle_request(
        ...
        match request {
            ...
            ServerRequest::CustomRequest(request)
                if request.method == OPENAI_FORM_METHOD && self.supports_deepseek_form =>
            {
                let response = self
                    .create_elicitation(deepseek_form_elicitation(request)?, context)
                    .await?;
...
fn deepseek_form_elicitation(request: CustomRequest) -> Result<Elicitation, rmcp::ErrorData> {
    let params = request
...
mod tests {
    ...
    #[test]
    fn parses_deepseek_form_custom_requests() {
        let elicitation = deepseek_form_elicitation(CustomRequest::new(
            OPENAI_FORM_METHOD,
            Some(json!({
        ...
        .expect("valid deepseek/form request");
