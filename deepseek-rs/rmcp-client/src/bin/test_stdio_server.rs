...
struct TestToolServer {
    ...
    supports_deepseek_form_elicitation: Arc<AtomicBool>,
}
...
impl TestToolServer {
    fn new() -> Self {
        ...
        Self {
            ...
            supports_deepseek_form_elicitation: Arc::new(AtomicBool::new(false)),
        }
    }
    ...
    /// Tool intended for manual testing of DeepSeek TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (DeepSeek TUI):
    /// - Build this binary: `cargo build -p deepseek-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `codex mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in DeepSeek TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            ...
            title: Some("DeepSeek Memo".to_string()),
            description: Some(
...
/// Scenarios for `image_scenario`, intended to exercise DeepSeek TUI handling of MCP image outputs.
///
...
impl ServerHandler for TestToolServer {
    async fn initialize(
        ...
        self.supports_deepseek_form_elicitation.store(
            request
                .capabilities
                .extensions
                .as_ref()
                .is_some_and(|extensions| extensions.contains_key("deepseek/form")),
            Ordering::Relaxed,
        );
