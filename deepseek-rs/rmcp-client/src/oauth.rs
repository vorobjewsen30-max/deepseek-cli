...
//!
//! If the keyring is not available or fails, we fall back to DEEPSEEK_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "DeepSeek MCP Credentials";
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_deepseek_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    struct TempDeepSeekHome {
        _guard: MutexGuard<'static, ()>,
        _dir: tempfile::TempDir,
    }

    impl TempDeepSeekHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create DEEPSEEK_HOME temp dir");
            unsafe {
                std::env::set_var("DEEPSEEK_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempDeepSeekHome {
        fn drop(&mut self) {
            unsafe {
