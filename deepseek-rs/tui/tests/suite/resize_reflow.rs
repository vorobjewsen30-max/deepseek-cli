...
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let deepseek_base_url_config = format!("deepseek_base_url=\"{}/v1\"", server.uri());
    write_config(deepseek_home.path(), &repo_root)?;
    write_auth(deepseek_home.path())?;
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("DEEPSEEK_HOME={}", deepseek_home.path().display()))
            .arg("DEEPSEEK_API_KEY=dummy")
            .arg(codex)
            .arg("-c")
            .arg("analytics.enabled=false")
            .arg("-c")
            .arg(&deepseek_base_url_config)
            .arg("--no-alt-screen")
            .arg("-C")
            .arg(&repo_root)
            .arg(prompt),
...
fn codex_binary(repo_root: &Path) -> Result<PathBuf> {
    ...
    anyhow::ensure!(
        fallback.is_file(),
        "codex binary is unavailable; run `cargo build -p deepseek-cli` first"
    );
    Ok(fallback)
}

fn write_config(deepseek_home: &Path, repo_root: &Path) -> Result<()> {
...
"#
    ...
    std::fs::write(deepseek_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(deepseek_home: &Path) -> Result<()> {
    std::fs::write(
        deepseek_home.join("auth.json"),
        r#"{"DEEPSEEK_API_KEY":"dummy","tokens":null,"last_refresh":null}"#,
    )?;
    Ok(())
}
