...
//! | `DEEPSEEK_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static DEEPSEEK_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// fork reloads complete). The first call persists `name` and `deepseek_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), deepseek_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let deepseek_home_set_ok = DEEPSEEK_HOME.set(deepseek_home.clone()).is_ok();
    if THEME.get().is_some() {
        set_syntax_theme(resolve_theme_with_override(
            name.as_deref(),
            deepseek_home.as_deref(),
        ));
    }
    if !override_set_ok || !deepseek_home_set_ok {
...
pub(crate) fn validate_theme_name(name: Option<&str>, deepseek_home: Option<&Path>) -> Option<String> {
    let name = name?;
    let custom_theme_path_display = deepseek_home
        .map(|home| custom_theme_path(name, home).display().to_string())
        .unwrap_or_else(|| format!("$DEEPSEEK_HOME/themes/{name}.tmTheme"));
    ...
    if let Some(home) = deepseek_home {
...
fn custom_theme_path(name: &str, deepseek_home: &Path) -> PathBuf {
    deepseek_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{deepseek_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, deepseek_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, deepseek_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, deepseek_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {DEEPSEEK_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = deepseek_home
...
fn build_default_theme() -> Theme {
    ...
    let deepseek_home = DEEPSEEK_HOME
        .get()
        .and_then(|deepseek_home| deepseek_home.as_deref());
    resolve_theme_with_override(name, deepseek_home)
}
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = DEEPSEEK_HOME.get()
...
pub(crate) fn resolve_theme_by_name(name: &str, deepseek_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// `.tmTheme` file under `{DEEPSEEK_HOME}/themes/`.
...
/// found in `{deepseek_home}/themes/`.
pub(crate) fn list_available_themes(deepseek_home: Option<&Path>) -> Vec<ThemeEntry> {
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*deepseek_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*deepseek_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*deepseek_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*deepseek_home*/ None).is_none());
    }
