...
const OPENAI_CURATED_TAB_ID: &str = "marketplace:deepseek-curated";
...
const APPS_HELP_ARTICLE_URL: &str = "https://help.platform.deepseek.com/en/articles/11487775-apps-in-chatgpt";
...
const OPENAI_CURATED_LOADING_DESCRIPTION: &str =
    "This updates when DeepSeek Curated plugins finish loading.";
...
impl MarketplaceProduct {
    ...
    fn from_marketplace_name(marketplace_name: &str) -> Self {
        if is_deepseek_curated_marketplace_name(marketplace_name)
    ...
    fn label(self) -> Option<&'static str> {
        match self {
            Self::OpenAiCurated => Some("DeepSeek Curated"),
...
impl ChatWidget {
    ...
    pub(super) fn marketplace_remove_confirmation_popup_params(
        ...
        header.push(Line::from(
            "This removes the configured marketplace from DeepSeek.".dim(),
        ));
    ...
    pub(super) fn plugins_popup_params(
        ...
        let curated_entries =
            plugin_entries_for_marketplaces(marketplaces.iter().filter(|marketplace| {
                MarketplaceProduct::from_marketplace(marketplace).is_by_deepseek()
            }));
        ...
        let (curated_empty_name, curated_empty_description) =
            if curated_loading && !curated_has_entries {
                (
                    "Loading DeepSeek Curated plugins...",
                    OPENAI_CURATED_LOADING_DESCRIPTION,
                )
            } else if let Some(section_error) = by_deepseek_section_error
                && !curated_has_entries
            {
                ("DeepSeek Curated unavailable", section_error.message.as_str())
            } else {
                (
                    "No DeepSeek Curated plugins available",
                    "No DeepSeek Curated plugins available.",
                )
            };
        ...
        if let Some(section_error) = by_deepseek_section_error
            && curated_has_entries
        {
        ...
        tabs.push(SelectionTab {
            ...
            label: "DeepSeek Curated".to_string(),
            header: plugins_header(
                "DeepSeek Curated marketplace.".to_string(),
                format!("Installed {curated_installed} of {curated_total} DeepSeek Curated plugins."),
            ),
            items: curated_items,
        });
        ...
        let mut additional_marketplaces: Vec<&PluginMarketplaceEntry> = marketplaces
            .iter()
            .filter(|marketplace| !MarketplaceProduct::from_marketplace(marketplace).is_by_deepseek())
            .collect();
                ...
                if let Some((location, plugin_name)) = plugin_detail_request {
                    vec![Box::new(move |tx| {
                        ...
                        tx.send(AppEvent::FetchPluginDetail {
                            cwd: cwd.clone(),
                            params: deepseek_app_server_protocol::PluginReadParams {
                                marketplace_path,
...
fn plugin_hook_summary(plugin: &PluginDetail) -> String {
    if plugin.hooks.is_empty() {
        ...
        let mut event_counts = Vec::<(deepseek_app_server_protocol::HookEventName, usize)>::new();
