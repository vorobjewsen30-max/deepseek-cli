...
use deepseek_app_server_protocol::HookErrorInfo;
use deepseek_app_server_protocol::HooksListEntry;
use deepseek_app_server_protocol::HooksListResponse;
use deepseek_app_server_protocol::MarketplaceLoadErrorInfo;
...
async fn plugins_popup_truncates_long_descriptions_in_list_rows() {
    ...
    insta::assert_snapshot!(
        verbose_row,
        @"  [-] Verbose Plugin  Available · DeepSeek Curated · This description…"
    );
    assert!(
        !popup
...
async fn plugin_detail_popup_snapshot_labels_personal_marketplace_as_local() {
    ...
    let mut plugin = plugins_test_detail(
        ...
        &[
            (deepseek_app_server_protocol::HookEventName::PreToolUse, 1),
            (deepseek_app_server_protocol::HookEventName::Stop, 2),
        ],
        &["Figma", "Slack"],
        &["figma-mcp", "docs-mcp"],
    );
...
async fn plugin_detail_popup_distinguishes_admin_installed_from_enabled() {
    ...
    insta::assert_snapshot!(
        popup
            ...
            .trim(),
        @"Figma · Enabled by Admin · DeepSeek Marketplace"
    );
}
...
async fn plugin_detail_popup_shows_local_share_context_as_read_only_snapshot() {
    ...
    let summary = PluginSummary {
        share_context: Some(PluginShareContext {
            ...
            share_url: Some("https://platform.deepseek.com)),
...
async fn plugin_detail_popup_shows_admin_disabled_status_snapshot() {
    ...
    insta::assert_snapshot!(
        status_row,
        @"  Admin Blocked · Disabled by admin · DeepSeek Marketplace"
    );
    assert!(
...
async fn plugins_popup_remote_section_fallback_states_when_remote_plugin_disabled_snapshot() {
    ...
    let curated_loading_popup =
        select_tab_containing(&mut chat, "Loading DeepSeek Curated plugins...");
    ...
    chat.on_plugin_remote_sections_loaded(
        ...
        vec![crate::app_event::PluginRemoteSectionError {
            ...
            message: "Sign in to DeepSeek to load workspace plugins.".to_string(),
        }],
    );
    ...
    let remote_curated_empty_popup =
        select_tab_containing(&mut remote_chat, "No DeepSeek Curated plugins available");
    ...
    insta::assert_snapshot!(
        ...
        .join("\n\n"),
        @r###"
        DeepSeek Curated marketplace.
        Loading DeepSeek Curated plugins...  This updates when DeepSeek Curated plugins finish loading.
        ...
        Workspace unavailable  Sign in to DeepSeek to load workspace plugins.

        DeepSeek Curated marketplace.
        No DeepSeek Curated plugins available  No DeepSeek Curated plugins available.
        "###
    );
}
...
#[tokio::test]
async fn plugins_popup_deepseek_curated_tab_omits_marketplace_in_rows() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Plugins, /*enabled*/ true);
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert!(
        popup.contains("DeepSeek Curated marketplace."),
        "expected DeepSeek Curated tab header, got:\n{popup}"
    );
    assert!(
        popup.contains("Calendar") && !popup.contains("Repo Plugin"),
        "expected DeepSeek Curated tab to show only official marketplace plugins, got:\n{popup}"
    );
    assert!(
        !popup.contains("DeepSeek Marketplace ·"),
...
async fn apps_popup_keeps_existing_full_snapshot_while_partial_refresh_loads() {
    ...
    chat.on_connectors_loaded(
        Ok(ConnectorsSnapshot {
            connectors: vec![
                ...
                AppInfo {
                    id: "connector_deepseek_hidden".to_string(),
                    name: "Hidden DeepSeek".to_string(),
                    ...
                    app_metadata: None,
                    labels: None,
                    install_url: Some("https://example.test/hidden-deepseek".to_string()),
                    is_accessible: true,
                    is_enabled: true,
                    plugin_display_names: Vec::new(),
                },
    ...
    assert!(
        !popup.contains("Hidden DeepSeek"),
...
async fn server_overloaded_error_does_not_switch_models() {
    ...
    handle_error(
        &mut chat,
        "server overloaded",
        Some(DeepSeekErrorInfo::ServerOverloaded),
    );
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &deepseek_feedback::FeedbackDiagnostics::new(vec![deepseek_feedback::FeedbackDiagnostic {
