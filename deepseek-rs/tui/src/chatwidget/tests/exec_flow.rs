...
async fn unified_exec_wait_after_final_agent_message_snapshot() {
    ...
    begin_unified_exec_startup(&mut chat, "call-wait", "proc-1", "cargo test -p deepseek-core");
...
async fn unified_exec_wait_status_renders_command_in_single_details_row_snapshot() {
    ...
    begin_unified_exec_startup(
        ...
        "call-wait-ui",
        "proc-ui",
        "cargo test -p deepseek-core -- --exact some::very::long::test::name",
    );
...
async fn apply_patch_approval_sends_op_with_call_id() {
    ...
    while let Ok(app_ev) = rx.try_recv() {
        ...
        } = app_ev
        {
            ...
            assert_matches!(
                decision,
                deepseek_app_server_protocol::FileChangeApprovalDecision::Accept
            );
            found = true;
            break;
        }
