...
pub(super) async fn test_config() -> Config {
    ...
    let deepseek_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_deepseek_home(deepseek_home.clone(), Vec::new())
            .await
            .expect("config");
    config.deepseek_home = deepseek_home.abs();
    config.sqlite_home = deepseek_home.clone();
    config.log_dir = deepseek_home.join("log");
...
pub(super) async fn make_chatwidget_manual_with_auth(
    ...
    let mut widget = ChatWidget::new_with_op_target(common, super::DeepSeekOpTarget::Direct(op_tx));
    widget.transcript.active_cell = None;
    widget.transcript.active_cell_revision = 0;
    widget.normal_placeholder_text = "Ask DeepSeek to do anything".to_string();
...
fn token_usage_breakdown(usage: TokenUsage) -> deepseek_app_server_protocol::TokenUsageBreakdown {
    deepseek_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    deepseek_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: deepseek_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    codex_error_info: Option<DeepSeekErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = deepseek_shell_command::parse_command::parse_command(&command)
        .into_iter()
    ...
    let item = AppServerThreadItem::CommandExecution {
        ...
        command: deepseek_shell_command::parse_command::shlex_join(&command),
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            deepseek_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_absolute_path(path: &str) -> AbsolutePathBuf {
    std::env::temp_dir()
        .join("deepseek-plugin-menu-tests")
        .join(path)
        .abs()
}
...
pub(super) fn plugins_test_curated_marketplace(
    ...
    PluginMarketplaceEntry {
        ...
        interface: Some(MarketplaceInterface {
            display_name: Some("DeepSeek Marketplace".to_string()),
        }),
        plugins,
    }
}
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(deepseek_app_server_protocol::HookEventName, usize)],
    apps: &[&str],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        marketplace_name: "DeepSeek Marketplace".to_string(),
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    deepseek_app_server_protocol::PluginHookSummary {
...
pub(super) fn hook_run(
    run_id: &str,
    event_name: deepseek_app_server_protocol::HookEventName,
    status: deepseek_app_server_protocol::HookRunStatus,
    status_message: &str,
    entries: Vec<deepseek_app_server_protocol::HookOutputEntry>,
) -> deepseek_app_server_protocol::HookRunSummary {
    deepseek_app_server_protocol::HookRunSummary {
        id: run_id.to_string(),
        event_name,
        handler_type: deepseek_app_server_protocol::HookHandlerType::Command,
        execution_mode: deepseek_app_server_protocol::HookExecutionMode::Sync,
        scope: deepseek_app_server_protocol::HookScope::Turn,
        source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
        source: deepseek_app_server_protocol::HookSource::User,
        display_order: 0,
        status,
        ...
        completed_at: matches!(
            status,
            deepseek_app_server_protocol::HookRunStatus::Completed
                | deepseek_app_server_protocol::HookRunStatus::Failed
                | deepseek_app_server_protocol::HookRunStatus::Blocked
                | deepseek_app_server_protocol::HookRunStatus::Stopped
        )
        .then_some(11),
        duration_ms: matches!(
            status,
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    handle_hook_started(
        ...
        hook_run(
            run_id,
            event_name,
            deepseek_app_server_protocol::HookRunStatus::Running,
            status_message,
            Vec::new(),
        ),
    );
    ...
    handle_hook_completed(
        ...
        hook_run(
            ...
            status_message,
            vec![
                deepseek_app_server_protocol::HookOutputEntry {
                    kind: deepseek_app_server_protocol::HookOutputEntryKind::Warning,
                    text: "Heads up from the hook".to_string(),
                },
                deepseek_app_server_protocol::HookOutputEntry {
                    kind: deepseek_app_server_protocol::HookOutputEntryKind::Context,
...
fn hook_event_label(event_name: deepseek_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        deepseek_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        deepseek_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        deepseek_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        deepseek_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        deepseek_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        deepseek_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        deepseek_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        deepseek_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        deepseek_app_server_protocol::HookEventName::SubagentStop => "SubagentStop",
        deepseek_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
