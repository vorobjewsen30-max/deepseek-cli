...
use deepseek_app_server_protocol::NetworkAccess;
...
async fn forked_thread_history_line_without_name_shows_id_once_snapshot() {
    ...
    chat.config.deepseek_home =
        deepseek_utils_absolute_path::AbsolutePathBuf::from_absolute_path(temp.path())
...
async fn app_server_forked_thread_history_line_uses_app_server_title_snapshot() {
    ...
    assert!(
        ...
        "app-server fork title lookup should not read local DEEPSEEK_HOME"
    );
...
async fn replayed_in_progress_mcp_tool_call_stays_active() {
    ...
    chat.replay_thread_item(
        AppServerThreadItem::McpToolCall {
            ...
            status: deepseek_app_server_protocol::McpToolCallStatus::InProgress,
