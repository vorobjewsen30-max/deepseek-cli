...
impl ChatWidget {
    ...
    fn model_menu_warning_line(&self) -> Option<Line<'static>> {
        let base_url = self.custom_deepseek_base_url()?;
        let warning = format!(
            "Warning: DeepSeek base URL is overridden to {base_url}. Selecting models may not be supported or work properly."
        );
        Some(Line::from(warning.red()))
    }

    fn custom_deepseek_base_url(&self) -> Option<String> {
