...
impl ChatWidget {
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        codex_error_info: &AppServerDeepSeekErrorInfo,
    ) -> bool {
        matches!(
            codex_error_info,
            AppServerDeepSeekErrorInfo::ActiveTurnNotSteerable { .. }
    ...
    pub(super) fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "DeepSeek is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(super) fn handle_non_retry_error(
        &mut self,
        message: String,
        codex_error_info: Option<AppServerDeepSeekErrorInfo>,
    ) {
        if codex_error_info
            .as_ref()
