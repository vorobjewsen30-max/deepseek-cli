...
pub(super) fn app_server_rate_limit_error_kind(
    info: &AppServerDeepSeekErrorInfo,
) -> Option<RateLimitErrorKind> {
    match info {
        AppServerDeepSeekErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServerDeepSeekErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServerDeepSeekErrorInfo::ResponseTooManyFailedAttempts {
...
pub(super) fn is_app_server_cyber_policy_error(info: &AppServerDeepSeekErrorInfo) -> bool {
    matches!(info, AppServerDeepSeekErrorInfo::CyberPolicy)
}
...
impl ChatWidget {
    ...
    pub(super) fn should_prefetch_rate_limits(&self) -> bool {
        self.config.model_provider.requires_deepseek_auth && self.has_chatgpt_account
    }
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::DeepSeekOp(AppCommand::override_turn_context(
                /*cwd*/ None,
