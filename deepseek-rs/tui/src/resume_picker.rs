            ...
            ThreadItem::UserMessage { content, .. } => Some(TranscriptPreviewLine {
                ...
                text: content
                    ...
                    .filter_map(|input| match input {
                        deepseek_app_server_protocol::UserInput::Text { text, .. } => {
                            Some(text.as_str())
                        }
                        _ => None,
                    })
...
impl PickerState {
    ...
    async fn persist_density(&self) -> Result<()> {
        ...
        ConfigEditsBuilder::new(&persistence.deepseek_home)
...
mod tests {
    ...
    fn local_picker_thread_list_params_include_cwd_filter() {
        ...
        let params = thread_list_params(
            ...
            cwd_filter.as_deref(),
            ProviderFilter::MatchDefault(String::from("deepseek")),
...
"#
    ...
    async fn ctrl_o_keeps_toggled_density_when_persistence_fails() {
        let tmp = tempdir().expect("tmpdir");
        let deepseek_home_file = tmp.path().join("deepseek-home-file");
        std::fs::write(&deepseek_home_file, "not a directory").expect("write codex home file");
        let loader = page_only_loader(|_| {});
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
            loader,
        ...
        state.view_persistence = Some(SessionPickerViewPersistence {
            deepseek_home: deepseek_home_file,
        });
    ...
    fn app_server_row_keeps_pathless_threads() {
        ...
        let thread = Thread {
            ...
            model_provider: String::from("deepseek"),
            created_at: 1,
            updated_at: 2,
            recency_at: Some(2),
    ...
    fn thread_to_transcript_cells_renders_core_message_types() {
        ...
        let thread = Thread {
            ...
            git_info: None,
            name: None,
            turns: vec![deepseek_app_server_protocol::Turn {
