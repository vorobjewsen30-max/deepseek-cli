...
const GOAL_FILE_PREFIX: &str = "Read the DeepSeek goal objective file at ";
...
pub(crate) async fn materialize_goal_draft(
    ...
    deepseek_home: Option<&GoalFilePath>,
    draft: GoalDraft,
    ...
    for (placeholder, text) in draft.pending_pastes.iter() {
        ...
        let path = ensure_goal_output_dir(app_server, deepseek_home, &mut output_dir)
            .await?
...
pub(crate) async fn objective_text_for_edit(
    ...
    let Some(path) = objective_file_path(objective, deepseek_home) else {
...
async fn ensure_goal_output_dir(
    ...
    let deepseek_home = deepseek_home
        .context("App server did not report $DEEPSEEK_HOME; cannot materialize goal files")?;
    let path = deepseek_home
        .join(GOAL_ATTACHMENT_DIR)
