...
// The standalone `deepseek-tui` binary prints a short help message before the
...
use deepseek_app_server_client::AppServerClient;
use deepseek_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use deepseek_app_server_client::InProcessAppServerClient;
use deepseek_app_server_client::InProcessClientStartArgs;
use deepseek_app_server_client::RemoteAppServerClient;
use deepseek_app_server_client::RemoteAppServerConnectArgs;
pub use deepseek_app_server_client::RemoteAppServerEndpoint;
use deepseek_app_server_protocol::Account as AppServerAccount;
use deepseek_app_server_protocol::AskForApproval;
use deepseek_app_server_protocol::ConfigWarningNotification;
use deepseek_app_server_protocol::Thread as AppServerThread;
use deepseek_app_server_protocol::ThreadListCwdFilter;
use deepseek_app_server_protocol::ThreadListParams;
use deepseek_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
use deepseek_app_server_protocol::ThreadSourceKind;
use deepseek_cloud_config::cloud_config_bundle_loader_for_storage;
use deepseek_config::CloudConfigBundleLoader;
use deepseek_config::ConfigLoadError;
use deepseek_config::LoaderOverrides;
use deepseek_config::format_config_error_with_source;
use deepseek_exec_server::EnvironmentManager;
use deepseek_exec_server::ExecServerRuntimePaths;
use deepseek_login::AuthConfig;
use deepseek_login::default_client::originator;
use deepseek_login::default_client::set_default_client_residency_requirement;
use deepseek_login::enforce_login_restrictions;
...
use deepseek_utils_home_dir::find_deepseek_home;
use deepseek_utils_oss::ensure_oss_provider_ready;
use deepseek_utils_oss::get_default_model_for_oss_provider;
...
pub(crate) use deepseek_app_server_client::legacy_core;
...
const TUI_LOG_FILE_NAME: &str = "deepseek-tui.log";
...
async fn start_embedded_app_server(
    ...
    feedback: deepseek_feedback::DeepSeekFeedback,
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = deepseek_state::runtime_db_path_for_corruption_error(&err)
                .unwrap_or_else(|| deepseek_state::state_db_path(config.sqlite_home.as_path()));
...
fn remove_legacy_tui_log_file(deepseek_home: &Path) {
    ...
    let _ = std::fs::remove_file(deepseek_home.join("log").join(TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let deepseek_home = find_deepseek_home().wrap_err("failed to resolve DEEPSEEK_HOME")?;
            deepseek_app_server_client::app_server_control_socket_path(&deepseek_home)
...
#[cfg(unix)]
async fn maybe_probe_default_daemon_socket(deepseek_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = deepseek_app_server_client::app_server_control_socket_path(deepseek_home).ok()?;
...
#[cfg(not(unix))]
async fn maybe_probe_default_daemon_socket(_deepseek_home: &Path) -> Option<AbsolutePathBuf> {
    None
}
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        deepseek_feedback::DeepSeekFeedback::new(),
        /*log_db*/ None,
        state_db,
        environment_manager,
    )
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = deepseek_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let deepseek_home = match find_deepseek_home() {
        Ok(deepseek_home) => deepseek_home.to_path_buf(),
        Err(err) => {
    ...
    if let Some(profile_v2) = cli.config_profile_v2.as_ref() {
        let user_config_path = resolve_profile_v2_config_path(&deepseek_home, profile_v2);
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        maybe_probe_default_daemon_socket(&deepseek_home).await
    } else {
        None
    };
    ...
    let environment_manager =
        if should_load_configured_environments(&loader_overrides, &app_server_target) {
            EnvironmentManager::from_deepseek_home(deepseek_home.clone(), Some(local_runtime_paths)).await
        } else {
    ...
    remove_legacy_tui_log_file(config.deepseek_home.as_path());

    let otel_originator = originator().value;
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        deepseek_app_server_client::build_otel_provider(
            &config,
    ...
    if let Some(metrics) = otel.as_ref().and_then(deepseek_otel::OtelProvider::metrics) {
        let _ = deepseek_otel::record_process_start_once(metrics, otel_originator.as_str());
        let telemetry =
            deepseek_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
        let _ = deepseek_state::install_process_db_telemetry(telemetry);
    }
    ...
    match effective_toml.try_into() {
        Ok(config_toml) => {
            match deepseek_app_server_client::migrate_personality_if_needed(
    ...
    let (tui_file_layer, _tui_file_log_guard) = if config_toml_log_dir_configured {
        ...
        let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("deepseek_core=info,deepseek_tui=info,deepseek_rmcp_client=info")
        });
    ...
    let feedback = deepseek_feedback::DeepSeekFeedback::new();
...
async fn run_ratatui_app(
    ...
    let login_status = if initial_config.model_provider.requires_deepseek_auth {
    ...
    let config = if should_show_onboarding {
        ...
        if show_login_screen && !uses_remote_workspace {
            cloud_config_bundle = cloud_config_bundle_loader_for_storage(
                initial_config.deepseek_home.to_path_buf(),
                /*enable_deepseek_api_key_env*/ false,
    ...
    if let Some(w) = crate::render::highlight::set_theme_override(
        ...
        find_deepseek_home().ok().map(AbsolutePathBuf::into_path_buf),
    ) {
    ...
    let required_elevated_sandbox_needs_setup = windows_sandbox_level
        ...
        && config
            ...
            .source
            .is_some()
        && !crate::windows_sandbox::sandbox_setup_is_complete(config.deepseek_home.as_path());
    #[cfg(target_os = "windows")]
...
async fn get_login_status(
    ...
    if !config.model_provider.requires_deepseek_auth {
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, deepseek_config::TomlValue)>,
    loader_overrides: LoaderOverrides,
    strict_config: bool,
    cloud_config_bundle: CloudConfigBundleLoader,
    ...
    match load_config_toml_with_layer_stack(
        ...
        cwd,
        cli_kv_overrides,
        deepseek_config::ConfigLoadOptions {
            loader_overrides,
            strict_config,
            cloud_config_bundle,
        },
...
fn should_show_login_screen(login_status: LoginStatus, config: &Config) -> bool {
    // Only show the login screen for providers that actually require DeepSeek auth
    // (DeepSeek or equivalents). For OSS/other providers, skip login entirely.
    if !config.model_provider.requires_deepseek_auth {
        return false;
    }
...
mod tests {
    ...
    async fn build_config(temp_dir: &TempDir) -> std::io::Result<Config> {
        ConfigBuilder::default()
            .deepseek_home(temp_dir.path().to_path_buf())
            .build()
            .await
    }
    ...
    fn write_session_rollout(
        ...
        let rollout_path = deepseek_home
            .join("sessions")
            .join(year)
            .join(month)
            .join(day)
        ...
        let session_meta = deepseek_protocol::protocol::SessionMeta {
            ...
            source: deepseek_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(deepseek_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: deepseek_app_server_client::app_server_control_socket_path(&deepseek_home)?,
            }
        );
        Ok(())
    }
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        ...
        assert!(
            maybe_probe_default_daemon_socket(deepseek_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let deepseek_home = TempDir::new()?;
        let socket_path =
            deepseek_app_server_client::app_server_control_socket_path(deepseek_home.path())?;
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(deepseek_home.path()).await,
            Some(socket_path)
        );
        Ok(())
    }
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            deepseek_app_server_client::AppServerClient::InProcess(
    ...
    -> std::io::Result<()> {
        ...
        let environment_manager = EnvironmentManager::create_for_tests(
            ...
            Some(ExecServerRuntimePaths::new(
                ...
                /*deepseek_linux_sandbox_exe*/ None,
            )?),
        )
        .await;
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        Box::pin(async {
            ...
            let state_runtime = deepseek_state::StateRuntime::init(
            ...
            let mut builder = deepseek_state::ThreadMetadataBuilder::new(
                thread_id,
                rollout_path.clone(),
                created_at,
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            deepseek_state::state_db_path(occupied_sqlite_home.as_path()).as_path()
        );
        assert!(
            startup_error
                .detail()
    ...
    -> color_eyre::Result<()> {
        ...
        let logs_db_path = deepseek_state::logs_db_path(&sqlite_home);
        ...
        assert!(
            deepseek_state::sqlite_error_detail_is_corruption(startup_error.detail()),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let deepseek_home = temp_dir.path().to_path_buf();
...
"#
        ...
        let trusted_config = ConfigBuilder::default()
            ...
            .deepseek_home(deepseek_home.clone())
