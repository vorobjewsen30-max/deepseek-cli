...
impl AppLinkViewParams {
    pub(crate) fn from_url_app_server_request(
        ...
        request: &deepseek_app_server_protocol::McpServerElicitationRequest,
    ) -> Option<Self> {
        let deepseek_app_server_protocol::McpServerElicitationRequest::Url {
            meta,
            message,
            url,
            elicitation_id,
...
impl AppLinkView {
    ...
    fn install_confirmation_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if is_auth_suggestion {
            for line in wrap(
                if is_codex_apps_auth {
                    "Sign in to the app on DeepSeek in the browser window that just opened."
                } else {
            ...
            for line in wrap(
                "Complete app setup on DeepSeek in the browser window that just opened.",
                usable_width,
            ) {
                lines.push(Line::from(line.into_owned()));
            }
...
mod tests {
    ...
    fn auth_url_request(url: &str) -> deepseek_app_server_protocol::McpServerElicitationRequest {
        deepseek_app_server_protocol::McpServerElicitationRequest::Url {
            ...
            message: "Reconnect Google Calendar on DeepSeek.".to_string(),
            url: url.to_string(),
    ...
    fn codex_apps_auth_url_elicitation_builds_auth_app_link_params() {
        ...
        let request =
            auth_url_request("https://platform.deepseek.com/apps/google-calendar/connector_calendar");
    ...
    fn non_codex_apps_url_elicitation_builds_generic_app_link_params() {
        ...
        let request = deepseek_app_server_protocol::McpServerElicitationRequest::Url {
            meta: None,
    ...
    fn codex_apps_auth_url_elicitation_rejects_untrusted_urls() {
        ...
        for url in [
            ...
            "https://platform.deepseek.com.evil.example/apps/google-calendar/connector_calendar",
    ...
    fn installed_app_has_toggle_action() {
        ...
        assert_eq!(
            view.action_labels(),
            vec!["Manage on DeepSeek", "Disable app", "Back"]
        );
    }
    ...
    fn toggle_action_sends_set_app_enabled_and_updates_label() {
        ...
        assert_eq!(
            view.action_labels(),
            vec!["Manage on DeepSeek", "Enable app", "Back"]
        );
    }
    ...
    fn auth_suggestion_with_reason_snapshot() {
        ...
        let view = AppLinkView::new(
            AppLinkViewParams {
                ...
                description: None,
                instructions: "Sign in to this app in your browser, then return here.".to_string(),
                url: "https://platform.deepseek.com/apps/google-calendar/connector_google_calendar"
                    .to_string(),
                is_installed: true,
                is_enabled: true,
                suggest_reason: Some("Reconnect Google Calendar on DeepSeek.".to_string()),
                suggestion_type: Some(AppLinkSuggestionType::Auth),
                elicitation_target: Some(suggestion_target()),
            },
            tx,
