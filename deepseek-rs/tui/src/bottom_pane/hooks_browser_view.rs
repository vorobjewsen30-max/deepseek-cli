...
impl HooksBrowserView {
    ...
    fn event_rows(&self) -> Vec<EventRow> {
        deepseek_protocol::protocol::HookEventName::iter()
            .map(|event_name| {
    ...
    fn selected_event(&self) -> Option<HookEventName> {
        self.state
            .selected_idx
            .and_then(|idx| deepseek_protocol::protocol::HookEventName::iter().nth(idx))
            .map(Into::into)
    }
    ...
    fn page_len(&self) -> usize {
        match self.page {
            HooksBrowserPage::Events => deepseek_protocol::protocol::HookEventName::iter().count(),
...
fn event_description(event_name: HookEventName) -> &'static str {
    match event_name {
        ...
        HookEventName::Stop => "Right before DeepSeek ends its turn",
    }
}
        ...
        HooksBrowserView::new(
            vec![
                hook(
                    ...
                    HookSource::Plugin,
                    Some("superpowers@deepseek-curated"),
