use deepseek_feedback::DOCTOR_REPORT_ATTACHMENT_FILENAME;
use deepseek_feedback::FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME;
use deepseek_feedback::FeedbackDiagnostics;
use deepseek_feedback::WINDOWS_SANDBOX_LOG_ATTACHMENT_FILENAME;
...
const CODEX_FEEDBACK_INTERNAL_URL: &str = "http://go/deepseek-feedback-internal";
...
pub(crate) fn feedback_success_cell(
    ...
    let mut lines = vec![Line::from(match issue_url.as_ref() {
        Some(_) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            format!("{prefix} Please report this in #deepseek-feedback:")
        }
    ...
    match issue_url {
        Some(url) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            lines.extend([
                ...
                Line::from(vec![
                    "    ".into(),
                    format!("https://go/deepseek-feedback/{thread_id}").bold(),
                ]),
            ]);
        }
        Some(url) => {
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current DeepSeek session logs and diagnostics with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    use crate::app_event::AppEvent;
    use crate::app_event_sender::AppEventSender;
    use deepseek_feedback::FeedbackDiagnostic;
    ...
    fn issue_url_available_for_bug_bad_result_safety_check_and_other() {
        ...
        let expected_slack_url = "http://go/deepseek-feedback-internal".to_string();
        ...
        let expected_external_url = "https://github.com/vorobjewsen30-max/deepseek-cli
    ...
    fn feedback_success_cell_matches_external_bug_copy() {
        ...
        assert_eq!(
            rendered,
            "• Feedback uploaded. Please open an issue using the following URL:\n\n  https://github.com/vorobjewsen30-max/deepseek-cli  Or mention your thread ID thread-1 in an existing issue."
        );
    }
    ...
    fn feedback_success_cell_matches_employee_bug_copy() {
        ...
        assert_eq!(
            rendered,
            "• Feedback uploaded. Please report this in #deepseek-feedback:\n\n  http://go/deepseek-feedback-internal\n\n  Share this and add some info about your problem:\n    https://go/deepseek-feedback/thread-2"
        );
    }
