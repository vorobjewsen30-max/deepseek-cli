...
use deepseek_app_server_client::AppServerPath;
use deepseek_app_server_protocol::AdditionalFileSystemPermissions;
use deepseek_app_server_protocol::AdditionalNetworkPermissions;
use deepseek_app_server_protocol::AdditionalPermissionProfile;
use deepseek_app_server_protocol::AgentMessageDeltaNotification;
...
use deepseek_app_server_protocol::FileUpdateChange;
use deepseek_app_server_protocol::ItemStartedNotification;
use deepseek_app_server_protocol::JSONRPCErrorError;
use deepseek_app_server_protocol::McpServerElicitationRequest;
...
use deepseek_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use deepseek_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use deepseek_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use deepseek_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use deepseek_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
...
use deepseek_app_server_protocol::SessionSource;
...
use deepseek_app_server_protocol::TokenUsageBreakdown;
...
use deepseek_app_server_protocol::UserInput as AppServerUserInput;
...
use deepseek_protocol::models::NetworkPermissions;
use deepseek_protocol::models::PermissionProfile;
use deepseek_protocol::protocol::MAX_THREAD_GOAL_OBJECTIVE_CHARS;
use deepseek_protocol::request_permissions::RequestPermissionProfile;
use deepseek_protocol::user_input::TextElement;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let deepseek_app_server_client::AppServerEvent::ServerNotification(
...
async fn handle_mcp_inventory_result_respects_origin_thread() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: deepseek_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        feedback: deepseek_feedback::DeepSeekFeedback::new(),
        is_first_run: false,
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::DeepSeekOp(Op::UserTurn { items, .. }) => {
                submitted_items = Some(items);
            }
            _ => {}
        }
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: deepseek_app_server_protocol::CollabAgentTool::Wait,
                status: deepseek_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemCompleted(deepseek_app_server_protocol::ItemCompletedNotification {
            thread_id: ThreadId::new().to_string(),
            turn_id: "turn-1".to_string(),
            completed_at_ms: 0,
            item: ThreadItem::CollabAgentToolCall {
                id: "send-1".to_string(),
                tool: deepseek_app_server_protocol::CollabAgentTool::SendInput,
                status: deepseek_app_server_protocol::CollabAgentToolCallStatus::Failed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                ...
                agents_states: HashMap::from([(
                    ...
                    deepseek_app_server_protocol::CollabAgentState {
                        status: deepseek_app_server_protocol::CollabAgentStatus::NotFound,
                        message: None,
                    },
                )]),
            },
...
async fn update_memory_settings_persists_and_updates_widget_config() -> Result<()> {
    ...
    app.config.deepseek_home = deepseek_home.path().to_path_buf().abs();
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let deepseek_home = tempdir()?;
        app.config.deepseek_home = deepseek_home.path().to_path_buf().abs();
        app.config.sqlite_home = deepseek_home.path().to_path_buf();
        ...
        let state_db = deepseek_state::StateRuntime::init(
...
async fn reset_memories_clears_local_memory_directories() -> Result<()> {
    Box::pin(async {
        ...
        let memory_root = deepseek_home.path().join("memories");
...
"#,
    ...
    let op = match app_event_rx.try_recv() {
        Ok(AppEvent::DeepSeekOp(op)) => op,
        other => panic!("expected DeepSeekOp event, got {other:?}"),
    };
    assert_eq!(
        op,
...
-> Result<()> {
    ...
    let deepseek_home = tempdir()?;
    app.config.deepseek_home = deepseek_home.path().to_path_buf().abs();
    let config_toml_path = deepseek_home.path().join("config.toml").abs();
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        available_decisions,
        vec![
            deepseek_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            deepseek_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            deepseek_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            deepseek_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: deepseek_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: deepseek_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: deepseek_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                updated_at: 2,
                recency_at: Some(2),
                status: deepseek_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.0".to_string(),
                source: deepseek_app_server_protocol::SessionSource::Unknown,
                thread_source: None,
                agent_nickname: Some("Robie".to_string()),
                agent_role: Some("explorer".to_string()),
                git_info: None,
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            deepseek_app_server_protocol::ServerRequestResolvedNotification {
...
async fn primary_thread_ignores_child_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        deepseek_app_server_client::AppServerEvent::ServerNotification(
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            deepseek_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        deepseek_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
async fn set_thread_goal_draft_materializes_long_objective_and_confirms_before_paste() -> Result<()>
{
    ...
    let deepseek_home = app_server
        .deepseek_home_path(&app.chat_widget.config_ref().deepseek_home)
        .expect("codex home");
    assert!(goal_files::objective_file_path(&goal.objective, Some(&deepseek_home)).is_some());
    assert_eq!(
        goal_files::objective_text_for_edit(&mut app_server, Some(&deepseek_home), &goal.objective)
            .await
    ...
    for path in [
        format!("/tmp/{suffix}"),
        format!("{deepseek_home}/../other/{suffix}"),
        format!("{deepseek_home}/other/{suffix}"),
    ] {
        assert!(!is_managed(&deepseek_home, &path));
    }
    assert!(!is_managed(
    ...
    let attachments_dir = app.chat_widget.config_ref().deepseek_home.join("attachments");
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        codex_error_info: Some(AppServerDeepSeekErrorInfo::ActiveTurnNotSteerable {
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(
                ServerNotification::ItemStarted(
                    deepseek_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: deepseek_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                deepseek_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                deepseek_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
            /*windows_sandbox_level*/ None,
...
async fn inactive_thread_settings_notification_updates_cached_collaboration_mode() {
    ...
    let notification = ThreadSettingsUpdatedNotification {
        ...
        thread_settings: ThreadSettings {
            ...
            approval_policy: AskForApproval::OnRequest,
            approvals_reviewer: deepseek_app_server_protocol::ApprovalsReviewer::AutoReview,
            sandbox_policy: deepseek_app_server_protocol::SandboxPolicy::ReadOnly {
                network_access: false,
            },
            active_permission_profile: Some(
                deepseek_app_server_protocol::ActivePermissionProfile::read_only(),
            ),
            model: "gpt-plan".to_string(),
            model_provider: "deepseek".to_string(),
            service_tier: None,
