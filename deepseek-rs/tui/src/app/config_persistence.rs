...
impl App {
    ...
    pub(super) async fn apply_permission_profile_selection(
        ...
        self.sync_active_thread_permission_settings_to_cached_session()
            .await;
        self.app_event_tx
            .send(AppEvent::DeepSeekOp(AppCommand::override_turn_context(
                /*cwd*/ None,
                approval_policy,
                approvals_reviewer,
    ...
    pub(super) async fn read_effective_config_after_overridden_write(
        ...
        match crate::config_update::read_effective_config(app_server.request_handle(), cwd).await {
            ...
            Err(err) => {
                ...
                self.chat_widget.add_error_message(format!(
                    "{setting} were saved, but DeepSeek could not refresh the effective config: {err}"
                ));
                None
            }
        }
    ...
    pub(super) fn restore_runtime_theme_from_config(&self) {
        if let Some(name) = self.config.tui_theme.as_deref()
            && let Some(theme) =
                crate::render::highlight::resolve_theme_by_name(name, Some(&self.config.deepseek_home))
        {
        ...
        if let Some(theme) = crate::render::highlight::resolve_theme_by_name(
            auto_theme_name,
            Some(&self.config.deepseek_home),
        ) {
            crate::render::highlight::set_syntax_theme(theme);
        }
    }
...
fn windows_sandbox_mode_from_effective_config(
    effective_config: &ConfigReadResponse,
) -> Option<deepseek_config::types::WindowsSandboxModeToml> {
...
mod tests {
    ...
    -> Result<()> {
        let mut app = make_test_app().await;
        let deepseek_home = tempdir()?;
        let required_policy = deepseek_protocol::protocol::AskForApproval::Never;
        let cloud_config_bundle =
            deepseek_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
...
"#
    ...
    -> Result<()> {
        ...
        let deepseek_home = tempdir()?;
        app.config.deepseek_home = deepseek_home.path().to_path_buf().abs();
        std::fs::write(deepseek_home.path().join("config.toml"), "[broken")?;
