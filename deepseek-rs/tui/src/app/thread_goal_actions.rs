...
const EPHEMERAL_THREAD_GOAL_ERROR_MESSAGE: &str = concat!(
    ...
    "Run `codex` to start a saved session, or `deepseek resume` / `/resume` to reopen one.",
);
...
impl App {
    ...
    pub(super) async fn open_thread_goal_editor(
        ...
        let deepseek_home = app_server.deepseek_home_path(&self.config.deepseek_home);
        match goal_files::objective_text_for_edit(app_server, deepseek_home.as_ref(), &goal.objective)
            .await
        {
