...
pub(super) async fn fetch_account_token_activity(
    request_handle: AppServerRequestHandle,
) -> Result<deepseek_app_server_protocol::GetAccountTokenUsageResponse> {
...
pub(super) async fn fetch_workspace_messages(
    request_handle: AppServerRequestHandle,
) -> Result<deepseek_app_server_protocol::GetWorkspaceMessagesResponse> {
...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<deepseek_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: deepseek_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
...
fn plugin_remote_section_error_next_step(label: &str, err: &str) -> &'static str {
    ...
    if err.contains("api key auth is not supported") {
        "Sign in with DeepSeek auth; API key auth cannot load remote plugin catalogs."
    } else if err.contains("authentication required")
        || err.contains("not signed in")
        || err.contains("not logged in")
    {
        "Sign in to DeepSeek, then try loading this section again."
    ...
    {
        "Ask a workspace admin to enable DeepSeek plugins or plugin sharing."
        ...
        "Update DeepSeek, then try opening the shared plugin again."
...
fn plugin_sharing_disabled_remote_section_error() -> PluginRemoteSectionError {
    PluginRemoteSectionError {
        ...
        message: "Plugin sharing is disabled for this DeepSeek session. Enable plugin sharing to load shared plugins.".to_string(),
    }
}

const CLI_HIDDEN_PLUGIN_MARKETPLACES: &[&str] = &["deepseek-bundled"];
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![deepseek_app_server_protocol::ConfigEdit {
...
pub(super) type McpInventoryMaps = (
    HashMap<String, deepseek_protocol::mcp::Tool>,
    HashMap<String, Vec<deepseek_protocol::mcp::Resource>>,
    HashMap<String, Vec<deepseek_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                deepseek_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
                deepseek_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
                deepseek_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
                deepseek_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
...
mod tests {
    ...
    #[test]
    fn hide_cli_only_plugin_marketplaces_removes_deepseek_bundled() {
        let mut response = PluginListResponse {
            marketplaces: vec![
                PluginMarketplaceEntry {
                    name: "deepseek-bundled".to_string(),
                    path: Some(test_absolute_path("/marketplaces/deepseek-bundled")),
                    interface: None,
                    plugins: Vec::new(),
                },
                PluginMarketplaceEntry {
                    name: "deepseek-curated".to_string(),
                    path: Some(test_absolute_path("/marketplaces/deepseek-curated")),
                    interface: None,
                    plugins: Vec::new(),
                },
            ],
