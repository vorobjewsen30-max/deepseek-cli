...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<deepseek_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<deepseek_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(deepseek_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, deepseek_protocol::mcp::Tool>,
