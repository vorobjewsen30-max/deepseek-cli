//! The main DeepSeek TUI chat surface.
//!
...
use deepseek_app_server_protocol::DeepSeekErrorInfo as AppServerDeepSeekErrorInfo;
...
use deepseek_app_server_protocol::CommandExecutionSource as ExecCommandSource;
use deepseek_app_server_protocol::CreditsSnapshot;
...
use deepseek_app_server_protocol::ModelVerification as AppServerModelVerification;
use deepseek_app_server_protocol::RateLimitReachedType;
...
use deepseek_app_server_protocol::SkillMetadata as ProtocolSkillMetadata;
use deepseek_app_server_protocol::SkillsListResponse;
use deepseek_app_server_protocol::ThreadGoal as AppThreadGoal;
use deepseek_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use deepseek_config::Constrained;
use deepseek_config::ConstraintResult;
...
use deepseek_core_skills::model::SkillMetadata;
use deepseek_features::FEATURES;
use deepseek_features::Feature;
#[cfg(test)]
use deepseek_git_utils::CommitLogEntry;
use deepseek_git_utils::current_branch_name;
use deepseek_git_utils::get_git_repo_root;
use deepseek_git_utils::local_git_branches;
use deepseek_git_utils::recent_commits;
use deepseek_otel::RuntimeMetricsSummary;
use deepseek_otel::SessionTelemetry;
use deepseek_plugin::PluginCapabilitySummary;
use deepseek_protocol::ThreadId;
use deepseek_protocol::account::PlanType;
use deepseek_protocol::approvals::GuardianAssessmentAction;
use deepseek_protocol::approvals::GuardianAssessmentDecisionSource;
use deepseek_protocol::approvals::GuardianAssessmentEvent;
use deepseek_protocol::approvals::GuardianAssessmentStatus;
...
use deepseek_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use deepseek_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
use deepseek_protocol::request_permissions::RequestPermissionsEvent;
use deepseek_protocol::user_input::ByteRange;
...
use deepseek_utils_cli::resume_hint;
use deepseek_utils_path_uri::PathUri;
use deepseek_utils_plugins::mention_syntax::PLUGIN_TEXT_MENTION_SIGIL;
use deepseek_utils_plugins::mention_syntax::TOOL_MENTION_SIGIL;
...
const TRUSTED_ACCESS_FOR_CYBER_VERIFICATION_WARNING: &str = "Your conversations have multiple flags for possible cybersecurity risk. Responses may take longer because extra safety checks are on. To get authorized for security work, join the Trusted Access for Cyber program: https://platform.deepseek.com/cyber";
const MEMORIES_DOC_URL: &str = "https://platform.deepseek.com/docs
...
use deepseek_utils_approval_presets::ApprovalPreset;
use deepseek_utils_approval_presets::builtin_approval_presets;
...
const DEFAULT_OPENAI_BASE_URL: &str = "https://api.deepseek.com/v1";
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) feedback: deepseek_feedback::DeepSeekFeedback,
...
/// and by sending requests back to deepseek-core.
///
...
pub(crate) struct ChatWidget {
    ...
    codex_op_target: DeepSeekOpTarget,
...
fn request_permissions_from_params(
    params: deepseek_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
    ...
    pub(crate) fn open_feedback_consent(&mut self, category: crate::app_event::FeedbackCategory) {
        ...
        let include_windows_sandbox_log =
            codex_windows_sandbox::current_log_file_path_for_deepseek_home(&self.config.deepseek_home)
                .is_file();
                ...
                .filter_map(|item| match item {
                    ...
                    UserInput::Mention { name, path } => {
                        ...
                        let mention = if let Some(plugin_id) = plugin_id {
                            ...
                            deepseek_connectors::metadata::connector_mention_slug_from_name(name)
                        } else {
                            name.clone()
                        };
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.codex_op_target {
            DeepSeekOpTarget::Direct(codex_op_tx) => {
            ...
            DeepSeekOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::DeepSeekOp(op));
            }
        }
        true
    }
