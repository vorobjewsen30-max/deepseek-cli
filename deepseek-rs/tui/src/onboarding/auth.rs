...
//!
//! This module owns the auth-step state machine (вход через DeepSeek/device-code/API
...
use deepseek_app_server_protocol::AuthMode as ApiAuthMode;
...
const API_KEY_DISABLED_MESSAGE: &str = "Вход по API-ключу отключён.";
fn onboarding_request_id() -> deepseek_app_server_protocol::RequestId {
    deepseek_app_server_protocol::RequestId::String(Uuid::new_v4().to_string())
}
...
pub(super) async fn cancel_login_attempt(
    ...
    let _ = request_handle
        .request_typed::<deepseek_app_server_protocol::CancelLoginAccountResponse>(
...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "  ".into(),
                "Войдите в DeepSeek CLI, используя API-ключ DeepSeek".into(),
            ]),
            Line::from(vec![
                "  ".into(),
                "Получите ключ на https://platform.deepseek.com/api_keys".into(),
            ]),
            "".into(),
        ];
        ...
        if !self.is_api_login_allowed() {
            lines.push(
                "  Вход по API-ключу отключён в этом окружении."
                    .dim()
                    .into(),
            );
            lines.push("".into());
    ...
    fn render_chatgpt_success_message(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "✓ Вход выполнен успешно!"
                ...
                .into(),
            "".into(),
            "  Перед началом работы:".into(),
            "".into(),
            "  Настройте уровень автономности DeepSeek CLI".into(),
            Line::from(vec![
                "  Подробнее читайте в ".into(),
                crate::terminal_hyperlinks::osc8_hyperlink(
                    "https://platform.deepseek.com/docs",
                    "документации DeepSeek",
                )
                .underlined(),
            ])
            .dim(),
            "".into(),
            "  DeepSeek CLI может ошибаться".into(),
            "  Проверяйте код и команды перед выполнением"
                .dim()
                .into(),
            "".into(),
            "  Работает на DeepSeek API".into(),
            Line::from(vec![
                "  Использует тарификацию по факту использования через ".into(),
                crate::terminal_hyperlinks::osc8_hyperlink(
            ...
            "".into(),
            Line::from(vec![
                "  Нажмите ".fg(Color::Cyan),
                self.confirm_binding().into(),
                " для продолжения".fg(Color::Cyan),
            ]),
        ];
    ...
    fn render_api_key_configured(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "✓ API-ключ DeepSeek настроен".fg(Color::Green).into(),
            "".into(),
            "  DeepSeek CLI будет использовать оплату по факту использования.".into(),
        ];
    ...
    fn render_api_key_entry(&self, area: Rect, buf: &mut Buffer, state: &ApiKeyInputState) {
        ...
        let mut intro_lines: Vec<Line> = vec![
            Line::from(vec![
                "> ".into(),
                "Введите API-ключ DeepSeek".bold(),
            ]),
            "".into(),
            "  Вставьте или введите ваш ключ. Он будет сохранён локально в auth.json.".into(),
            "  Получите ключ на https://platform.deepseek.com/api_keys".into(),
            "".into(),
        ];
        if state.prepopulated_from_env {
            intro_lines.push("  Обнаружена переменная окружения DEEPSEEK_API_KEY.".into());
            intro_lines.push(
        ...
        let content_line: Line = if state.value.is_empty() {
            vec!["Вставьте или введите ваш API-ключ".dim()].into()
        } else {
    ...
    fn save_api_key(&mut self, api_key: String) {
        ...
        tokio::spawn(async move {
                ...
                .await
            {
                ...
                Err(err) => {
                    *error.write().unwrap() = Some(format!("Ошибка сохранения API-ключа: {err}"));
                    *sign_in_state.write().unwrap() = SignInState::ApiKeyEntry(ApiKeyInputState {
                        value: api_key,
                        prepopulated_from_env: false,
                    });
    ...
    /// Kicks off the DeepSeek auth flow and keeps the UI state consistent with the attempt.
    fn start_chatgpt_login(&mut self) {
        // If we're already authenticated with DeepSeek, don't start a new login –
...
mod tests {
    ...
    async fn widget_forced_chatgpt() -> (AuthModeWidget, TempDir) {
        ...
        let config = ConfigBuilder::default()
            .deepseek_home(deepseek_home_path.clone())
            .build()
            .await
            .unwrap();
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            environment_manager: Arc::new(
                deepseek_app_server_client::EnvironmentManager::default_for_tests(),
            ),
    ...
    async fn cancel_active_attempt_notifies_device_code_login() {
        ...
        *widget.sign_in_state.write().unwrap() =
            SignInState::ChatGptDeviceCode(ContinueWithDeviceCodeState::ready(
                "request-1".to_string(),
                "login-1".to_string(),
                "https://platform.deepseek.com/device".to_string(),
                "ABCD-EFGH".to_string(),
            ));
