...
use deepseek_ansi_escape::ansi_escape_line;
...
use deepseek_app_server_protocol::SandboxMode as AppServerSandboxMode;
...
use deepseek_app_server_protocol::SkillErrorInfo;
...
use deepseek_app_server_protocol::WriteStatus;
...
use deepseek_models_manager::model_presets::HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG;
use deepseek_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
#[cfg(target_os = "windows")]
use deepseek_protocol::permissions::FileSystemSandboxKind;
...
use deepseek_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        deepseek_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&deepseek_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&deepseek_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[deepseek_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&deepseek_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<deepseek_app_server_protocol::CommandExecutionApprovalDecision> {
...
fn resume_hint_for_resumable_thread(
    ...
    deepseek_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        ...
        Some(AppServerDeepSeekErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
impl App {
    ...
    pub async fn run(
        ...
        let requires_deepseek_auth = bootstrap.requires_deepseek_auth;
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            deepseek_login::default_client::originator().value,
...
See the раскладка клавиш documentation for supported actions and examples."
            )
        })?;
        #[cfg(not(debug_assertions))]
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.deepseek_home.clone();
        ...
        if requires_deepseek_auth && has_chatgpt_account {
