...
const CHATGPT_USAGE_URL: &str = "https://platform.deepseek.com
...
impl StatusHistoryCell {
    ...
    fn new(
        ...
        let show_chatgpt_usage_link = config.model_provider.requires_deepseek_auth;
    ...
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            StatusAccountDisplay::ChatGpt { email, plan } => match (email, plan) {
                ...
                (None, None) => "DeepSeek".to_string(),
            },
            StatusAccountDisplay::ApiKey => {
                "API key configured (run codex login to use DeepSeek)".to_string()
            }
        });
        ...
        // The DeepSeek usage page only applies to providers backed by DeepSeek auth;
        ...
        lines.push(Line::from(Vec::<Span<'static>>::new()));
        // Hide token usage only for DeepSeek subscribers
...
fn format_model_provider(config: &Config, runtime_base_url: Option<&str>) -> Option<String> {
    ...
    let is_default_deepseek = provider.is_deepseek() && base_url.is_none();
    if is_default_deepseek {
        return None;
    }
