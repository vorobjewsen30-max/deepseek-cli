...
mod tests {
    ...
    async fn test_config(deepseek_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let deepseek_home = TempDir::new().expect("temp codex home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = deepseek_home.path().join("global.md");
        let config = test_config(&deepseek_home, &cwd).await;
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let deepseek_home = TempDir::new().expect("temp codex home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = deepseek_home.path().join("override.md");
        let config = test_config(&deepseek_home, &cwd).await;
