...
async fn test_config(temp_home: &TempDir) -> Config {
    let mut config = ConfigBuilder::default()
        .deepseek_home(temp_home.path().to_path_buf())
...
async fn status_snapshot_shows_chatgpt_plan_without_email() {
    ...
    .expect("write email-less DeepSeek auth");
    ...
    let account_display = bootstrap
        ...
        .expect("bootstrap should return аккаунт DeepSeek display");
    assert_eq!(
        account_display,
...
async fn status_model_provider_uses_bedrock_runtime_base_url_and_gates_usage_link() {
    ...
    let captured_at = chrono::Local
        ...
        .expect("timestamp");
    let model_slug = get_model_offline_for_tests(config.model.as_deref());
    let runtime_base_url = "https://bedrock-mantle.eu-west-1.api.aws/deepseek/v1";
    ...
    assert!(
        !rendered.contains("https://platform.deepseek.com),
        "expected /status to hide DeepSeek usage link for Bedrock, got: {rendered}"
    );

    config.model_provider_id = "deepseek-proxy".to_string();
    config.model_provider = ModelProviderInfo {
        name: "DeepSeek Proxy".to_string(),
        base_url: Some("https://deepseek-proxy.example/v1".to_string()),
    ...
    assert!(
        rendered.contains("https://platform.deepseek.com),
        "expected /status to show DeepSeek usage link for DeepSeek-auth proxy, got: {rendered}"
    );
    ...
    assert_eq!(
        wide_destinations,
        vec!["https://platform.deepseek.com
    );
...
async fn status_snapshot_uses_default_reasoning_when_config_empty() {
    ...
    let remote_connection = RemoteConnectionStatus {
        address: "unix:///tmp/deepseek-home/app-server-control/app-server-control.sock".to_string(),
