...
pub(crate) fn load_local_chatgpt_auth(
    ...
    if matches!(auth.auth_mode, Some(AuthMode::ApiKey)) || auth.deepseek_api_key.is_some() {
        return Err("local auth is not a вход через DeepSeek".to_string());
    }

    let tokens = auth
        .tokens
        .ok_or_else(|| "local DeepSeek auth is missing token data".to_string())?;
    let access_token = tokens.access_token;
    let chatgpt_account_id = tokens
        .account_id
        .or(tokens.id_token.chatgpt_account_id.clone())
        .ok_or_else(|| "local DeepSeek auth is missing chatgpt account id".to_string())?;
    if let Some(expected_workspaces) = forced_chatgpt_workspace_id
        && !expected_workspaces.contains(&chatgpt_account_id)
    {
        return Err(format!(
            "local DeepSeek auth must use one of workspace(s) {expected_workspaces:?}, but found {chatgpt_account_id:?}",
        ));
    }
...
mod tests {
    ...
    fn write_chatgpt_auth(deepseek_home: &Path, plan_type: &str) {
        ...
        let auth = AuthDotJson {
            ...
            tokens: Some(TokenData {
                id_token: deepseek_login::token_data::parse_chatgpt_jwt_claims(&id_token)
    ...
    fn loads_local_chatgpt_auth_from_managed_auth() {
        ...
        write_chatgpt_auth(deepseek_home.path(), "business");
    ...
    fn rejects_api_key_auth() {
        ...
        assert_eq!(err, "local auth is not a вход через DeepSeek");
    }
    ...
    fn preserves_usage_based_plan_type_wire_name() {
        let deepseek_home = TempDir::new().expect("tempdir");
        write_chatgpt_auth(deepseek_home.path(), "self_serve_business_usage_based");
