...
use deepseek_app_server_client::AppServerEvent;
use deepseek_app_server_client::AppServerPath;
use deepseek_app_server_client::AppServerRequestHandle;
use deepseek_app_server_client::TypedRequestError;
...
use deepseek_app_server_protocol::ConfigBatchWriteParams;
use deepseek_app_server_protocol::ConfigRequirementsReadResponse;
use deepseek_app_server_protocol::ConfigWriteResponse;
use deepseek_app_server_protocol::ExternalAgentConfigDetectParams;
use deepseek_app_server_protocol::ExternalAgentConfigDetectResponse;
use deepseek_app_server_protocol::ExternalAgentConfigImportParams;
use deepseek_app_server_protocol::ExternalAgentConfigImportResponse;
use deepseek_app_server_protocol::ExternalAgentConfigMigrationItem;
...
use deepseek_app_server_protocol::Model as ApiModel;
use deepseek_app_server_protocol::ModelListParams;
use deepseek_app_server_protocol::ModelListResponse;
use deepseek_app_server_protocol::NewThreadModelDefaults;
use deepseek_app_server_protocol::RateLimitSnapshot;
use deepseek_app_server_protocol::RequestId;
use deepseek_app_server_protocol::ReviewDelivery;
...
use deepseek_app_server_protocol::ThreadMemoryMode;
...
use deepseek_protocol::deepseek_models::ModelAvailabilityNux;
use deepseek_protocol::deepseek_models::ModelPreset;
use deepseek_protocol::deepseek_models::ModelServiceTier;
use deepseek_protocol::deepseek_models::ModelUpgrade;
use deepseek_protocol::deepseek_models::ReasoningEffortPreset;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<deepseek_protocol::account::PlanType>,
    /// Whether the configured model provider needs DeepSeek-style auth. Combined
    /// with `has_chatgpt_account` to decide if a startup rate-limit prefetch
    /// should be fired.
    pub(crate) requires_deepseek_auth: bool,
...
impl AppServerSession {
    ...
    pub(crate) fn deepseek_home_path(
        &self,
        local_deepseek_home: &AbsolutePathBuf,
    ) -> Option<AppServerPath> {
        self.client.deepseek_home(local_deepseek_home)
    }
    ...
    pub(crate) async fn bootstrap(&mut self, config: &Config) -> Result<AppServerBootstrap> {
        ...
        let (
            ...
            Some(Account::Chatgpt { email, plan_type }) => {
                let feedback_audience = if email
                    .as_deref()
                    .is_some_and(|email| email.ends_with("@platform.deepseek.com"))
                {
        ...
        Ok(AppServerBootstrap {
            ...
            status_account_display,
            plan_type,
            requires_deepseek_auth: account.requires_deepseek_auth,
            default_model,
            feedback_audience,
            has_chatgpt_account,
            available_models,
    ...
    pub(crate) async fn turn_start(
        ...
        approvals_reviewer: deepseek_protocol::config_types::ApprovalsReviewer,
        permissions_override: TurnPermissionsOverride,
        workspace_roots: &[AbsolutePathBuf],
        model: String,
        effort: Option<deepseek_protocol::deepseek_models::ReasoningEffort>,
        summary: Option<deepseek_protocol::config_types::ReasoningSummary>,
        service_tier: Option<Option<String>>,
        collaboration_mode: Option<deepseek_protocol::config_types::CollaborationMode>,
        personality: Option<deepseek_protocol::config_types::Personality>,
...
pub(crate) fn status_account_display_from_auth_mode(
    ...
    plan_type: Option<deepseek_protocol::account::PlanType>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<deepseek_app_server_protocol::ApprovalsReviewer> {
...
fn sandbox_mode_from_permission_profile(
    ...
    cwd: &std::path::Path,
) -> Option<deepseek_app_server_protocol::SandboxMode> {
    match permission_profile {
        PermissionProfile::Disabled => {
            Some(deepseek_app_server_protocol::SandboxMode::DangerFullAccess)
        }
        ...
        PermissionProfile::Managed { .. } => {
            ...
            if file_system_policy.has_full_disk_write_access() {
                permission_profile
                    ...
                    .is_enabled()
                    .then_some(deepseek_app_server_protocol::SandboxMode::DangerFullAccess)
            } else if file_system_policy.can_write_path_with_cwd(cwd, cwd) {
                Some(deepseek_app_server_protocol::SandboxMode::WorkspaceWrite)
            } else {
                Some(deepseek_app_server_protocol::SandboxMode::ReadOnly)
            }
        }
    }
}
...
fn turn_permissions_overrides(
    ...
    Option<deepseek_app_server_protocol::SandboxPolicy>,
    Option<String>,
) {
...
fn display_permission_profile_from_thread_response(
    sandbox: &deepseek_app_server_protocol::SandboxPolicy,
    cwd: &std::path::Path,
    config: &Config,
    thread_params_mode: ThreadParamsMode,
) -> PermissionProfile {
...
async fn thread_session_state_from_thread_response(
    ...
    reasoning_effort: Option<deepseek_protocol::deepseek_models::ReasoningEffort>,
    config: &Config,
) -> Result<ThreadSessionState, String> {
    ...
    let history_config =
        deepseek_message_history::HistoryConfig::new(config.deepseek_home.clone(), &config.history);
    let (log_id, entry_count) = deepseek_message_history::history_metadata(&history_config).await;
...
mod tests {
    ...
    fn rate_limit_snapshot(limit_id: &str) -> RateLimitSnapshot {
        RateLimitSnapshot {
            ...
            limit_name: None,
            primary: Some(deepseek_app_server_protocol::RateLimitWindow {
                used_percent: 0,
    ...
    fn legacy_turn_permissions_project_to_sandbox_when_explicitly_overridden() {
        ...
        assert_eq!(
            sandbox_policy,
            Some(deepseek_app_server_protocol::SandboxPolicy::ReadOnly {
    ...
    async fn resume_response_restores_turns_from_thread_items() {
        ...
        let response = ThreadResumeResponse {
            thread: deepseek_app_server_protocol::Thread {
                ...
                source: deepseek_app_server_protocol::SessionSource::Cli,
                thread_source: None,
                agent_nickname: None,
                agent_role: None,
                git_info: None,
                ...
                turns: vec![Turn {
                    ...
                    items: vec![
                        deepseek_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            client_id: None,
                            content: vec![deepseek_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        deepseek_app_server_protocol::ThreadItem::AgentMessage {
            ...
            approval_policy: deepseek_app_server_protocol::AskForApproval::Never,
            approvals_reviewer: deepseek_app_server_protocol::ApprovalsReviewer::User,
        ...
        let embedded_config = ConfigBuilder::default()
            .deepseek_home(temp_dir.path().join("embedded-deepseek-home"))
            .harness_overrides(ConfigOverrides {
                default_permissions: Some(BUILT_IN_PERMISSION_PROFILE_WORKSPACE.to_string()),
                ..ConfigOverrides::default()
            })
    ...
    async fn embedded_thread_response_uses_local_config_profile() {
        ...
        assert_eq!(
            display_permission_profile_from_thread_response(
                &deepseek_app_server_protocol::SandboxPolicy::DangerFullAccess,
                cwd.as_path(),
                &config,
                ThreadParamsMode::Embedded,
            ),
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        deepseek_message_history::append_entry("older", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        deepseek_message_history::append_entry("newer", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        ...
        let session = thread_session_state_from_thread_response(
            ...
            deepseek_protocol::config_types::ApprovalsReviewer::User,
            PermissionProfile::read_only(),
            /*active_permission_profile*/ None,
            test_path_buf("/tmp/project").abs(),
            Vec::new(),
    ...
    fn status_account_display_from_auth_mode_uses_remapped_plan_labels() {
        let business = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(deepseek_protocol::account::PlanType::EnterpriseCbpUsageBased),
        );
        assert!(matches!(
            business,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let team = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(deepseek_protocol::account::PlanType::SelfServeBusinessUsageBased),
        );
        assert!(matches!(
            team,
            Some(StatusAccountDisplay::ChatGpt {
