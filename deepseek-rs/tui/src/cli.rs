...
use clap::Parser;
use deepseek_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `deepseek resume` subcommand.
    // These are not exposed as user flags on the base `deepseek` команда.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `deepseek fork` subcommand.
    // These are not exposed as user flags on the base `deepseek` команда.
    #[clap(skip)]
