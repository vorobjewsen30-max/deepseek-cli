//! Ambient terminal rendering for the DeepSeek companion.
//!
...
impl AmbientPet {
    ...
    /// DEEPSEEK_HOME cache, and records the terminal protocol support snapshot used
    ...
    pub(crate) fn load(
        ...
        let pet = Pet::load_with_deepseek_home(
            ...
            /*deepseek_home*/ Some(deepseek_home),
        )
        .with_context(|| "load ambient pet")?;
        let cache_dir = deepseek_home
            .join("cache")
            .join("tui-pets")
            .join("frame-cache")
            .join(&pet.id)
