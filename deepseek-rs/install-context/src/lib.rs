...
pub struct InstallContext {
    ...
    pub package_layout: Option<DeepSeekPackageLayout>,
}
...
pub enum InstallMethod {
    ...
    /// A DeepSeek binary launched through the npm-managed `codex.js` shim.
    Npm,
    /// A DeepSeek binary launched through the bun-managed `codex.js` shim.
    Bun,
    /// A DeepSeek binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled DeepSeek binaries, custom
    /// internal launchers, and tests that execute DeepSeek from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let deepseek_home = deepseek_utils_home_dir::find_deepseek_home().ok();
        Self::from_exe_with_deepseek_home(
            is_macos,
            current_exe,
            managed_by_npm,
            managed_by_bun,
    ...
    fn from_exe_with_deepseek_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        managed_by_npm: bool,
        managed_by_bun: bool,
        deepseek_home: Option<&Path>,
    ) -> Self {
        let package_layout = current_exe.and_then(DeepSeekPackageLayout::from_exe);
        let method = if managed_by_npm {
            InstallMethod::Npm
        } else if managed_by_bun {
            InstallMethod::Bun
        } else if let Some(exe_path) = current_exe {
            install_method_from_exe(exe_path, deepseek_home, package_layout.as_ref(), is_macos)
        } else {
            InstallMethod::Other
        };
...
fn install_method_from_exe(
    ...
    if let Some(standalone_method) = standalone_install_method(exe_path, deepseek_home, package_layout)
    {
...
fn standalone_install_method(
    ...
    let canonical_deepseek_home = canonical_absolute_path(deepseek_home?)?;
    ...
    let releases_root = canonical_deepseek_home
        .join("packages")
...
mod tests {
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let context = InstallContext::from_exe_with_deepseek_home(
            /*is_macos*/ false,
    ...
    fn detects_package_layout_independently_from_install_method() -> std::io::Result<()> {
        ...
        let package_layout = DeepSeekPackageLayout {
    ...
    fn npm_and_bun_take_precedence() {
        let npm_context = InstallContext::from_exe_with_deepseek_home(
            /*is_macos*/ false,
        ...
        let bun_context = InstallContext::from_exe_with_deepseek_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/codex")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ true,
