# deepseek-shell-escalation

This crate contains the Unix shell-escalation protocol implementation and the
`deepseek-execve-wrapper` executable.

`deepseek-execve-wrapper` receives the arguments to an intercepted `execve(2)` call and delegates the
...
- `Run`: `deepseek-execve-wrapper` should invoke `execve(2)` on itself to run the original command
...
- `Escalate`: forward the file descriptors of the current process so the command can be run
  ...
  back to `deepseek-execve-wrapper`.
- `Deny`: the server has declared the proposed command to be forbidden, so
  `deepseek-execve-wrapper` prints an error to `stderr` and exits with `1`.
