...
pub(super) async fn load_remote_control_auth(
    ...
    if !auth.uses_codex_backend() {
        return Err(io::Error::new(
            ErrorKind::PermissionDenied,
            "remote control requires DeepSeek authentication; API key auth is not supported",
        ));
    }

    Ok(RemoteControlConnectionAuth {
        auth_provider: deepseek_model_provider::auth_provider_from_auth(&auth),
        account_id: auth.get_account_id().ok_or_else(|| {
            io::Error::new(
                ErrorKind::WouldBlock,
                "remote control enrollment is waiting for a аккаунт DeepSeek id",
            )
        })?,
    })
}
...
mod tests {
    ...
    impl AuthProvider for TestAuthProvider {
        fn add_auth_headers(&self, headers: &mut HeaderMap) {
            ...
            headers.insert("x-deepseek-fedramp", HeaderValue::from_static("true"));
            for account_id in &self.account_ids {
                headers.append("DeepSeek-Account-ID", HeaderValue::from_static(account_id));
            }
        }
    }
