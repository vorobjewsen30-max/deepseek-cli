...
use deepseek_core::util::backoff;
...
mod tests {
    ...
    pub(super) async fn remote_control_state_runtime(deepseek_home: &TempDir) -> Arc<StateRuntime> {
    ...
    async fn connect_remote_control_websocket_requires_chatgpt_auth() {
        ...
        assert_eq!(
            err.to_string(),
            "remote control requires DeepSeek authentication"
        );
        assert_eq!(*current_enrollment.lock().await, None);
        assert_eq!(
            status_rx.borrow().clone(),
