...
pub use deepseek_state::LogEntry;
...
pub type StateDbHandle = Arc<deepseek_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        deepseek_state::StateRuntime::init(sqlite_home.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    deepseek_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == deepseek_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, deepseek_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = deepseek_state::state_db_path(config.sqlite_home());
    if !tokio::fs::try_exists(&state_path).await.unwrap_or(false) {
        deepseek_state::record_fallback(
            "get_state_db",
            "db_unavailable",
    ...
    let runtime = match deepseek_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    ...
    originator: &str,
) -> deepseek_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == deepseek_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<deepseek_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&deepseek_state::StateRuntime>,
    deepseek_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.deepseek_home() != deepseek_home {
        warn!(
            "state db deepseek_home mismatch: expected {}, got {}",
            ctx.deepseek_home().display(),
            deepseek_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => deepseek_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => deepseek_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => deepseek_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
    ...
    relation_filter: Option<deepseek_state::ThreadRelationFilter>,
    archived: bool,
    search_term: Option<&str>,
) -> Option<deepseek_state::ThreadsPage> {
    let ctx = context?;
    if ctx.deepseek_home() != deepseek_home {
        warn!(
            "state db deepseek_home mismatch: expected {}, got {}",
    ...
    let filters = deepseek_state::ThreadFilterOptions {
        ...
        sort_direction: match sort_direction {
            SortDirection::Asc => deepseek_state::SortDirection::Asc,
            SortDirection::Desc => deepseek_state::SortDirection::Desc,
        },
        search_term,
    };
