...
use deepseek_state::BackfillStatus;
...
async fn extract_metadata_from_rollout_uses_session_meta() {
    ...
    let outcome = extract_metadata_from_rollout(&path, "deepseek")
        .await
        .expect("extract");

    let builder = builder_from_session_meta(&session_meta_line, path.as_path()).expect("builder");
    let mut expected = builder.build("deepseek");
    apply_rollout_item(&mut expected, &rollout_line.item, "deepseek");
...
async fn extract_metadata_from_rollout_rejects_unknown_history_mode() {
    ...
    assert!(
        extract_metadata_from_rollout(&path, "deepseek")
            .await
            .is_err()
    );
}
...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let deepseek_home = dir.path().to_path_buf();
    ...
    let runtime = deepseek_state::StateRuntime::init(deepseek_home.clone(), "test-provider".to_string())
        .await
        .expect("initialize runtime");
    let first_watermark = backfill_watermark_for_path(deepseek_home.as_path(), first_path.as_path());
    ...
    backfill_sessions(runtime.as_ref(), deepseek_home.as_path(), "test-provider").await;
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let deepseek_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = deepseek_home.join(".");
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = deepseek_home.join("sessions");
