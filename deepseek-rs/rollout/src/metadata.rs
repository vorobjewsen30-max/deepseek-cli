...
use deepseek_state::BackfillState;
use deepseek_state::BackfillStats;
use deepseek_state::BackfillStatus;
use deepseek_state::DB_ERROR_METRIC;
use deepseek_state::DB_METRIC_BACKFILL;
use deepseek_state::DB_METRIC_BACKFILL_DURATION_MS;
use deepseek_state::ExtractionOutcome;
use deepseek_state::ThreadMetadataBuilder;
use deepseek_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = deepseek_otel::global();
    ...
    let sessions_root = deepseek_home.join(SESSIONS_SUBDIR);
    let archived_root = deepseek_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(deepseek_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(deepseek_home: &Path, path: &Path) -> String {
    path.strip_prefix(deepseek_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
