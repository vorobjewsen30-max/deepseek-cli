//! Persist DeepSeek session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use deepseek_protocol::protocol::GitInfo as ProtocolGitInfo;
...
use deepseek_protocol::protocol::SessionContextWindow;
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided DeepSeek home directory.
    #[allow(clippy::too_many_arguments)]
    ...
    async fn list_threads_with_db_fallback(
        ...
        let deepseek_home = config.deepseek_home();
        ...
        deepseek_state::record_fallback("list_threads", "db_error", /*telemetry_override*/ None);
...
async fn list_threads_from_files_desc(
    ...
    if let Some(search_term) = search_term {
        ...
        loop {
            ...
            filter_thread_items_by_search_term(deepseek_home, &mut page.items, Some(search_term))
                .await?;
...
async fn list_threads_from_files_desc_unfiltered(
    ...
    if archived {
        let root = deepseek_home.join(ARCHIVED_SESSIONS_SUBDIR);
        get_threads_in_root(
            root,
            page_size,
            cursor,
...
async fn list_threads_from_files_asc(
    ...
    filter_thread_items_by_search_term(deepseek_home, &mut all_items, search_term).await?;
...
async fn filter_thread_items_by_search_term(
    ...
    let thread_ids = items
        ...
        .filter_map(|item| item.thread_id)
        .collect::<HashSet<_>>();
    let thread_names = find_thread_names_by_ids(deepseek_home, &thread_ids).await?;
    items.retain(|item| {
        item.thread_id
...
fn precompute_log_file_info(
    ...
    let mut dir = config.deepseek_home().to_path_buf();
...
impl From<deepseek_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: deepseek_state::ThreadsPage) -> Self {
        let deepseek_state::ThreadsPage {
            items,
            parent_thread_ids,
            next_anchor,
            num_scanned_rows,
...
fn thread_item_from_state_metadata(
    item: deepseek_state::ThreadMetadata,
    parent_thread_id: Option<ThreadId>,
) -> ThreadItem {
    ThreadItem {
...
async fn select_resume_path_from_db_page(
    page: &deepseek_state::ThreadsPage,
    filter_cwd: Option<&Path>,
    default_provider: &str,
) -> Option<PathBuf> {
    match filter_cwd {
