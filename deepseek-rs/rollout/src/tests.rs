...
async fn insert_state_db_thread(
    ...
    let runtime = deepseek_state::StateRuntime::init(home.to_path_buf(), TEST_PROVIDER.to_string())
        .await
...
async fn test_model_provider_filter_selects_only_matching_sessions() -> Result<()> {
    ...
    let deepseek_id = Uuid::from_u128(1);
    ...
    let deepseek_id_str = deepseek_id.to_string();
    let none_id_str = none_id.to_string();
    let deepseek_filter = provider_vec(&["deepseek"]);
    let deepseek_sessions = get_threads(
        home,
        /*page_size*/ 10,
        /*cursor*/ None,
        ThreadSortKey::CreatedAt,
        NO_SOURCE_FILTER,
        Some(deepseek_filter.as_slice()),
        /*cwd_filters*/ None,
        "deepseek",
    )
    .await?;
    assert_eq!(deepseek_sessions.items.len(), 2);
    let deepseek_ids: Vec<_> = deepseek_sessions
        .items
        .iter()
        .filter_map(|item| item.thread_id.as_ref().map(ToString::to_string))
        .collect();
    assert!(deepseek_ids.contains(&deepseek_id_str));
    assert!(deepseek_ids.contains(&none_id_str));
