...
pub async fn append_thread_name(
    ...
    append_session_index_entry(deepseek_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(deepseek_home);
...
fn session_index_path(deepseek_home: &Path) -> PathBuf {
    deepseek_home.join(SESSION_INDEX_FILE)
}
