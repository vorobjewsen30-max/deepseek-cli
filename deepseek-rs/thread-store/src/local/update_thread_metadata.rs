...
use deepseek_rollout::ARCHIVED_SESSIONS_SUBDIR;
...
pub(super) async fn update_thread_metadata(
    ...
    deepseek_rollout::state_db::reconcile_rollout(
...
async fn refresh_resolved_rollout_path(resolved: &mut ResolvedRolloutPath) {
    if let Some(path) = deepseek_rollout::existing_rollout_path(resolved.path.as_path()).await {
        resolved.path = path;
    }
}
...
async fn metadata_for_missing_sqlite_row(
    ...
    patch: &ThreadMetadataPatch,
) -> ThreadStoreResult<deepseek_state::ThreadMetadata> {
...
async fn canonical_history_mode(
    ...
    let session_meta = match read_session_meta_line(rollout_path).await {
        ...
        Err(err) => {
            if deepseek_rollout::existing_rollout_path(rollout_path)
                .await
                .is_none()
...
async fn apply_thread_git_info_to_rollout(
    ...
    session_meta.git = Some(GitInfo {
        commit_hash: sha.as_deref().map(deepseek_git_utils::GitSha::new),
        branch: branch.clone(),
...
async fn apply_thread_name(
    ...
    append_thread_name(store.config.deepseek_home.as_path(), thread_id, &name)
        .await
        .map_err(|err| ThreadStoreError::Internal {
...
fn rollout_path_is_archived(store: &LocalThreadStore, path: &Path) -> bool {
    path.starts_with(store.config.deepseek_home.join(ARCHIVED_SESSIONS_SUBDIR))
}
...
mod tests {
    ...
    async fn update_thread_metadata_sets_name_on_active_rollout_and_indexes_name() {
        ...
        let latest_name = deepseek_rollout::find_thread_name_by_id(home.path(), &thread_id)
            .await
    ...
    async fn update_thread_metadata_sets_git_info() {
            ...
            .update_thread_metadata(UpdateThreadMetadataParams {
                ...
                patch: ThreadMetadataPatch {
                    git_info: Some(GitInfoPatch {
                        ...
                        origin_url: Some(Some("https://github.com/vorobjewsen30-max/deepseek-cli))),
                    }),
                    ..Default::default()
                },
                include_archived: false,
        ...
        assert_eq!(
            ...
            Some("https://github.com/vorobjewsen30-max/deepseek-cli)
        );
    }
