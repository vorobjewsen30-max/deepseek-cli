...
use deepseek_rollout::find_thread_names_by_ids;
use deepseek_rollout::parse_cursor;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => deepseek_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => deepseek_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => deepseek_rollout::ThreadSortKey::RecencyAt,
    };
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => deepseek_rollout::SortDirection::Asc,
        SortDirection::Desc => deepseek_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        deepseek_home: store.config.deepseek_home.clone(),
        sqlite_home: store.config.sqlite_home.clone(),
        cwd: store.config.deepseek_home.clone(),
    ...
    if names.len() < thread_ids.len()
        && let Ok(legacy_names) =
            find_thread_names_by_ids(store.config.deepseek_home.as_path(), &thread_ids).await
    {
...
pub(super) async fn list_rollout_threads(
    state_db: Option<deepseek_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&deepseek_rollout::Cursor>,
    sort_key: deepseek_rollout::ThreadSortKey,
    sort_direction: deepseek_rollout::SortDirection,
) -> ThreadStoreResult<deepseek_rollout::ThreadsPage> {
    if let Some(relation_filter) = params.relation_filter {
        let relation_filter = match relation_filter {
            ThreadRelationFilter::DirectChildrenOf(parent_thread_id) => {
                deepseek_state::ThreadRelationFilter::DirectChildrenOf(parent_thread_id)
            }
            ThreadRelationFilter::DescendantsOf(ancestor_thread_id) => {
                deepseek_state::ThreadRelationFilter::DescendantsOf(ancestor_thread_id)
            }
        };
        let page = deepseek_rollout::state_db::list_threads_db(
            state_db.as_deref(),
