...
// store to persist аккаунт DeepSeek, session, auth, or other user-specific cookie
// data.
...
/// Adds the process-local DeepSeek Cloudflare cookie jar used by DeepSeek HTTP clients.
///
/// WARNING: this jar is global within the process. It is only acceptable because it hardcodes a
/// small allowlist of Cloudflare cookie names and refuses all other DeepSeek cookies. Do not store
/// аккаунт DeepSeek, session, auth, or other user-specific cookies here. If a future caller needs
...
mod tests {
    ...
    fn stores_and_returns_cloudflare_cookies_for_chatgpt_hosts() {
        ...
        let url = reqwest::Url::parse("https://api.deepseek.com/v1/responses").unwrap();
    ...
    fn does_not_return_chatgpt_cloudflare_cookies_for_other_hosts() {
        let store = ChatGptCloudflareCookieStore::default();
        let chatgpt_url =
            reqwest::Url::parse("https://api.deepseek.com/v1/responses").unwrap();
        let api_url = reqwest::Url::parse("https://api.deepseek.com/v1/responses").unwrap();
        let set_cookie = HeaderValue::from_static("_cfuvid=visitor; Path=/; Secure; HttpOnly");
    ...
    fn rejects_plain_http_chatgpt_cookie_urls() {
        ...
        let https_url = reqwest::Url::parse("https://api.deepseek.com/v1/responses")
            .expect("URL should parse");
        let set_cookie = HeaderValue::from_static("_cfuvid=visitor; Path=/; Secure; HttpOnly");
