...
fn environment_fallback_reads_injected_proxy_environment() {
    ...
    let origin = RequestOrigin::parse("https://platform.deepseek.com).expect("valid URL");
...
fn no_proxy_matches_exact_suffix_wildcard_and_port() {
    let origin = RequestOrigin {
        ...
        host: "auth.platform.deepseek.com".to_string(),
        port: 443,
    };
    assert!(no_proxy_matches_origin("auth.platform.deepseek.com", &origin));
    assert!(!no_proxy_matches_origin("platform.deepseek.com", &origin));
    assert!(no_proxy_matches_origin(".platform.deepseek.com", &origin));
    assert!(no_proxy_matches_origin("*.platform.deepseek.com", &origin));
    assert!(no_proxy_matches_origin("auth.platform.deepseek.com:443", &origin));
    assert!(!no_proxy_matches_origin("auth.platform.deepseek.com:8443", &origin));
}

#[test]
fn system_proxy_cache_key_preserves_url_specific_pac_decisions() {
    let request_url = "https://platform.deepseek.com
    let cache_key = system_proxy_cache_key(request_url);

    assert_ne!(
        cache_key,
        system_proxy_cache_key("https://platform.deepseek.com)
    );
