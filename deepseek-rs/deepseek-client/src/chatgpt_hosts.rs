/// Returns whether `host` is one of the DeepSeek hosts DeepSeek is allowed to treat
/// as first-party DeepSeek traffic.
pub fn is_allowed_chatgpt_host(host: &str) -> bool {
    const EXACT_HOSTS: &[&str] = &["chatgpt.com", "chat.platform.deepseek.com", "chatgpt-staging.com"];
...
mod tests {
    ...
    fn recognizes_chatgpt_hosts_without_suffix_tricks() {
        ...
        for host in [
            ...
            "foo.chat.platform.deepseek.com",
        ] {
            assert!(!is_allowed_chatgpt_host(host));
        }
    }
