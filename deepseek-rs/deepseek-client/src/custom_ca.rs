//! Custom CA handling for DeepSeek outbound HTTP and websocket clients.
//!
//! DeepSeek constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!
//! - read CA material from `DEEPSEEK_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
//!   because DeepSeek itself commonly runs spawned test processes under seatbelt, so this is not just
...
use deepseek_utils_rustls_provider::ensure_rustls_crypto_provider;
...
pub const CODEX_CA_CERT_ENV: &str = "DEEPSEEK_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set DEEPSEEK_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// Builds a reqwest client that honors DeepSeek custom CA environment variables.
///
/// Callers supply the baseline builder configuration they need, and this helper layers in custom
/// CA handling before finally constructing the client. `DEEPSEEK_CA_CERTIFICATE` takes precedence
...
/// easy mistake to make when adding a new outbound DeepSeek HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a DeepSeek custom CA bundle is configured.
///
/// This is the websocket-facing sibling of [`build_reqwest_client_with_custom_ca`]. When
/// `DEEPSEEK_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
fn maybe_build_rustls_client_config_with_env(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the DeepSeek custom
...
trait EnvSource {
    ...
    ///
    /// `DEEPSEEK_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the DeepSeek-specific override.
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for DeepSeek CA overrides.
    ///
    /// This accepts a few common real-world variants so DeepSeek behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// DeepSeek only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
