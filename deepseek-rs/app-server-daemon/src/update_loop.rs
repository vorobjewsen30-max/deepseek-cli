...
pub(crate) fn reexec_managed_updater(managed_codex_bin: &std::path::Path) -> Result<()> {
    ...
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed DeepSeek binary {}",
...
async fn install_latest_standalone() -> Result<()> {
    let script = reqwest::get("")
        .await
        .context("failed to fetch standalone DeepSeek updater")?
        .error_for_status()
        .context("standalone DeepSeek updater request failed")?
        .bytes()
        .await
        .context("failed to read standalone DeepSeek updater")?;
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone DeepSeek updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone DeepSeek updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone DeepSeek updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone DeepSeek updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone DeepSeek updater exited with status {status}")
    }
}
