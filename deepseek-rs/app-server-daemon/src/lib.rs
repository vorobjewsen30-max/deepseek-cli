...
use deepseek_app_server_transport::app_server_control_socket_path;
...
impl Daemon {
    fn from_environment() -> Result<Self> {
        ...
        let socket_path = app_server_control_socket_path(deepseek_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = deepseek_home.as_path().join(STATE_DIR_NAME);
        Ok(Self {
            socket_path,
            ...
            managed_codex_bin: managed_codex_bin(deepseek_home.as_path()),
        })
    }
    ...
    fn ensure_managed_codex_bin(&self) -> Result<()> {
        ...
        Err(anyhow!(
            "managed standalone DeepSeek install not found at {managed_codex_path}\n\n\
             This command requires the standalone install managed by the DeepSeek installer, because \
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                "backend": "pid",
                "managedDeepSeekPath": "codex",
                "managedDeepSeekVersion": "1.2.3",
