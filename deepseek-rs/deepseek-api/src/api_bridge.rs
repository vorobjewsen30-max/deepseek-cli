...
use chrono::DateTime;
use chrono::Utc;
use deepseek_protocol::auth::PlanType;
use deepseek_protocol::error::DeepSeekErr;
use deepseek_protocol::error::RetryLimitReachedError;
use deepseek_protocol::error::UnexpectedResponseError;
use deepseek_protocol::error::UsageLimitReachedError;
use http::HeaderMap;
use serde::Deserialize;
use serde_json::Value;

pub fn map_api_error(err: ApiError) -> DeepSeekErr {
    match err {
        ApiError::ContextWindowExceeded => DeepSeekErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => DeepSeekErr::QuotaExceeded,
        ApiError::UsageNotIncluded => DeepSeekErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => DeepSeekErr::Stream(message, delay),
        ApiError::Stream(msg) => DeepSeekErr::Stream(msg, None),
        ApiError::ServerOverloaded => DeepSeekErr::ServerOverloaded,
        ApiError::Api { status, message } => {
            let user_message = api_error_user_message(status, &message);
            DeepSeekErr::UnexpectedStatus(UnexpectedResponseError {
                status,
                body: message,
                user_message,
                url: None,
        ...
        ApiError::InvalidRequest { message } => DeepSeekErr::InvalidRequest(message),
        ApiError::CyberPolicy { message } => DeepSeekErr::CyberPolicy { message },
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return DeepSeekErr::ServerOverloaded;
                }
                ...
                if status == http::StatusCode::BAD_REQUEST {
                            ...
                            == Some(CYBER_POLICY_ERROR_CODE)
                    {
                        ...
                        DeepSeekErr::CyberPolicy { message }
                    } else if body_text
                        .contains("The image data you provided does not represent a valid image")
                    {
                        DeepSeekErr::InvalidImageRequest()
                    } else {
                        DeepSeekErr::InvalidRequest(body_text)
                    }
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return DeepSeekErr::UsageLimitReached(UsageLimitReachedError {
                            ...
                            return DeepSeekErr::UsageNotIncluded;
                        }
                    }

                    DeepSeekErr::RetryLimit(RetryLimitReachedError {
                        status,
            ...
            TransportError::RetryLimit => DeepSeekErr::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
            }),
            TransportError::Timeout => DeepSeekErr::RequestTimeout,
            TransportError::Network(msg) | TransportError::Build(msg) => {
                DeepSeekErr::Stream(msg, None)
            }
        },
        ApiError::RateLimit(msg) => DeepSeekErr::Stream(msg, None),
    }
}
...
const X_OPENAI_AUTHORIZATION_ERROR_HEADER: &str = "x-deepseek-authorization-error";
