...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, DeepSeekErr::ServerOverloaded));
}
...
fn map_api_error_maps_cloudflare_blocked_response_to_user_message() {
    ...
    let DeepSeekErr::UnexpectedStatus(err) = err else {
        panic!("expected DeepSeekErr::UnexpectedStatus, got {err:?}");
    };
    assert_eq!(
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let DeepSeekErr::CyberPolicy { message } = err else {
        panic!("expected DeepSeekErr::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let DeepSeekErr::InvalidRequest(message) = err else {
        panic!("expected DeepSeekErr::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    let DeepSeekErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected DeepSeekErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
