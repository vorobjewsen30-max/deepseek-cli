...
impl<T: HttpTransport> RealtimeCallClient<T> {
    ...
    pub async fn create_with_session_and_headers(
        ...
        trace!(target: "deepseek_api::realtime_websocket::wire", "realtime call request SDP: {sdp}");
...
mod tests {
    ...
    async fn sends_sdp_offer_as_raw_body() {
        ...
        assert_eq!(request.url, "https://api.deepseek.com/v1/realtime/calls");
        assert_eq!(
    ...
    async fn extracts_call_id_from_forwarded_backend_location() {
        ...
        assert_eq!(
            request.url,
            "https://api.deepseek.com/v1/realtime/calls"
        );
        assert_eq!(
            request.body,
            Some(RequestBody::Raw(Bytes::from_static(b"v=offer\r\n")))
    ...
    async fn sends_api_session_call_as_multipart_body() {
        ...
        assert_eq!(
            request.url,
            "https://api.deepseek.com/v1/realtime/calls?intent=quicksilver&architecture=avas"
        );
        assert_eq!(
            request.headers.get(CONTENT_TYPE).unwrap(),
            HeaderValue::from_static(MULTIPART_CONTENT_TYPE)
