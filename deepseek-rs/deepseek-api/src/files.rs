...
pub enum OpenAiFileError {
    ...
    #[error("failed to send DeepSeek file request to {url}: {source}")]
    Request {
        url: String,
        #[source]
        source: reqwest::Error,
    },
    #[error("DeepSeek file request to {url} failed with status {status}: {body}")]
    UnexpectedStatus {
        url: String,
        status: StatusCode,
        body: String,
    },
    #[error("failed to parse DeepSeek file response from {url}: {source}")]
    Decode {
        url: String,
        #[source]
        source: serde_json::Error,
    },
    #[error("DeepSeek file upload for `{file_id}` is not ready yet")]
    UploadNotReady { file_id: String },
    #[error("DeepSeek file upload for `{file_id}` failed: {message}")]
...
pub fn deepseek_file_uri(file_id: &str) -> String {
        ...
        match finalize_payload.status.as_str() {
            "success" => {
                return Ok(UploadedOpenAiFile {
                    ...
                    uri: deepseek_file_uri(&create_payload.file_id),
...
fn build_reqwest_client() -> reqwest::Client {
    build_reqwest_client_with_custom_ca(reqwest::Client::builder()).unwrap_or_else(|error| {
        tracing::warn!(error = %error, "failed to build DeepSeek file upload client");
        reqwest::Client::new()
    })
}
...
mod tests {
    ...
    impl AuthProvider for ChatGptTestAuth {
        fn add_auth_headers(&self, headers: &mut reqwest::header::HeaderMap) {
            ...
            headers.insert("DeepSeek-Account-ID", HeaderValue::from_static("account_id"));
        }
    }
    ...
    #[tokio::test]
    async fn upload_deepseek_file_returns_canonical_uri() {
