...
use deepseek_api::AuthError;
use deepseek_api::AuthProvider;
use deepseek_api::Compression;
use deepseek_api::Provider;
use deepseek_api::ResponsesApiRequest;
use deepseek_api::ResponsesClient;
use deepseek_api::ResponsesOptions;
use deepseek_client::HttpTransport;
use deepseek_client::Request;
use deepseek_client::RequestBody;
use deepseek_client::Response;
...
impl AuthProvider for StaticAuth {
    fn add_auth_headers(&self, headers: &mut HeaderMap) {
        ...
        if let Ok(header) = HeaderValue::from_str(&self.account_id) {
            headers.insert("DeepSeek-Account-ID", header);
        }
    }
}
...
fn provider(name: &str) -> Provider {
    Provider {
        ...
        query_params: None,
        headers: HeaderMap::new(),
        retry: deepseek_api::RetryConfig {
            max_attempts: 1,
...
struct FlakyTransportState {
    attempts: i64,
    requests: Vec<(RequestBody, HeaderMap, deepseek_client::RequestCompression)>,
}
...
impl FlakyTransport {
    ...
    fn requests(&self) -> Vec<(RequestBody, HeaderMap, deepseek_client::RequestCompression)> {
        self.state
            .lock()
            .expect("flaky transport state mutex should not be poisoned")
            .requests
...
impl AuthProvider for FailsOnceAuth {
    ...
    fn apply_auth(&self, request: Request) -> deepseek_api::AuthProviderFuture<'_> {
...
async fn streaming_client_adds_auth_headers() -> Result<()> {
    ...
    let client = ResponsesClient::new(transport, provider("deepseek"), auth);
    ...
    let account_header = req.headers.get("DeepSeek-Account-ID");
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(requests[0].2, deepseek_client::RequestCompression::None);
    Ok(())
}
