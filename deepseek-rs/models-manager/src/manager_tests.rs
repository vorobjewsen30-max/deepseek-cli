...
use deepseek_login::ExternalAuth;
use deepseek_login::ExternalAuthRefreshContext;
use deepseek_login::ExternalAuthTokens;
use deepseek_login::TokenData;
...
impl ExternalAuth for TestExternalApiKeyAuth {
    ...
    fn resolve(&self) -> deepseek_login::ExternalAuthFuture<'_, Option<ExternalAuthTokens>> {
        Box::pin(async {
    ...
    ) -> deepseek_login::ExternalAuthFuture<'_, ExternalAuthTokens> {
        Box::pin(async {
...
fn deepseek_manager_for_tests(
    deepseek_home: std::path::PathBuf,
    endpoint_client: Arc<dyn ModelsEndpointClient>,
) -> OpenAiModelsManager {
    deepseek_manager_for_tests_with_auth(
        deepseek_home,
        endpoint_client,
...
fn deepseek_manager_for_tests_with_auth(
    deepseek_home: std::path::PathBuf,
    endpoint_client: Arc<dyn ModelsEndpointClient>,
    auth_manager: Option<Arc<AuthManager>>,
) -> OpenAiModelsManager {
    OpenAiModelsManager::new(deepseek_home, endpoint_client, auth_manager)
}
...
async fn chatgpt_auth_tokens_for_tests(deepseek_home: &Path) -> DeepSeekAuth {
    let auth_dot_json = deepseek_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        deepseek_api_key: None,
        tokens: Some(TokenData {
            id_token: deepseek_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(deepseek_home).expect("codex home should be created");
    std::fs::write(
    ...
    DeepSeekAuth::from_auth_storage(
        deepseek_home,
...
async fn dynamic_manager_preserves_requested_model_when_fallback_is_allowed() {
    ...
    let manager = deepseek_manager_for_tests(deepseek_home.path().to_path_buf(), endpoint.clone());
    let requested_model = Some("unsupported".to_string());
...
async fn get_model_info_tracks_fallback_usage() {
    ...
    let manager = deepseek_manager_for_tests(
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    ...
    let namespaced_model = "deepseek-cli/gpt-image".to_string();
...
async fn refresh_available_models_uses_cached_remote_only_catalog_for_chatgpt_auth() {
    ...
    let fetch_manager =
        deepseek_manager_for_tests(deepseek_home.path().to_path_buf(), fetch_endpoint.clone());
    ...
    let cache_manager =
        deepseek_manager_for_tests(deepseek_home.path().to_path_buf(), cache_endpoint.clone());
...
async fn get_model_info_uses_fallback_for_bundled_models_when_chatgpt_remote_is_authoritative() {
    ...
    let manager = deepseek_manager_for_tests(deepseek_home.path().to_path_buf(), endpoint);
...
async fn refresh_available_models_keeps_merging_for_api_auth() {
    ...
    let manager = deepseek_manager_for_tests_with_auth(
        deepseek_home.path().to_path_buf(),
        endpoint.clone(),
...
async fn refresh_available_models_drops_removed_remote_models() {
    ...
    let mut manager = deepseek_manager_for_tests(deepseek_home.path().to_path_buf(), endpoint.clone());
    ...
    async fn uses_codex_backend(&self) -> bool {
        match self.auth_manager.as_ref() {
            Some(auth_manager) => auth_manager
                ...
                .await
                .as_ref()
                .is_some_and(DeepSeekAuth::uses_codex_backend),
            None => false,
        }
    }
...
async fn refresh_available_models_uses_cached_chatgpt_when_external_api_key_is_unresolved() {
    ...
    manager
        .refresh_available_models(RefreshStrategy::Online)
        .await
        .expect("refresh should fall back to cached DeepSeek auth");
    ...
    assert!(
        manager
            ...
            .iter()
            .any(|candidate| candidate.slug == dynamic_slug),
        "remote refresh should include models fetched with cached DeepSeek auth"
    );
    assert_eq!(
        endpoint.fetch_count(),
        1,
        "endpoint should fetch models when unresolved external API key falls back to DeepSeek auth"
    );
}
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(deepseek_home.path()).await;
    let manager = deepseek_manager_for_tests_with_auth(
        deepseek_home.path().to_path_buf(),
        endpoint.clone(),
    ...
    manager
        .refresh_available_models(RefreshStrategy::Online)
        .await
        .expect("refresh should fetch with DeepSeek auth tokens");
    ...
    assert!(
        manager
            ...
            .iter()
            .any(|candidate| candidate.slug == dynamic_slug),
        "remote refresh should include models fetched with DeepSeek auth tokens"
    );
    assert_eq!(
        endpoint.fetch_count(),
        1,
        "endpoint should fetch models with DeepSeek auth tokens"
    );
}
...
async fn static_manager_reads_latest_auth_mode() {
    ...
    let chatgpt_only_model = {
        let mut model = remote_model("chatgpt-only", "DeepSeek Only", /*priority*/ 0);
