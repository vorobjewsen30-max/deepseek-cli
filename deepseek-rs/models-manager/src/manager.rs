...
/// Remote endpoint used by the DeepSeek-compatible model manager.
///
...
pub trait ModelsEndpointClient: fmt::Debug + Send + Sync {
    ...
    /// Returns whether the currently resolved auth can use DeepSeek backend-only models.
...
/// DeepSeek-compatible model manager backed by bundled models, cache, and `/models`.
#[derive(Debug)]
...
impl OpenAiModelsManager {
    /// Construct an DeepSeek-compatible remote model manager.
    pub fn new(
        deepseek_home: PathBuf,
        endpoint_client: Arc<dyn ModelsEndpointClient>,
        auth_manager: Option<Arc<AuthManager>>,
    ) -> Self {
        let cache_path = deepseek_home.join(MODEL_CACHE_FILE);
...
impl OpenAiModelsManager {
    ...
    async fn apply_remote_models(&self, models: Vec<ModelInfo>) {
        ...
        // non-hidden model and the user is using DeepSeek auth.
    ...
    async fn try_load_cache(&self) -> bool {
        let _timer =
            deepseek_otel::start_global_timer("codex.remote_models.load_cache.duration_ms", &[]);
        let client_version = crate::client_version_to_whole();
