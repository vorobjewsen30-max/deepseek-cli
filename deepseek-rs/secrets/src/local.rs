...
pub enum LocalSecretsNamespace {
    ...
    #[default]
    ManagedSecrets,
    /// DeepSeek authentication credentials used by the CLI, TUI, app server, and other clients.
    DeepSeekAuth,
...
impl LocalSecretsBackend {
    pub fn new(deepseek_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.deepseek_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(deepseek_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let deepseek_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(deepseek_home.path());
        keyring.set_error(
            &account,
    ...
    fn local_namespaces_write_separate_files() -> Result<()> {
        ...
        assert!(!deepseek_home.path().join("secrets").join("local.age").exists());
        Ok(())
    }
}
