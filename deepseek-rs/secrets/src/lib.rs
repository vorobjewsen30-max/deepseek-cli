...
impl SecretsManager {
    pub fn new(deepseek_home: PathBuf, backend_kind: SecretsBackendKind) -> Self {
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind::Local => {
                let keyring_store: Arc<dyn KeyringStore> = Arc::new(DefaultKeyringStore);
                Arc::new(LocalSecretsBackend::new(deepseek_home, keyring_store))
            }
        };
        Self { backend }
    }
...
pub fn compute_keyring_account(deepseek_home: &Path) -> String {
