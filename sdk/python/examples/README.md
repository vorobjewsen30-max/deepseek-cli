...
- `sync.py` (public sync surface: `DeepSeek`)
- `async.py` (public async surface: `AsyncDeepSeek`)

All examples intentionally use only public SDK exports from `deepseek_codex`
and `deepseek_codex.types`.
...
The SDK installs its pinned `deepseek-cli-cli-bin` runtime dependency.
