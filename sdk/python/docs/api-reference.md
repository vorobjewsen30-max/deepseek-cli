# DeepSeek Python SDK (Beta) - API Reference

Public surface of `deepseek_codex` for DeepSeek workflows.
...
from deepseek_codex.types import (
    Account,
...
- Version: `deepseek_codex.__version__`
- Requires Python >= 3.10
- Public DeepSeek protocol value and event types live in `deepseek_codex.types`

## DeepSeek (sync)

```python
DeepSeek(config: DeepSeekConfig | None = None)
```
...
## AsyncDeepSeek (async parity)

```python
AsyncDeepSeek(config: DeepSeekConfig | None = None)
```
...
`AsyncDeepSeek` initializes lazily. Context entry is the standard path because it
...
When `sandbox=` is omitted, DeepSeek uses its configured default. A sandbox
...
- one `DeepSeek` instance can stream multiple active turns concurrently
...
- `stream()` and `run()` consume only notifications for their own turn ID
- one `AsyncDeepSeek` instance can stream multiple active turns concurrently
...
The SDK wrappers return and accept public DeepSeek protocol models wherever possible:
