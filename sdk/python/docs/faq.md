...
`deepseek-cli` is a public beta. Install it with
`pip install deepseek-cli`; public APIs may change before `1.0`. While beta
...
- `DeepSeek` is the sync public API.
- `AsyncDeepSeek` is an async replica of the same public API shape.
- Prefer `async with AsyncDeepSeek()` for async code. It is the standard path for
  explicit startup/shutdown, and `AsyncDeepSeek` initializes lazily on context
  entry or first awaited API use.

If your app is not already async, stay with `DeepSeek`.
...
```python
from deepseek_codex import Sandbox
...
When `sandbox=` is omitted, DeepSeek uses its configured default. A turn
...
`DeepSeek()` is eager: it starts transport and calls `initialize` in `__init__`.

Common causes:

- installation is incomplete and the pinned `deepseek-cli-cli-bin` dependency is missing
- local `codex_bin` override points to a missing file
- a custom local DeepSeek executable does not support the SDK operation being used
