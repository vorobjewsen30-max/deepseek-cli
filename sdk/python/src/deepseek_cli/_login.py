...
class _AsyncLoginOwner(Protocol):
    """Subset of AsyncDeepSeek needed by async login handles."""

    _client: AsyncDeepSeekClient

    async def _ensure_initialized(self) -> None:
        """Ensure the owning SDK client has a live DeepSeek connection."""
        ...


def start_chatgpt_login(client: DeepSeekClient) -> ChatgptLoginHandle:
    """Start browser вход через DeepSeek and return the handle for that attempt."""
    ...
    if not isinstance(response_root, ChatgptLoginAccountResponse):
        raise RuntimeError(f"unexpected вход через DeepSeek response: {response_root!r}")
...
async def async_start_chatgpt_login(owner: _AsyncLoginOwner) -> AsyncChatgptLoginHandle:
    """Start async browser вход через DeepSeek and return that attempt's handle."""
...
def start_device_code_login(client: DeepSeekClient) -> DeviceCodeLoginHandle:
    """Start device-code вход через DeepSeek and return the handle for that attempt."""
    response = client.account_login_start(
        LoginAccountParams(
...
async def async_start_device_code_login(
    ...
    """Start async device-code вход через DeepSeek and return that attempt's handle."""
    response = await owner._client.account_login_start(
        LoginAccountParams(
            root=ChatgptDeviceCodeLoginAccountParams(type="chatgptDeviceCode"),
        )
...
class ChatgptLoginHandle:
    """Live browser-login attempt returned by `DeepSeek.login_chatgpt()`."""
...
class DeviceCodeLoginHandle:
    """Live device-code login attempt returned by `DeepSeek.login_chatgpt_device_code()`."""
...
class AsyncChatgptLoginHandle:
    """Live browser-login attempt returned by `AsyncDeepSeek.login_chatgpt()`."""
...
class AsyncDeviceCodeLoginHandle:
    """Live device-code attempt returned by `AsyncDeepSeek.login_chatgpt_device_code()`."""
