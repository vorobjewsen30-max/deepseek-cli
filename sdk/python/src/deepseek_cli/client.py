...
from .errors import DeepSeekError, InvalidRequestError, TransportClosedError
...
RUNTIME_PKG_NAME = "deepseek-cli-cli-bin"
...
def _installed_codex_path() -> Path:
    try:
        ...
        raise FileNotFoundError(
            "Unable to locate the pinned DeepSeek runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set DeepSeekConfig.codex_bin "
            "explicitly."
        ) from exc
...
def _installed_codex_path_dirs() -> tuple[Path, ...]:
    try:
        from deepseek_cli_bin import bundled_path_dir
...
class DeepSeekBinResolverOps:
    installed_codex_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_codex_bin_resolver_ops() -> DeepSeekBinResolverOps:
    return DeepSeekBinResolverOps(
        installed_codex_path=_installed_codex_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_codex_bin(config: "DeepSeekConfig", ops: DeepSeekBinResolverOps) -> Path:
    if config.codex_bin is not None:
        codex_bin = Path(config.codex_bin)
        if not ops.path_exists(codex_bin):
            raise FileNotFoundError(
                f"DeepSeek binary not found at {codex_bin}. Set DeepSeekConfig.codex_bin "
...
def _resolve_codex_bin(config: "DeepSeekConfig") -> Path:
    return resolve_codex_bin(config, _default_codex_bin_resolver_ops())


@dataclass(slots=True)
class DeepSeekConfig:
    """Configuration for launching and identifying the local DeepSeek runtime.

    Most callers can use ``DeepSeek()`` without configuration. Set ``codex_bin``
    only when intentionally using a specific local DeepSeek executable.
    """
    ...
    client_title: str = "DeepSeek Python SDK"
...
class DeepSeekClient:
    ...
    def request(
        ...
        *,
        ...
        if not isinstance(result, dict):
            raise DeepSeekError(f"{method} response must be a JSON object")
    ...
    def _write_message(self, payload: JsonObject) -> None:
        if self._proc is None or self._proc.stdin is None:
            raise TransportClosedError("DeepSeek process is not running")
        with self._lock:
    ...
    def _read_message(self) -> dict[str, JsonValue]:
        ...
        if not line:
            raise TransportClosedError(
                f"DeepSeek process closed stdout. stderr_tail={self._stderr_tail()[:2000]}"
            )

        try:
            message = json.loads(line)
        except json.JSONDecodeError as exc:
            raise DeepSeekError(f"Invalid JSON-RPC line: {line!r}") from exc

        if not isinstance(message, dict):
            raise DeepSeekError(f"Invalid JSON-RPC payload: {message!r}")
        return message


def default_deepseek_home() -> str:
