...
class JsonRpcError(DeepSeekError):
...
class TransportClosedError(DeepSeekError):
    """Raised when the DeepSeek transport closes unexpectedly."""


class DeepSeekRpcError(JsonRpcError):
    """Base typed error for JSON-RPC failures."""


class ParseError(DeepSeekRpcError):
    """Raised when a request or response cannot be parsed."""


class InvalidRequestError(DeepSeekRpcError):
    """Raised when the runtime rejects the request shape."""


class MethodNotFoundError(DeepSeekRpcError):
    """Raised when the requested operation is unavailable."""


class InvalidParamsError(DeepSeekRpcError):
    """Raised when an operation receives invalid parameters."""


class InternalRpcError(DeepSeekRpcError):
    """Raised when the runtime reports an internal RPC failure."""


class ServerBusyError(DeepSeekRpcError):
...
def map_jsonrpc_error(code: int, message: str, data: Any = None) -> JsonRpcError:
    ...
    if -32099 <= code <= -32000:
        ...
        if _contains_retry_limit_text(message):
            return RetryLimitExceededError(code, message, data)
        return DeepSeekRpcError(code, message, data)
