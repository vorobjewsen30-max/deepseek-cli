...
class AsyncDeepSeekClient:
    """Async wrapper around DeepSeekClient using thread offloading."""

    def __init__(self, config: DeepSeekConfig | None = None) -> None:
        """Create the wrapped sync client that owns the transport process."""
        self._sync = DeepSeekClient(config=config)

    async def __aenter__(self) -> "AsyncDeepSeekClient":
        """Start the DeepSeek process when entering an async context."""
        await self.start()
        return self

    async def __aexit__(self, _exc_type, _exc, _tb) -> None:
        """Close the DeepSeek process when leaving an async context."""
        await self.close()
    ...
    async def initialize(self) -> InitializeResponse:
        """Initialize the DeepSeek session."""
