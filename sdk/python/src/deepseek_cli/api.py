...
class DeepSeek:
    """Synchronous client for creating threads and running DeepSeek turns.
    ...
    def __init__(self, config: DeepSeekConfig | None = None) -> None:
        self._client = DeepSeekClient(config=config)
        try:
            self._client.start()
    ...
    def login_api_key(self, api_key: str) -> None:
        """Authenticate DeepSeek with an API key."""
...
class AsyncDeepSeek:
    ...
    def metadata(self) -> InitializeResponse:
        if self._init is None:
            raise RuntimeError(
                "AsyncDeepSeek is not initialized yet. Prefer `async with AsyncDeepSeek()`; "
    ...
    # BEGIN GENERATED: AsyncDeepSeek.flat_methods
    ...
    # END GENERATED: AsyncDeepSeek.flat_methods
