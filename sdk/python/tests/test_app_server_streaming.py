...
from deepseek_codex import AsyncDeepSeek, DeepSeek
...
def test_async_stream_routes_text_deltas_and_completion(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncDeepSeek(config=harness.app_server_config()) as codex:
...
def test_low_level_sync_stream_text_uses_real_turn_routing(tmp_path) -> None:
    """DeepSeekClient.stream_text should stream through a real app-server turn."""
    with AppServerHarness(tmp_path) as harness:
        harness.responses.enqueue_sse(
