...
import deepseek_codex.types as public_types
from deepseek_codex import (
    ApprovalMode,
    AsyncDeepSeek,
    AsyncThread,
    AsyncTurnHandle,
...
from deepseek_codex._initialize_metadata import validate_initialize_metadata
from deepseek_codex.types import InitializeResponse
...
def test_root_exports_deepseek_config() -> None:
    """The root package should expose the process configuration object."""
    assert DeepSeekConfig.__name__ == "DeepSeekConfig"
...
def test_package_and_default_client_versions_follow_project_version() -> None:
    ...
    assert deepseek_codex.__version__ == pyproject["project"]["version"]
    assert DeepSeekConfig().client_version == deepseek_codex.__version__
...
def test_curated_public_api_has_builtin_help_documentation() -> None:
    ...
    documented = {
        "module": deepseek_codex,
        "DeepSeek": DeepSeek,
        "AsyncDeepSeek": AsyncDeepSeek,
        ...
        "Sandbox": Sandbox,
        "thread_start": DeepSeek.thread_start,
        "thread_resume": DeepSeek.thread_resume,
...
def test_package_includes_py_typed_marker() -> None:
    ...
    marker = resources.files("deepseek_codex").joinpath("py.typed")
...
def test_package_root_exports_only_public_api() -> None:
    ...
    assert deepseek_codex.__all__ == EXPECTED_ROOT_EXPORTS
    assert {name: hasattr(deepseek_codex, name) for name in EXPECTED_ROOT_EXPORTS} == dict.fromkeys(
        EXPECTED_ROOT_EXPORTS, True
    )
    assert {
        "DeepSeekClient": hasattr(deepseek_codex, "DeepSeekClient"),
        "AsyncDeepSeekClient": hasattr(deepseek_codex, "AsyncDeepSeekClient"),
        "InitializeResponse": hasattr(deepseek_codex, "InitializeResponse"),
        "ThreadStartParams": hasattr(deepseek_codex, "ThreadStartParams"),
        "TurnStartParams": hasattr(deepseek_codex, "TurnStartParams"),
        "TurnCompletedNotification": hasattr(deepseek_codex, "TurnCompletedNotification"),
        "TurnStatus": hasattr(deepseek_codex, "TurnStatus"),
    } == {
        "DeepSeekClient": False,
        "AsyncDeepSeekClient": False,
...
def test_package_star_import_matches_public_api() -> None:
    ...
    exec("from deepseek_codex import *", namespace)
...
def test_types_module_exports_curated_public_types() -> None:
    """The public type module should expose DeepSeek protocol models."""
...
def test_types_star_import_matches_public_types() -> None:
    ...
    namespace: dict[str, object] = {}
    exec("from deepseek_codex.types import *", namespace)
...
def test_examples_use_public_import_surfaces() -> None:
    ...
    private_import_markers = [
        "deepseek_codex.api",
        "deepseek_codex.client",
        "deepseek_codex.generated",
        "deepseek_codex.models",
        "deepseek_codex.retry",
    ]
...
def test_lifecycle_methods_are_codex_scoped() -> None:
    ...
    assert hasattr(DeepSeek, "thread_resume")
    assert hasattr(DeepSeek, "thread_fork")
    assert hasattr(DeepSeek, "thread_archive")
    assert hasattr(DeepSeek, "thread_unarchive")
    assert hasattr(AsyncDeepSeek, "thread_resume")
    assert hasattr(AsyncDeepSeek, "thread_fork")
    assert hasattr(AsyncDeepSeek, "thread_archive")
    assert hasattr(AsyncDeepSeek, "thread_unarchive")
    assert not hasattr(DeepSeek, "thread")
    assert not hasattr(AsyncDeepSeek, "thread")
    ...
    for fn in (
        DeepSeek.thread_archive,
        DeepSeek.thread_unarchive,
        AsyncDeepSeek.thread_archive,
        AsyncDeepSeek.thread_unarchive,
    ):
        _assert_no_any_annotations(fn)
...
def test_initialize_metadata_parses_user_agent_shape() -> None:
    ...
    payload = InitializeResponse.model_validate({"userAgent": "deepseek-cli/1.2.3"})
    parsed = validate_initialize_metadata(payload)
    assert parsed is payload
    assert parsed.userAgent == "deepseek-cli/1.2.3"
    assert parsed.serverInfo is not None
    assert parsed.serverInfo.name == "deepseek-cli"
