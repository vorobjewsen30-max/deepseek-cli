...
import deepseek_codex.api as public_api_module
from deepseek_codex.api import (
    ApprovalMode,
    AsyncDeepSeek,
    DeepSeek,
    Sandbox,
)
from deepseek_codex.generated.v2_all import TurnStartParams
from deepseek_codex.models import InitializeResponse
...
def test_codex_init_failure_closes_client(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(public_api_module, "DeepSeekClient", FakeClient)
...
def test_async_codex_initializes_only_once_under_concurrency() -> None:
    async def scenario() -> None:
        ...
        async def fake_initialize() -> InitializeResponse:
            ...
            return InitializeResponse.model_validate(
                {
                    "userAgent": "deepseek-cli/1.2.3",
                    "serverInfo": {"name": "deepseek-cli", "version": "1.2.3"},
                }
            )
