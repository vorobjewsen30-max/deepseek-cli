...
from deepseek_codex.generated.v2_all import (
    ChatgptAuthTokensLoginAccountParams,
    LoginAccountParams,
)


def _app_server_config(harness: AppServerHarness) -> DeepSeekConfig:
    """Build an isolated login config without inheriting ambient API-key auth."""
    config = harness.app_server_config()
    config.env = {**(config.env or {}), "DEEPSEEK_API_KEY": ""}
    return config
...
def test_api_key_login_authenticates_follow_up_model_requests(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path, requires_deepseek_auth=True) as harness:
        harness.responses.enqueue_assistant_message("api key auth", response_id="api-key-auth")

        with DeepSeek(config=_app_server_config(harness)) as codex:
...
def test_chatgpt_token_login_authenticates_follow_up_model_requests(tmp_path) -> None:
    """DeepSeek token handoff should authorize later Responses requests with that token."""
    ...
    # App-server parses claims from the access token before persisting external DeepSeek auth.
