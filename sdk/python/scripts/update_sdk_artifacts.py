...
SDK_DISTRIBUTION_NAME = "deepseek-cli"
RUNTIME_DISTRIBUTION_NAME = "deepseek-cli-cli-bin"
RUNTIME_PACKAGE_ROOT = Path("src") / "deepseek_cli_bin"
...
def schema_bundle_path(schema_dir: Path) -> Path:
    ...
    return schema_dir / "deepseek_app_server_protocol.v2.schemas.json"
...
def pinned_runtime_codex_path() -> Path:
    """Return the bundled DeepSeek binary from the installed pinned runtime wheel."""
    ...
    try:
        from deepseek_cli_bin import bundled_codex_path
    ...
    if not codex_path.exists():
        raise RuntimeError(f"Pinned DeepSeek runtime binary not found at {codex_path}.")
    return codex_path
...
def normalize_codex_version(version: str) -> str:
    ...
    if not re.fullmatch(r"[0-9]+(?:\.[0-9]+)*(?:(?:a|b|rc)[0-9]+)?", normalized):
        raise RuntimeError(f"Could not normalize DeepSeek version {version!r} to a PEP 440 version")
    return normalized
...
def stage_python_sdk_package(staging_dir: Path, sdk_version: str) -> Path:
    ...
    sdk_bin_dir = staging_dir / "src" / "deepseek_codex" / "bin"
...
def _extract_codex_package_archive(package_archive: Path, runtime_package_root: Path) -> None:
    if not package_archive.name.endswith(".tar.gz"):
        raise RuntimeError(f"Expected a .tar.gz DeepSeek package archive: {package_archive}")
...
def _validate_codex_package_layout(package_dir: Path, package_archive: Path) -> None:
    ...
    if missing_entries:
        ...
        raise RuntimeError(f"Missing DeepSeek package layout entries in {package_archive}: {missing}")
...
def _make_chatgpt_account_email_nullable(schema: dict[str, Any]) -> None:
    ...
    for variant in account.get("oneOf", []):
        ...
        if not isinstance(email, dict):
            raise RuntimeError("аккаунт DeepSeek schema is missing email")
        email["type"] = ["string", "null"]
        return

    raise RuntimeError("Schema bundle is missing the аккаунт DeepSeek variant")
...
def generate_v2_all(schema_dir: Path) -> None:
    ...
    out_path = sdk_root() / "src" / "deepseek_codex" / "generated" / "v2_all.py"
...
def _notification_specs(schema_dir: Path) -> list[tuple[str, str]]:
    ...
    generated_source = (sdk_root() / "src" / "deepseek_codex" / "generated" / "v2_all.py").read_text()
...
def generate_notification_registry(schema_dir: Path) -> None:
    ...
    out = sdk_root() / "src" / "deepseek_codex" / "generated" / "notification_registry.py"
...
def _load_public_fields(
    ...
    if module_name == "deepseek_codex.generated.v2_all":
...
def _load_generated_v2_all_module() -> types.ModuleType:
    ...
    module_name = "_deepseek_codex_generated_v2_all_for_artifacts"
    sys.modules.pop(module_name, None)
    module_path = sdk_root() / "src" / "deepseek_codex" / "generated" / "v2_all.py"
...
def _render_codex_block(
    ...
    lines = [
        ...
        *_kw_signature_lines(thread_start_fields),
        "    ) -> Thread:",
        '        """Create a new DeepSeek conversation thread."""',
...
def generate_public_api_flat_methods() -> None:
    ...
    public_api_path = src_dir / "deepseek_codex" / "api.py"
    ...
    thread_start_fields = _load_public_fields(
        "deepseek_codex.generated.v2_all",
        "ThreadStartParams",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    stage_runtime_parser.add_argument(
        "package_archive",
        type=Path,
        help="Path to a DeepSeek package .tar.gz archive for this platform.",
    )
    stage_runtime_parser.add_argument(
        "--codex-version",
        required=True,
        help=(
            "DeepSeek release version to write into the staged runtime package. "
