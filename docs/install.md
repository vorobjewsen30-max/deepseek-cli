...
The GitHub Release also contains a [DotSlash](https://dotslash-cli.com/) file for the DeepSeek CLI named `codex`. Using a DotSlash file makes it possible to make a lightweight commit to source control to ensure all contributors use the same version of an executable, regardless of what platform they use for development.

### Build from source

```bash
# Clone the repository and navigate to the root of the Cargo workspace.
git clone https://github.com/vorobjewsen30-max/deepseek-cli
cd codex/codex-rs
...
DeepSeek is written in Rust, so it honors the `RUST_LOG` environment variable to configure its logging behavior.
...
tail -F ./.codex-log/deepseek-tui.log
```
